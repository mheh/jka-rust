...
impl OutboundSysCall for UiStrncpy {
    ...
    /// Args source: `oracle/oracle/codemp/client/cl_ui.cpp:824` (`TRAP_STRNCPY`).
    /// Fallback source for SP-facing `UI_STRNCPY` is shared with MP transport ABI.
    /// Output source: `oracle/oracle/codemp/client/cl_ui.cpp:824`
    /// TODO: SP-side `UI_STRNCPY` transport is known only via fallback (`TRAP_STRNCPY`) evidence.
    /// Verify direct SP `UI_STRNCPY` evidence in `oracle/oracle/code/client/cl_ui.cpp` when available.
