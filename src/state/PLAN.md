...
## Summary
`src/state` is the JKA engine reimagined in idiomatic Rust. `src/state/boundary/` is the
typed seam between the faithfully-ported game module (`src/codemp/game`, `src/trap`) and the
engine: every engine↔game call is a typed `OutboundSysCall` (game→engine) or `InboundVmCall`
