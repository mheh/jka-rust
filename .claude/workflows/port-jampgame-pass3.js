...
- Fn you fill carries a pre-existing PORT-ESCALATION(...) marker (skeleton-staged or otherwise) -> CONVERT it to // PORT-NOTE(<topic>): <one line>. A filled fn NEVER carries PORT-ESCALATION — the manifest scan counts any PORT-ESCALATION as an open fn, so leaving (or adding) one gets the fn pointlessly re-ported.`

const DISCIPLINE = `MECHANICS (kills the pass-2 error classes): raw pointers are dereferenced explicitly — (*ent).client, never ent.client; every EntThink/EntUse/EntTouch/EntDie/EntPain/EntBlocked/EntReached use comes with its import line; add 'use crate::trap;' if you call traps; vec3 helpers use the reshaped q_math signatures shown in your packet (by-value inputs, &mut outputs); stored entity fields are EntityId per ruling 17 — assign/compare ids, not pointers; bg-tier bodies use PmoveContext/BgState/BgTraps/GameCallbacks exactly as your packet's threading digest shows; va()/printf family per the packet's mapping table (format!, owned String). NO LOCAL SHIMS: NEVER define a local helper/shim fn in your file — if a helper seems missing, use the canonical mapped form from your packet's EXAMPLE SYNTAX section, or report it in missing_symbols; never re-invent it. vec3 macro family (VectorCopy/VectorSubtract/VectorAdd/VectorScale/VectorMA/DotProduct/CrossProduct/VectorNormalize/…): use the canonical reshaped q_math helpers per your packet's EXAMPLE SYNTAX (by-value inputs, &mut outputs, _-prefixed assignment forms) — never report them missing, never shim, never write Raven macro call syntax. WRITE DISCIPLINE: few LARGE Edit calls; touch ONLY fns listed in your packet; skip any fn already implemented. PARSE GATE (mandatory before returning): after your final edit, run \`rustfmt --edition 2021 --emit stdout <your file> > /dev/null\` — output discarded, this is a parse check only; any rustfmt error means the file does not parse (rustc stops at parse errors, so a broken file masks all cargo triage downstream). Fix every syntax error it reports and re-run until it exits clean. This is the ONLY compiler-adjacent command you may run — cargo remains forbidden.`
