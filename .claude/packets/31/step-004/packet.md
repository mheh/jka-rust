...
**2026-08-05 - lane-review close-out, pass with dispositioned findings.** The clerk's review ran in two sittings, because its first gate battery died against the 1.12-million-file `target/debug/deps` bloat (see the memory record `target-deps-bloat-trap`); after the `cargo clean` it re-ran every gate itself: 517 workspace tests passed with zero failures, all four golden suites green, the ghoul2 tripwire SHA-identical across master, branch, and worktree (`897352f5...51c3d`), and `entity_duel1.png` byte-exact against the lane's provenance (592713 bytes, `a1ccf23a...67d5`). No dependency moved and no read-only file was touched.

Dispositions, all accepted without reopening the lane: (1) four pass-through signatures (`R_AddEntitySurfaces`, `R_GenerateDrawSurfs`, `R_MirrorViewBySurface`, `R_SortDrawSurfs`) changed beyond the contract's named three - the contract under-enumerated the mandatory chain, and the lane widened nothing; (2) four ledger mismatches, each equivalence-checked by the clerk: the brush predicate is the contract's own mandate, `G2SurfaceRef.model`'s handle equals the old pool index under DEC-42.2, one warn flag now guards the two ghoul2 legs, and the dropped `md3_name` call prints identical text through `read_qpath`; (3) the new `entity_golden.rs` was column-wrapped at about 112 columns, fixed at `6118f834` with both model goldens re-confirmed green after; the semicolons in its `#[ignore]` and mismatch strings stay because they mirror the sibling goldens verbatim, deferred to the parked hygiene stage; (4) accepted unverified: tip-only gate verification (corroborated for the merged result by the tripwire's byte identity at tip), the image's visual content (the stat asserts prove both arms drew and neither decoder failed), and the `boot.rs` spike resolving entities against a registry no drain fills, inert today and recorded as a step-005 radar item beside the deferred per-slot aliasing ruling.

The user ruled the merge on 2026-08-05.

**2026-08-05 - the bundle restructures, session ruling.** The lane found that the planned commits 2 and 3 cannot compile separately: commit 2 dissolves `EntityWalkHost.models`, which is the exact field commit 3's `backend_models` reads. Only one honest structure exists, so the session accepts the lane's restructure without a user round: the drain lands as its own behavior-neutral commit (`b4fa7e5f`, a better bisect point), and the arm and decoder migration lands as one combined commit. The scope is unchanged. Two notes ride along: `boot.rs` is a fifth `EntityWalkHost` construction site the contract's list of four missed, edited under the compile-fallout catch-all, and the migrated arms gain `None` early-return skips where the old code read a raw `model_t` unconditionally, the defined-behavior skip survey section G anticipated.
