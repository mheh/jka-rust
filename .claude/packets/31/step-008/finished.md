...
The walk closed two more rulings, and `08b818d4 fix(gh#31 s008): the carried-spelling note` carries the one that needs code. The module doc spells the word Rasterisation the British way, carried in from `scene_golden.rs`, and the ruling keeps it as a carried defect with a one-line note at the site. The lowercase openings of the two assert messages stand as the repo's panic-message and assert-message idiom, so no message changed. That commit adds one comment line, so `cargo build --workspace` warning-free and `cargo test --workspace -- --test-threads=1` green were the whole gate.
