...
### 9. Lane-review close-out, four deviations accepted

**2026-08-04.** The lane returned seven commits and the finished file. Lane-review ran in full-read mode, because this leg carries the renderer's first `unsafe impl Send`/`Sync` and its first cross-thread publication. The session read the whole diff and re-ran every gate rather than trusting the finished file: `cargo check --workspace --all-targets` reports zero warnings and zero errors, `cargo test --workspace` is green with zero failures, both world goldens pass byte-identical, and no commit carries a trailer. Four deviations were found, and all four are this packet being wrong rather than the lane widening scope. The user accepted all four and ruled the merge.

1. **The sidecars sit behind their own `Arc`.** The surface contract spells the private fields `Option<MdxmParsed>` and `Option<MdxaParsed>`. `Arc::make_mut` needs `ModelBlock: Clone`, neither parsed type implements `Clone`, and deriving it writes to `crates/mp/host-interface/`, which the write scopes exclude. The fields are `Option<Arc<MdxmParsed>>` and `Option<Arc<MdxaParsed>>` instead. Every listed signature holds unchanged. The shape is better than the one this packet spelled, because the index shares by refcount rather than deep-copying on every copy-on-write poke, and its address stays stable across that poke, which `model_mdxm_ptrs` needs when it hands the index out as a raw pointer.
2. **`empty_render_assets()` is not the single construction site.** `crates/mp/renderer-gpu/src/bin/dev_harness.rs:328` is a second `RenderAssets` struct literal, so the new field does not compile without naming it there. The lane edited that one file outside the write scopes: one field line plus one import, no behavior. This is the class of compile fallout the write scopes already authorize for the `RE_EndFrame` signature change.
3. **Six helpers the contract implies but does not list.** `ModelBlock::new` and `bump_generation` exist because the contract gives the type private fields and asks the poke to bump the generation. `block_base_ptr` and `block_containing` exist because `disk_image` is private to `cached_model_binary.rs`, so this packet's own post-replay re-fetch and its pointer-to-block resolution have nowhere else to live. `PublishedModel::holds` is a private helper beside its one caller, and `mark_block` is `pub(super)` because the two registration-completion sites are sibling modules inside `tr_model`. None of the six is `pub`.
4. **Three commits past the four-commit bundle.** The finished file, a comment lint over the lane's own added lines, and a finished-file correction. The lane-review skill requires the finished file, so its commit is expected.

The memory-safety argument behind the `Send`/`Sync` impls was checked in the review and holds. Every live `model_t` pointer's block is kept alive by either its cache entry or the published registry. Where a copy-on-write poke splits a block in two, the post-replay re-fetch moves `model_t` onto the live block while a frame in flight keeps the old one alive for as long as it reads it. On the fresh-load arm `Arc::make_mut` finds a unique block and writes it in place, so no split occurs there at all. All three cache-removal sites call `remove_block` or `clear`, which is what keeps `r_modelpoolmegs` reclamation freeing memory.

One correction to the lane brief, not to this packet. The world goldens need `--test-threads=1`. Two engine boots in parallel threads crash in the pk3 inflate path before any pixel comparison, which the test file's own module doc already records.
