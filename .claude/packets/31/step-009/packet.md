...
The oracle's header comment says `e->origin` is the bottom point, `e->oldorigin` the top, and `e->radius` the radius (`:849-851`). The code maps the two radii the opposite way from Raven's own ring names. `e->radius` scales `v1`, which builds `lower_points` and translates them to `e->oldorigin`, the top point. `e->rotation` scales `vu`, which builds `upper_points` and translates them to `e->origin`, the bottom point (`:892-906`). Transcribe the code, and do not carry Raven's upper and lower names into a comment as if they matched the header. `NUM_CYLINDER_SEGMENTS` is 32 (`:815`).
...
**`RB_SurfaceElectricity` writes its own input entity.** The `oldorigin` write at `:1159` mutates `backEnd.currentEntity->e`, which points into the shared `backEnd.refdef.entities` array. A frame with two views draws the same entity twice, and the second draw reads both the mutated `oldorigin` and the mutated `frame` seed. Open row 6 rules the disposition.
...
**Row 6 - the `oldorigin` write (user ruling, ratified as amended).** Compute the grown endpoint into a local `end` and never write the entity. DEC-66 ruling 2 fixes the dispatch at an immutable entity borrow, so the local is the only shape. The site note words the write as an accepted divergence rather than as parity. The oracle's write survives past one draw chain: a mirror or portal frame runs `R_RenderView` per view, both views index the one shared `backEnd.refdef.entities` array, and the second `RB_SurfaceElectricity` call of the frame reads the `oldorigin` and the `frame` seed the first call mutated (`oracle/codemp/renderer/tr_surface.cpp:1159`). Nothing observes it today, because no committed golden and no current backend path draws a portal view. The `≤2`-line note takes the shape of the `RT_SABER_GLOW` radius note (`pipeline3d.rs:4615-4621`) and names portals and mirrors as the condition that would make the gap visible.
...
- `cargo build --workspace`. An intermediate commit may carry warnings, and the bundle's final state must build with zero warnings. Commits 1 through 3 leave `refdef_time`, `rng` and `shape` unread inside `build_entity_geometry`, because commit 4 is the first consumer of `refdef_time` and `shape` and commit 5 is the last new reader of `rng`. Those unused-parameter and unused-field warnings are expected and need no placeholder. Every commit from 5 onward builds clean.
...
**2026-08-28 - the ratification walk closed all seven rows.** The rulings are folded into the body above.

- Row 1, where the three live arms land: ratified as proposed. The arms land in `pipeline3d.rs::build_entity_geometry`.
- Row 2, the saber-glow hilt radius: ratified as proposed. Close it this step and re-bless `scene_saber_glow.png`.
- Row 3, the four goldens and the bless flow: ratified as proposed. Three new scene goldens plus the one re-bless, the step-007 procedure, a STOP per PNG, and a named defect condition per image.
- Row 6, the `oldorigin` write: ratified as amended per the audit challenge. The local `end` plan stands, and the site note words the write as an accepted divergence, unobservable until portals or mirrors draw. The oracle's second draw of the shared entities array reads the mutated `oldorigin` and the mutated `frame` seed (`oracle/codemp/renderer/tr_surface.cpp:1159`). The "parity, not a divergence" sentence is dropped.
