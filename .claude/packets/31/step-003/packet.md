...
**2026-08-04 - lane-review close-out, clean pass.** Three commits landed (`0def8283`, `849251f9`, `1de794bb`), and the conformance clerk found no letter violation: every surface in the diff maps to a contract line, all ten files sit in the write scopes, and no dependency, hook, cvar, or layout moved outside the list. The clerk re-ran all five gates itself (build, workspace tests 516/0/18, the ghoul2 golden, both world goldens, the seven scene goldens, all exit 0), verified the fixture bit-identical three ways (`897352f5...51c3d`), re-derived the 22-surface multiset digest to equality, and confirmed the empty-shader-name cause by reading all 81 `mdxmSurfHierarchy_t` entries out of the retail `assets1.pk3`.

The headline outcome corrects the survey: the re-bless reproduced the committed fixture byte for byte, identity permutation, because every stormtrooper surface carries an empty shader name and `R_FindShader` resolves slot zero on the client leg, the value the server leg forced. One fixture has now matched under three configurations. Step-004 must not assume the ghoul2 draw order is free to move.

Accepted ledger notes from the clerk, none reopening the lane: (A) the finished file's `pub` inventory sentence omits the contract-listed `pub re` field; (B) `re.automap` joins the inert list; (C) four load-time `boot.rs` sites seat the `re` pointer while an `Arc::make_mut` borrow is live, which conforms to the contract's seat-everywhere rule and to the pre-existing per-slot cast discipline (`hook_install.rs:7-11`), because no `re`-casting hook fires inside those calls today; (D) `ShaderHashTableExists` also casts the slot, unreachable here because its one caller gates on `vm_current_is_game()` and the harness holds no VM; (E) the `screenshot`/`screenshot_tga` console handlers change from a guaranteed null crash to a live cast, unexercised in the rig; (F, G) one comment rewrite and three call reformats unconfessed. Two accepted procedural deviations as confessed: the scene-golden invocation carries no `--ignored`, and commit 2 has no fixture diff. Accepted unverified: the commit-1-only byte-identity claim was not re-run at `0def8283`; it is corroborated by composition, since the tip passes byte-identical on the client leg and the gh#35 control run proved the server leg against the same bytes.

**Open ruling carried forward (pre-step-004):** the seated `re` slot's per-field aliasing discipline. The pin closes the one live write behind a read (`sim.published`); items C and E above are the same per-slot class the `rm` slot has always lived under, and the rule deserves an explicit ruling before the harness grows a second in-frame hook or executes a console handler. Recorded for the step-004 packet alongside rulings 3 and 4.
