...
`R_LoadLightmaps` zeroes `tr.numLightmaps` only for the main world, `if (&worldData == &s_worldData) { tr.numLightmaps = 0; }` (`oracle/codemp/renderer/tr_bsp.cpp:176-179`). It then sets `tr.numLightmaps = len / (LIGHTMAP_SIZE * LIGHTMAP_SIZE * 3)` for every world (`:191`) and writes `tr.lightmaps[i]` from index 0 (`:240`). An instance load therefore replaces the whole lightmap table, and the main world's surfaces then sample the instance's lightmaps. See row 6.
...
`WorldSurfaceRef` is a `Copy` five-variant enum over one bare `u32` (`crates/mp/renderer/src/tr_main.rs:273-285`), built by `WorldSurfaceRef::of` (`:291-299`). DEC-43.3 ruling 3 chose that shape, and DEC-43.1 states the flat index is the oracle's own `worldData.surfaces` subscript.
...
`R_LoadLightmaps` replaces `assets.lightmaps` wholesale for every world (`crates/mp/renderer/src/tr_bsp.rs:2417,2519`), which is the oracle's own last-world-wins behavior.

## Surface contract

### `crates/mp/renderer/src/render_state/render_assets.rs`

Three new methods on `RenderAssets`. They define the flat surface-index space by summation over the worlds it already owns, so there is no stored table to go stale and no new state home.
...
/// The surfaces the main world and every loaded instance hold together, the length of the flat index space.
pub fn total_world_surfaces(&self) -> usize
```

The ordering law all three share, stated once in `world_surface_base`'s doc: the main world first, then `bsp_models` in slot order. `build_world_mesh` concatenates in that same order, and the two must never diverge.
...
`RenderModels::bmodel_index` is removed. After commit 3 and commit 5 its only callers are the three assertions in this file's test module, and those move to `bmodel_location`.
...
**Row 1 - the fix shape (user ruling).** The issue names two shapes. **Proposed default: (a), append with offsets.** The evidence, in order of weight.

The oracle's draw path never asks which world a surface came from. Cull, sort, batch break, and every backend arm are world-agnostic, and the only world-keyed read in the whole path is the fog lookup, which reads the main world unconditionally (`oracle/codemp/renderer/tr_shade.cpp:1192`). A world tag on `WorldSurfaceRef` would carry information the oracle deliberately does not carry.

DEC-43.3 ruling 3 chose the bare `u32` index handle, and DEC-43.1 states flat-index addressing preserves the oracle's one surface-index space. Shape (b) contradicts both, so it is a DEC divergence and needs its own ruling row on top of this one.

The backend binds the world vertex and index buffers once for the whole pass (`crates/mp/renderer-gpu/src/pipeline3d.rs:1601,1615`), and the item list is sorted by shader, so worlds interleave through it. Per-world buffers would force a rebind per item and break the one-pass shape.
...
The blast radius is smaller. Shape (a) changes eight sites and adds three methods on a struct that already owns both worlds. Shape (b) changes all eight `WorldSurfaceRef` consumers plus the geometry, the scratch, and the pass structure.
...
**Row 6 - the lightmap clobber (mechanical, a preserved quirk).** **Proposed default: no change, and a named pause trigger so the lane does not correct it.** `R_LoadLightmaps` zeroes the counter only for the main world (`oracle/codemp/renderer/tr_bsp.cpp:176-179`) and then writes `tr.numLightmaps` and `tr.lightmaps[0..n]` for every world (`:191,240`). An instance load therefore replaces the whole table, and the main world's surfaces sample the instance's lightmaps afterwards. The port matches with `assets.lightmaps = lightmaps` (`crates/mp/renderer/src/tr_bsp.rs:2519`). Porting rule 20 keeps an emergent per-mode quirk. The consequence for row 8 is direct: the golden's main world will light differently from `world_ffa2.png`, and that is the correct image, not a regression.
...
**Row 8 - the live gate (user ruling).** **Proposed default: one new world golden, `world_subbsp_ffa2.png`, plus unit tests on the index space.**
...
Named defect conditions. The image must show instance geometry that is not in `world_ffa2.png`. An image identical to `world_ffa2.png` except for lighting means the instance submodel drew nothing, which is the bug this step closes. A panic in `world.bmodels` or `surf_dlight_bits` means an index escaped its offset. The main world's lighting differing from `world_ffa2.png` is expected, per row 6.
...
**Row 9 - the removal of `RenderModels::bmodel_index` (mechanical).** **Proposed default: remove it in the commit that takes its last live caller, and move the three assertions in the file's test module to `bmodel_location`.** After commits 3 and 5 nothing else calls it, and a second accessor that silently drops the world index is the exact shape that caused this ticket.
...
- The lightmap table looks like it should be per world, or the main world's lighting change looks like a regression. STOP, per row 6. It is Raven's behavior.
...
5. **`RE_GetBModelVerts` resolves its owning world.** The row-7 fix, plus the removal of `RenderModels::bmodel_index` and the three test moves of row 9. Files: `crates/mp/renderer/src/tr_world.rs`, `crates/mp/renderer/src/tr_model/render_models.rs`. Subject: `fix(gh#50 s001): RE_GetBModelVerts reads the owning world`. Gates: the full battery, all twenty fixtures byte-identical.
...
- `crates/mp/renderer/src/render_state/render_assets.rs` - the three index-space methods and their tests only.
...
- `crates/mp/renderer/src/tr_model/render_models.rs` - `worlds_dirty`, `take_worlds_dirty`, the `bmodel_index` removal, and the test moves.
