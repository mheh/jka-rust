...
# The subsystem map of the active module, set by configure() from the spec
# (modules/<name>.py). subsystem_of() reads it, in this tool and engineorder.
SUBSYSTEMS: list = []


def configure(module: str):
    """The module spec supplies the subsystem map. Call before any
    subsystem_of() use."""
    global SUBSYSTEMS
    sp = C.spec(module)
    if "subsystems" not in sp:
        sys.exit(f"module '{module}' spec has no `subsystems` key "
                 f"(modules/{module}.py)")
    SUBSYSTEMS = sp["subsystems"]


def module_label(module: str) -> str:
    """The output-prefix label from the spec. mp-engine-ded keeps its
    historical "engine" prefix (out/engine/engine-fn-manifest.json etc.);
    other modules default to their name with the "mp-" prefix stripped."""
    return C.spec(module).get(
        "label", module[3:] if module.startswith("mp-") else module)
...
def render_md(man):
    s = man["stats"]
    msp = C.spec(s["module"])
    title = msp.get("sweep_title", s["module"])
    desc = msp.get("sweep_desc", f"the `{s['module']}` profile")
    o = []
    o.append(f"# {title} — function sweep stats\n")
    o.append(f"Generated by `tools/closure-prototype/enginesweep.py` from one "
             f"unity libclang parse of {desc}.\n")
...
def main():
    ...
    ap.add_argument("--out", default=None,
                    help="default: out/engine for mp-engine-ded, out/<label> otherwise")
    args = ap.parse_args()
    man = build(args.module)
    man.update(C.freshness_stamp())
    prefix = man["stats"]["label"]
    outdir = Path(args.out) if args.out else (
        Path(__file__).resolve().parent / "out" / prefix)
    outdir.mkdir(parents=True, exist_ok=True)
    (outdir / f"{prefix}-fn-manifest.json").write_text(json.dumps(man, indent=1))
    (outdir / f"{prefix}-fn-stats.md").write_text(render_md(man))
    (outdir / f"{prefix}-wave-partition.json").write_text(
        json.dumps(F.build_wave_partition(man), indent=1))
    ...
    print(f"[enginesweep] wrote {outdir}/{prefix}-fn-manifest.json + "
          f"{prefix}-fn-stats.md + {prefix}-wave-partition.json")
