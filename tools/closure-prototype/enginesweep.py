...
# Output-prefix + report title/description, per module (R1 generalization:
# enginesweep.py originally hardwired both to the mp-engine-ded/WinDed case).
# mp-engine-ded keeps its historical "engine" prefix (out/engine/engine-fn-
# manifest.json etc. — unchanged filenames); other modules default to their
# module name with the "mp-" prefix stripped.
MODULE_LABELS = {"mp-engine-ded": "engine", "mp-renderer": "renderer"}
MODULE_DESC = {
    "mp-engine-ded": (
        "mp-engine (DEDICATED server)",
    ...
    "mp-renderer": (
        "mp-renderer (full MP client renderer)",
...
def module_label(module: str) -> str:
    return MODULE_LABELS.get(module, module[3:] if module.startswith("mp-") else module)
...
def render_md(man):
    s = man["stats"]
    title, desc = MODULE_DESC.get(
        s["module"], (s["module"], f"the `{s['module']}` profile"))
    o = []
