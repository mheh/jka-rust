...
def render_fn(fn, ported):
    ...
    cite = f"oracle/codemp/game/{fn['file']}:{fn['line']}-{fn['end_line']}"
    src_line = f"oracle/codemp/game/{fn['file']}:{fn['line']}"
    unported = set()
...
def render_file(cfile, fns, ported):
    o = []
    o.append(f"//! FAITHFUL signature skeleton for `oracle/codemp/game/{cfile}`.")
    o.append("//!")
