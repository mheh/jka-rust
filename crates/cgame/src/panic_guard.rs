...
//! fires: PRINTS a readable `cgame panic: file:line:col — message` to stderr
...
mod tests {
    ...
    fn format_record_names_file_line_and_message() {
        let r = format_record(Some("cg_main.rs:42:9"), "boom");
        assert!(
            r.contains("cg_main.rs:42:9"),
    ...
    fn store_take_roundtrips_and_clears() {
        store("cg_main.rs:7:1 — kaboom".to_string());
        let got = take().expect("a record was stored");
        assert!(got.contains("cg_main.rs:7:1"));
