//! `GpGroup` — one GP2 parse-tree group node (SP).
...
pub(crate) struct GpGroupNode {
    ...
    /// SP's `AddGroup` constructs subgroups without a parent
    /// (`genericparser2.cpp:655`, unlike MP), so parsing never sets this; it
    /// exists because `CGPGroup::mParent` does, and stays `None`.
...
/// Raven `strcmpi` ordering as used by `SortObject`/`FindSubGroup`
/// (ASCII case-insensitive; the MSVC `strcmpi` is locale-dependent for
...
impl<'a> GpGroup<'a> {
    ...
    ///
    /// Always `None` for parsed trees: SP's `AddGroup(name, textPool)` never
    /// passes a parent (unlike MP). SP consumers null-check accordingly
    /// (`code/RMG/RM_Mission.cpp:1251`).
    ...
    /// Raven `CGPGroup::GetNumSubGroups`.
    ...
    /// case-insensitively, in insertion order. SP has no `"||"` multi-key
    /// search (that is MP-only).
    pub fn find_pair(&self, key: &str) -> Option<&'a GpValue> {
        self.pairs().find(|p| p.name().eq_ignore_ascii_case(key))
    }
