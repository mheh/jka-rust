//! Shared pmove parity harness for `pmove_parity.rs` and `pmove_saber_parity.rs`.
//!
//! The axial-brush world stub (verbatim `tools/jampgame-oracle/pmworld.h`), the
//! fixture grammar, the `TestTraps`/`TestCallbacks` seam implementations, and the
//! scenario row parser are identical between the melee and saber slices and live
//! here; each test file keeps only its divergent baseline, override table, dump
//! format, and scenario list.
...
use mp_game::bg_channel::{BgTraps, GameCallbacks};
use mp_game::prelude::*;
...
/// One axis-aligned brush of the pmove world stub.
#[derive(Clone, Copy)]
pub struct Brush {
    pub mins: [f32; 3],
    pub maxs: [f32; 3],
    pub surface_flags: c_int,
}

/// A fresh `trace_t` with `fraction == 1` (a clean miss).
...
/// Parse a fixture token as an f32: `0x…` is a raw bit pattern, else a decimal.
pub fn parse_float(tok: &str) -> f32 {
    let b = tok.as_bytes();
...
/// Parse a fixture token as a `c_int`: `0x…` is hex, else decimal.
pub fn parse_int(tok: &str) -> c_int {
    let b = tok.as_bytes();
    if b.len() >= 2 && b[0] == b'0' && (b[1] == b'x' || b[1] == b'X') {
...
/// Parse an optional `surf=<hex|dec>` brush token; missing token is `0`.
pub fn parse_surf(tok: &str) -> c_int {
    match tok.find('=') {
        Some(i) => {
            let v = &tok[i + 1..];
            let b = v.as_bytes();
...
/// Strip a trailing `#` line comment from a fixture line.
...
/// `BgTraps` world stub: axial-brush trace/pointcontents, `rintf` snap_vector,
/// and the read-only FS path that serves the synthetic `animation.cfg`.
pub struct TestTraps {
    pub brushes: Vec<Brush>,
    pub trace_count: Cell<i64>,
    fixdir: PathBuf,
    files: std::cell::RefCell<Vec<Option<(Vec<u8>, usize)>>>, // handle -> (bytes,pos)
}

impl TestTraps {
    pub fn new(fixdir: PathBuf) -> Self {
        Self {
            brushes: Vec::new(),
...
impl BgTraps for TestTraps {
    ...
    // --- everything below is off the basic pmove path ---
...
/// `GameCallbacks` stub: serves the anim restart-check reads from the prior-frame
/// anim mirror; every other entry is off the basic pmove path.
pub struct TestCallbacks {
    pub legs_mirror: c_int,
    pub torso_mirror: c_int,
}
...
impl GameCallbacks for TestCallbacks {
    ...
    fn my_saber(&mut self, _c: c_int, _s: c_int) -> *mut saberInfo_t {
        // The entity arena is zeroed, so BG_MySaber returns NULL (matches the C
        // dumper); no per-saber saberInfo is read on the basic pmove path.
        core::ptr::null_mut()
    }
...
/// Reinterpret an f32 as its raw `u32` bit pattern for bit-exact golden dumps.
pub fn f2b(v: f32) -> u32 {
    v.to_bits()
}

// ============================ scenario driver ================================

/// One `cmd` fixture row: a usercmd plus optional repeat count and yaw increment.
pub struct Cmd {
    pub dt: c_int,
    pub fwd: c_int,
    pub right: c_int,
...
/// Parse a scenario fixture file into its ordered row list.
pub fn parse_scenario(path: &PathBuf) -> Vec<Row> {
