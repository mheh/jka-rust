...
/// Raven `renderInfo_t`.
///
/// This holds per-client model-rendering state: part yaw and pitch ranges, muzzle points,
/// tag points, the look target, bolt indices, and `lastG2`.
///
/// `lastG2` is a pointer, so its layout is architecture-dependent.
/// The asserts below pin the host 64-bit layout.
///
/// Type definition source: `oracle/codemp/game/g_local.h:460-532`
#[repr(C)]
#[derive(Clone, Copy)]
