//! Port of `oracle/codemp/game/NPC_AI_Utils.c`.
//!
//! Ambient globals (`level`, `g_entities`, cvars) are reached through the `GameContext<'_>` receiver, per the engine seam
//! (`docs/architecture/engine-seam.md`).
//! `level` becomes `ctx.world.level`, `g_entities[i]` becomes `ctx.world.g_entities[i]`, and cvars become `ctx.world.cvars`.
//! Traps go through `trap::X(ctx.engine, ...)`.
//! `ent->client` and `ent->NPC` are `*mut c_void` placeholders in `gentity_t` (see the tiering note in `mp_qshared::gentity_t`).
//! Each use site casts them to `*mut gclient_t` or `*mut gNPC_t`.
//!
//! The entity half of this file uses `ctx.world.entity(id)`/`entity_mut(id)` accessor borrows at the point of use.
//! Entity parameters stay `EntityId`/`Option<EntityId>` handles.
//! Three irreducible raw-deref regimes remain, each flagged inline with a `FLAG` comment.
//! `AIGroupInfo_t` is a `level`-owned struct with no safe accessor, and it stores raw `gentity_t*` `commander`/`enemy` fields itself.
//! `gNPC_t` (`NPC`) has no accessor.
//! NPCs carry a `BG_Alloc`'d pool `gclient_t` (`gClPtrs`, `g_utils.c:430`), not a `level.clients` slot, so `ent->client` is dereffed
//! raw exactly as Raven does.
//! Parity rests on the compile and the golden test suite.
#![allow(non_snake_case, unused, clippy::all)]

use crate::prelude::*;
use crate::trap;
use crate::world::GameContext;

use mp_abi::game::syscalls::G_ENTITIES_IN_BOX::GEntitiesInBoxArgs;
use mp_abi::game::syscalls::G_IN_PVS::GInPvsArgs;
use mp_abi::game::syscalls::G_NAV_GETPATHCOST::GNavGetpathcostArgs;

use crate::ai::consts::{MAX_FRAME_GROUPS, MAX_GROUP_MEMBERS, NUM_SQUAD_STATES};
use crate::ai::group_member::AIGroupMember_t;
use crate::level::alert_event::alertEventLevel_e;

use crate::ai_wpnav::G_TestLine;
use crate::g_nav::{NAV_FindClosestWaypointForEnt, NAV_FindClosestWaypointForPoint};
use crate::g_timer::TIMER_Done;
use crate::NPC_AI_Stormtrooper::{ST_AggressionAdjust, ST_MarkToCover, ST_StartFlee};
use crate::NPC_combat::G_SetEnemy;
use crate::NPC_move::NAV_GetLastMove;

pub const MAX_RADIUS_ENTS: usize = 128;

use crate::npc::script_flags::SCF_NO_GROUPS;

// `DistanceSquared` is canonical in `crate::q_math::DistanceSquared`, reached through the prelude glob.
// This file carries no per-file copy.

/// Raven `AI_GetGroupSize`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:23-67`
pub fn AI_GetGroupSize(
    ctx: &mut GameContext,
    origin: vec3_t,
    radius: c_int,
    playerTeam: team_t,
    avoid: Option<EntityId>,
) -> c_int {
    let mut radiusEnts = [0i32; MAX_RADIUS_ENTS];
    let mut mins: vec3_t = [0.0; 3];
    let mut maxs: vec3_t = [0.0; 3];
    for i in 0..3 {
        mins[i] = origin[i] - radius as f32;
        maxs[i] = origin[i] + radius as f32;
    }

    let numEnts = trap::EntitiesInBox(
        ctx.engine,
        GEntitiesInBoxArgs::new(
            &mins as *const vec3_t,
            &maxs as *const vec3_t,
            radiusEnts.as_mut_ptr(),
            MAX_RADIUS_ENTS as c_int,
        ),
    );

    let mut realCount = 0;
    for j in 0..numEnts {
        let check = EntityId(radiusEnts[j as usize] as u32);

        // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
        let client = ctx.world.entity(check).client;
        if client.is_null() {
            continue;
        }
        if avoid == Some(check) {
            continue;
        }
        if unsafe { (*client).playerTeam } != playerTeam {
            continue;
        }
        if ctx.world.entity(check).health <= 0 {
            continue;
        }
        realCount += 1;
    }

    realCount
}

/// Raven `AI_GetGroupSize2`: overload.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:71-77`
pub fn AI_GetGroupSize2(ctx: &mut GameContext, ent: Option<EntityId>, radius: c_int) -> c_int {
    let Some(ent_id) = ent else {
        return -1;
    };
    // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
    let client = ctx.world.entity(ent_id).client;
    if client.is_null() {
        return -1;
    }
    let origin = ctx.world.entity(ent_id).r.currentOrigin;
    let playerTeam = unsafe { (*client).playerTeam };
    AI_GetGroupSize(ctx, origin, radius, playerTeam, Some(ent_id))
}

/// Raven `AI_ClosestGroupEntityNumToPoint`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:80-110`
pub fn AI_ClosestGroupEntityNumToPoint(
    ctx: &mut GameContext,
    group: *mut AIGroupInfo_t,
    point: vec3_t,
) -> c_int {
    unsafe {
        let mut markerWP = WAYPOINT_NONE;
        let mut cost: c_int;
        let mut bestCost = Q3_INFINITE;
        let mut closest = ENTITYNUM_NONE;

        if group.is_null() || (*group).numGroup <= 0 {
            return ENTITYNUM_NONE;
        }

        markerWP = NAV_FindClosestWaypointForPoint(
            ctx,
            EntityId::from_num((*group).member[0].number as c_int).unwrap(),
            point,
        );

        if markerWP == WAYPOINT_NONE {
            return ENTITYNUM_NONE;
        }

        for i in 0..(*group).numGroup {
            cost = trap::Nav_GetPathCost(
                ctx.engine,
                GNavGetpathcostArgs::new((*group).member[i as usize].waypoint, markerWP),
            );
            if cost < bestCost {
                bestCost = cost;
                closest = (*group).member[i as usize].number;
            }
        }

        closest
    }
}

/// Raven `AI_SetClosestBuddy`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:112-132`
pub fn AI_SetClosestBuddy(ctx: &mut GameContext, group: *mut AIGroupInfo_t) {
    unsafe {
        for i in 0..(*group).numGroup {
            (*group).member[i as usize].closestBuddy = ENTITYNUM_NONE;

            let mut bestDist: f32 = Q3_INFINITE as f32;
            for j in 0..(*group).numGroup {
                let dist = DistanceSquared(
                    ctx.world.g_entities[(*group).member[i as usize].number as usize]
                        .r
                        .currentOrigin,
                    ctx.world.g_entities[(*group).member[j as usize].number as usize]
                        .r
                        .currentOrigin,
                );
                if dist < bestDist {
                    bestDist = dist;
                    (*group).member[i as usize].closestBuddy = (*group).member[j as usize].number;
                }
            }
        }
    }
}

/// Raven `AI_SortGroupByPathCostToEnemy`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:134-210`
pub fn AI_SortGroupByPathCostToEnemy(ctx: &mut GameContext, group: *mut AIGroupInfo_t) {
    unsafe {
        let mut bestMembers: [AIGroupMember_t; MAX_GROUP_MEMBERS] =
            [core::mem::zeroed(); MAX_GROUP_MEMBERS];
        let mut sort = false;

        if !(*group).enemy.is_null() {
            // FIXME: just use enemy->waypoint?
            (*group).enemyWP = NAV_FindClosestWaypointForEnt(
                ctx,
                ctx.entity_id_of((*group).enemy).unwrap(),
                WAYPOINT_NONE,
            );
        } else {
            (*group).enemyWP = WAYPOINT_NONE;
        }

        for i in 0..(*group).numGroup {
            if (*group).enemyWP == WAYPOINT_NONE {
                // FIXME: just use member->waypoint?
                (*group).member[i as usize].waypoint = WAYPOINT_NONE;
                (*group).member[i as usize].pathCostToEnemy = Q3_INFINITE;
            } else {
                // FIXME: just use member->waypoint?
                (*group).member[i as usize].waypoint = NAV_FindClosestWaypointForEnt(
                    ctx,
                    ctx.entity_id_of((*group).enemy).unwrap(),
                    WAYPOINT_NONE,
                );
                if (*group).member[i as usize].waypoint != WAYPOINT_NONE {
                    (*group).member[i as usize].pathCostToEnemy = trap::Nav_GetPathCost(
                        ctx.engine,
                        GNavGetpathcostArgs::new(
                            (*group).member[i as usize].waypoint,
                            (*group).enemyWP,
                        ),
                    );
                    // at least one of us has a path, so do sorting
                    sort = true;
                } else {
                    (*group).member[i as usize].pathCostToEnemy = Q3_INFINITE;
                }
            }
        }

        // Now sort
        if sort {
            // initialize bestMembers data
            for j in 0..(*group).numGroup {
                bestMembers[j as usize].number = ENTITYNUM_NONE;
            }

            for i in 0..(*group).numGroup {
                for j in 0..(*group).numGroup {
                    if bestMembers[j as usize].number != ENTITYNUM_NONE {
                        // slot occupied
                        if (*group).member[i as usize].pathCostToEnemy
                            < bestMembers[j as usize].pathCostToEnemy
                        {
                            // this guy has a shorter path than the one currenly in this spot, bump him and put myself in here
                            // Raven's shift loop is `for (k = numGroup; k > j; k++)`, a non-terminating buffer overrun (UB).
                            // The port takes the intended downward shift (`k -= 1`), which stays in bounds because numGroup <= MAX_GROUP_MEMBERS - 1.
                            let mut k = (*group).numGroup;
                            while k > j {
                                bestMembers[k as usize] = bestMembers[(k - 1) as usize];
                                k -= 1;
                            }
                            bestMembers[j as usize] = (*group).member[i as usize];
                            break;
                        }
                    } else {
                        // slot unoccupied, reached end of list, throw self in here
                        bestMembers[j as usize] = (*group).member[i as usize];
                        break;
                    }
                }
            }

            // Okay, now bestMembers is a sorted list, just copy it into group->members
            for i in 0..(*group).numGroup {
                (*group).member[i as usize] = bestMembers[i as usize];
            }
        }
    }
}

/// Raven `AI_FindSelfInPreviousGroup`.
///
/// Raven: go through other groups made this frame and see if any of those contain me already.
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:212-230`
pub fn AI_FindSelfInPreviousGroup(ctx: &mut GameContext, self_: EntityId) -> qboolean {
    unsafe {
        for i in 0..MAX_FRAME_GROUPS {
            // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers, dereffed raw as Raven does.
            let group = &mut ctx.world.level.groups[i] as *mut AIGroupInfo_t;
            if (*group).numGroup != 0 {
                for j in 0..(*group).numGroup as usize {
                    if (*group).member[j].number == ctx.world.entity(self_).s.number {
                        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
                        let npc = ctx.world.entity(self_).NPC;
                        (*npc).group = group;
                        return 1;
                    }
                }
            }
        }
        0
    }
}

/// Raven `AI_InsertGroupMember`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:232-257`
pub fn AI_InsertGroupMember(group: *mut AIGroupInfo_t, member: &mut gentity_t) {
    // `member` is a safe `&mut gentity_t`, and its `s.number`/`NPC` fields read directly.
    // FLAG: the stored `commander` field keeps a raw `gentity_t*`, so this retains a raw pointer to `member` just for that store.
    let member_number = member.s.number;
    // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
    let npc = member.NPC;
    // FLAG: `AIGroupInfo_t` stores a raw `gentity_t*` `commander` field.
    let member_ptr: *mut gentity_t = member;
    unsafe {
        let mut i: usize = 0;
        while (i as c_int) < (*group).numGroup {
            if (*group).member[i].number == member_number {
                //already in here
                break;
            }
            i += 1;
        }
        if (i as c_int) < (*group).numGroup {
            //found him in group already
        } else {
            //add him in
            let idx = (*group).numGroup as usize;
            (*group).member[idx].number = member_number;
            (*group).numGroup += 1;
            (*group).numState[(*npc).squadState as usize] += 1;
        }
        let commanderRank = if (*group).commander.is_null() {
            RANK_CIVILIAN
        } else {
            let cnpc = (*(*group).commander).NPC;
            (*cnpc).rank
        };
        if (*group).commander.is_null() || (*npc).rank > commanderRank {
            //keep track of highest rank
            (*group).commander = member_ptr;
        }
        (*npc).group = group;
    }
}

/// Raven `AI_TryJoinPreviousGroup`.
///
/// Raven: go through other groups made this frame and see if any of those have the same enemy as me... if so, add me in!
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:259-277`
pub fn AI_TryJoinPreviousGroup(ctx: &mut GameContext, self_: EntityId) -> qboolean {
    unsafe {
        for i in 0..MAX_FRAME_GROUPS {
            // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
            let group = &mut ctx.world.level.groups[i] as *mut AIGroupInfo_t;
            let self_enemy_ptr = match ctx.world.entity(self_).enemy {
                Some(id) => &mut ctx.world.g_entities[id.index()] as *mut gentity_t,
                None => core::ptr::null_mut(),
            };
            if (*group).numGroup != 0
                && (*group).numGroup < (MAX_GROUP_MEMBERS as c_int - 1)
                && (*group).enemy == self_enemy_ptr
            {
                //has members, not full and has my enemy
                if AI_ValidateGroupMember(ctx, group, Some(self_)) != 0 {
                    //I am a valid member for this group
                    let self_ent = &mut ctx.world.g_entities[self_.index()];
                    AI_InsertGroupMember(group, self_ent);
                    return 1;
                }
            }
        }
        0
    }
}

/// Raven `AI_GetNextEmptyGroup`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:279-308`
pub fn AI_GetNextEmptyGroup(ctx: &mut GameContext, self_: EntityId) -> qboolean {
    unsafe {
        if AI_FindSelfInPreviousGroup(ctx, self_) != 0 {
            //already in one, no need to make a new one
            return 0;
        }

        if AI_TryJoinPreviousGroup(ctx, self_) != 0 {
            //try to just put us in one that already exists
            return 0;
        }

        //okay, make a whole new one, then
        for i in 0..MAX_FRAME_GROUPS {
            // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
            let group = &mut ctx.world.level.groups[i] as *mut AIGroupInfo_t;
            if (*group).numGroup == 0 {
                //make a new one
                // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
                let npc = ctx.world.entity(self_).NPC;
                (*npc).group = group;
                return 1;
            }
        }

        //WTF?  Out of groups!
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let npc = ctx.world.entity(self_).NPC;
        (*npc).group = core::ptr::null_mut();
        0
    }
}

/// Raven `AI_ValidateNoEnemyGroupMember`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:310-340`
pub fn AI_ValidateNoEnemyGroupMember(
    ctx: &mut GameContext,
    group: *mut AIGroupInfo_t,
    member: Option<EntityId>,
) -> qboolean {
    unsafe {
        let mut center: vec3_t = [0.0; 3];

        if group.is_null() {
            return 0;
        }
        // FLAG: `AIGroupInfo_t` stores a raw `gentity_t*` `commander` field.
        if !(*group).commander.is_null() {
            let commander_id = ctx.entity_id_of((*group).commander).unwrap();
            crate::q_math::_VectorCopy(ctx.world.entity(commander_id).r.currentOrigin, &mut center);
        } else {
            // hmm, just pick the first member
            if (*group).member[0].number < 0 || (*group).member[0].number >= ENTITYNUM_WORLD {
                return 0;
            }
            crate::q_math::_VectorCopy(
                ctx.world.g_entities[(*group).member[0].number as usize]
                    .r
                    .currentOrigin,
                &mut center,
            );
        }
        let member_id = member.unwrap();
        let member_origin = ctx.world.entity(member_id).r.currentOrigin;
        // FIXME: maybe it should be based on the center of the mass of the group, not the commander?
        if DistanceSquared(center, member_origin) > 147456.0 {
            // 384*384
            return 0;
        }
        if trap::InPVS(
            ctx.engine,
            GInPvsArgs::new(&member_origin as *const vec3_t, &center as *const vec3_t),
        ) == 0
        {
            // not within PVS of the group enemy
            return 0;
        }
        1
    }
}

/// Raven `AI_ValidateGroupMember`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:342-435`
pub fn AI_ValidateGroupMember(
    ctx: &mut GameContext,
    group: *mut AIGroupInfo_t,
    member: Option<EntityId>,
) -> qboolean {
    unsafe {
        //Validate ents
        let Some(member_id) = member else {
            return 0;
        };

        //Validate clients
        // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
        let client = ctx.world.entity(member_id).client;
        if client.is_null() {
            return 0;
        }

        //Validate NPCs
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let npc = ctx.world.entity(member_id).NPC;
        if npc.is_null() {
            return 0;
        }

        //must be aware
        if (*npc).confusionTime > ctx.world.level.time {
            return 0;
        }

        //must be allowed to join groups
        if (*npc).scriptFlags & SCF_NO_GROUPS != 0 {
            return 0;
        }

        //Must not be in another group
        if !(*npc).group.is_null() && (*npc).group != group {
            //FIXME: if that group's enemy is mine, why not absorb that group into mine?
            return 0;
        }

        //Must be alive
        if ctx.world.entity(member_id).health <= 0 {
            return 0;
        }

        //can't be in an emplaced gun
        //rwwFIXMEFIXME: support this flag

        //Must be on the same team
        if (*client).playerTeam != (*group).team {
            return 0;
        }

        let weapon = (*client).ps.weapon;
        if weapon == mp_bg::weapons::weapon_t::WP_SABER
            || weapon == mp_bg::weapons::weapon_t::WP_THERMAL
            || weapon == mp_bg::weapons::weapon_t::WP_DISRUPTOR
            || weapon == mp_bg::weapons::weapon_t::WP_EMPLACED_GUN
            || weapon == mp_bg::weapons::weapon_t::WP_STUN_BATON
            || weapon == mp_bg::weapons::weapon_t::WP_TURRET
        {
            //not really a squad-type guy
            return 0;
        }

        let npc_class = (*client).NPC_class;
        if npc_class == class_t::CLASS_ATST
            || npc_class == class_t::CLASS_PROBE
            || npc_class == class_t::CLASS_SEEKER
            || npc_class == class_t::CLASS_REMOTE
            || npc_class == class_t::CLASS_SENTRY
            || npc_class == class_t::CLASS_INTERROGATOR
            || npc_class == class_t::CLASS_MINEMONSTER
            || npc_class == class_t::CLASS_HOWLER
            || npc_class == class_t::CLASS_MARK1
            || npc_class == class_t::CLASS_MARK2
        {
            //these kinds of enemies don't actually use this group AI
            return 0;
        }

        //should have same enemy
        let member_enemy = ctx.world.entity(member_id).enemy;
        let member_enemy_ptr = match member_enemy {
            Some(id) => &mut ctx.world.g_entities[id.index()] as *mut gentity_t,
            None => core::ptr::null_mut(),
        };
        if member_enemy_ptr != (*group).enemy {
            if !member_enemy.is_none() {
                //he's fighting someone else, leave him out
                return 0;
            }
            let group_enemy_id = ctx.entity_id_of((*group).enemy).unwrap();
            let groupEnemyOrigin = ctx.world.entity(group_enemy_id).r.currentOrigin;
            let member_origin = ctx.world.entity(member_id).r.currentOrigin;
            if trap::InPVS(
                ctx.engine,
                GInPvsArgs::new(
                    &member_origin as *const vec3_t,
                    &groupEnemyOrigin as *const vec3_t,
                ),
            ) == 0
            {
                //not within PVS of the group enemy
                return 0;
            }
        } else if (*group).enemy.is_null() {
            //if the group is a patrol group, only take those within the room and radius
            if AI_ValidateNoEnemyGroupMember(ctx, group, Some(member_id)) == 0 {
                return 0;
            }
        }
        //must be actually in combat mode
        if TIMER_Done(ctx, Some(member_id), c"interrogating".as_ptr()) == 0 {
            return 0;
        }
        //FIXME: need to have a route to enemy and/or clear shot?
        1
    }
}

/// Raven `AI_GetGroup`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:443-551`
pub fn AI_GetGroup(ctx: &mut GameContext, self_: Option<EntityId>) {
    unsafe {
        let Some(self_id) = self_ else {
            return;
        };
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let npc = ctx.world.entity(self_id).NPC;
        if npc.is_null() {
            return;
        }

        if ctx.world.cvars.d_noGroupAI.integer != 0 {
            (*npc).group = core::ptr::null_mut();
            return;
        }

        // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
        let client = ctx.world.entity(self_id).client;
        if client.is_null() {
            (*npc).group = core::ptr::null_mut();
            return;
        }

        if (*npc).scriptFlags & SCF_NO_GROUPS != 0 {
            (*npc).group = core::ptr::null_mut();
            return;
        }

        if let Some(enemy_id) = ctx.world.entity(self_id).enemy {
            let enemyClient = ctx.world.entity(enemy_id).client;
            if enemyClient.is_null() || (ctx.world.level.time - (*npc).enemyLastSeenTime > 7000) {
                (*npc).group = core::ptr::null_mut();
                return;
            }
        }

        if AI_GetNextEmptyGroup(ctx, self_id) == 0 {
            //either no more groups left or we're already in a group built earlier
            return;
        }

        //create a new one
        // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
        let group = (*npc).group;
        *group = core::mem::zeroed::<AIGroupInfo_t>();

        (*group).enemy = match ctx.world.entity(self_id).enemy {
            Some(id) => &mut ctx.world.g_entities[id.index()] as *mut gentity_t,
            None => core::ptr::null_mut(),
        };
        (*group).team = (*client).playerTeam;
        (*group).processed = 0;
        // FLAG: `AIGroupInfo_t` stores a raw `gentity_t*` `commander` field.
        (*group).commander = &mut ctx.world.g_entities[self_id.index()] as *mut gentity_t;
        (*group).memberValidateTime = ctx.world.level.time + 2000;
        (*group).activeMemberNum = 0;

        if !(*group).enemy.is_null() {
            (*group).lastSeenEnemyTime = ctx.world.level.time;
            (*group).lastClearShotTime = ctx.world.level.time;
            let group_enemy_id = ctx.entity_id_of((*group).enemy).unwrap();
            (*group).enemyLastSeenPos = ctx.world.entity(group_enemy_id).r.currentOrigin;
        }

        let numEntities = ctx.world.level.num_entities;
        for i in 0..numEntities {
            let member_id = EntityId(i as u32);

            if ctx.world.entity(member_id).inuse == 0 {
                continue;
            }

            if AI_ValidateGroupMember(ctx, group, Some(member_id)) == 0 {
                //FIXME: keep track of those who aren't angry yet and see if we should wake them after we assemble the core group
                continue;
            }

            //store it
            let member_ent = &mut ctx.world.g_entities[member_id.index()];
            AI_InsertGroupMember(group, member_ent);

            if (*group).numGroup >= (MAX_GROUP_MEMBERS as c_int - 1) {
                //full
                break;
            }
        }

        if (*group).numGroup <= 0 {
            //none in group
            (*npc).group = core::ptr::null_mut();
            return;
        }

        AI_SortGroupByPathCostToEnemy(ctx, group);
        AI_SetClosestBuddy(ctx, group);
    }
}

/// Raven `AI_SetNewGroupCommander`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:553-568`
pub fn AI_SetNewGroupCommander(ctx: &mut GameContext, group: *mut AIGroupInfo_t) {
    unsafe {
        // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
        (*group).commander = core::ptr::null_mut();
        for i in 0..(*group).numGroup as usize {
            let member_id = EntityId((*group).member[i].number as u32);
            let member_ptr = &mut ctx.world.g_entities[member_id.index()] as *mut gentity_t;

            let commanderNpc = if (*group).commander.is_null() {
                core::ptr::null_mut()
            } else {
                let commander_id = ctx.entity_id_of((*group).commander).unwrap();
                ctx.world.entity(commander_id).NPC
            };
            // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
            let memberNpc = ctx.world.entity(member_id).NPC;
            if (*group).commander.is_null()
                || (!member_ptr.is_null()
                    && !memberNpc.is_null()
                    && !commanderNpc.is_null()
                    && (*memberNpc).rank > (*commanderNpc).rank)
            {
                //keep track of highest rank
                (*group).commander = member_ptr;
            }
        }
    }
}

/// Raven `AI_DeleteGroupMember`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:570-600`
pub fn AI_DeleteGroupMember(ctx: &mut GameContext, group: *mut AIGroupInfo_t, memberNum: c_int) {
    unsafe {
        // FLAG: `AIGroupInfo_t` stores a raw `gentity_t*` `commander` field.
        if !(*group).commander.is_null() {
            let commander_id = ctx.entity_id_of((*group).commander).unwrap();
            if ctx.world.entity(commander_id).s.number == (*group).member[memberNum as usize].number
            {
                (*group).commander = core::ptr::null_mut();
            }
        }
        let ent_id = EntityId((*group).member[memberNum as usize].number as u32);
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let ent_npc = ctx.world.entity(ent_id).NPC;
        if !ent_npc.is_null() {
            (*ent_npc).group = core::ptr::null_mut();
        }
        let mut i = memberNum;
        while i < (*group).numGroup - 1 {
            (*group).member[i as usize] = (*group).member[(i + 1) as usize];
            i += 1;
        }
        if memberNum < (*group).activeMemberNum {
            (*group).activeMemberNum -= 1;
            if (*group).activeMemberNum < 0 {
                (*group).activeMemberNum = 0;
            }
        }
        (*group).numGroup -= 1;
        if (*group).numGroup < 0 {
            (*group).numGroup = 0;
        }
        AI_SetNewGroupCommander(ctx, group);
    }
}

/// Raven `AI_DeleteSelfFromGroup`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:602-615`
pub fn AI_DeleteSelfFromGroup(ctx: &mut GameContext, self_: EntityId) {
    unsafe {
        //FIXME: if killed, keep track of how many in group killed?  To affect morale?
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let npc = ctx.world.entity(self_).NPC;
        // FLAG: `AIGroupInfo_t` is `level`-owned and has no accessor.
        let group = (*npc).group;
        for i in 0..(*group).numGroup as usize {
            if (*group).member[i].number == ctx.world.entity(self_).s.number {
                AI_DeleteGroupMember(ctx, group, i as c_int);
                return;
            }
        }
    }
}

/// Raven `AI_GroupMemberKilled`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:620-697`
pub fn AI_GroupMemberKilled(ctx: &mut GameContext, self_: Option<EntityId>) {
    unsafe {
        let Some(self_id) = self_ else {
            return;
        };
        // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
        let selfNpc = ctx.world.entity(self_id).NPC;
        // FLAG: `AIGroupInfo_t` is `level`-owned and has no accessor.
        let group = if selfNpc.is_null() {
            core::ptr::null_mut()
        } else {
            (*selfNpc).group
        };

        if group.is_null() {
            //what group?
            return;
        }
        if selfNpc.is_null() || (*selfNpc).rank < RANK_ENSIGN {
            //I'm not an officer, let's not really care for now
            return;
        }
        //temporarily drop group morale for a few seconds
        (*group).moraleAdjust -= (*selfNpc).rank as c_int;
        //go through and drop aggression on my teammates (more cover, worse aim)
        let mut noflee = false;
        for i in 0..(*group).numGroup as usize {
            let member_id = EntityId((*group).member[i].number as u32);
            if member_id == self_id {
                continue;
            }
            // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
            let memberNpc = ctx.world.entity(member_id).NPC;
            if (*memberNpc).rank > RANK_ENSIGN {
                //officers do not panic
                noflee = true;
            } else {
                ST_AggressionAdjust(ctx.world.entity(member_id), -1);
                (*memberNpc).currentAim -= ctx.world.bg_state.rng.Q_irand(0, 10);
            }
        }
        //okay, if I'm the group commander, make everyone else flee
        if ctx.entity_id_of((*group).commander) != Some(self_id) {
            //I'm not the commander... hmm, should maybe a couple flee... maybe those near me?
            return;
        }
        //now see if there is another of sufficient rank to keep them from fleeing
        if !noflee {
            (*group).speechDebounceTime = 0;
            for i in 0..(*group).numGroup as usize {
                let member_id = EntityId((*group).member[i].number as u32);
                if member_id == self_id {
                    continue;
                }
                // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
                let memberNpc = ctx.world.entity(member_id).NPC;
                if (*memberNpc).rank < RANK_ENSIGN {
                    //grunt
                    let member_origin = ctx.world.entity(member_id).r.currentOrigin;
                    let group_enemy = ctx.entity_id_of((*group).enemy);
                    if !(*group).enemy.is_null()
                        && DistanceSquared(
                            member_origin,
                            ctx.world.entity(group_enemy.unwrap()).r.currentOrigin,
                        ) < 65536.0
                    {
                        //those close to enemy run away!
                        ST_StartFlee(
                            ctx,
                            Some(member_id),
                            group_enemy,
                            member_origin,
                            alertEventLevel_e::AEL_DANGER_GREAT as c_int,
                            3000,
                            5000,
                        );
                    } else if DistanceSquared(
                        member_origin,
                        ctx.world.entity(self_id).r.currentOrigin,
                    ) < 65536.0
                    {
                        //those close to me run away!
                        ST_StartFlee(
                            ctx,
                            Some(member_id),
                            group_enemy,
                            member_origin,
                            alertEventLevel_e::AEL_DANGER_GREAT as c_int,
                            3000,
                            5000,
                        );
                    } else {
                        //else, maybe just a random chance
                        if ctx.world.bg_state.rng.Q_irand(0, (*selfNpc).rank as c_int)
                            > (*memberNpc).rank as c_int
                        {
                            //lower rank they are, higher rank I am, more likely they are to flee
                            ST_StartFlee(
                                ctx,
                                Some(member_id),
                                group_enemy,
                                member_origin,
                                alertEventLevel_e::AEL_DANGER_GREAT as c_int,
                                3000,
                                5000,
                            );
                        } else {
                            ST_MarkToCover(ctx, Some(member_id));
                        }
                    }
                    (*memberNpc).currentAim -= ctx.world.bg_state.rng.Q_irand(1, 15);
                }
                (*memberNpc).currentAim -= ctx.world.bg_state.rng.Q_irand(1, 15);
            }
        }
    }
}

/// Raven `AI_GroupUpdateEnemyLastSeen`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:699-707`
pub fn AI_GroupUpdateEnemyLastSeen(ctx: &mut GameContext, group: *mut AIGroupInfo_t, spot: vec3_t) {
    unsafe {
        if group.is_null() {
            return;
        }
        (*group).lastSeenEnemyTime = ctx.world.level.time;
        (*group).enemyLastSeenPos = spot;
    }
}

/// Raven `AI_GroupUpdateClearShotTime`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:709-716`
pub fn AI_GroupUpdateClearShotTime(ctx: &mut GameContext, group: *mut AIGroupInfo_t) {
    unsafe {
        if group.is_null() {
            return;
        }
        (*group).lastClearShotTime = ctx.world.level.time;
    }
}

/// Raven `AI_GroupUpdateSquadstates`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:718-738`
pub fn AI_GroupUpdateSquadstates(
    group: *mut AIGroupInfo_t,
    member: &mut gentity_t,
    newSquadState: c_int,
) {
    // `member` is a safe `&mut gentity_t`, and `s.number` reads directly.
    // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
    let npc = member.NPC;
    let member_number = member.s.number;
    unsafe {
        if group.is_null() {
            (*npc).squadState = newSquadState;
            return;
        }

        for i in 0..(*group).numGroup as usize {
            if (*group).member[i].number == member_number {
                (*group).numState[(*npc).squadState as usize] -= 1;
                (*npc).squadState = newSquadState;
                (*group).numState[(*npc).squadState as usize] += 1;
                return;
            }
        }
    }
}

/// Raven `AI_RefreshGroup`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:740-962`
pub fn AI_RefreshGroup(ctx: &mut GameContext, group: *mut AIGroupInfo_t) -> qboolean {
    unsafe {
        //see if we should merge with another group
        for i in 0..MAX_FRAME_GROUPS {
            // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
            let other = &mut ctx.world.level.groups[i] as *mut AIGroupInfo_t;
            if other == group {
                break;
            } else if (*other).enemy == (*group).enemy {
                //2 groups with same enemy
                if (*other).numGroup + (*group).numGroup < (MAX_GROUP_MEMBERS as c_int - 1) {
                    //combining the members would fit in one group
                    let mut deleteWhenDone = true;
                    let mut j: c_int = 0;

                    //combine the members of mine into theirs
                    while j < (*group).numGroup {
                        let member_id = EntityId((*group).member[j as usize].number as u32);
                        if (*other).enemy.is_null() {
                            //special case for groups without enemies, must be in range
                            if AI_ValidateNoEnemyGroupMember(ctx, other, Some(member_id)) == 0 {
                                deleteWhenDone = false;
                                j += 1;
                                continue;
                            }
                        }
                        //remove this member from this group
                        AI_DeleteGroupMember(ctx, group, j);
                        //keep marker at same place since we deleted this guy and shifted everyone up one
                        j -= 1;
                        //add them to the earlier group
                        let member_ent = &mut ctx.world.g_entities[member_id.index()];
                        AI_InsertGroupMember(other, member_ent);
                        j += 1;
                    }
                    //return and delete this group
                    if deleteWhenDone {
                        return 0;
                    }
                }
            }
        }
        //clear numStates
        for i in 0..NUM_SQUAD_STATES {
            (*group).numState[i] = 0;
        }

        //go through group and validate each membership
        (*group).commander = core::ptr::null_mut();
        let mut i: c_int = 0;
        while i < (*group).numGroup {
            let member_id = EntityId((*group).member[i as usize].number as u32);

            //Must be alive
            if ctx.world.entity(member_id).health <= 0 {
                AI_DeleteGroupMember(ctx, group, i);
                //keep marker at same place since we deleted this guy and shifted everyone up one
                i -= 1;
            } else if (*group).memberValidateTime < ctx.world.level.time
                && AI_ValidateGroupMember(ctx, group, Some(member_id)) == 0
            {
                //remove this one from the group
                AI_DeleteGroupMember(ctx, group, i);
                //keep marker at same place since we deleted this guy and shifted everyone up one
                i -= 1;
            } else {
                //membership is valid
                //keep track of squadStates
                // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
                let npc = ctx.world.entity(member_id).NPC;
                (*group).numState[(*npc).squadState as usize] += 1;
                let commanderRank = if (*group).commander.is_null() {
                    RANK_CIVILIAN
                } else {
                    let commander_id = ctx.entity_id_of((*group).commander).unwrap();
                    let cnpc = ctx.world.entity(commander_id).NPC;
                    (*cnpc).rank
                };
                if (*group).commander.is_null() || (*npc).rank > commanderRank {
                    //keep track of highest rank
                    (*group).commander =
                        &mut ctx.world.g_entities[member_id.index()] as *mut gentity_t;
                }
            }
            i += 1;
        }
        if (*group).memberValidateTime < ctx.world.level.time {
            (*group).memberValidateTime =
                ctx.world.level.time + ctx.world.bg_state.rng.Q_irand(500, 2500);
        }

        //calc the morale of this group
        (*group).morale = (*group).moraleAdjust;
        for i in 0..(*group).numGroup as usize {
            let member_id = EntityId((*group).member[i].number as u32);
            // FLAG: `gNPC_t` (`NPC`) has no accessor, so this derefs it raw.
            let npc = ctx.world.entity(member_id).NPC;
            if (*npc).rank < RANK_ENSIGN {
                //grunts
                (*group).morale += 1;
            } else {
                (*group).morale += (*npc).rank as c_int;
            }
            if !(*group).commander.is_null() && ctx.world.cvars.debugNPCAI.integer != 0 {
                let commander_id = ctx.entity_id_of((*group).commander).unwrap();
                let commander_origin = ctx.world.entity(commander_id).r.currentOrigin;
                let member_origin = ctx.world.entity(member_id).r.currentOrigin;
                G_TestLine(ctx, commander_origin, member_origin, 0x0000ff, FRAMETIME);
            }
        }
        if !(*group).enemy.is_null() {
            //modify morale based on enemy health and weapon
            let enemy_id = ctx.entity_id_of((*group).enemy).unwrap();
            let enemy_health = ctx.world.entity(enemy_id).health;
            if enemy_health < 10 {
                (*group).morale += 10;
            } else if enemy_health < 25 {
                (*group).morale += 5;
            } else if enemy_health < 50 {
                (*group).morale += 2;
            }
            let weapon = ctx.world.entity(enemy_id).s.weapon;
            if weapon == mp_bg::weapons::weapon_t::WP_SABER {
                (*group).morale -= 5;
            } else if weapon == mp_bg::weapons::weapon_t::WP_BRYAR_PISTOL {
                (*group).morale += 3;
            } else if weapon == mp_bg::weapons::weapon_t::WP_DISRUPTOR {
                (*group).morale += 2;
            } else if weapon == mp_bg::weapons::weapon_t::WP_REPEATER {
                (*group).morale -= 1;
            } else if weapon == mp_bg::weapons::weapon_t::WP_FLECHETTE {
                (*group).morale -= 2;
            } else if weapon == mp_bg::weapons::weapon_t::WP_ROCKET_LAUNCHER {
                (*group).morale -= 10;
            } else if weapon == mp_bg::weapons::weapon_t::WP_THERMAL {
                (*group).morale -= 5;
            } else if weapon == mp_bg::weapons::weapon_t::WP_TRIP_MINE {
                (*group).morale -= 3;
            } else if weapon == mp_bg::weapons::weapon_t::WP_DET_PACK {
                (*group).morale -= 10;
            } else if weapon == mp_bg::weapons::weapon_t::WP_STUN_BATON {
                (*group).morale += 10;
            } else if weapon == mp_bg::weapons::weapon_t::WP_EMPLACED_GUN {
                (*group).morale -= 8;
            }
        }
        if (*group).moraleDebounce < ctx.world.level.time {
            //slowly degrade whatever moraleAdjusters we may have
            if (*group).moraleAdjust > 0 {
                (*group).moraleAdjust -= 1;
            } else if (*group).moraleAdjust < 0 {
                (*group).moraleAdjust += 1;
            }
            (*group).moraleDebounce = ctx.world.level.time + 1000; //FIXME: define?
        }
        //mark this group as not having been run this frame
        (*group).processed = 0;

        ((*group).numGroup > 0) as qboolean
    }
}

/// Raven `AI_UpdateGroups`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:964-980`
pub fn AI_UpdateGroups(ctx: &mut GameContext) {
    unsafe {
        if ctx.world.cvars.d_noGroupAI.integer != 0 {
            return;
        }
        //Clear all Groups
        for i in 0..MAX_FRAME_GROUPS {
            // FLAG: `AIGroupInfo_t` is `level`-owned, has no accessor, and stores raw entity pointers.
            let group = &mut ctx.world.level.groups[i] as *mut AIGroupInfo_t;
            if (*group).numGroup == 0 || AI_RefreshGroup(ctx, group) == 0 {
                *group = core::mem::zeroed::<AIGroupInfo_t>();
            }
        }
    }
}

/// Raven `AI_GroupContainsEntNum`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:982-998`
pub fn AI_GroupContainsEntNum(group: *mut AIGroupInfo_t, entNum: c_int) -> qboolean {
    unsafe {
        if group.is_null() {
            return 0;
        }
        for i in 0..(*group).numGroup as usize {
            if (*group).member[i].number == entNum {
                return 1;
            }
        }
        0
    }
}

/// Raven `AI_CheckEnemyCollision`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:1030-1055`
pub fn AI_CheckEnemyCollision(
    ctx: &mut GameContext,
    ent: Option<EntityId>,
    takeEnemy: qboolean,
) -> qboolean {
    unsafe {
        let Some(ent_id) = ent else {
            return 0;
        };

        let mut info: navInfo_t = core::mem::zeroed();
        NAV_GetLastMove(ctx, &mut info as *mut navInfo_t);

        //See if we've hit something
        if !info.blocker.is_null()
            && ctx.entity_id_of(info.blocker) != ctx.world.entity(ent_id).enemy
        {
            let blocker_id = ctx.entity_id_of(info.blocker).unwrap();
            // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
            let blockerClient = ctx.world.entity(blocker_id).client;
            let entClient = ctx.world.entity(ent_id).client;
            if !blockerClient.is_null() && (*blockerClient).playerTeam == (*entClient).enemyTeam {
                if takeEnemy != 0 {
                    G_SetEnemy(ctx, ent_id, ctx.entity_id_of(info.blocker));
                }

                return 1;
            }
        }

        0
    }
}

/// Raven `AI_DistributeAttack`.
///
/// Source: `oracle/codemp/game/NPC_AI_Utils.c:1065-1139`
pub fn AI_DistributeAttack(
    ctx: &mut GameContext,
    attacker: Option<EntityId>,
    enemy: Option<EntityId>,
    team: team_t,
    threshold: c_int,
) -> *mut gentity_t {
    let enemy_id = enemy.unwrap();
    unsafe {
        //Don't take new targets
        let enemy_origin = ctx.world.entity(enemy_id).r.currentOrigin;
        let numSurrounding = AI_GetGroupSize(ctx, enemy_origin, 48, team, attacker);

        //First, see if we should look for the player
        if enemy_id != EntityId(0) {
            //rwwFIXMEFIXME: care about all clients not just 0
            let world0_origin = ctx.world.entity(EntityId(0)).r.currentOrigin;
            let aroundPlayer = AI_GetGroupSize(ctx, world0_origin, 48, team, attacker);

            //See if we're above our threshold
            if aroundPlayer < threshold {
                return &mut ctx.world.g_entities[0] as *mut gentity_t;
            }
        }

        //See if our current enemy is still ok
        if numSurrounding < threshold {
            return &mut ctx.world.g_entities[enemy_id.index()] as *mut gentity_t;
        }

        //Otherwise we need to take a new enemy if possible

        //Setup the bbox to search in
        let mut mins: vec3_t = [0.0; 3];
        let mut maxs: vec3_t = [0.0; 3];
        for i in 0..3 {
            mins[i] = enemy_origin[i] - 512.0;
            maxs[i] = enemy_origin[i] + 512.0;
        }

        //Get the number of entities in a given space
        let mut radiusEnts = [0i32; MAX_RADIUS_ENTS];
        let numEnts = trap::EntitiesInBox(
            ctx.engine,
            GEntitiesInBoxArgs::new(
                &mins as *const vec3_t,
                &maxs as *const vec3_t,
                radiusEnts.as_mut_ptr(),
                MAX_RADIUS_ENTS as c_int,
            ),
        );

        //Cull this list
        for j in 0..numEnts {
            let check = EntityId(radiusEnts[j as usize] as u32);

            //Validate clients
            // FLAG: NPC pool `gclient_t` (`gClPtrs`) dereffed raw as Raven does.
            let checkClient = ctx.world.entity(check).client;
            if checkClient.is_null() {
                continue;
            }

            //Skip the requested avoid ent if present
            if check == enemy_id {
                continue;
            }

            let enemyClient = ctx.world.entity(enemy_id).client;
            //Must be on the same team
            if (*checkClient).playerTeam != (*enemyClient).playerTeam {
                continue;
            }

            //Must be alive
            if ctx.world.entity(check).health <= 0 {
                continue;
            }

            //Must not be overwhelmed
            let check_origin = ctx.world.entity(check).r.currentOrigin;
            if AI_GetGroupSize(ctx, check_origin, 48, team, attacker) > threshold {
                continue;
            }

            return &mut ctx.world.g_entities[check.index()] as *mut gentity_t;
        }

        core::ptr::null_mut()
    }
}
