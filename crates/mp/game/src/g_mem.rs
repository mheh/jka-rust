...
//!
//! Raven's file-scope `static memoryPool`/`allocPoint` become `GameWorld` fields.
//! `G_Alloc` reaches them through the `ctx.world` channel the rest of the module already threads.
//! There is no global or `static mut` (porting-rules §B3).
//! `G_InitMemory` and `Svcmd_GameMem_f` already took `ctx`.
//!
//! After the prefix-string arena landed (the `G_NewString` deletion), the pool's only remaining consumer is
//! the ICARUS `parms_t` block (`g_ICARUScb.rs`).
//! The string half of `G_Alloc` moved to `GameWorld::prefixStrings`.
//! The pool itself is unchanged.
#![allow(non_snake_case, unused, clippy::all)]

use crate::prelude::*;

/// Raven `G_Alloc`, the game-tier bump allocator over the module island's `memoryPool`/`allocPoint`.
/// Raven's file-scope `static memoryPool` becomes a `GameWorld` field reached through `ctx.world`.
/// The pool fields already existed on `GameWorld`, and only the accessor was missing.
///
...
pub fn G_Alloc(ctx: &mut GameContext, size: c_int) -> *mut c_void {
    ...
    // Raven's `#define POOLSIZE (256 * 1024)` is local to `g_mem.c`.
