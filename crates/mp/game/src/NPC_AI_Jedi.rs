...
pub fn NPC_Jedi_RateNewEnemy(ctx: &mut GameContext, self_: EntityId, enemy: Option<EntityId>) {
    ...
    unsafe {
        ...
        newAggression =
            ((healthAggression + weaponAggression + (*((*self_).NPC)).stats.aggression as f32)
                / 3.0)
                .ceil() as c_int;
...
pub fn Jedi_Retreat(ctx: &mut GameContext) {
    unsafe {
        ...
        Jedi_Move(ctx, ctx.entity_id_of(enemy), qtrue);
    }
}
...
pub fn Jedi_SaberBlockGo(
    ...
    unsafe {
        // STAGE-1: EntityId/Option params, raw body re-derived verbatim (Stage-2
        ...
        if incoming.is_null() {
            ...
            if (*client).ps.saberInFlight != qfalse {
                ...
                saberBusy = qtrue;
            } else if Jedi_QuickReactions(ctx, ctx.entity_id_of(self_).unwrap()) != qfalse {
            ...
            || (zdiff < -36.0 && (zdiff < -44.0 || ctx.world.bg_state.rng.Q_irand(0, 2) == 0))
        {
            ...
            if (*client).ps.groundEntityNum == ENTITYNUM_NONE {
                ...
                evasionType =
                    Jedi_CheckFlipEvasions(ctx, ctx.entity_id_of(self_).unwrap(), rightdot, zdiff);
