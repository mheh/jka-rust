...
// Raven's `qboolean` is `c_int`. We keep the source spelling at assignment sites.
...
pub fn SP_misc_bsp(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        // This is faithful UB per porting-rules §19.
        // We keep the one Raven-defined behavior instead of inventing a fix.
...
// Raven's `goto justthink;` is ported as an early return after we inline the shared tail.
...
pub fn shield_power_converter_use(
    ...
    if gametype == GT_SIEGE && !other_cl.is_null() && unsafe { (*other_cl).siegeClass } != 0 {
        ...
        if ctx.world.bg_state.bgSiegeClasses[sc as usize].maxarmor == 0 {
            // can't use it!
            let snd = G_SoundIndex(ctx, "sound/interface/shieldcon_empty");
    ...
    if set_time < level_time {
        ...
        if ctx.world.entity(self_).s.loopSound == 0 {
            let snd = G_SoundIndex(ctx, "sound/interface/shieldcon_run");
            let e = ctx.world.entity_mut(self_);
            e.s.loopSound = snd;
            e.s.loopIsSoundset = qfalse;
        }
...
pub fn ammo_generic_power_converter_use(
    ...
    if set_time < level_time {
        ...
        if ctx.world.entity(self_).s.loopSound == 0 {
            let snd = G_SoundIndex(ctx, "sound/interface/ammocon_run");
            let e = ctx.world.entity_mut(self_);
            e.s.loopSound = snd;
            e.s.loopIsSoundset = qfalse;
        }
    ...
    if stop || ctx.world.entity(self_).count <= 0 {
        ...
        if loop_sound != 0 && set_time < level_time {
            if ctx.world.entity(self_).count <= 0 {
                let snd = G_SoundIndex(ctx, "sound/interface/ammocon_empty");
                G_Sound(ctx, Some(self_), CHAN_AUTO as c_int, snd);
            } else {
                let gv7 = ctx.world.entity(self_).genericValue7;
                G_Sound(ctx, Some(self_), CHAN_AUTO as c_int, gv7);
...
pub fn SP_misc_ammo_floor_unit(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        let model = ctx.world.entity(ent).model.clone();
        let mi = G_ModelIndex(ctx, model.as_deref().unwrap_or(""));
        {
            let e = ctx.world.entity_mut(ent);
            e.s.modelindex = mi;
        ...
        G_SoundIndex(ctx, "sound/interface/ammocon_run");
        let snd = G_SoundIndex(ctx, "sound/interface/ammocon_done");
        ctx.world.entity_mut(ent).genericValue7 = snd;
        G_SoundIndex(ctx, "sound/interface/ammocon_empty");
...
pub fn SP_misc_shield_floor_unit(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        G_SoundIndex(ctx, "sound/interface/shieldcon_run");
        let snd = G_SoundIndex(ctx, "sound/interface/shieldcon_done");
        ctx.world.entity_mut(ent).genericValue7 = snd;
        G_SoundIndex(ctx, "sound/interface/shieldcon_empty");
...
pub fn SP_misc_model_shield_power_converter(ctx: &mut GameContext, ent: EntityId) {
    ...
    let mi2 = G_ModelIndex(ctx, "/models/items/psd_big.md3");
...
pub fn ammo_power_converter_use(
    ...
    if set_time < level_time {
        if ctx.world.entity(self_).s.loopSound == 0 {
            let snd = G_SoundIndex(ctx, "sound/player/pickupshield.wav");
...
pub fn health_power_converter_use(
    ...
    if set_time < level_time {
        if ctx.world.entity(self_).s.loopSound == 0 {
            let snd = G_SoundIndex(ctx, "sound/player/pickuphealth.wav");
            ctx.world.entity_mut(self_).s.loopSound = snd;
        }
        ctx.world.entity_mut(self_).setTime = level_time + 100;
...
pub fn SP_misc_model_health_power_converter(ctx: &mut GameContext, ent: EntityId) {
    ...
    //G_SoundIndex("sound/movers/objects/useshieldstation.wav");
    G_SoundIndex(ctx, "sound/player/pickuphealth.wav");
    let snd = G_SoundIndex(ctx, "sound/interface/shieldcon_done");
    ctx.world.entity_mut(ent).genericValue7 = snd;
...
pub fn SP_fx_runner(ctx: &mut GameContext, ent: EntityId) {
    ...
    let mi = G_EffectIndex(ctx, &fx_file);
    let level_time = ctx.world.level.time;
    {
        let e = ctx.world.entity_mut(ent);
        e.s.modelindex = mi;
...
pub fn SP_CreateSpaceDust(ctx: &mut GameContext, ent: EntityId) {
    let count = ctx.world.entity(ent).count;
    G_EffectIndex(ctx, &format!("*spacedust {}", count));
...
pub fn SP_CreateSnow(ctx: &mut GameContext, ent: EntityId) {
    G_EffectIndex(ctx, "*snow");
    G_EffectIndex(ctx, "*fog");
    G_EffectIndex(ctx, "*constantwind (100 100 -100)");
}
...
pub fn SP_CreateRain(ctx: &mut GameContext, ent: EntityId) {
    let count = ctx.world.entity(ent).count;
    G_EffectIndex(ctx, &format!("*rain init {}", count));
}
...
pub fn SP_misc_maglock(ctx: &mut GameContext, self_: EntityId) {
    ...
    let mi = G_ModelIndex(ctx, "models/map_objects/imp_detention/door_lock.md3");
    ctx.world.entity_mut(self_).s.modelindex = mi;
    let ei = G_EffectIndex(ctx, "maglock/explosion");
...
pub fn faller_touch(
    ...
    if epv2 < -100.0 && gv7 < level_time {
        ...
        let snd = if r == 1 {
            G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain25")
        } else if r == 2 {
            G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain50")
        } else {
            G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain75")
        };
...
pub fn misc_faller_create(
    ...
    let s10 = G_SoundIndex(ctx, "sound/player/fallsplat");
    ctx.world.entity_mut(faller_id).genericValue10 = s10;
    let s9 = G_SoundIndex(ctx, "sound/chars/stofficer1/misc/falling1");
    {
    ...
    let mi = G_ModelIndex(ctx, "models/players/stormtrooper/model.glm");
    {
        let f = ctx.world.entity_mut(faller_id);
        f.s.modelindex = mi;
        f.s.g2radius = 100;
...
pub fn SP_misc_faller(ctx: &mut GameContext, ent: EntityId) {
    G_ModelIndex(ctx, "models/players/stormtrooper/model.glm");
    G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain25");
    G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain50");
    G_SoundIndex(ctx, "sound/chars/stofficer1/misc/pain75");
    G_SoundIndex(ctx, "sound/chars/stofficer1/misc/falling1");
    G_SoundIndex(ctx, "sound/player/fallsplat");
...
// We reshape it to `&mut vec3_t`.
// This file has no same-file callers to fix. Cross-file callers are the fixer's job per the packet.
/// Raven `TAG_GetOrigin`.
///
...
pub fn TAG_GetAngles(
    ...
    if tag.is_null() {
        ...
        // We take the one defined behavior per porting-rules §19: report failure.
        return 0;
    }
    unsafe {
        *angles = (*tag).angles;
    ...
    unsafe {
        ...
        if ctx.world.globals.g_shooterClientInit == qfalse {
            ...
            for slot in ctx.world.globals.g_shooterClients.iter_mut() {
                ...
                // `cl.pers.netname`'s `String` is not zero-valid, so we reset the whole slot instead of byte-zeroing over it.
...
// We drop the local `G_SpawnInt` shim that used to live here, which adapted byte-string literals to `*const c_char`.
// Its only callers (`EnergyShieldStationSettings`/`EnergyAmmoStationSettings`/`EnergyHealthStationSettings`) are parked under `seam-threading`.
// The threading effort left them stranded, because `G_SpawnInt` needs a `GameContext` this shim had no way to supply.
// It has zero live callers, per porting-rules §20.
