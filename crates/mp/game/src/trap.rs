//! `mod trap` holds outbound-call wrappers for every `trap_*` call the jampgame manifest records.
//! One non-generic function serves each call.
//! C signatures live in `docs/abi-traps.md`.
//! Token mapping lives in `oracle/codemp/game/g_syscalls.c`.
//!
//! This module is hand-maintained, originally generated by fngen.py, and wrappers own the C seam.
//! String args are `&str`.
//! Engine-filled `char` buffers come back as `String` (`buffer_len` keeps the engine-side truncation width at the call site).
//! `qboolean` is `bool`.
//! The raw pointer and `Args` shapes live only inside this module and `mp_abi`.
...
pub fn Argv(engine: &Engine, n: c_int, buffer_len: usize) -> String {
    ...
    // The engine serves argv as Latin-1 wire bytes, byte-transparent client input.
    // Decode it 1:1, so a non-ASCII chat or name payload round-trips back onto the wire untouched.
    // ASCII input is identical to a lossy decode.
...
///
/// `listbuf` stays a caller byte buffer.
/// The engine packs NUL-separated names, and callers walk them by offset.
...
/// `boneName` is nullable in Raven.
/// NULL selects the engine's init/reset-IK branch, so `None` here encodes a null pointer on the wire.
...
/// Raven passes the fill buffer through a `const char *`, though the engine writes through it anyway.
/// The buffer stays a caller pointer at this seam.
...
/// Raven `trap_Printf` (`g_syscalls.c:27-29`): `G_PRINT`.
/// This prints the message on the local console (`g_public.h:105`).
...
pub fn SendServerCommand(engine: &Engine, clientNum: c_int, text: &str) {
    // This encodes the text as Latin-1 wire bytes, one byte per char.
    // A chat or name payload the game read via `Argv` carries non-ASCII chars whose UTF-8 form (`cstr`) would re-widen on the wire.
    // `CString::new` over the Latin-1 bytes keeps the NUL-terminated seam shape.
    // ASCII input is byte-identical to `cstr(text)`.
...
// trap_-named MODULE-LOCAL helpers are defined in game/*.c, not g_syscalls.c.
// They are ported as normal functions, with no wrapper here:
