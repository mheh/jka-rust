...
//! (arrays-as-parameters), so the callee mutates the caller's storage; a
//! Rust `[f32; N]` passed by value cannot. Every param that Raven's body
//! writes through is changed here to `&mut vec3_t`/`&mut vec4_t` (or, for the
//! `vec3_t axis[3]`-shaped matrix params, the pre-generated `*mut vec3_t` is
//! reinterpreted as `&mut [vec3_t; 3]`) — read-only params keep the
//! by-value skeleton shape. `AngleVectors`'s optional out-params (`if
//! (forward)` null checks in the oracle) become `Option<&mut vec3_t>`.
//! `BoxOnPlaneSide`'s `struct cplane_s *` (the generator couldn't resolve the
//! elaborated-type-specifier to the already-ported `cplane_t`) is the same
//! type `SetPlaneSignbits` takes, so it is spelled `*mut cplane_t` here too.
