...
//! fields per fork-ruling 1) and engine traps (`trap_*`) through the threaded
//! `GameContext`/`GameWorld` handle; the shared RNG (ruling 3) is threaded
//! rather than read from a bare `crandom`/`Q_irand`, and fn-pointer fields
//! are set via the `EntThink`/`EntTouch`/`EntUse` enums (ruling 2, see
