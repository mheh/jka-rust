...
pub fn NPC_SetMoveGoal(
    ...
    unsafe {
        ...
        let targetEnt: *mut gentity_t = ent_ptr(ctx, targetEnt);
        if (*ent).NPC.is_null() {
            return;
        }
        let npc = (*ent).NPC;
...
pub fn NAV_AvoidCollision(
    ...
    unsafe {
        // STAGE-1: EntityId/Option params, raw body re-derived verbatim (Stage-2 debt).
        // `self_` is a plain `EntityId` (never NULL); Raven's `!self_` guard below
        // is vacuous (the body already deref'd `self_->r.currentOrigin`) and stays
        // as an always-true test.
        let self_: *mut gentity_t = ctx.entity_mut(self_);
        let goal: *mut gentity_t = ent_ptr(ctx, goal);
...
pub fn NAV_MoveToGoal(ctx: &mut GameContext, self_: EntityId, info: *mut navInfo_t) -> c_int {
    unsafe {
        ...
        if (*goal_ent).s.number == 0 {
            ...
            (*goal_ent).waypoint = NAV_GetNearestNode(
                ctx,
                ctx.entity_id_of(goal_ent).unwrap(),
                (*goal_ent).waypoint,
            );
            if (*goal_ent).waypoint == WAYPOINT_NONE {
                return WAYPOINT_NONE;
        ...
        // Find our waypoint
        (*self_).waypoint =
            NAV_GetNearestNode(ctx, ctx.entity_id_of(self_).unwrap(), (*self_).lastWaypoint);
...
pub fn SP_waypoint(ctx: &mut GameContext, ent: EntityId) {
    unsafe {
        ...
        if ctx.world.globals.navCalculatePaths != 0 {
            ...
            let radius = waypoint_getRadius(ctx, ctx.entity_id_of(ent).unwrap());
            ...
            NAV_StoreWaypoint(ctx, ctx.entity_id_of(ent).unwrap());
            G_FreeEntity(ctx, ctx.entity_id_of(ent));
            return;
        }
...
pub fn NAV_ShowDebugInfo(ctx: &mut GameContext) {
    unsafe {
        ...
        if ctx.world.globals.NAVDEBUG_showTestPath != 0 {
            ...
            nearestNode = NAV_TestBestNode(
                ctx,
                ctx.entity_id_of(ent0).unwrap(),
                nearestNode,
                testNode,
                qfalse,
            );
