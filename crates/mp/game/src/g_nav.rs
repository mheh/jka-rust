...
// `game_globals.rs`); only the `fatalErrorString` rolling-buffer write logic
// remains deferred.
...
pub fn NAV_WaypointsTooFar(
    ...
    unsafe {
        if Distance((*wp1).r.currentOrigin, (*wp2).r.currentOrigin) > 1024.0 {
            ...
            //TODO: Port fatalErrorString append
            // Source: oracle/oracle/codemp/game/g_nav.c:1618-1655
            // The `fatalErrorString`/`fatalErrorPointer` buffer + cursor now
            // exist as real fields; the rolling append of `temp` into the
            // buffer is still deferred (logic-port gap, not a type gap).
            let len = temp.len();
            ...
            // (*ctx.world).globals.fatalErrorPointer += temp — deferred append
            // into `fatalErrorString` (see the `//TODO: Port` above).
...
pub fn NAV_CalculatePaths(
    ...
    unsafe {
        ...
        //TODO: Port fatalErrorString reset
        // Source: oracle/oracle/codemp/game/g_nav.c:1734-1838
        // Buffer/cursor reset (`fatalErrorString[0]=0; fatalErrorPointer=...`)
        // deferred — logic-port gap; the fields themselves now exist.
