...
//!
//! Safe-state migration **Stage 2c** (deref regime): entity reaches go through
//! the checked `ctx.entity(id)` / `ctx.entity_mut(id)` accessors (§B5), not raw
//! `gentity_t*` re-derives — the Stage-1 fn-top `*mut gentity_t` shims are gone.
//! The raw derefs that remain are, by design:
//!   * the ABI seam — entity pointers handed to engine `trap::Nav_*`/`LinkEntity`
//!     syscalls, and the `*mut trace_t` / `*mut navInfo_t` / `*mut c_int`
//!     out-param pointers threaded verbatim (not `gentity_t`);
//!   * pool-allocated `gclient_t` (`ent.client`) and `gNPC_t` (`ent.NPC`,
//!     `globals.NPCInfo`) structs, which have no arena accessor — each such
//!     deref is a tight `unsafe` block through a copied pointer value and is
//!     FLAGged at the site (recipe 2b/2c).
...
pub fn FlyingCreature(ent: &gentity_t) -> qboolean {
    if !ent.client.is_null() {
        ...
        // FLAG: gclient_t deref (ctx-free leaf helper, no arena accessor);
        // stays raw (recipe 2b).
...
pub fn NPC_Blocked(ctx: &mut GameContext, self_: EntityId, blocker: Option<EntityId>) {
    // FLAG: gNPC_t (self->NPC) has no arena accessor; derefs stay raw (recipe 2c).
    ...
    if !blocker_client.is_null() {
        ...
        // FLAG: gclient_t derefs stay raw (recipe 2b).
    ...
    // FLAG: gNPC_t writes stay raw (recipe 2c).
    unsafe {
...
pub fn NPC_SetMoveGoal(
    ...
    // FLAG: gNPC_t (ent->NPC) has no arena accessor; derefs stay raw (recipe 2c).
...
pub fn NAV_ClearPathToPoint(
    ...
    // `self_` is never mutated here; snapshot the fields it reads.
...
pub fn NAV_CheckAhead(
    ...
    // `trace` is a caller-owned out-param pointer (not an entity); derefs stay raw.
...
pub fn NAV_ResolveBlock(
    ...
    let blocker = blocker.unwrap();
    // Stop double waiting
    // FLAG: gNPC_t (blocker->NPC) deref stays raw (recipe 2c).
...
pub fn NAV_TrueCollision(
    ...
    // FLAG: gclient_t deref stays raw (ctx-free leaf helper, recipe 2b).
...
pub fn NAV_StackedCanyon(
    ...
    // `self_` is never mutated below; snapshot the trace inputs.
    let self_mins = ctx.entity(self_).r.mins;
    let self_number = ctx.entity(self_).s.number;
    let self_clipmask = ctx.entity(self_).clipmask;
    let blocker_origin = ctx.entity(blocker).r.currentOrigin;
...
pub fn NAV_TestForBlocked(
    ...
    {
        // `flags` is a caller-owned out-param pointer (not an entity); deref stays raw.
        unsafe {
            *flags |= NIF_BLOCKED;
        }
...
pub fn NAV_AvoidCollision(
    ...
    // is vacuous (the body already reads `self_`'s origin) and stays an
    // always-true test. `info` is a caller-owned `*mut navInfo_t` out-param
    // (not an entity); its derefs stay raw.
    ...
    // FLAG: gNPC_t (self_->NPC) deref stays raw (recipe 2c).
    let npc = ctx.entity(self_).NPC;
    if !npc.is_null() {
...
pub fn NAV_MoveToGoal(ctx: &mut GameContext, self_: EntityId, info: *mut navInfo_t) -> c_int {
    // `info` is a caller-owned `*mut navInfo_t` out-param (not an entity); its
    // derefs stay raw.
