...
pub fn ImperialProbe_FireBlaster(ctx: GameContext<'_>) {
  unsafe {
    ...
    G_Sound(ctx, npc, CHAN_AUTO, G_SoundIndex(c"sound/chars/probe/misc/fire".as_ptr()));
    ...
    if (*npc).health != 0 {
        ...
        AngleVectors((*npc).r.currentAngles, Some(&mut forward), Some(&mut vright), Some(&mut up));
    }
...
pub fn ImperialProbe_Ranged(
  ...
  unsafe {
    ...
    if TIMER_Done(ctx, npc, c"attackDelay".as_ptr()) != 0 {
        ...
        TIMER_Set(ctx, npc, c"attackDelay".as_ptr(), world.bg_state.rng.Q_irand(500, 3000));
  ...
  unsafe {
    ...
    if TIMER_Done(ctx, npc, c"patrolNoise".as_ptr()) != 0 {
        if TIMER_Done(ctx, npc, c"angerNoise".as_ptr()) != 0 {
            ...
            TIMER_Set(ctx, npc, c"patrolNoise".as_ptr(), world.bg_state.rng.Q_irand(4000, 10000));
        }
    }
  ...
  unsafe {
    ...
    if (*npc).enemy.is_none() {
        ...
        if UpdateGoal(ctx) != core::ptr::null_mut() {
            ...
            (*npc).s.loopSound = G_SoundIndex(c"sound/chars/probe/misc/probedroidloop".as_ptr());
        ...
        if TIMER_Done(ctx, npc, c"patrolNoise".as_ptr()) != 0 {
            ...
            TIMER_Set(ctx, npc, c"patrolNoise".as_ptr(), world.bg_state.rng.Q_irand(2000, 4000));
        }
    } else {
        // He's got an enemy. Make him angry.
        G_SoundOnEnt(ctx, npc, CHAN_AUTO, c"sound/chars/probe/misc/anger1".as_ptr());
