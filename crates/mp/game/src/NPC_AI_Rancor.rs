...
pub fn Rancor_Patrol(ctx: GameContext<'_>) {
    unsafe {
        ...
        crate::g_timer::TIMER_Set(ctx, npc, c"lookForNewEnemy".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(5000, 15000));
    }
}
...
pub fn Rancor_DropVictim(
    ...
    unsafe {
        let activator = crate::ent_id::resolve((*ctx.world).g_entities.as_mut_ptr(), (*self_).activator);
...
pub fn Rancor_Swing(
    ...
    unsafe {
        ...
        let numEnts = crate::NPC_utils::NPC_GetEntsNearBolt(ctx, radiusEntNums.as_mut_ptr(), radius, (*((*npc).client as *mut gclient_t)).renderInfo.handRBolt, &mut boltOrg);
        ...
        for i in 0..(numEnts as usize) {
            ...
            if DistanceSquared((*radiusEnt).r.currentOrigin, boltOrg) <= radiusSquared {
                if tryGrab != 0
                    ...
                    //wait to attack
                    crate::g_timer::TIMER_Set(ctx, npc, c"attacking".as_ptr(), (*((*npc).client as *mut gclient_t)).ps.legsTimer + (*ctx.world).bg_state.rng.Q_irand(500, 2500));
                    ...
                    */
                    ...
                    if (*((*radiusEnt).client as *mut gclient_t)).NPC_class != CLASS_RANCOR
                        && (*((*radiusEnt).client as *mut gclient_t)).NPC_class != CLASS_ATST {
                        crate::g_combat::G_Damage(ctx, radiusEnt, npc, npc, Some(&mut [0.0; 3]), (*radiusEnt).r.currentOrigin, (*ctx.world).bg_state.rng.Q_irand(25, 40), DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK, MOD_MELEE as c_int);
...
pub fn Rancor_Smash(ctx: GameContext<'_>) {
    unsafe {
        ...
        let numEnts = crate::NPC_utils::NPC_GetEntsNearBolt(ctx, radiusEntNums.as_mut_ptr(), radius, (*((*npc).client as *mut gclient_t)).renderInfo.handLBolt, &mut boltOrg);
        ...
        for i in 0..(numEnts as usize) {
            ...
            if distSq <= radiusSquared {
                ...
                if distSq < halfRadSquared {
                    ...
                    crate::g_combat::G_Damage(ctx, radiusEnt, npc, npc, Some(&mut [0.0; 3]), (*radiusEnt).r.currentOrigin, (*ctx.world).bg_state.rng.Q_irand(10, 25), DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK, MOD_MELEE as c_int);
                }
                if (*radiusEnt).health > 0
                    ...
                    && (*((*radiusEnt).client as *mut gclient_t)).NPC_class != CLASS_ATST {
                    if distSq < halfRadSquared
                        || (*((*radiusEnt).client as *mut gclient_t)).ps.groundEntityNum != ENTITYNUM_NONE {
...
pub fn Rancor_Bite(ctx: GameContext<'_>) {
    unsafe {
        ...
        let numEnts = crate::NPC_utils::NPC_GetEntsNearBolt(ctx, radiusEntNums.as_mut_ptr(), radius, (*((*npc).client as *mut gclient_t)).renderInfo.crotchBolt, &mut boltOrg);//was gutBolt?
        ...
        for i in 0..(numEnts as usize) {
            ...
            if DistanceSquared((*radiusEnt).r.currentOrigin, boltOrg) <= radiusSquared {
                crate::g_combat::G_Damage(ctx, radiusEnt, npc, npc, Some(&mut [0.0; 3]), (*radiusEnt).r.currentOrigin, (*ctx.world).bg_state.rng.Q_irand(15, 30), DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK, MOD_MELEE as c_int);
    ...
    unsafe {
        ...
        if crate::g_timer::TIMER_Exists(ctx, npc, c"attacking".as_ptr()) == qfalse {
            if (*npc).count == 2 && !(*npc).activator.is_none() {
                ...
                let mut yawAng: vec3_t = [0.0, (*((*npc).client as *mut gclient_t)).ps.viewangles[1], 0.0];
            ...
            crate::g_timer::TIMER_Set(ctx, npc, c"attacking".as_ptr(), ((*((*npc).client as *mut gclient_t)).ps.legsTimer as f32 + (*ctx.world).bg_state.rng.random() * 200.0) as c_int);
        }
        ...
        if crate::g_timer::TIMER_Done2(ctx, npc, c"attack_dmg".as_ptr(), qtrue) != 0 {
            ...
            _ if (*((*npc).client as *mut gclient_t)).ps.legsAnim == BOTH_ATTACK1 as c_int => {
                if (*npc).count == 1 && !(*npc).activator.is_none() {
                    ...
                    if (*activator).health <= 0 {
                        ...
                        let activator_client = (*activator).client as *mut gclient_t;
                        crate::g_combat::G_Dismember(ctx, activator, npc, (*activator).r.currentOrigin, G2_MODELPART_HEAD as c_int, 90.0, 0.0, (*activator_client).ps.torsoAnim, qtrue);
            ...
            _ if (*((*npc).client as *mut gclient_t)).ps.legsAnim == BOTH_ATTACK3 as c_int => {
                if (*npc).count == 1 && !(*npc).activator.is_none() {
                    ...
                    if !(*activator).client.is_null() {
                        //NPC->activator->client->dismembered = qfalse;
                        crate::g_combat::G_Dismember(ctx, activator, npc, (*activator).r.currentOrigin, G2_MODELPART_WAIST as c_int, 90.0, 0.0, (*((*activator).client as *mut gclient_t)).ps.torsoAnim, qtrue);
            ...
            _ if (*((*npc).client as *mut gclient_t)).ps.legsAnim == BOTH_ATTACK3 as c_int => {
                if (*npc).count == 1 && !(*npc).activator.is_none() {
                    ...
                    if (*activator).health > 0 {
                        ...
                        //NPC->activator->client->dismembered = qfalse;
                        let activator_client = (*activator).client as *mut gclient_t;
                        crate::g_combat::G_Dismember(ctx, activator, npc, (*activator).r.currentOrigin, G2_MODELPART_WAIST as c_int, 90.0, 0.0, (*activator_client).ps.torsoAnim, qtrue);
                        //G_DoDismemberment( NPC->activator, NPC->enemy->r.currentOrigin, MOD_SABER, 1000, HL_WAIST, qtrue );
                        //KILL
                        crate::g_combat::G_Damage(ctx, activator, npc, npc, Some(&mut [0.0; 3]), (*activator).r.currentOrigin, (*crate::ent_id::resolve(ent_base, (*npc).enemy)).health+10, DAMAGE_NO_PROTECTION|DAMAGE_NO_ARMOR|DAMAGE_NO_KNOCKBACK|DAMAGE_NO_HIT_LOC, MOD_MELEE as c_int);//, HL_NONE );
                        (*activator_client).ps.forceHandExtend = HANDEXTEND_NONE as c_int;
            ...
            if (*((*npc).client as *mut gclient_t)).ps.legsTimer >= 1200 && (*((*npc).client as *mut gclient_t)).ps.legsTimer <= 1350 {
                if (*ctx.world).bg_state.rng.Q_irand(0, 2) != 0 {
                    Rancor_Swing(ctx, qfalse);
                } else {
                    Rancor_Swing(ctx, qtrue);
                }
            } else if (*((*npc).client as *mut gclient_t)).ps.legsTimer >= 1100 && (*((*npc).client as *mut gclient_t)).ps.legsTimer <= 1550 {
                Rancor_Swing(ctx, qtrue);
            }
        }
...
pub fn Rancor_Combat(ctx: GameContext<'_>) {
    unsafe {
        ...
        if crate::NPC_utils::NPC_ClearLOS4(ctx, crate::ent_id::resolve((*ctx.world).g_entities.as_mut_ptr(), (*npc).enemy)) == qfalse {
            //|| UpdateGoal( ))
...

        {
            ...
            let mut advance = if distance > ((*npc).r.maxs[0] + MIN_DISTANCE as f32) { qtrue } else { qfalse };
...
pub fn NPC_Rancor_Pain(
    ...
    unsafe {
        ...
        if !attacker.is_null() && !(*attacker).client.is_null() && (*((*attacker).client as *mut gclient_t)).NPC_class == CLASS_RANCOR {
            hitByRancor = qtrue;
        }
        let self_enemy = crate::ent_id::resolve((*ctx.world).g_entities.as_mut_ptr(), (*self_).enemy);
        if !attacker.is_null()
            && (*attacker).inuse != 0
            && attacker != self_enemy
            ...
            if (*self_).count == 0 {
                if ((*attacker).s.number == 0 && (*ctx.world).bg_state.rng.Q_irand(0,3) == 0)
                    ...
                    crate::g_timer::TIMER_Set(ctx, self_, c"lookForNewEnemy".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(5000, 15000));
                    if hitByRancor != 0 {
                        //stay mad at this Rancor for 2-5 secs before looking for attacker enemies
                        crate::g_timer::TIMER_Set(ctx, self_, c"rancorInfight".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(2000, 5000));
                    }
                }
            }
        }
...
pub fn Rancor_CheckDropVictim(ctx: GameContext<'_>) {
    unsafe {
        let npc = (*ctx.world).globals.NPC;
        let activator = crate::ent_id::resolve((*ctx.world).g_entities.as_mut_ptr(), (*npc).activator);
...
pub fn Rancor_Crush(ctx: GameContext<'_>) {
    unsafe {
        ...
        let crush = &mut (*ctx.world).g_entities[(*((*npc).client as *mut gclient_t)).ps.groundEntityNum as usize];
        if crush.inuse != 0 && !crush.client.is_null() && crush.localAnimIndex == 0 {
            //a humanoid, smash them good.
            crate::g_combat::G_Damage(ctx, crush as *mut gentity_t, npc, npc, None, (*npc).r.currentOrigin, 200, 0, MOD_CRUSH as c_int);
        }
    }
}
...
pub fn NPC_BSRancor_Default(ctx: GameContext<'_>) {
    unsafe {
        ...
        (*((*npc).client as *mut gclient_t)).ps.eFlags2 &= !(EF2_USE_ALT_ANIM|EF2_GENERIC_NPC_FLAG);
        if (*npc).count != 0 {
            //holding someone
            ...
            */
            if crate::g_timer::TIMER_Done(ctx, npc, c"angrynoise".as_ptr()) != 0 {
                crate::g_utils::G_Sound(ctx, npc, CHAN_AUTO, crate::g_utils::G_SoundIndex(
                    cstr(&format!("sound/chars/rancor/misc/anger{}.wav", (*ctx.world).bg_state.rng.Q_irand(1, 3))).as_ptr()
                ));

                crate::g_timer::TIMER_Set(ctx, npc, c"angrynoise".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(5000, 10000));
            } else {
                crate::NPC_senses::AddSoundEvent(ctx, npc, (*npc).r.currentOrigin, 512.0, AEL_DANGER_GREAT, qfalse);//, qfalse );
            }
            if (*npc).count == 2 && (*((*npc).client as *mut gclient_t)).ps.legsAnim == BOTH_ATTACK3 as c_int {
            ...
            if !(*npc_enemy).client.is_null() && (*((*npc_enemy).client as *mut gclient_t)).NPC_class == CLASS_RANCOR {
                ...
                if crate::NPC_combat::ValidEnemy(ctx, crate::ent_id::resolve((*ctx.world).g_entities.as_mut_ptr(), (*npc).enemy)) == qfalse {
                    crate::g_timer::TIMER_Remove(ctx, npc, c"lookForNewEnemy".as_ptr());//make them look again right now
                    if (*npc_enemy).inuse == 0 || (*ctx.world).level.time - (*npc_enemy).s.time > (*ctx.world).bg_state.rng.Q_irand(10000, 15000) {
                ...
                if crate::g_timer::TIMER_Done(ctx, npc, c"lookForNewEnemy".as_ptr()) != 0 {
                    ...
                    (*npc).enemy = sav_enemy;
                    if !newEnemy.is_null() && ent_id_opt((*ctx.world).g_entities.as_mut_ptr(), newEnemy) != sav_enemy {
                        ...
                        crate::g_timer::TIMER_Set(ctx, npc, c"lookForNewEnemy".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(2000, 5000));
                    }
                }
            }
            Rancor_Combat(ctx);
            ...
            if crate::g_timer::TIMER_Done(ctx, npc, c"idlenoise".as_ptr()) != 0 {
                crate::g_utils::G_Sound(ctx, npc, CHAN_AUTO, crate::g_utils::G_SoundIndex(
                    cstr(&format!("sound/chars/rancor/snort_{}.wav", (*ctx.world).bg_state.rng.Q_irand(1, 2))).as_ptr()
                ));

                crate::g_timer::TIMER_Set(ctx, npc, c"idlenoise".as_ptr(), (*ctx.world).bg_state.rng.Q_irand(2000, 4000));
