...
pub fn G_SayTo(
    ...
    unsafe {
        ...
        if ctx.world.cvars.g_gametype.integer == GT_SIEGE
            && !(*ent).client.is_null()
            && ((*((*ent).client)).tempSpectate >= ctx.world.level.time
...
pub fn Cmd_Tell_f(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        let netname_ent = cstr_to_str((*((*ent).client)).pers.netname.as_ptr());
        let netname_target = cstr_to_str((*((*target).client)).pers.netname.as_ptr());
        let logmsg = format!(
            "tell: {} to {}: {}\n",
            netname_ent,
            netname_target,
...
pub fn Cmd_EngageDuel_f(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        if tr.fraction != 1.0 && tr.entityNum < MAX_CLIENTS as c_short {
            ...
            let challengedClient = (*challenged).client;
