...
//! handle) and are parked; likewise a couple of function-scope statics
//! (`ConcatArgs`'s rotating scratch buffer) that the fork-1/fork-5 rulings
//! resolve to owned return values but whose skeleton signature is a raw
//! `*mut c_char` cannot be threaded either. See PORT-ESCALATION markers.
