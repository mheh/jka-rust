...
//!
//! Relocated to the qshared tier (`q_shared.h` is a shared header) so the bg
//! crate can reach the `SFL_*` bits; re-exported here so game importers and the
//! prelude keep resolving `crate::saber::saber_flags::*` unchanged.
