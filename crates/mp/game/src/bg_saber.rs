...
// ============================================================================
// Pass-3 real bodies: the pmove working-set functions above stay parked
// (`todo!()`) as faithful free-fn C-signature skeletons — matching the
// `bg_pmove.rs` precedent (`PM_BGEntForNum`) where the resolved shape for a
// state-touching bg fn is a method on `PmoveContext` (ruling 12/8a: "pmove
// working set -> methods on PmoveContext, reach the working set via
// self.pm/self.pml/..."). The bodies below are the real ported logic; the old
// stubs are left as discoverable, dead PORT-ESCALATION markers per that
// established convention. `unsafe` here is the confined pmove/entity-overlay
// deref (porting-rules §D11; ruling 14).
// ============================================================================
impl PmoveContext<'_> {
    /// Raven `PM_irand_timesync`.
    /// Source: `oracle/oracle/codemp/game/bg_saber.c:10-25`
