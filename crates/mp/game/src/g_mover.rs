...
pub fn G_TryPushingEntity(
    ...
    unsafe {
        ...
        ctx.world.globals.pushed.push(entry);
        ctx.world.globals.pushed_p += 1;
        ...
        let check_eid = ctx.entity_id_of(check).unwrap();
        // add movement
        for i in 0..3 {
        ...
        let mut block = G_TestEntityPosition(ctx, check_eid);
        if block.is_null() {
            // pushed ok
            if !(*check).client.is_null() {
                let client = (*check).client;
        ...
        let check_eid = ctx.entity_id_of(check).unwrap();
        block = G_TestEntityPosition(ctx, check_eid);
        if block.is_null() {
            (*check).s.groundEntityNum = -1;
            ctx.world.globals.pushed_p -= 1;
            ctx.world.globals.pushed.pop();
            return qtrue;
        }
    ...
    unsafe {
        *obstacle = core::ptr::null_mut();
        ...
        for e in 0..listed_entities {
            ...
            if G_TryPushingEntity(
                ctx,
                ctx.entity_id_of(check).unwrap(),
                ctx.entity_id_of(pusher).unwrap(),
                r#move,
                amove,
            ) != 0
            {
...
pub fn Reached_BinaryMover(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        if (*ent).moverState == MOVER_1TO2 {
            ...
            let level_time = ctx.world.level.time;
            SetMoverState(ctx, ent_eid, MOVER_POS2, level_time);
            ...
            let ent_eid = ctx.entity_id_of(ent).unwrap();
            let level_time = ctx.world.level.time;
            SetMoverState(ctx, ent_eid, MOVER_POS1, level_time);
            CalcTeamDoorCenter(ctx, ctx.entity_id_of(ent).unwrap(), &mut doorcenter);
            G_PlayDoorSound(ctx, ctx.entity_id_of(ent).unwrap(), BMS_END);
            ...
            let activator_eid = ctx.entity_id_of(activator_ptr);
            // close areaportals
            if crate::ent_id::resolve(ctx.world.g_entities.as_mut_ptr(), (*ent).teammaster) == ent
...
pub fn Use_BinaryMover_Go(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        if (*ent).moverState == MOVER_POS2 {
            ...
            G_UseTargets2(ctx, ent_eid, activator_eid, target2.as_deref());
            return;
        }
...
pub fn UnLockDoors(ctx: &mut GameContext, ent: EntityId) {
    ...
    while let Some(sid) = slave {
        // want to allow locked toggle doors, so keep the targetname
        if ctx.world.entity(sid).spawnflags & MOVER_TOGGLE == 0 {
            // not usable ever again
            ctx.ent_set(sid, PrefixSet::Targetname(None));
        }
        let s = ctx.world.entity_mut(sid);
        s.spawnflags &= !MOVER_LOCKED;
        s.s.frame = 1; // second stage of anim
        slave = ctx.world.entity(sid).teamchain;
    }
}
...
pub fn Use_BinaryMover(
    ...
    unsafe {
        ...
        if (*ent).spawnflags & MOVER_LOCKED != 0 {
            ...
            UnLockDoors(ctx, ctx.entity_id_of(ent).unwrap());
            return;
        }
...
/// Source: `oracle/codemp/game/g_mover.c:913-934`
...
pub fn InitMover(ctx: &mut GameContext, ent: EntityId) {
    ...
    if !model2.is_empty() {
        ...
        if model2.contains(".glm") {
            ...
            ctx.entity_mut(ent).s.modelindex2 = G_ModelIndex(ctx, &model2);
        }
    }
    ...
    unsafe {
        ...
        if (*ent).takedamage != 0 {
            ...
            while !other.is_null() {
                ...
                other =
                    crate::ent_id::resolve(ctx.world.g_entities.as_mut_ptr(), (*other).teamchain);
            }
        }
        ...
        ctx.ent_set(other_eid, PrefixSet::ClassnameStatic(c"trigger_door"));
...
pub fn G_EntIsDoor(ctx: &mut GameContext, entityNum: c_int) -> qboolean {
    ...
    let classname = ctx.world.g_entities[entityNum as usize].classname_str();
    if Q_stricmp(&classname, "func_door") == 0 {
        // blocked by a door
        return qtrue;
    }
    qfalse
...
pub fn G_FindDoorTrigger(ctx: &mut GameContext, ent: EntityId) -> *mut gentity_t {
    ...
    unsafe {
        ...
        if let Some(door_targetname) = (*door).targetname_str() {
            // find out what is targeting it
            loop {
                owner = G_Find(ctx, ctx.entity_id_of(owner), EntFindField::Target, &door_targetname);
                if owner.is_null() {
                    break;
                }
        ...
        owner = core::ptr::null_mut();
        loop {
            owner = G_Find(ctx, ctx.entity_id_of(owner), EntFindField::Classname, "trigger_door");
            if owner.is_null() {
                break;
            }
        ...
        if G_EntIsDoor(ctx, entityNum) != 0 {
            ...
            if let Some(ent_targetname) = (*ent).targetname_str() {
                ...
                loop {
                    ...
                    if Q_stricmp(&(*owner).classname_str(), "trigger_multiple") == 0
...
pub fn SP_func_door(ctx: &mut GameContext, ent: EntityId) {
    ...
    if ctx.entity(ent).flags & flags::FL_TEAMSLAVE == 0 {
        ...
        if ctx.entity(ent).spawnflags & MOVER_LOCKED == 0
            && (ctx.entity(ent).targetname_str().is_some()
                || health != 0
...
pub fn SP_func_plat(ctx: &mut GameContext, ent: EntityId) {
    ...
    if ctx.entity(ent).targetname_str().is_none() {
...
pub fn Reached_Train(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        let level_time = ctx.world.level.time;
        // start it going
        SetMoverState(ctx, ent_eid, MOVER_1TO2, level_time);
...
pub fn Think_SetupTrainTargets(ctx: &mut GameContext, ent: EntityId) {
    ...
    unsafe {
        ...
        while path != start {
            ...
            let Some(path_target) = (*path).target.clone() else {
                // end of path
                break;
            };
        ...
        if (*ent).targetname_str().is_none() || ((*ent).spawnflags & 1) != 0
        /* start on */
        {
...
pub fn SP_func_train(ctx: &mut GameContext, self_: EntityId) {
    ...
    if ctx.entity(self_).target.is_none() {
...
pub fn CacheChunkEffects(ctx: &mut GameContext, material: material_t) {
    match material {
        MAT_GLASS => {
            G_EffectIndex(ctx, "chunks/glassbreak");
        }
        MAT_GLASS_METAL => {
            G_EffectIndex(ctx, "chunks/glassbreak");
            G_EffectIndex(ctx, "chunks/metalexplode");
        }
        MAT_ELECTRICAL | MAT_ELEC_METAL => {
            G_EffectIndex(ctx, "chunks/sparkexplode");
        }
        MAT_METAL | MAT_METAL2 | MAT_METAL3 | MAT_CRATE1 | MAT_CRATE2 => {
            G_EffectIndex(ctx, "chunks/metalexplode");
        }
        MAT_GRATE1 => {
            G_EffectIndex(ctx, "chunks/grateexplode");
        }
        MAT_DRK_STONE | MAT_LT_STONE | MAT_GREY_STONE | MAT_WHITE_METAL | MAT_SNOWY_ROCK => {
            G_EffectIndex(ctx, "chunks/rockbreaklg");
            G_EffectIndex(ctx, "chunks/rockbreakmed");
        }
        MAT_ROPE => {
            G_EffectIndex(ctx, "chunks/ropebreak");
        }
        _ => {}
    }
}
...
pub fn funcBBrushDieGo(ctx: &mut GameContext, self_: EntityId) {
    ...
    unsafe {
        ...
        if (*self_).splashDamage > 0 && (*self_).splashRadius > 0 {
            ...
            let te = ctx.entity_mut(te_eid) as *mut gentity_t;
            (*te).s.eventParm = G_SoundIndex(ctx, "sound/weapons/explosions/cargoexplode.wav");
        }
...
pub fn funcBBrushDie(
    ...
    unsafe {
        ...
        funcBBrushDieGo(ctx, ctx.entity_id_of(self_).unwrap());
    }
}
...
pub fn funcBBrushUse(
    ...
    unsafe {
        ...
        if (*self_).spawnflags & 64 != 0 {
            ...
            if (*self_).target.as_deref().is_some_and(|s| !s.is_empty()) {
...
///
/// Empty in Raven.
/// Source: `oracle/codemp/game/g_mover.c:2663-2665`
pub fn funcBBrushTouch(ent: EntityId, other: Option<EntityId>, trace: *mut trace_t) {}
...
pub fn SP_func_breakable(ctx: &mut GameContext, self_: EntityId) {
    ...
    if !s.is_empty() {
        ...
        ctx.entity_mut(self_).genericValue15 = G_EffectIndex(ctx, &s);
    } else {
    ...
    G_SoundIndex(ctx, "sound/weapons/explosions/cargoexplode.wav"); // precaching
    ...
    let team = ctx.entity(self_).team.clone();
...
pub fn G_EntIsBreakable(ctx: &mut GameContext, entityNum: c_int) -> qboolean {
    ...
    let classname = ent.classname_str();

    if svFlags & SVF_GLASS_BRUSH != 0 {
        return qtrue;
    }

    if Q_stricmp(&classname, "func_breakable") == 0 {
        return qtrue;
    }

    if Q_stricmp(&classname, "misc_model_breakable") == 0 {
        return qtrue;
    }
    if Q_stricmp(&classname, "misc_maglock") == 0 {
        return qtrue;
    }
...
/// Source: `oracle/codemp/game/g_mover.c:2924-2928`
pub fn GlassPain(self_: EntityId, attacker: Option<EntityId>, damage: c_int) {
...
pub fn func_wait_return_solid(ctx: &mut GameContext, self_: EntityId) {
    ...
    if ctx.entity(self_).spawnflags & 16 == 0 || G_TestEntityPosition(ctx, self_).is_null() {
        ...
        if target2.as_deref().is_some_and(|s| !s.is_empty()) {
            ...
            G_UseTargets2(ctx, Some(self_), activator_eid, target2.as_deref());
        }
    } else {
        ctx.entity_mut(self_).clipmask = 0;
...
///
/// Source: `oracle/codemp/game/g_mover.c:3027-3035`
...
pub fn G_EntIsRemovableUsable(ctx: &mut GameContext, entNum: c_int) -> qboolean {
    ...
    let targetname = ent.targetname_str();
    if Q_stricmp(&classname, "func_usable") == 0 {
        if (eFlags & EF_SHADER_ANIM) == 0 && (spawnflags & 8) == 0 && targetname.is_some() {
...
pub fn func_usable_use(
    ...
    if ctx.entity(self_).s.eFlags & EF_SHADER_ANIM != 0 {
        ...
        if ctx.entity(self_).target.as_deref().is_some_and(|s| !s.is_empty()) {
            G_UseTargets(ctx, Some(self_), activator);
        }
...
pub fn SP_func_usable(ctx: &mut GameContext, self_: EntityId) {
    ...
    if !model2.is_empty() {
        ...
        if model2.contains(".glm") {
            ...
            ctx.entity_mut(self_).s.modelindex2 = G_ModelIndex(ctx, &model2);
        }
    }
