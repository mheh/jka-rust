//! Port of `oracle/codemp/game/g_turret_G2.c`.
//!
//! think/use/pain/die assignments are wired through the `EntThink`/`EntUse`/`EntPain`/`EntDie`
//! dispatch enums that `gentity_t` carries.
//!
//! Entity-pointer params are `EntityId` / `Option<EntityId>` handles (§B5), and bodies reach
//! entity fields through the `ctx.world.entity(id)` / `entity_mut(id)` accessor borrows at the
//! point of use.
//! A turret target/enemy/attacker can be an NPC carrying a `BG_Alloc`'d pool `gclient_t`
//! (`gClPtrs`, g_utils.c:430) that is not a `level.clients` slot, so `ent->client` is read via the
//! safe entity borrow and dereffed raw exactly as Raven does.
...
/// Raven `bg_public.h:625-660` `EFFECT_*` enum ordinal (`combatEffects_e`, not yet type-ported).
/// Transcribed locally as its raw ordinal (10).
...
// The shadowing local copies are removed.

/// `material_t` is `c_int` (fork per type-port).
/// `MAT_METAL = 0` (`q_shared.h:969`).
const MAT_METAL: c_int = 0;

/// Raven `G2Tur_SetBoneAngles`.
///
/// Raven: special routine for tracking angles between client and server -rww.
/// `angles` is only ever read (copied into the entity state / handed to the trap), never written
/// back, kept by-value.
...
pub fn G2Tur_SetBoneAngles(
    ...
    // Walk the 4 fixed bone-index/bone-angle slot pairs looking for an existing match, else the
    // first free slot.
...
pub fn turretG2_set_models(ctx: &mut GameContext, self_: EntityId, dying: qboolean) {
    ...
    if dying != 0 {
        ...
        // Raven `vec3_origin` resolves via the crate prelude.
        if ctx.world.entity(self_).spawnflags & SPF_TURRETG2_TURBO != 0 {
...
pub fn TurretG2Pain(
    ...
    // FLAG: `attacker` may be an NPC carrying a BG_Alloc'd pool client (gClPtrs), not a
    // `level.clients` slot.
    // The pointer is read via the safe entity borrow and dereffed raw exactly as Raven does.
...
pub fn TurboLaser_SetBoneAnim(
    ...
    // `mp_qshared::common::mp::ghoul2::bone_flags` module (crate prelude glob).
    // See `EWeb_SetBoneAnim` in `g_items.rs` for the same idiom.
...
pub fn turretG2_fire(ctx: &mut GameContext, ent: EntityId, start: vec3_t, dir: &mut vec3_t) {
    ...
    if ctx.world.entity(ent).spawnflags & SPF_TURRETG2_TURBO != 0 {
        ...
        // Raw-ptr-into-ctx seam because `set()` needs `ctx` while the entity is borrowed.
...
pub fn turretG2_aim(ctx: &mut GameContext, self_: EntityId) {
    ...
    if let Some(enemy_id) = enemy_opt {
        ...
        // Aim at enemy
        // FLAG: enemy may be an NPC carrying a BG_Alloc'd pool client (gClPtrs), not a
        // `level.clients` slot.
        // The pointer is read via the safe entity borrow and dereffed raw exactly as Raven does.
    ...
    // The commented-out sweep assignment below in Raven is not transcribed here.
...
pub fn turretG2_find_enemies(ctx: &mut GameContext, self_: EntityId) -> qboolean {
    ...
    // The redundant local alias is removed.
    ...
    for i in 0..count {
        ...
        // FLAG: `target` may be an NPC/bbrush carrying a BG_Alloc'd pool client (gClPtrs), not a
        // `level.clients` slot.
        // `client` is read via the safe entity borrow and dereffed raw exactly as Raven does.
...
pub fn turretG2_base_think(ctx: &mut GameContext, self_: EntityId) {
    ...
    if ctx.world.entity(self_).last_move_time < ctx.world.level.time {
        ...
        if turretG2_find_enemies(ctx, self_) != 0 {
            ...
            // FLAG: enemy may be an NPC pool client.
            // Only the null test is used here, so the pointer value alone suffices (no deref).
            let enemy_client = ctx.world.entity(enemy_id).client;
            let time = ctx.world.level.time;
    ...
    if let Some(enemy_id) = enemy_opt2 {
        // FLAG: enemy may be an NPC carrying a pool client.
        // `sessionTeam` / `eyePoint` are dereffed raw exactly as Raven does.
        let enemy_client = ctx.world.entity(enemy_id).client;
