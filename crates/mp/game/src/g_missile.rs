// PORT-COMPLETE: g_missile.c 1/9
//! FAITHFUL port of `oracle/oracle/codemp/game/g_missile.c`.
//!
//! Generated by `tools/closure-prototype/fnskel.py` from the fnsweep
//! manifest, then filled by the jampgame mega-pass. STAGING ONLY — not
//! wired into crates/. Functions that reach file-scope game state
//! (`level`, `g_entities`, cvars `d_projectileGhoul2Collision`/
//! `g_g2TraceLod`), an engine trap (`trap_LinkEntity`/`trap_Trace`/
//! `trap_G2Trace`), a fn-pointer *write*/zero-out (ruling 2 — needs an
//! `EntXxx` enum variant and a threaded `GameWorld`/engine handle this
//! raw-pointer signature carries no channel for), or a `vec3_t` out-param
//! whose staged signature passes the (`Copy`) array by value with no
//! write-back channel to the caller, are parked; see PORT-ESCALATION
//! markers at each site (same convention as `g_misc.rs`).
#![allow(non_snake_case, unused, clippy::all)]

use crate::prelude::*;
use crate::g_utils::{G_EffectIndex, G_PlayEffectID, G_TempEntity};
use crate::trap;

use mp_bg::public::entity_event::entity_event_t::EV_SABER_BLOCK;
use mp_bg::weapons::weapon_t::{WP_BLASTER, WP_BOWCASTER, WP_BRYAR_PISTOL};

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads `g_entities` and
// `level.time` (global game state not reachable from this raw-pointer
// signature) and writes the `think`/`nextthink` fn-pointer fields (ruling 2 —
// needs an `EntThink` enum + zero/`None` variant, not a bare `0`).
/// Raven `G_ReflectMissile`.
///
/// Reflect the missile roughly back at it's owner.
/// Source: `oracle/oracle/codemp/game/g_missile.c:24-89`
pub fn G_ReflectMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t, missile: *mut gentity_t, forward: vec3_t) {
    unsafe {
        let mut bounce_dir: vec3_t = [0.0; 3];
        let mut owner = ent;
        let mut isowner = 0;

        // Find the owner
        if (*ent).r.ownerNum != 0 {
            owner = &mut (*ctx.world).entities[(*ent).r.ownerNum as usize];
        }

        // Check if original owner is bouncing the missile
        if (*missile).r.ownerNum == (*ent).s.number {
            isowner = 1;
        }

        // Save the original speed
        let speed = crate::q_math::VectorNormalize(&mut (*missile).s.pos.trDelta);

        // Determine bounce direction
        if !owner.is_null() && (*missile).s.weapon != WP_SABER && (*missile).s.weapon != G2_MODEL_PART && isowner == 0 {
            // Bounce back at them if you can
            let mut missile_to_owner: vec3_t = [0.0; 3];
            crate::q_math::_VectorSubtract(
                &(*ctx.world).entities[(*missile).r.ownerNum as usize].r.currentOrigin,
                &(*missile).r.currentOrigin,
                &mut missile_to_owner
            );
            crate::q_math::VectorNormalize(&mut missile_to_owner);
            bounce_dir = missile_to_owner;
        } else if isowner != 0 {
            // Push the missile away from owner with boost
            let mut missile_dir: vec3_t = [0.0; 3];
            crate::q_math::_VectorSubtract(&(*missile).r.currentOrigin, &(*ent).r.currentOrigin, &mut missile_dir);
            crate::q_math::_VectorCopy(&(*missile).s.pos.trDelta, &mut bounce_dir);
            let dot = crate::q_math::_DotProduct(forward, missile_dir);
            crate::q_math::_VectorScale(&mut bounce_dir, dot);
            crate::q_math::VectorNormalize(&mut bounce_dir);
        } else {
            // Default deflection
            let mut missile_dir: vec3_t = [0.0; 3];
            crate::q_math::_VectorSubtract(&(*ent).r.currentOrigin, &(*missile).r.currentOrigin, &mut missile_dir);
            crate::q_math::_VectorCopy(&(*missile).s.pos.trDelta, &mut bounce_dir);
            let dot = crate::q_math::_DotProduct(forward, missile_dir);
            crate::q_math::_VectorScale(&mut bounce_dir, dot);
            crate::q_math::VectorNormalize(&mut bounce_dir);
        }

        // Add randomness
        for i in 0..3 {
            bounce_dir[i] += crate::w_saber::RandFloat(ctx, -0.2, 0.2);
        }

        crate::q_math::VectorNormalize(&mut bounce_dir);
        crate::q_math::_VectorScale(&bounce_dir, speed, &mut (*missile).s.pos.trDelta);
        (*missile).s.pos.trTime = (*ctx.world).level.time;
        crate::q_math::_VectorCopy(&(*missile).r.currentOrigin, &mut (*missile).s.pos.trBase);

        if (*missile).s.weapon != WP_SABER && (*missile).s.weapon != G2_MODEL_PART {
            (*missile).r.ownerNum = (*ent).s.number;
        }

        if (*missile).s.weapon == WP_ROCKET_LAUNCHER {
            (*missile).think = None;
            (*missile).nextthink = 0;
        }
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads `level.time` and
// writes the `think`/`nextthink` fn-pointer fields (ruling 2).
/// Raven `G_DeflectMissile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:91-140`
pub fn G_DeflectMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t, missile: *mut gentity_t, forward: vec3_t) {
    unsafe {
        let mut bounce_dir: vec3_t = [0.0; 3];
        let mut missile_dir: vec3_t = [0.0; 3];
        let mut isowner = 0;

        // Check if original owner is bouncing the missile
        if (*missile).r.ownerNum == (*ent).s.number {
            isowner = 1;
        }

        // Save the original speed
        let speed = crate::q_math::VectorNormalize(&mut (*missile).s.pos.trDelta);

        if !(*ent).client.is_null() {
            crate::q_math::AngleVectors((*(*ent).client).ps.viewangles, Some(&mut missile_dir), None, None);
            crate::q_math::_VectorCopy(&missile_dir, &mut bounce_dir);
            let dot = crate::q_math::_DotProduct(forward, missile_dir);
            crate::q_math::_VectorScale(&bounce_dir, dot);
            crate::q_math::VectorNormalize(&mut bounce_dir);
        } else {
            crate::q_math::_VectorCopy(&forward, &mut bounce_dir);
            crate::q_math::VectorNormalize(&mut bounce_dir);
        }

        // Add randomness
        for i in 0..3 {
            bounce_dir[i] += crate::w_saber::RandFloat(ctx, -1.0, 1.0);
        }

        crate::q_math::VectorNormalize(&mut bounce_dir);
        crate::q_math::_VectorScale(&bounce_dir, speed, &mut (*missile).s.pos.trDelta);
        (*missile).s.pos.trTime = (*ctx.world).level.time;
        crate::q_math::_VectorCopy(&(*missile).r.currentOrigin, &mut (*missile).s.pos.trBase);

        if (*missile).s.weapon != WP_SABER && (*missile).s.weapon != G2_MODEL_PART {
            (*missile).r.ownerNum = (*ent).s.number;
        }

        if (*missile).s.weapon == WP_ROCKET_LAUNCHER {
            (*missile).think = None;
            (*missile).nextthink = 0;
        }
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads `level.time`.
/// Raven `G_BounceMissile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:148-205`
pub fn G_BounceMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t, trace: *mut trace_t) {
    unsafe {
        let mut velocity: vec3_t = [0.0; 3];
        let tr = &*trace;

        // Reflect the velocity on the trace plane
        let hitTime = (*ctx.world).level.previousTime + ((*ctx.world).level.time - (*ctx.world).level.previousTime) as c_float * tr.fraction;
        crate::bg_misc::BG_EvaluateTrajectoryDelta(&(*ent).s.pos, hitTime as c_int, &mut velocity);
        let dot = crate::q_math::_DotProduct(&velocity, &tr.plane.normal);
        crate::q_math::_VectorMA(&velocity, -2.0 * dot, &tr.plane.normal, &mut (*ent).s.pos.trDelta);

        if ((*ent).flags & FL_BOUNCE_SHRAPNEL) != 0 {
            crate::q_math::_VectorScale(&(*ent).s.pos.trDelta, 0.25, &mut (*ent).s.pos.trDelta);
            (*ent).s.pos.trType = TR_GRAVITY;

            // Check for stop
            if tr.plane.normal[2] > 0.7 && (*ent).s.pos.trDelta[2] < 40.0 {
                G_SetOrigin(ent, tr.endpos);
                (*ent).nextthink = (*ctx.world).level.time + 100;
                return;
            }
        } else if ((*ent).flags & FL_BOUNCE_HALF) != 0 {
            crate::q_math::_VectorScale(&(*ent).s.pos.trDelta, 0.65, &mut (*ent).s.pos.trDelta);
            // Check for stop
            if tr.plane.normal[2] > 0.2 && crate::q_math::VectorLength((*ent).s.pos.trDelta) < 40.0 {
                G_SetOrigin(ent, tr.endpos);
                return;
            }
        }

        // Play bounce sound
        if (*ent).s.weapon == WP_THERMAL {
            let sound_name = crate::q_shared::va(c"sound/weapons/thermal/bounce%i.wav".as_ptr(), crate::q_math::Q_irand(1, 2));
            G_Sound(ctx, ent, CHAN_BODY as c_int, G_SoundIndex(sound_name));
        } else if (*ent).s.weapon == WP_SABER {
            let sound_name = crate::q_shared::va(c"sound/weapons/saber/bounce%i.wav".as_ptr(), crate::q_math::Q_irand(1, 3));
            G_Sound(ctx, ent, CHAN_BODY as c_int, G_SoundIndex(sound_name));
        }

        crate::q_math::_VectorAdd(&(*ent).r.currentOrigin, &tr.plane.normal, &mut (*ent).r.currentOrigin);
        crate::q_math::_VectorCopy(&(*ent).r.currentOrigin, &mut (*ent).s.pos.trBase);
        (*ent).s.pos.trTime = (*ctx.world).level.time;

        if (*ent).bounceCount != -5 {
            (*ent).bounceCount -= 1;
        }
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads `level.time`,
// writes `g_entities[ent->r.ownerNum].client->accuracy_hits`, and calls
// `trap_LinkEntity`.
/// Raven `G_ExplodeMissile`.
///
/// Explode a missile without an impact.
/// Source: `oracle/oracle/codemp/game/g_missile.c:215-257`
pub fn G_ExplodeMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t) {
    unsafe {
        let mut dir: vec3_t = [0.0, 0.0, 1.0];
        let mut origin: vec3_t = [0.0; 3];

        crate::bg_misc::BG_EvaluateTrajectory(&(*ent).s.pos, (*ctx.world).level.time, &mut origin);
        trap::SnapVector(ctx.engine, crate::trap::GSnapvector::new(&mut origin));
        G_SetOrigin(ent, origin);

        (*ent).s.eType = ET_GENERAL;
        G_AddEvent(ent, EV_MISSILE_MISS as c_int, DirToByte(dir));

        (*ent).freeAfterEvent = qtrue;
        (*ent).takedamage = qfalse;

        // Splash damage
        if (*ent).splashDamage != 0 {
            // Check if we need to set parent for damage credit
            if (*ent).s.eType == ET_MISSILE && (((*ent).s.eFlags & EF_JETPACK_ACTIVE) != 0) && ((*ent).r.ownerNum as usize) < MAX_CLIENTS {
                (*ent).parent = &mut (*ctx.world).entities[(*ent).r.ownerNum as usize];
            }

            if G_RadiusDamage(ctx, (*ent).r.currentOrigin, (*ent).parent, (*ent).splashDamage as f32, (*ent).splashRadius as f32, ent, ent, (*ent).splashMethodOfDeath) != 0 {
                if !(*ent).parent.is_null() {
                    let parent_num = (*(*ent).parent).s.number as usize;
                    if !(*ctx.world).entities[parent_num].client.is_null() {
                        (*(*ctx.world).entities[parent_num].client).accuracy_hits += 1;
                    }
                } else if !(*ent).activator.is_null() {
                    let activator_num = (*(*ent).activator).s.number as usize;
                    if !(*ctx.world).entities[activator_num].client.is_null() {
                        (*(*ctx.world).entities[activator_num].client).accuracy_hits += 1;
                    }
                }
            }
        }

        trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): indexes `g_entities` and
// reads the shared `vec3_origin` global.
/// Raven `G_RunStuckMissile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:259-277`
pub fn G_RunStuckMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t) {
    unsafe {
        if (*ent).takedamage != 0 {
            if (*ent).s.groundEntityNum >= 0 && ((*ent).s.groundEntityNum as usize) < ENTITYNUM_WORLD as usize {
                let other = &mut (*ctx.world).entities[(*ent).s.groundEntityNum as usize];

                let delta_moving = !VectorCompare(crate::q_math::vec3_origin, (*other).s.pos.trDelta) && (*other).s.pos.trType != TR_STATIONARY;
                let apos_moving = !VectorCompare(crate::q_math::vec3_origin, (*other).s.apos.trDelta) && (*other).s.apos.trType != TR_STATIONARY;

                if delta_moving || apos_moving {
                    // Thing I stuck to is moving or rotating now, kill me
                    G_Damage(ent, other, other, core::ptr::null_mut(), core::ptr::null_mut(), 99999, 0, MOD_CRUSH as c_int);
                    return;
                }
            }
        }
        // Check think function
        G_RunThink(ctx, ent);
    }
}

// PORT-ESCALATION(vec3-outparam-seam): `endout` is a C out-param, but the
// staged skeleton signature passes `vec3_t` (`[f32; 3]`, `Copy`) by value with
// no `&mut`/return channel to write it back to the caller — same seam noted
// at `locateCamera` in `g_misc.rs`. Also calls the still-unported
// `VectorNormalize` (q_math.c, in-file skeleton is `todo!()`).
/// Raven `G_BounceProjectile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:284-294`
pub fn G_BounceProjectile(start: vec3_t, impact: vec3_t, dir: vec3_t, endout: &mut [f32;3]) {
    let mut v: vec3_t = [0.0; 3];
    let mut newv: vec3_t = [0.0; 3];

    crate::q_math::_VectorSubtract(&impact, &start, &mut v);
    let dot = crate::q_math::_DotProduct(&v, &dir);
    crate::q_math::_VectorMA(&v, -2.0 * dot, &dir, &mut newv);

    crate::q_math::VectorNormalize(&mut newv);
    crate::q_math::_VectorMA(&impact, 8192.0, &newv, endout);
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): calls `G_Spawn` (arena
// allocation) and reads `level.time`; writes the `think` fn-pointer field to
// `G_FreeEntity` (ruling 2 — needs `EntThink::G_FreeEntity`, not a bare fn
// pointer store).
/// Raven `CreateMissile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:298-329`
pub fn CreateMissile(
    ctx: GameContext<'_>,
    org: vec3_t,
    dir: vec3_t,
    vel: f32,
    life: c_int,
    owner: *mut gentity_t,
    altFire: qboolean,
) -> *mut gentity_t {
    unsafe {
        let missile = G_Spawn(ctx);

        (*missile).nextthink = (*ctx.world).level.time + life;
        (*missile).think = Some(EntThink::G_FreeEntity);
        (*missile).s.eType = ET_MISSILE;
        (*missile).r.svFlags = SVF_USE_CURRENT_ORIGIN;
        (*missile).parent = owner;
        (*missile).r.ownerNum = (*owner).s.number;

        if altFire != 0 {
            (*missile).s.eFlags |= EF_ALT_FIRING;
        }

        (*missile).s.pos.trType = TR_LINEAR;
        (*missile).s.pos.trTime = (*ctx.world).level.time;
        (*missile).target_ent = core::ptr::null_mut();

        let mut snapped_org = org;
        trap::SnapVector(ctx.engine, crate::trap::GSnapvector::new(&mut snapped_org));
        crate::q_math::_VectorCopy(&snapped_org, &mut (*missile).s.pos.trBase);
        crate::q_math::_VectorScale(&dir, vel, &mut (*missile).s.pos.trDelta);
        crate::q_math::_VectorCopy(&snapped_org, &mut (*missile).r.currentOrigin);
        trap::SnapVector(ctx.engine, crate::trap::GSnapvector::new(&mut (*missile).s.pos.trDelta));

        missile
    }
}

/// Raven `G_MissileBounceEffect`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:331-354`
pub fn G_MissileBounceEffect(
    ctx: GameContext<'_>,ent: *mut gentity_t, org: vec3_t, dir: vec3_t) {
    //FIXME: have an EV_BOUNCE_MISSILE event that checks the s.weapon and does the appropriate effect
    unsafe {
        match (*ent).s.weapon {
            WP_BOWCASTER => {
                let fx = G_EffectIndex(c"bowcaster/deflect".as_ptr());
                G_PlayEffectID(fx, (*ent).r.currentOrigin, dir);
            }
            WP_BLASTER | WP_BRYAR_PISTOL => {
                let fx = G_EffectIndex(c"blaster/deflect".as_ptr());
                G_PlayEffectID(fx, (*ent).r.currentOrigin, dir);
            }
            _ => {
                let te = G_TempEntity(ctx, org, EV_SABER_BLOCK as c_int);
                (*te).s.origin = org;
                (*te).s.angles = dir;
                (*te).s.eventParm = 0;
                (*te).s.weapon = 0; //saberNum
                (*te).s.legsAnim = 0; //bladeNum
            }
        }
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads `level.time`,
// indexes `g_entities` (owner/otherOwner lookups), and calls
// `trap_LinkEntity`.
/// Raven `G_MissileImpact`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:362-801`
pub fn G_MissileImpact(
    ctx: GameContext<'_>,ent: *mut gentity_t, trace: *mut trace_t) {
    unsafe {
        let tr = &*trace;
        let mut other = &mut (*ctx.world).entities[tr.entityNum as usize];
        let mut hitClient = qfalse;
        let mut isKnockedSaber = qfalse;

        // Check for bounce
        if (*other).takedamage == 0 && ((*ent).bounceCount > 0 || (*ent).bounceCount == -5) && (((*ent).flags & (FL_BOUNCE | FL_BOUNCE_HALF)) != 0) {
            G_BounceMissile(ctx, ent, trace);
            G_AddEvent(ent, EV_GRENADE_BOUNCE as c_int, 0);
            return;
        } else if (*ent).neverFree != 0 && (*ent).s.weapon == WP_SABER && (((*ent).flags & FL_BOUNCE_HALF) != 0) {
            // This is a knocked-away saber
            if (*ent).bounceCount > 0 || (*ent).bounceCount == -5 {
                G_BounceMissile(ctx, ent, trace);
                G_AddEvent(ent, EV_GRENADE_BOUNCE as c_int, 0);
                return;
            }
            isKnockedSaber = qtrue;
        }

        // Bounce shrapnel and force field handling
        if (!(*other).takedamage != 0 && ((*ent).bounceCount > 0 || (*ent).bounceCount == -5) && (((*ent).flags & FL_BOUNCE_SHRAPNEL) != 0)) || (((tr.surfaceFlags & SURF_FORCEFIELD) != 0) && (*ent).splashDamage == 0 && (*ent).splashRadius == 0 && ((*ent).bounceCount > 0 || (*ent).bounceCount == -5)) {
            G_BounceMissile(ctx, ent, trace);
            if (*ent).bounceCount < 1 {
                (*ent).flags &= !FL_BOUNCE_SHRAPNEL;
            }
            return;
        }

        // Saber block checks
        if (((*other).r.contents & CONTENTS_LIGHTSABER) != 0) && isKnockedSaber == 0 {
            let otherOwner = &mut (*ctx.world).entities[(*other).r.ownerNum as usize];
            if (*otherOwner).takedamage != 0 && !(*otherOwner).client.is_null() && (*(*otherOwner).client).ps.duelInProgress != 0 && (*(*otherOwner).client).ps.duelIndex != (*ent).r.ownerNum {
                // Jump to killProj label
                let mut dir: vec3_t = [0.0, 0.0, 1.0];
                if (*other).takedamage != 0 && !(*other).client.is_null() && isKnockedSaber == 0 {
                    G_AddEvent(ent, EV_MISSILE_HIT as c_int, DirToByte(tr.plane.normal));
                    (*ent).s.otherEntityNum = (*other).s.number;
                } else if (tr.surfaceFlags & SURF_METALSTEPS) != 0 {
                    G_AddEvent(ent, EV_MISSILE_MISS_METAL as c_int, DirToByte(tr.plane.normal));
                } else if (*ent).s.weapon != G2_MODEL_PART && isKnockedSaber == 0 {
                    G_AddEvent(ent, EV_MISSILE_MISS as c_int, DirToByte(tr.plane.normal));
                }
                if isKnockedSaber == 0 {
                    (*ent).freeAfterEvent = qtrue;
                    (*ent).s.eType = ET_GENERAL;
                }
                crate::q_math::SnapVectorTowards(tr.endpos, (*ent).s.pos.trBase);
                G_SetOrigin(ent, tr.endpos);
                (*ent).takedamage = qfalse;
                if (*ent).splashDamage != 0 {
                    if G_RadiusDamage(ctx, tr.endpos, (*ent).parent, (*ent).splashDamage as f32, (*ent).splashRadius as f32, other, ent, (*ent).splashMethodOfDeath) != 0 {
                        if hitClient == 0 && !(*ctx.world).entities[(*ent).r.ownerNum as usize].client.is_null() {
                            (*(*ctx.world).entities[(*ent).r.ownerNum as usize].client).accuracy_hits += 1;
                        }
                    }
                }
                if (*ent).s.weapon == G2_MODEL_PART {
                    (*ent).freeAfterEvent = qfalse;
                }
                trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));
                return;
            }
        } else if isKnockedSaber == 0 {
            if (*other).takedamage != 0 && !(*other).client.is_null() && (*(*other).client).ps.duelInProgress != 0 && (*(*other).client).ps.duelIndex != (*ent).r.ownerNum {
                // Jump to killProj label
                let mut dir: vec3_t = [0.0, 0.0, 1.0];
                if (*other).takedamage != 0 && !(*other).client.is_null() && isKnockedSaber == 0 {
                    G_AddEvent(ent, EV_MISSILE_HIT as c_int, DirToByte(tr.plane.normal));
                    (*ent).s.otherEntityNum = (*other).s.number;
                } else if (tr.surfaceFlags & SURF_METALSTEPS) != 0 {
                    G_AddEvent(ent, EV_MISSILE_MISS_METAL as c_int, DirToByte(tr.plane.normal));
                } else if (*ent).s.weapon != G2_MODEL_PART && isKnockedSaber == 0 {
                    G_AddEvent(ent, EV_MISSILE_MISS as c_int, DirToByte(tr.plane.normal));
                }
                if isKnockedSaber == 0 {
                    (*ent).freeAfterEvent = qtrue;
                    (*ent).s.eType = ET_GENERAL;
                }
                crate::q_math::SnapVectorTowards(tr.endpos, (*ent).s.pos.trBase);
                G_SetOrigin(ent, tr.endpos);
                (*ent).takedamage = qfalse;
                if (*ent).splashDamage != 0 {
                    if G_RadiusDamage(ctx, tr.endpos, (*ent).parent, (*ent).splashDamage as f32, (*ent).splashRadius as f32, other, ent, (*ent).splashMethodOfDeath) != 0 {
                        if hitClient == 0 && !(*ctx.world).entities[(*ent).r.ownerNum as usize].client.is_null() {
                            (*(*ctx.world).entities[(*ent).r.ownerNum as usize].client).accuracy_hits += 1;
                        }
                    }
                }
                if (*ent).s.weapon == G2_MODEL_PART {
                    (*ent).freeAfterEvent = qfalse;
                }
                trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));
                return;
            }
        }

        // Heavy weapon deflection check
        if ((*other).flags & FL_DMG_BY_HEAVY_WEAP_ONLY) != 0 {
            if (*ent).methodOfDeath != MOD_REPEATER_ALT as c_int && (*ent).methodOfDeath != MOD_ROCKET as c_int && (*ent).methodOfDeath != MOD_FLECHETTE_ALT_SPLASH as c_int && (*ent).methodOfDeath != MOD_ROCKET_HOMING as c_int && (*ent).methodOfDeath != MOD_THERMAL as c_int && (*ent).methodOfDeath != MOD_THERMAL_SPLASH as c_int && (*ent).methodOfDeath != MOD_TRIP_MINE_SPLASH as c_int && (*ent).methodOfDeath != MOD_TIMED_MINE_SPLASH as c_int && (*ent).methodOfDeath != MOD_DET_PACK_SPLASH as c_int && (*ent).methodOfDeath != MOD_VEHICLE as c_int && (*ent).methodOfDeath != MOD_CONC as c_int && (*ent).methodOfDeath != MOD_CONC_ALT as c_int && (*ent).methodOfDeath != MOD_SABER as c_int && (*ent).methodOfDeath != MOD_TURBLAST as c_int && (*ent).methodOfDeath != MOD_TARGET_LASER as c_int {
                let mut fwd: vec3_t = [0.0; 3];
                if !trace.is_null() {
                    crate::q_math::_VectorCopy(&tr.plane.normal, &mut fwd);
                } else {
                    crate::q_math::AngleVectors((*other).r.currentAngles, Some(&mut fwd), None, None);
                }
                G_DeflectMissile(ctx, other, ent, fwd);
                G_MissileBounceEffect(ctx, ent, (*ent).r.currentOrigin, fwd);
                return;
            }
        }

        // Shielded entity deflection
        if (((*other).flags & FL_SHIELDED) != 0) && (*ent).s.weapon != WP_ROCKET_LAUNCHER && (*ent).s.weapon != WP_THERMAL && (*ent).s.weapon != WP_TRIP_MINE && (*ent).s.weapon != WP_DET_PACK && (*ent).s.weapon != WP_DEMP2 && (*ent).s.weapon != WP_EMPLACED_GUN && (*ent).methodOfDeath != MOD_REPEATER_ALT as c_int && (*ent).methodOfDeath != MOD_FLECHETTE_ALT_SPLASH as c_int && (*ent).methodOfDeath != MOD_TURBLAST as c_int && (*ent).methodOfDeath != MOD_TARGET_LASER as c_int && (*ent).methodOfDeath != MOD_VEHICLE as c_int && (*ent).methodOfDeath != MOD_CONC as c_int && (*ent).methodOfDeath != MOD_CONC_ALT as c_int && ((*ent).dflags & DAMAGE_HEAVY_WEAP_CLASS) == 0 {
            let mut fwd: vec3_t = [0.0; 3];
            if !(*other).client.is_null() {
                crate::q_math::AngleVectors((*(*other).client).ps.viewangles, Some(&mut fwd), None, None);
            } else {
                crate::q_math::AngleVectors((*other).r.currentAngles, Some(&mut fwd), None, None);
            }
            G_DeflectMissile(ctx, other, ent, fwd);
            G_MissileBounceEffect(ctx, ent, (*ent).r.currentOrigin, fwd);
            return;
        }

        // Saber block handling
        if (*other).takedamage != 0 && !(*other).client.is_null() && (*ent).s.weapon != WP_ROCKET_LAUNCHER && (*ent).s.weapon != WP_THERMAL && (*ent).s.weapon != WP_TRIP_MINE && (*ent).s.weapon != WP_DET_PACK && (*ent).s.weapon != WP_DEMP2 && (*ent).methodOfDeath != MOD_REPEATER_ALT as c_int && (*ent).methodOfDeath != MOD_FLECHETTE_ALT_SPLASH as c_int && (*ent).methodOfDeath != MOD_CONC as c_int && (*ent).methodOfDeath != MOD_CONC_ALT as c_int && (*(*other).client).ps.saberBlockTime < (*ctx.world).level.time && isKnockedSaber == 0 && WP_SaberCanBlock(ctx, other, (*ent).r.currentOrigin, 0, 0, qtrue, 0) != 0 {
            let mut fwd: vec3_t = [0.0; 3];
            let otherDefLevel = (*(*other).client).ps.fd.forcePowerLevel[FP_SABER_DEFENSE as usize];

            let te = G_TempEntity(ctx, (*ent).r.currentOrigin, EV_SABER_BLOCK as c_int);
            crate::q_math::_VectorCopy(&(*ent).r.currentOrigin, &mut (*te).s.origin);
            crate::q_math::_VectorCopy(&tr.plane.normal, &mut (*te).s.angles);
            (*te).s.eventParm = 0;
            (*te).s.weapon = 0;
            (*te).s.legsAnim = 0;

            let mut adjusted_def_level = otherDefLevel;
            if (*(*other).client).ps.velocity[2] > 0.0 || (*(*other).client).pers.cmd.forwardmove < 0 {
                adjusted_def_level -= 1;
                if adjusted_def_level < 0 {
                    adjusted_def_level = 0;
                }
            }

            crate::q_math::AngleVectors((*(*other).client).ps.viewangles, Some(&mut fwd), None, None);
            if adjusted_def_level == FORCE_LEVEL_1 as c_int {
                // Kill the projectile
            } else if adjusted_def_level == FORCE_LEVEL_2 as c_int {
                G_DeflectMissile(ctx, other, ent, fwd);
            } else {
                G_ReflectMissile(ctx, other, ent, fwd);
            }
            (*(*other).client).ps.saberBlockTime = (*ctx.world).level.time + (350 - (adjusted_def_level * 100));
            (*(*other).client).ps.saberEventFlags |= SEF_DEFLECTED;

            if adjusted_def_level == FORCE_LEVEL_3 as c_int {
                (*(*other).client).ps.saberBlockTime = 0;
            }

            if adjusted_def_level == FORCE_LEVEL_1 as c_int {
                // Fall through to killProj
            } else {
                return;
            }
        } else if (((*other).r.contents & CONTENTS_LIGHTSABER) != 0) && isKnockedSaber == 0 {
            let otherOwner = &mut (*ctx.world).entities[(*other).r.ownerNum as usize];
            if (*otherOwner).takedamage != 0 && !(*otherOwner).client.is_null() && (*ent).s.weapon != WP_ROCKET_LAUNCHER && (*ent).s.weapon != WP_THERMAL && (*ent).s.weapon != WP_TRIP_MINE && (*ent).s.weapon != WP_DET_PACK && (*ent).s.weapon != WP_DEMP2 && (*ent).methodOfDeath != MOD_REPEATER_ALT as c_int && (*ent).methodOfDeath != MOD_FLECHETTE_ALT_SPLASH as c_int && (*ent).methodOfDeath != MOD_CONC as c_int && (*ent).methodOfDeath != MOD_CONC_ALT as c_int {
                let mut fwd: vec3_t = [0.0; 3];
                let te = G_TempEntity(ctx, (*ent).r.currentOrigin, EV_SABER_BLOCK as c_int);
                crate::q_math::_VectorCopy(&(*ent).r.currentOrigin, &mut (*te).s.origin);
                crate::q_math::_VectorCopy(&tr.plane.normal, &mut (*te).s.angles);
                (*te).s.eventParm = 0;
                (*te).s.weapon = 0;
                (*te).s.legsAnim = 0;

                let mut other_def_level = (*(*otherOwner).client).ps.fd.forcePowerLevel[FP_SABER_DEFENSE as usize];
                if (*(*otherOwner).client).ps.velocity[2] > 0.0 || (*(*otherOwner).client).pers.cmd.forwardmove < 0 {
                    other_def_level -= 1;
                    if other_def_level < 0 {
                        other_def_level = 0;
                    }
                }

                crate::q_math::AngleVectors((*(*otherOwner).client).ps.viewangles, Some(&mut fwd), None, None);

                if other_def_level == FORCE_LEVEL_1 as c_int {
                    // Kill projectile
                } else if other_def_level == FORCE_LEVEL_2 as c_int {
                    G_DeflectMissile(ctx, otherOwner, ent, fwd);
                } else {
                    G_ReflectMissile(ctx, otherOwner, ent, fwd);
                }
                (*(*otherOwner).client).ps.saberBlockTime = (*ctx.world).level.time + (350 - (other_def_level * 100));
                (*(*otherOwner).client).ps.saberEventFlags |= SEF_DEFLECTED;

                if other_def_level == FORCE_LEVEL_3 as c_int {
                    (*(*otherOwner).client).ps.saberBlockTime = 0;
                }

                if other_def_level == FORCE_LEVEL_1 as c_int {
                    // Fall through to killProj
                } else {
                    return;
                }
            }
        }

        // Sticky missiles
        if (*other).takedamage == 0 && (((*ent).s.eFlags & EF_MISSILE_STICK) != 0) {
            laserTrapStick(ctx, ent, tr.endpos, tr.plane.normal);
            G_AddEvent(ent, EV_MISSILE_STICK as c_int, 0);
            return;
        }

        // Impact damage
        if (*other).takedamage != 0 && isKnockedSaber == 0 {
            if (*ent).damage != 0 {
                let mut velocity: vec3_t = [0.0; 3];
                let mut didDmg = qfalse;

                if LogAccuracyHit(ctx, other, &mut (*ctx.world).entities[(*ent).r.ownerNum as usize]) != 0 {
                    (*(*ctx.world).entities[(*ent).r.ownerNum as usize].client).accuracy_hits += 1;
                    hitClient = qtrue;
                }

                crate::bg_misc::BG_EvaluateTrajectoryDelta(&(*ent).s.pos, (*ctx.world).level.time, &mut velocity);
                if crate::q_math::VectorLength(velocity) == 0.0 {
                    velocity[2] = 1.0;
                }

                if (*ent).s.weapon == WP_BOWCASTER || (*ent).s.weapon == WP_FLECHETTE || (*ent).s.weapon == WP_ROCKET_LAUNCHER {
                    if (*ent).s.weapon == WP_FLECHETTE && (((*ent).s.eFlags & EF_ALT_FIRING) != 0) {
                        if let Some(think_fn) = (*ent).think {
                            // Call the think function
                            (think_fn)(ctx, ent);
                        }
                    } else {
                        G_Damage(other, ent, &mut (*ctx.world).entities[(*ent).r.ownerNum as usize], velocity, (*ent).r.currentOrigin, (*ent).damage, DAMAGE_HALF_ABSORB as c_int, (*ent).methodOfDeath);
                        didDmg = qtrue;
                    }
                } else {
                    G_Damage(other, ent, &mut (*ctx.world).entities[(*ent).r.ownerNum as usize], velocity, (*ent).r.currentOrigin, (*ent).damage, 0, (*ent).methodOfDeath);
                    didDmg = qtrue;
                }

                if didDmg != 0 && !other.is_null() && !(*other).client.is_null() {
                    let npc_class = (*(*other).client).NPC_class;
                    if npc_class == CLASS_SEEKER || npc_class == CLASS_PROBE || npc_class == CLASS_MOUSE || npc_class == CLASS_GONK || npc_class == CLASS_R2D2 || npc_class == CLASS_R5D2 || npc_class == CLASS_REMOTE || npc_class == CLASS_MARK1 || npc_class == CLASS_MARK2 || npc_class == CLASS_INTERROGATOR || npc_class == CLASS_ATST || npc_class == CLASS_SENTRY {
                        if (*(*other).client).ps.electrifyTime < (*ctx.world).level.time + 100 {
                            (*(*other).client).ps.electrifyTime = (*ctx.world).level.time + 450;
                        }
                    }
                }
            }

            if (*ent).s.weapon == WP_DEMP2 {
                if !other.is_null() && !(*other).client.is_null() && (*(*other).client).NPC_class == CLASS_VEHICLE {
                    if !(*other).m_pVehicle.is_null() && !(*(*other).m_pVehicle).m_pVehicleInfo.is_null() && ((*(*(*other).m_pVehicle).m_pVehicleInfo).type_ == VH_SPEEDER || ((*(*(*other).m_pVehicle).m_pVehicleInfo).type_ == VH_FIGHTER && !(*ent).classname.is_null() && crate::q_shared::Q_stricmp((*ent).classname, c"vehicle_proj".as_ptr() as *const c_char) == 0)) && FighterIsLanded((*other).m_pVehicle, &mut (*(*other).client).ps) == 0 && (((*other).spawnflags & 2) == 0) {
                        if (*(*other).client).ps.electrifyTime > (*ctx.world).level.time {
                            (*(*other).client).ps.electrifyTime += crate::q_math::Q_irand(200, 500);
                            if (*(*other).client).ps.electrifyTime > (*ctx.world).level.time + 4000 {
                                (*(*other).client).ps.electrifyTime = (*ctx.world).level.time + 4000;
                            }
                        } else {
                            (*(*other).client).ps.electrifyTime = (*ctx.world).level.time + crate::q_math::Q_irand(200, 500);
                        }
                    }
                } else if !other.is_null() && !(*other).client.is_null() && (*(*other).client).ps.powerups[PW_CLOAKED as usize] != 0 {
                    Jedi_Decloak(ctx, other);
                    if (*ent).methodOfDeath == MOD_DEMP2_ALT as c_int {
                        (*(*other).client).cloakToggleTime = Q3_INFINITE;
                    } else {
                        (*(*other).client).cloakToggleTime = (*ctx.world).level.time + crate::q_math::Q_irand(3000, 10000);
                    }
                }
            }
        }

        // killProj label - generate impact event
        let mut dir: vec3_t = [0.0, 0.0, 1.0];
        if (*other).takedamage != 0 && !(*other).client.is_null() && isKnockedSaber == 0 {
            G_AddEvent(ent, EV_MISSILE_HIT as c_int, DirToByte(tr.plane.normal));
            (*ent).s.otherEntityNum = (*other).s.number;
        } else if (tr.surfaceFlags & SURF_METALSTEPS) != 0 {
            G_AddEvent(ent, EV_MISSILE_MISS_METAL as c_int, DirToByte(tr.plane.normal));
        } else if (*ent).s.weapon != G2_MODEL_PART && isKnockedSaber == 0 {
            G_AddEvent(ent, EV_MISSILE_MISS as c_int, DirToByte(tr.plane.normal));
        }

        if isKnockedSaber == 0 {
            (*ent).freeAfterEvent = qtrue;
            (*ent).s.eType = ET_GENERAL;
        }

        crate::q_math::SnapVectorTowards(tr.endpos, (*ent).s.pos.trBase);
        G_SetOrigin(ent, tr.endpos);

        (*ent).takedamage = qfalse;
        // Splash damage
        if (*ent).splashDamage != 0 {
            if G_RadiusDamage(ctx, tr.endpos, (*ent).parent, (*ent).splashDamage as f32, (*ent).splashRadius as f32, other, ent, (*ent).splashMethodOfDeath) != 0 {
                if hitClient == 0 && !(*ctx.world).entities[(*ent).r.ownerNum as usize].client.is_null() {
                    (*(*ctx.world).entities[(*ent).r.ownerNum as usize].client).accuracy_hits += 1;
                }
            }
        }

        if (*ent).s.weapon == G2_MODEL_PART {
            (*ent).freeAfterEvent = qfalse;
        }

        trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));
    }
}

// PORT-ESCALATION(raw-ptr-skeleton-no-world-handle): reads cvars
// `d_projectileGhoul2Collision`/`g_g2TraceLod`, indexes `g_entities`, reads
// `level.time`, and calls `trap_G2Trace`/`trap_Trace`/`trap_LinkEntity`.
/// Raven `G_RunMissile`.
///
/// Source: `oracle/oracle/codemp/game/g_missile.c:808-1019`
pub fn G_RunMissile(
    ctx: GameContext<'_>,ent: *mut gentity_t) {
    unsafe {
        let mut origin: vec3_t = [0.0; 3];
        let mut groundSpot: vec3_t = [0.0; 3];
        let mut tr: trace_t = std::mem::zeroed();
        let mut passent: c_int;
        let mut isKnockedSaber = qfalse;

        if (*ent).neverFree != 0 && (*ent).s.weapon == WP_SABER && (((*ent).flags & FL_BOUNCE_HALF) != 0) {
            isKnockedSaber = qtrue;
            (*ent).s.pos.trType = TR_GRAVITY;
        }

        // Get current position
        crate::bg_misc::BG_EvaluateTrajectory(&(*ent).s.pos, (*ctx.world).level.time, &mut origin);

        // Determine entity to pass (not collide with)
        if !(*ent).target_ent.is_null() {
            passent = (*(*ent).target_ent).s.number;
        } else if ((*ent).r.svFlags & SVF_OWNERNOTSHARED) != 0 && (((*ent).s.eFlags & EF_JETPACK_ACTIVE) != 0) {
            // Vehicle missile that should be solid to owner
            passent = (*ent).s.number;
        } else {
            passent = (*ent).r.ownerNum as c_int;
        }

        // Trace from previous position to current position
        if (*ctx.world).cvars.d_projectileGhoul2Collision.integer != 0 {
            trap::G2Trace(
                ctx.engine,
                crate::trap::GG2trace::new(
                    &mut tr,
                    &(*ent).r.currentOrigin,
                    &(*ent).r.mins,
                    &(*ent).r.maxs,
                    &origin,
                    passent,
                    (*ent).clipmask,
                    G2TRFLAG_DOGHOULTRACE | G2TRFLAG_GETSURFINDEX | G2TRFLAG_THICK | G2TRFLAG_HITCORPSES,
                    (*ctx.world).cvars.g_g2TraceLod.integer,
                ),
            );

            if tr.fraction != 1.0 && (tr.entityNum as usize) < ENTITYNUM_WORLD as usize {
                let g2Hit = &mut (*ctx.world).entities[tr.entityNum as usize];
                if (*g2Hit).inuse != 0 && !(*g2Hit).client.is_null() && !(*g2Hit).ghoul2.is_null() {
                    (*(*g2Hit).client).g2LastSurfaceHit = tr.surfaceFlags as c_int;
                    (*(*g2Hit).client).g2LastSurfaceTime = (*ctx.world).level.time;
                }
                if !(*g2Hit).ghoul2.is_null() {
                    tr.surfaceFlags = 0;
                }
            }
        } else {
            trap::Trace(
                ctx.engine,
                crate::trap::GTrace::new(
                    &mut tr,
                    &(*ent).r.currentOrigin,
                    &(*ent).r.mins,
                    &(*ent).r.maxs,
                    &origin,
                    passent,
                    (*ent).clipmask,
                ),
            );
        }

        if tr.startsolid != 0 || tr.allsolid != 0 {
            // Make sure tr.entityNum is set to the entity we're stuck in
            trap::Trace(
                ctx.engine,
                crate::trap::GTrace::new(
                    &mut tr,
                    &(*ent).r.currentOrigin,
                    &(*ent).r.mins,
                    &(*ent).r.maxs,
                    &(*ent).r.currentOrigin,
                    passent,
                    (*ent).clipmask,
                ),
            );
            tr.fraction = 0.0;
        } else {
            crate::q_math::_VectorCopy(&tr.endpos, &mut (*ent).r.currentOrigin);
        }

        if (*ent).passThroughNum != 0 && tr.entityNum == ((*ent).passThroughNum - 1) as u32 {
            crate::q_math::_VectorCopy(&origin, &mut (*ent).r.currentOrigin);
            trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));
            // Fall through to passthrough label
        } else {
            trap::LinkEntity(ctx.engine, crate::trap::GLinkentity::new(ent));

            if (*ent).s.weapon == G2_MODEL_PART && (*ent).bounceCount == 0 {
                let mut lowerOrg: vec3_t = [0.0; 3];
                let mut trG: trace_t = std::mem::zeroed();

                crate::q_math::_VectorCopy(&(*ent).r.currentOrigin, &mut lowerOrg);
                lowerOrg[2] -= 1.0;
                trap::Trace(
                    ctx.engine,
                    crate::trap::GTrace::new(
                        &mut trG,
                        &(*ent).r.currentOrigin,
                        &(*ent).r.mins,
                        &(*ent).r.maxs,
                        &lowerOrg,
                        passent,
                        (*ent).clipmask,
                    ),
                );

                crate::q_math::_VectorCopy(&trG.endpos, &mut groundSpot);

                if trG.startsolid == 0 && trG.allsolid == 0 && trG.entityNum == ENTITYNUM_WORLD as u32 {
                    (*ent).s.groundEntityNum = trG.entityNum as c_int;
                } else {
                    (*ent).s.groundEntityNum = ENTITYNUM_NONE as c_int;
                }
            }

            if tr.fraction != 1.0 {
                // Never explode or bounce on sky
                if (tr.surfaceFlags & SURF_NOIMPACT) != 0 {
                    // If grapple, reset owner
                    if !(*ent).parent.is_null() && !(*(*ent).parent).client.is_null() && (*(*ent).parent).client.hook == ent {
                        (*(*ent).parent).client.hook = core::ptr::null_mut();
                    }

                    if ((*ent).s.weapon == WP_SABER && (*ent).isSaberEntity != 0) || isKnockedSaber != 0 {
                        G_RunThink(ctx, ent);
                        return;
                    } else if (*ent).s.weapon != G2_MODEL_PART {
                        G_FreeEntity(ctx, ent);
                        return;
                    }
                }

                // Ghoul2 mark handling code is #if 0, so we skip to the active part
                if (*ent).s.weapon > WP_NONE as c_int && (*ent).s.weapon < WP_NUM_WEAPONS as c_int && (tr.entityNum as usize) < MAX_CLIENTS || (*ctx.world).entities[tr.entityNum as usize].s.eType == ET_NPC {
                    crate::q_math::_VectorCopy(&(*ent).r.currentOrigin, &mut (*ent).s.origin);
                    crate::bg_misc::BG_EvaluateTrajectory(&(*ent).s.pos, (*ctx.world).level.time, &mut (*ent).s.origin2);

                    if VectorCompare((*ent).s.origin, (*ent).s.origin2) != 0 {
                        (*ent).s.origin2[2] += 2.0;
                    }
                }

                G_MissileImpact(ctx, ent, &mut tr);

                if tr.entityNum == (*ent).s.otherEntityNum as u32 {
                    (*ent).s.trickedentindex = 1;
                }

                if (*ent).s.eType != ET_MISSILE && (*ent).s.weapon != G2_MODEL_PART {
                    return; // Exploded
                }
            }
        }

        // passthrough label
        if (*ent).s.pos.trType == TR_STATIONARY && (((*ent).s.eFlags & EF_MISSILE_STICK) != 0) {
            // Stuck missiles should check some special stuff
            G_RunStuckMissile(ctx, ent);
            return;
        }

        if (*ent).s.weapon == G2_MODEL_PART {
            if (*ent).s.groundEntityNum == ENTITYNUM_WORLD as c_int {
                (*ent).s.pos.trType = TR_LINEAR;
                crate::q_math::VectorClear(&mut (*ent).s.pos.trDelta);
                (*ent).s.pos.trTime = (*ctx.world).level.time;

                crate::q_math::_VectorCopy(&groundSpot, &mut (*ent).s.pos.trBase);
                crate::q_math::_VectorCopy(&groundSpot, &mut (*ent).r.currentOrigin);

                if (*ent).s.apos.trType != TR_STATIONARY {
                    (*ent).s.apos.trType = TR_STATIONARY;
                    (*ent).s.apos.trTime = (*ctx.world).level.time;

                    (*ent).s.apos.trBase[ROLL as usize] = 0.0;
                    (*ent).s.apos.trBase[PITCH as usize] = 0.0;
                }
            }
        }

        // Check think function after bouncing
        G_RunThink(ctx, ent);
    }
}
