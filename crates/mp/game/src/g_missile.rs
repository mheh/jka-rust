//! Port of `oracle/codemp/game/g_missile.c`.
//!
//! Functions reach file-scope game state (`level`, `g_entities`, cvars
...
//! Fn-pointer fields (`think`) are set through the `EntThink` enum, not a bare fn pointer.
...
pub fn G_ReflectMissile(ctx: &mut GameContext, ent: EntityId, missile: EntityId, forward: vec3_t) {
    ...
    // Raven resolves an `owner` pointer here only to null-test it below.
    // The test is vacuously true (always a live entity), so the walk is dropped.
...
pub fn G_DeflectMissile(ctx: &mut GameContext, ent: EntityId, missile: EntityId, forward: vec3_t) {
    ...
    // `client` is a raw `gentity_t*` seam field.
    // The gclient deref stays unsafe.
    // The entity walk is by handle.
...
pub fn G_BounceMissile(ctx: &mut GameContext, ent: EntityId, trace: &trace_t) {
    ...
    if (ctx.entity(ent).flags & FL_BOUNCE_SHRAPNEL) != 0 {
        ...
        // check for stop.
        // C promotes `normal[2]` to double against the unsuffixed `0.7`.
        ...
        // Raven `VectorScale(..., 0.65, ...)`.
        // The unsuffixed `0.65` makes the macro multiply each component in double, narrowed on store.
        ...
        // check for stop.
        // `normal[2]` (float) and `VectorLength` (float) promote to double against the unsuffixed `0.2`/`40` in C.
        // The f32-nearest 0.2 exceeds double 0.2, so evaluate in f64 to keep the branch decision.
    ...
    if weapon == WP_THERMAL {
        ...
        let sound = G_SoundIndex(ctx, &sound_name);
        G_Sound(ctx, Some(ent), CHAN_BODY as c_int, sound);
...
pub fn G_ExplodeMissile(ctx: &mut GameContext, ent: EntityId) {
    ...
    if ctx.entity(ent).splashDamage != 0 {
        // Set the parent for damage credit on vehicle missiles.
        // `s.eType` was set to `ET_GENERAL` above, so the `ET_MISSILE` check below never fires.
        ...
        if G_RadiusDamage(
            ...
            if let Some(parent_id) = parent {
                ...
                // §19: oracle derefs `parent->...client->accuracy_hits++` unconditionally (UB if the parent is a non-client entity).
                // The port guards it.
...
pub fn G_MissileBounceEffect(ctx: &mut GameContext, ent: EntityId, org: vec3_t, dir: vec3_t) {
    ...
    match ctx.entity(ent).s.weapon {
        WP_BOWCASTER => {
            let fx = G_EffectIndex(ctx, "bowcaster/deflect");
            let currentOrigin = ctx.entity(ent).r.currentOrigin;
            G_PlayEffectID(fx, currentOrigin, dir);
        }
        WP_BLASTER | WP_BRYAR_PISTOL => {
            let fx = G_EffectIndex(ctx, "blaster/deflect");
            let currentOrigin = ctx.entity(ent).r.currentOrigin;
            G_PlayEffectID(fx, currentOrigin, dir);
        }
        _ => {
...
pub fn G_MissileImpact(ctx: &mut GameContext, ent: EntityId, trace: &mut trace_t) {
    // `ent`/`other`/`otherOwner` are `EntityId` handles reached through the `GameContext` accessors.
    // `gclient_t`/`Vehicle_t` seam pointers, read from an entity's `.client`/`.m_pVehicle` field, are not arena entities.
    // Those derefs stay raw in tight `unsafe` blocks, matching Raven's approach.
    // Each `(*other).client`/`(*ent).client` is re-read at the point of use, never cached across a `G_*`/trap call.
    // This mirrors Raven's per-access field read, so no read crosses a mutation.
        ...
        } != 0
    {
        ...
        // Raven: FORCE_LEVEL_1 is `goto killProj`.
        // It skips the sticky-missile and impact-damage sections.
        // Source: `oracle/codemp/game/g_missile.c:563-567`
        if adjusted_def_level == FORCE_LEVEL_1 as c_int {
            missile_kill_proj(ctx, ent, other, tr, hitClient, isKnockedSaber);
        }
        return;
            ...
            && ctx.entity(ent).methodOfDeath != MOD_CONC_ALT as c_int
        {
            ...
            // This triggers the defender's saber-block animation.
            ...
            // Raven: FORCE_LEVEL_1 is `goto killProj`.
            // It skips the sticky-missile and impact-damage sections.
            // Source: `oracle/codemp/game/g_missile.c:640-644`
            if other_def_level == FORCE_LEVEL_1 as c_int {
                missile_kill_proj(ctx, ent, other, tr, hitClient, isKnockedSaber);
            }
            return;
    ...
    if ctx.entity(other).takedamage != 0 && !isKnockedSaber {
        if ctx.entity(ent).damage != 0 {
                ...
                || ctx.entity(ent).s.weapon == WP_ROCKET_LAUNCHER
            {
                ...
                {
                    // §19: oracle calls `ent->think(ent)` unconditionally (UB if think is NULL).
                    // The port guards the dispatch.
...
/// Split from `G_MissileImpact`.
/// This is Raven's `killProj` label tail: impact event, endpos snap and origin set, splash damage with accuracy credit, G2-part unfree, and relink.
/// Reached by `goto` from the duel-isolation and FORCE_LEVEL_1 saber-block arms, and by fall-through after impact damage.
...
pub fn G_RunMissile(ctx: &mut GameContext, ent: EntityId) {
    // `ent` is an `EntityId` handle reached through the `GameContext` accessors.
    // The one seam deref is the grapple owner's `gclient_t` (`parent.client`, read from the entity's `.client` field).
    // It is not an arena entity, so it stays raw in a tight `unsafe` block.
    let mut origin: vec3_t = [0.0; 3];
        ...
        && tr.entityNum as u32 == (ctx.entity(ent).passThroughNum - 1) as u32
    {
        ...
        if tr.fraction != 1.0 {
            ...
            if (tr.surfaceFlags & SURF_NOIMPACT) != 0 {
                ...
                // `parent.client` is a `gclient_t*` seam pointer, not an arena entity, so the deref stays raw.
