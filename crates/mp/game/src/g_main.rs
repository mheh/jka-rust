...
//! Rust has no field reflection, so this stands in for the `cv->vmCvar` pointer Raven's `cvarTable_t` carries
//! directly.
//! The ctx-free function-pointer boundary functions `Com_Error`/`Com_Printf` route through a print/error sink
//! registered at `dllEntry`.
//! They fall back to panic/stderr when no sink is registered (in-process tests).
...
// The QVM entry `vmMain` and `G_InitGame`/`G_ShutdownGame` are kept as-is.
// `vmMain` is the shell plus the `impl Dispatch<C> for GameContext` seam (`world/game_context.rs`).
// The two commands dispatch to `crate::g_init_game::g_init_game` and
// `crate::g_shutdown_game::g_shutdown_game`.
// The skeleton duplicates were dropped here.
// Source: `oracle/codemp/game/g_main.c:515-696,897,1128`

/// Raven `G_Printf`.
///
/// Source: `oracle/codemp/game/g_main.c:703-712`
pub fn G_Printf(ctx: &mut GameContext, msg: &str) {
    trap::Printf(ctx.engine, msg);
}

/// Raven `G_Error`.
///
/// Source: `oracle/codemp/game/g_main.c:714-723`
pub fn G_Error(ctx: &mut GameContext, msg: &str) {
...
pub fn G_FindTeams(ctx: &mut GameContext) {
                ...
                && e.r.contents != mp_qshared::shared::surface_flags::CONTENTS_TRIGGER
            {
                ...
                while j < num_entities {
                        ...
                        && e.team == e2.team
                    {
                        ...
                        // The master inherits the slave's `targetname` allocation, and the slave relinquishes it.
                        // This mirrors Raven's pointer move.
    ...
    // This call is commented out in the oracle.
}
...
/// Maps one `GAME_CVAR_TABLE` row's `field` name to its `vmCvar_t` storage in `GameCvars`.
/// This is mechanical field dispatch, standing in for the `cv->vmCvar` pointer Raven's `cvarTable_t` carries
/// directly.
/// Rust has no runtime field reflection to derive this from the row's `&'static str` name.
impl GameCvars {
...
/// Resolves one `GAME_CVAR_TABLE` row's symbolic `flags` expression (e.g. `"CVAR_SERVERINFO | CVAR_LATCH"`,
/// verbatim from `g_main.c:230-475`) to the numeric `trap_Cvar_Register` flags argument.
/// The `CVAR_*` bit consts themselves are real (`q_shared_cvar_flags`).
/// Only the OR-of-named-tokens shape needs mechanical parsing, since the table stores the expression as written
/// rather than a pre-folded int.
fn resolve_cvar_flags(expr: &str) -> c_int {
    // `CVAR_INTERNAL` (`q_shared.h:1800`) has no crate-wide const yet.
    // Only this one `gameCvarTable` row (`g_npcspskill`) uses it.
    // It is defined locally, rather than added to `q_shared_cvar_flags.rs`, under the concurrent-edit boundary
    // for this file.
...
pub fn G_RegisterCvars(ctx: &mut GameContext) {
    ...
    for (i, entry) in GAME_CVAR_TABLE.iter().enumerate() {
        let cvar_opt: Option<&mut vmCvar_t> = match entry.field {
            Some(name) => Some(ctx.world.cvars.field_mut(name)),
            None => None,
        };
        trap::Cvar_Register(
            ctx.engine,
    ...
    if gt < 0 || gt >= GT_MAX_GAME_TYPE {
        ...
        G_Printf(ctx, &msg);
        trap::Cvar_Set(ctx.engine, "g_gametype", "0");
    } else if gt == GT_HOLOCRON {
        G_Printf(ctx, "This gametype is not supported.\n");
        trap::Cvar_Set(ctx.engine, "g_gametype", "0");
...
pub fn G_UpdateCvars(ctx: &mut GameContext) {
    unsafe {
        ...
        for (i, entry) in GAME_CVAR_TABLE.iter().enumerate() {
            if let Some(name) = entry.field {
                // The `&mut vmCvar_t` still crosses the seam to `Cvar_Update`.
                // The engine writes the fresh value in place.
                // The readback then reads it through the safe `field_mut` accessor.
                trap::Cvar_Update(ctx.engine, ctx.world.cvars.field_mut(name));
                ...
                if ctx.world.globals.gameCvarModCounts.0[i] != new_mod_count {
                    ...
                    if entry.track_change {
                        ...
                        trap::SendServerCommand(ctx.engine, -1, &format!(
...
pub fn AddTournamentPlayer(ctx: &mut GameContext) {
    ...
    // Raven tracks `next_in_line` as a `gclient_t*` into `level.clients`.
    // The client index is the only thing read back (`offset_from`), so this carries the index directly.
    ...
    // set them to free-for-all team
        SetTeam(ctx, EntityId(idx as u32), "f");
}
...
pub fn RemoveTournamentLoser(ctx: &mut GameContext) {
    ...
    // make them a spectator
        SetTeam(ctx, EntityId(clientNum as u32), "s");
}
...
pub fn G_PowerDuelCount(
    ...
    // Entity slots `0..MAX_CLIENTS` are the player entities, whose `client` points at `level.clients[i]`
    // (Raven `g_entities[i].client = clients + i`).
    // So Raven's `(*ent).client` dereference reads `clients[i]`, indexed safely.
    // `loners`/`doubles` stay raw out-pointers, since the signature has an out-of-file caller in `g_bot.rs`.
    // Their writes are the only remaining unsafe code here.
    let mut i: c_int = 0;
...
pub fn AdjustTournamentScores(ctx: &mut GameContext) {
        ...
        && ctx.world.clients[s1 as usize].pers.connected == CON_CONNECTED
    {
        ...
        if cl_failure != 2 {
            ...
            trap::SetConfigstring(ctx.engine, CS_CLIENT_DUELWINNER, &format!("{}", clientNum));
...
//    Examples: `(n/2)*es`, the `n>40` ninther offsets `(n/8)*es`, partition walks, `vecswap` byte counts, and
//    recursion sizes `r/es`.
//    Every decision point, `n < 7`, pivot selection, `swap_cnt`, `pb > pc`, and `r > es` (2 or more elements),
//    depends only on `n` and the comparator return value.
//    `swaptype` picks the byte-copy strategy. It never picks which elements move.
//    Widening `int` to `{int, world}` cannot change the permutation.
//
// 3. Ties happen on the first frame. They are not hypothetical.
//    The comparator's final fallback returns 0 for connected non-spectator clients with equal `PERS_SCORE`, which
...
//    The same algorithm as the oracle stays identical at every `n`.
//
// Known costs, accepted: a per-call scratch `Vec` (`n <= MAX_CLIENTS`), negligible next to string allocations,
// and 8 redundant bytes per element.
// Dormant non-bug: C keys `swaptype` off `sizeof(long)`, Rust keys it off `usize`.
// These differ on LLP64 Windows, but that only changes the copy strategy, never the permutation.

/// Raven `SortRanks`, the `qsort` comparator over `level.sortedClients` indices.
/// The signature widens to carry `world` alongside the indices, because a `qsort` callback has no context slot for `level`.
///
/// Raven sorts a bare `int[]` and reaches `level` through the C global.
/// §B3 forbids that global, and a `qsort` comparator is a bare two-argument C callback with no context slot.
/// Widening the element to carry `world` alongside the index is Raven `SortRanks`, the `qsort` comparator for
/// `level.sortedClients`.
///
/// Raven's `*(int *)a` void-pointer unpack becomes plain `c_int` params.
/// The C-global `level` reach becomes the threaded raw `world` (§B3).
/// The comparison logic below is a verbatim transcription of the oracle.
/// It sorts through `native_sort::qsort`, the canonical BSD body from `bg_lib.c`.
/// See that module's note on the retail-msvcrt tie-order question.
///
...
pub fn CalculateRanks(ctx: &mut GameContext) {
    ...
    // Raven UB (porting-rules §19): `for (i=0; i<TEAM_NUM_TEAMS; i++)` (=4, g_main.c:1768) zeroes a
    // `numteamVotingClients[2]` array (g_local.h:879).
    // This is a 2-int overrun that silently clobbers neighbors in C but panics in safe Rust.
    // Only `[0]`/`[1]` are ever read or incremented, so this clamps to the array's real length.
    let mut i: usize = 0;
    ...
    // Raven: `if (!g_warmup.integer)` was dead-coded to `if (1)` in the oracle.
    ctx.world.level.warmupTime = 0;

    // `qsort( level.sortedClients, level.numConnectedClients, ..., SortRanks )`.
    // Sort a scratch copy so the comparator's raw `world` reads never alias a live `&mut` into `level`.
    // §B3 forbids a global, so the closure captures the raw world pointer instead.
    // Copy the indices back afterward.
    ...
    if ctx.world.cvars.g_gametype.integer >= GT_TEAM {
        trap::SetConfigstring(ctx.engine, CS_SCORES1, &format!(
                    "{}",
                    ctx.world.level.teamScores[TEAM_RED as usize]
                ));
        trap::SetConfigstring(ctx.engine, CS_SCORES2, &format!(
                    "{}",
                    ctx.world.level.teamScores[TEAM_BLUE as usize]
                ));
    } else {
        if ctx.world.level.numConnectedClients == 0 {
            trap::SetConfigstring(ctx.engine, CS_SCORES1, &format!("{}", SCORE_NOT_PRESENT));
            trap::SetConfigstring(ctx.engine, CS_SCORES2, &format!("{}", SCORE_NOT_PRESENT));
            ...
            || ctx.world.cvars.g_gametype.integer != GT_POWERDUEL
        {
            if ctx.world.level.numConnectedClients >= 1 {
                trap::SetConfigstring(ctx.engine, CS_CLIENT_DUELWINNER, &format!("{}", ctx.world.level.sortedClients[0]));
            } else {
...
pub fn MoveClientToIntermission(ctx: &mut GameContext, id: EntityId) {
    // FLAG(pool-client): `client` is the entity's `gclient_t*`.
    // It is a real player client for the intermission callers, but pool-allocated for NPC entities.
    // Read the pointer value through the safe entity borrow, then dereference it raw, exactly as Raven does.
    // Never index it as `clients[id]`.
...
pub fn FindIntermissionPoint(ctx: &mut GameContext) {
    unsafe {
        ...
        if ent.is_null() {
            ...
            // This call is irreducible.
            // `SelectSpawnPoint` takes `ctx` plus `&mut vec3_t` out-params that alias into `ctx.world.level`.
            // The raw pointer keeps them disjoint from the `ctx` argument's borrow.
            let world_ptr = ctx.world_raw();
            SelectSpawnPoint(
                ctx,
                vec3_origin,
    ...
    unsafe {
        ...
        while i < ctx.world.level.maxclients {
            ...
            if ctx.world.entity(id).health <= 0 {
                ...
                // Dereference it raw, gated by the null check, exactly as Raven does.
                // Never index it as `clients[i]`.
...
pub fn ExitLevel(ctx: &mut GameContext) {
        ...
        || ctx.world.cvars.g_gametype.integer == GT_POWERDUEL
    {
        if DuelLimitHit(ctx) == qfalse {
            if ctx.world.level.restarted == qfalse {
                trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "map_restart 0\n");
        ...
        && ctx.world.globals.g_siegePersistant.beatingTime != qfalse
    {
        ...
        trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "vstr nextmap\n");
    }
    ctx.world.level.changemap = std::ptr::null_mut();
    ctx.world.level.intermissiontime = 0;
...
pub fn G_LogPrintf(ctx: &mut GameContext, msg: &str) {
    ...
    trap::FS_Write(ctx.engine, full.as_bytes(), ctx.world.level.logFile);
}
...
pub fn LogExit(ctx: &mut GameContext, string: &str) {
    unsafe {
        ...
        trap::SetConfigstring(ctx.engine, CS_INTERMISSION, "1");
        ...
        while i < num_sorted {
            let clientNum = ctx.world.level.sortedClients[i as usize];
            // `clientNum` is a sorted (real) client index.
            // Read it fresh through `ctx.world.clients[..]` at each use, so nothing is borrowed across the
            // `G_LogPrintf(ctx, …)` call.
            ...
            && ctx.world.level.time > ctx.world.level.intermissiontime + 2000
        {
            ...
            if DuelLimitHit(ctx) == qfalse {
                ...
                if ctx.world.cvars.g_gametype.integer == GT_POWERDUEL {
                    ...
                    {
                        trap::SetConfigstring(ctx.engine, CS_CLIENT_DUELISTS, &format!(
                                    "{}|{}|{}",
    ...
    unsafe {
        ...
        if ctx.world.globals.gEscaping != qfalse {
            ...
            while (i as usize) < MAX_CLIENTS {
                ...
                if inuse != qfalse && !client_ptr.is_null() && health > 0 {
                    // FLAG(pool-client): dereference the entity's `gclient_t*` raw, through the copied pointer
                    // value, rather than laundering a `&*` reference out of it.
        ...
        if ctx.world.cvars.g_gametype.integer != GT_SIEGE {
            if ctx.world.cvars.g_timelimit.integer != 0 && ctx.world.level.warmupTime == 0 {
                ...
                {
                    let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "TIMELIMIT_HIT");
                    let hit_str = sv_str;
        ...
        if ctx.world.cvars.g_gametype.integer < GT_SIEGE && ctx.world.cvars.g_fraglimit.integer != 0
        {
            ...
            {
                let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "HIT_THE_KILL_LIMIT");
                let hit_limit = sv_str;
            ...
            let mut i: c_int = 0;
            while i < ctx.world.cvars.g_maxclients.integer {
                // Read fresh through `ctx.world.clients[i]` at each use.
                // `i` is a real client index (`i < g_maxclients`), so nothing is borrowed across the
                // `LogExit`/`SendServerCommand`/`Com_Printf` calls.
                if ctx.world.clients[i as usize].pers.connected != CON_CONNECTED {
                    i += 1;
                    continue;
                }
            ...
            && ctx.world.cvars.g_capturelimit.integer != 0
        {
            ...
            {
                let red = G_GetStringEdString(ctx, "MP_SVGAME", "PRINTREDTEAM");
            ...
            if ctx.world.level.teamScores[TEAM_BLUE as usize]
                >= ctx.world.cvars.g_capturelimit.integer
            {
                let blue = G_GetStringEdString(ctx, "MP_SVGAME", "PRINTBLUETEAM");
                let hit = G_GetStringEdString(ctx, "MP_SVGAME", "HIT_CAPTURE_LIMIT");
                trap::SendServerCommand(ctx.engine, -1, &format!("print \"{} \"", blue));
                trap::SendServerCommand(ctx.engine, -1, &format!("print \"{}.\n\"", hit));
                LogExit(ctx, "Capturelimit hit.");
...
pub fn G_RemoveDuelist(ctx: &mut GameContext, team: c_int) {
    ...
    while i < MAX_CLIENTS {
        ...
        {
                        SetTeam(ctx, EntityId(i as u32), "s");
        }
        i += 1;
    }
}
        ...
        if ctx.world.cvars.g_gametype.integer == GT_DUEL {
            ...
            if ctx.world.level.numPlayingClients >= 2 {
                ...
                if ctx.world.cvars.g_showDuelHealths.integer >= 1 {
                    ...
                    trap::SetConfigstring(ctx.engine, CS_CLIENT_DUELHEALTHS, &format!("{}|{}|!", h0, h1));
                }
            }
                ...
                && ctx.world.globals.g_dontFrickinCheck == qfalse
            {
                    ...
                    || ctx.world.level.numConnectedClients > 0
                {
                    if ctx.world.globals.g_duelPrintTimer < ctx.world.level.time {
                        ...
                        if lone < 1 {
                            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "DUELMORESINGLE");
                            let msg = sv_str;
                            trap::SendServerCommand(ctx.engine, -1, &format!("cp \"{}\n\"", msg));
                        } else {
                            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "DUELMOREPAIRED");
                            let msg = sv_str;
                            trap::SendServerCommand(ctx.engine, -1, &format!("cp \"{}\n\"", msg));
                        }
            ...
            if not_enough != qfalse {
                if ctx.world.level.warmupTime != -1 {
                    ctx.world.level.warmupTime = -1;
                    trap::SetConfigstring(ctx.engine, CS_WARMUP, &format!("{}", ctx.world.level.warmupTime));
                    G_LogPrintf(ctx, "Warmup:\n");
                }
            ...
            if ctx.world.level.time > ctx.world.level.warmupTime {
                ...
                trap::Cvar_Set(ctx.engine, "g_restarted", "1");
                trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "map_restart 0\n");
                ctx.world.level.restarted = qtrue;
                return;
            }
...
pub fn G_KickAllBots(ctx: &mut GameContext) {
    unsafe {
        ...
        while i < ctx.world.cvars.g_maxclients.integer {
            ...
            // Cleaning it directly matches Raven's `strncpy(netname, ...)` plus `Q_CleanStr`.
...
pub fn CheckVote(ctx: &mut GameContext) {
    unsafe {
            ...
            if ctx.world.level.votingGametype != qfalse {
                ...
                if cur != ctx.world.level.votingGametypeTo {
                    ...
                    if !next_map.is_empty() {
                        trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), &format!("map {}\n", next_map));
                    }
                } else {
                ...
                if ctx.world.cvars.g_fraglimitVoteCorrection.integer != 0 {
                    ...
                    let current_gt = trap::Cvar_VariableIntegerValue(ctx.engine, "g_gametype");
                    let current_fl = trap::Cvar_VariableIntegerValue(ctx.engine, "fraglimit");
                    let current_tl = trap::Cvar_VariableIntegerValue(ctx.engine, "timelimit");
                        ...
                        && current_gt != GT_POWERDUEL
                    {
                        if current_fl > 3 || current_fl == 0 {
                            ...
                            trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "fraglimit 3\n");
                        }
                        if current_tl != 0 {
                            // if voting to duel, and timelimit is set, make it unlimited
                            trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "timelimit 0\n");
                        }
                        ...
                        && (current_gt == GT_DUEL || current_gt == GT_POWERDUEL)
                    {
                        if current_fl != 0 && current_fl < 20 {
                            ...
                            trap::SendConsoleCommand(ctx.engine, (EXEC_APPEND as c_int), "fraglimit 20\n");
                        }
                    }
                }
        ...
        if ctx.world.level.time - ctx.world.level.voteTime >= VOTE_TIME {
            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "VOTEFAILED");
            let msg = sv_str;
            trap::SendServerCommand(ctx.engine, -1, &format!("print \"{}\n\"", msg));
        } else if ctx.world.level.voteYes > ctx.world.level.numVotingClients / 2 {
            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "VOTEPASSED");
        ...
        trap::SetConfigstring(ctx.engine, CS_VOTE_TIME, "");
    }
}
...
pub fn CheckTeamVote(ctx: &mut GameContext, team: c_int) {
    unsafe {
        ...
        if ctx.world.level.time - ctx.world.level.teamVoteTime[cs_offset] >= VOTE_TIME {
            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "TEAMVOTEFAILED");
            let msg = sv_str;
            trap::SendServerCommand(ctx.engine, -1, &format!("print \"{}\n\"", msg));
        } else if ctx.world.level.teamVoteYes[cs_offset]
            > ctx.world.level.numteamVotingClients[cs_offset] / 2
        {
            let sv_str = G_GetStringEdString(ctx, "MP_SVGAME", "TEAMVOTEPASSED");
            // execute the command, then remove the vote
            let msg = sv_str;
            trap::SendServerCommand(ctx.engine, -1, &format!("print \"{}\n\"", msg));
        ...
        trap::SetConfigstring(ctx.engine, CS_TEAMVOTE_TIME + cs_offset as c_int, "");
    }
}
...
pub fn CheckCvars(ctx: &mut GameContext) {
    unsafe {
        ...
        // It is homed on `GameGlobals::checkCvarsLastMod` (seeds to -1).
        // The gate fires once per `g_password` modification, not every frame.
...
pub fn G_RunThink(ctx: &mut GameContext, id: EntityId) {
    // Raven casts `nextthink` to float and compares it against `level.time` as a float.
    // The int comparison here only diverges past f32's exact-integer range (~16.7M ms), so the cast is elided.
...
pub fn NAV_CheckCalcPaths(ctx: &mut GameContext) {
    unsafe {
            ...
            && ctx.world.globals.navCalcPathTime < ctx.world.level.time
        {
            ...
            trap::Cvar_Register(ctx.engine, Some(&mut mapname), "mapname", "", CVAR_SERVERINFO | CVAR_ROM);
            trap::Cvar_Register(ctx.engine, Some(&mut ck_sum), "sv_mapChecksum", "", CVAR_ROM);
...
/// Raven `level.time` accessor.
/// This has no standalone Raven symbol.
/// It mirrors the many `level.time` reads inlined at call sites.
///
    ...
    unsafe {
            ...
            && ctx.world.globals.g_siegeRespawnCheck < ctx.world.level.time
        {
            ...
            while (i as usize) < MAX_CLIENTS {
                ...
                // FLAG(pool-client): read the entity's `gclient_t*` value and dereference it raw.
                // `client_ptr` is a copied pointer held across `respawn(ctx, …)`, the sanctioned pool-client
                // pattern.
                let client_ptr = ctx.world.entity(id).client;
                if inuse != qfalse
                    && !client_ptr.is_null()
        ...
        if ctx.world.globals.gDoSlowMoDuel != qfalse {
            if ctx.world.level.restarted != qfalse {
                ...
                let t_fval = atof_bytes(buf.as_bytes()) as f32;
                trap::Cvar_Set(ctx.engine, "timescale", "1");
                if t_fval == 1.0f32 {
                ...
                if time_dif < 150.0 {
                    trap::Cvar_Set(ctx.engine, "timescale", "0.1f");
                    ...
                    trap::Cvar_Set(ctx.engine, "timescale", &format!("{:.6}", use_dif));
                } else {
                    let buf = trap::Cvar_VariableStringBuffer(ctx.engine, "timescale", 128);
                    let t_fval = atof_bytes(buf.as_bytes()) as f32;
                    trap::Cvar_Set(ctx.engine, "timescale", "1");
        ...
        //
        // FLAG(deferred): this `MAX_GENTITIES`-wide dispatch loop walks arbitrary entities.
        ...
        if ctx.world.cvars.g_listEntity.integer != 0 {
            ...
            while (i as usize) < MAX_GENTITIES {
                // §19: the oracle passes a possibly-NULL classname to `%s` (UB).
                // We print an empty string for unused slots. `classname_str` collapses NULL to "".
                let name = ctx.world.g_entities[i as usize].classname_str();
                G_Printf(ctx, &format!("{:4}: {}\n", i, name));
                i += 1;
            }
            trap::Cvar_Set(ctx.engine, "g_listEntity", "0");
        }
...
pub fn G_GetStringEdString(_ctx: &mut GameContext, _refSection: &str, refName: &str) -> String {
