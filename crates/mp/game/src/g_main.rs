...
//! `ctx: GameContext` (fork 8), and pass-3 blind-transcribed against the
//! settled fork rulings (rulings 1-22, the va/printf mapping table,
...
pub fn G_Printf(
    ...
    // PORT-NOTE(variadic-c-abi): Raven's `...` is resolved per ruling 18 — every
...
pub fn G_LogPrintf(
    ...
    // PORT-NOTE(variadic-c-abi): `fmt` arrives pre-formatted (ruling 18).
    unsafe {
        let world = &mut *ctx.world;
...
pub fn CheckCvars(ctx: GameContext<'_>) {
    unsafe {
        ...
        // genuine cross-frame static (fork ruling 1 class); it wants a
...
pub fn BG_GetTime() -> c_int {
    ...
    // worktree signature exactly"). `GameCallbacks`/`BgState` (ruling 16) is
...
// Returns 0 until GameCallbacks/BgState (ruling 16) wiring reaches this call
...
pub fn G_GetStringEdString(
    ...
    // this leaks a fresh `'static` allocation per call (ruling-5-adjacent —
    // an owned-return divergence, not a rotating-buffer aliasing bug).
    unsafe {
