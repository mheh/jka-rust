...
// (`mp_qshared::common::mp::game::g_public`), so the engine server can see this `g_public.h` server<->game contract.
// This file re-exports them, so `mp_game`'s own call sites and the `prelude` glob keep resolving.
