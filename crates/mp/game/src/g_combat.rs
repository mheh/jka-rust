//! Port of `oracle/codemp/game/g_combat.c`.
//!
//! `tools/closure-prototype/fnskel.py` generated this file from the fnsweep manifest.
//! All functions here are filled in against the settled fork rulings.
//! Those rulings are ctx threading, vec3 reshape, `Option<EntityId>` stored fields, fn-ptr dispatch enums,
//! the BgState/BgTraps/GameCallbacks bg-tier channel, and va/printf mapping.
#![allow(non_snake_case, unused, clippy::all)]

use crate::g_items::{Drop_Item, LaunchItem};
use crate::prelude::*;
use crate::g_utils::G_EffectIndex;
use crate::g_utils::G_SoundIndex;
use mp_bg::bg_misc::{BG_FindItemForPowerup, BG_FindItemForWeapon};

use crate::bg_channel::GameBgTraps;
use crate::ent_fn_enums::{EntDie, EntThink, EntTouch};
use crate::entity::hit_location::*;
use crate::g_main::CalculateRanks;
use crate::g_timer::TIMER_Set;
use crate::g_utils::{G_AddEvent, G_FreeEntity, G_TempEntity, G_UseTargets};
use crate::q_math::{
    _DotProduct, _VectorAdd, _VectorCopy, _VectorMA, _VectorScale, _VectorSubtract, AngleVectors,
    DirToByte, DistanceSquared, VectorCompare, VectorLengthSquared, VectorNormalize,
};
use native_string::{Q_stricmp, Q_strncmp};
use native_string::strncpyz_string;
use crate::g_vehicles::G_ShipSurfaceForSurfName;
use crate::teams::class::class_t;
use crate::trap;
use crate::w_saber::UpdateClientRenderBolts;
use crate::NPC_utils::G_ActivateBehavior;
use core::ffi::CStr;
use mp_abi::game::syscalls::G_TRACE::GTraceArgs;
use mp_abi::game::syscalls::G_UNLINKENTITY::GUnlinkentityArgs;
use mp_bg::bg_g2_utils::BG_GetRootSurfNameWithVariant;
use mp_bg::bg_misc::vectoyaw;
use mp_bg::bg_panimate::{BG_HasAnimation, BG_PickAnim};
use mp_bg::bg_pmove::BG_KnockDownable;
use mp_bg::public::anim_number::animNumber_t;
use mp_bg::public::duel_team::duelTeam_t::{DUELTEAM_DOUBLE, DUELTEAM_LONE};
use mp_bg::public::entity_event::entity_event_t;
use mp_bg::public::entity_type::entityType_t;
use mp_bg::public::g2_model_parts::g2ModelParts_t;
use mp_bg::public::stat_index::statIndex_t;
use mp_qshared::probe;
use mp_qshared::shared::MAX_CLIENTS;

// Raven `qboolean` is `c_int`. Assignment sites keep the source spelling.
// Source: `oracle/codemp/game/q_shared.h`

// `PITCH`/`YAW`/`ROLL` (`crate::q_math`) and `SVF_BROADCAST` (`crate::g_public_consts`) resolve via the crate prelude glob.
// Local shadowing copies of these constants were removed.

/// Raven `#define ARMOR_PROTECTION 0.50`.
/// Shields stop only 50% of armor-piercing damage.
/// Source: `oracle/codemp/game/bg_public.h:26`
const ARMOR_PROTECTION: f32 = 0.50;

/// Raven `#define ARMOR_REDUCTION_FACTOR 0.50`.
/// Some damage types do not take off armor as efficiently.
/// Source: `oracle/codemp/game/bg_public.h:27`
const ARMOR_REDUCTION_FACTOR: f32 = 0.50;

/// Raven `modNames`, the obituary name table indexed by `meansOfDeath_t`.
///
/// Raven declares `char *modNames[MOD_MAX]` (MOD_MAX = 45) with only 42 initializers.
/// The last 3 slots (MOD_TARGET_LASER, MOD_TRIGGER_HURT, MOD_TEAM_CHANGE) are NULL.
/// The bounds check and the null-print site below account for that.
/// Source: `oracle/codemp/game/g_combat.c:755-797`
pub(crate) const modNames: [*const c_char; meansOfDeath_t::MOD_MAX as usize] = [
    c"MOD_UNKNOWN".as_ptr(),
    c"MOD_STUN_BATON".as_ptr(),
    c"MOD_MELEE".as_ptr(),
    c"MOD_SABER".as_ptr(),
    c"MOD_BRYAR_PISTOL".as_ptr(),
    c"MOD_BRYAR_PISTOL_ALT".as_ptr(),
    c"MOD_BLASTER".as_ptr(),
    c"MOD_TURBLAST".as_ptr(),
    c"MOD_DISRUPTOR".as_ptr(),
    c"MOD_DISRUPTOR_SPLASH".as_ptr(),
    c"MOD_DISRUPTOR_SNIPER".as_ptr(),
    c"MOD_BOWCASTER".as_ptr(),
    c"MOD_REPEATER".as_ptr(),
    c"MOD_REPEATER_ALT".as_ptr(),
    c"MOD_REPEATER_ALT_SPLASH".as_ptr(),
    c"MOD_DEMP2".as_ptr(),
    c"MOD_DEMP2_ALT".as_ptr(),
    c"MOD_FLECHETTE".as_ptr(),
    c"MOD_FLECHETTE_ALT_SPLASH".as_ptr(),
    c"MOD_ROCKET".as_ptr(),
    c"MOD_ROCKET_SPLASH".as_ptr(),
    c"MOD_ROCKET_HOMING".as_ptr(),
    c"MOD_ROCKET_HOMING_SPLASH".as_ptr(),
    c"MOD_THERMAL".as_ptr(),
    c"MOD_THERMAL_SPLASH".as_ptr(),
    c"MOD_TRIP_MINE_SPLASH".as_ptr(),
    c"MOD_TIMED_MINE_SPLASH".as_ptr(),
    c"MOD_DET_PACK_SPLASH".as_ptr(),
    c"MOD_VEHICLE".as_ptr(),
    c"MOD_CONC".as_ptr(),
    c"MOD_CONC_ALT".as_ptr(),
    c"MOD_FORCE_DARK".as_ptr(),
    c"MOD_SENTRY".as_ptr(),
    c"MOD_WATER".as_ptr(),
    c"MOD_SLIME".as_ptr(),
    c"MOD_LAVA".as_ptr(),
    c"MOD_CRUSH".as_ptr(),
    c"MOD_TELEFRAG".as_ptr(),
    c"MOD_FALLING".as_ptr(),
    c"MOD_SUICIDE".as_ptr(),
    c"MOD_TARGET_LASER".as_ptr(),
    c"MOD_TRIGGER_HURT".as_ptr(),
    // Raven leaves indices 42-44 (MOD_TARGET_LASER, MOD_TRIGGER_HURT, MOD_TEAM_CHANGE) uninitialized in a `[MOD_MAX]` array.
    // C zero-inits file scope arrays, so these three read as NULL.
    core::ptr::null(),
    core::ptr::null(),
    core::ptr::null(),
];

/// Raven `hitLocName`, the hit-location display-name table indexed by `hitLoc`.
///
/// Source: `oracle/codemp/game/g_combat.c:3165-3189`
const hitLocName: [&core::ffi::CStr; 23] = [
    c"none",
    c"right foot",
    c"left foot",
    c"right leg",
    c"left leg",
    c"waist",
    c"back right shoulder",
    c"back left shoulder",
    c"back",
    c"front right shouler",
    c"front left shoulder",
    c"chest",
    c"right arm",
    c"left arm",
    c"right hand",
    c"left hand",
    c"head",
    c"generic1",
    c"generic2",
    c"generic3",
    c"generic4",
    c"generic5",
    c"generic6",
];

/// Raven `ObjectDie`.
///
/// Source: `oracle/codemp/game/g_combat.c:22-31`
pub fn ObjectDie(
    ctx: &mut GameContext,
    self_: EntityId,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    damage: c_int,
    meansOfDeath: c_int,
) {
    if ctx.entity(self_).target.is_some() {
        G_UseTargets(ctx, Some(self_), attacker);
    }

    //remove my script_targetname
    G_FreeEntity(ctx, Some(self_));
}

/// Raven `G_HeavyMelee`.
///
/// Source: `oracle/codemp/game/g_combat.c:33-44`
pub fn G_HeavyMelee(ctx: &mut GameContext, attacker: Option<EntityId>) -> qboolean {
    if ctx.world.cvars.g_gametype.integer == GT_SIEGE {
        if let Some(attacker) = attacker {
            // attacker may be an NPC. `.client` is a pool client, so the deref stays raw.
            let client = ctx.entity(attacker).client;
            unsafe {
                if !client.is_null()
                    && (*client).siegeClass != -1
                    && ((&ctx.world.bg_state.bgSiegeClasses)[(*client).siegeClass as usize]
                        .classflags
                        & (1 << CFL_HEAVYMELEE as c_int))
                        != 0
                {
                    return qtrue;
                }
            }
        }
    }
    qfalse
}

/// Raven `G_GetHitLocation`.
///
/// `ppoint` is read-only (the impact point is copied *from* it), so it stays by-value.
/// (vec3 params only become `&mut` when written through).
/// Source: `oracle/codemp/game/g_combat.c:46-266`
pub fn G_GetHitLocation(ctx: &mut GameContext, target: EntityId, ppoint: vec3_t) -> c_int {
    let e = ctx.entity(target);
    {
        let mut point: vec3_t = [0.0; 3];
        let mut point_dir: vec3_t = [0.0; 3];
        let mut forward: vec3_t = [0.0; 3];
        let mut right: vec3_t = [0.0; 3];
        let mut up: vec3_t = [0.0; 3];
        // When `target->client` is NULL, C leaves `tangles` uninitialized and feeds that garbage to `AngleVectors` below.
        // This is undefined behavior (§19), so we zero-init `tangles` instead.
        let mut tangles: vec3_t = [0.0; 3];
        let mut tcenter: vec3_t = [0.0; 3];

        // Get target forward, right and up.
        if !e.client.is_null() {
            // Ignore player's pitch and roll.
            tangles = [0.0, e.r.currentAngles[YAW], 0.0];
        }

        AngleVectors(tangles, Some(&mut forward), Some(&mut right), Some(&mut up));

        // Get center of target.
        for i in 0..3 {
            tcenter[i] = (e.r.absmin[i] + e.r.absmax[i]) * 0.5;
        }

        // Get radius width of target.
        // Raven computes `tradius` but never reads it below, so we keep it unused too.
        let _tradius =
            (e.r.maxs[0].abs() + e.r.maxs[1].abs() + e.r.mins[0].abs() + e.r.mins[1].abs()) / 4.0;

        // Get impact point.
        if ppoint[0] != 0.0 || ppoint[1] != 0.0 || ppoint[2] != 0.0 {
            point = ppoint;
        } else {
            return HL_NONE;
        }

        for i in 0..3 {
            point_dir[i] = point[i] - tcenter[i];
        }
        VectorNormalize(&mut point_dir);

        let dot = |a: &vec3_t, b: &vec3_t| a[0] * b[0] + a[1] * b[1] + a[2] * b[2];

        // Get bottom to top (vertical) position index.
        let udot = dot(&up, &point_dir);
        let vertical = if (udot as f64) > 0.800 {
            4
        } else if (udot as f64) > 0.400 {
            3
        } else if (udot as f64) > -0.333 {
            2
        } else if (udot as f64) > -0.666 {
            1
        } else {
            0
        };

        // Get back to front (forward) position index.
        let fdot = dot(&forward, &point_dir);
        let forward_idx = if (fdot as f64) > 0.666 {
            4
        } else if (fdot as f64) > 0.333 {
            3
        } else if (fdot as f64) > -0.333 {
            2
        } else if (fdot as f64) > -0.666 {
            1
        } else {
            0
        };

        // Get left to right (lateral) position index.
        let rdot = dot(&right, &point_dir);
        let lateral = if (rdot as f64) > 0.666 {
            4
        } else if (rdot as f64) > 0.333 {
            3
        } else if (rdot as f64) > -0.333 {
            2
        } else if (rdot as f64) > -0.666 {
            1
        } else {
            0
        };

        let hit_loc = vertical * 25 + forward_idx * 5 + lateral;

        if hit_loc <= 10 {
            // Feet.
            if rdot > 0.0 {
                HL_FOOT_RT
            } else {
                HL_FOOT_LT
            }
        } else if hit_loc <= 50 {
            // Legs.
            if rdot > 0.0 {
                HL_LEG_RT
            } else {
                HL_LEG_LT
            }
        } else if hit_loc == 56
            || hit_loc == 60
            || hit_loc == 61
            || hit_loc == 65
            || hit_loc == 66
            || hit_loc == 70
        {
            // Hands.
            if rdot > 0.0 {
                HL_HAND_RT
            } else {
                HL_HAND_LT
            }
        } else if hit_loc == 83
            || hit_loc == 87
            || hit_loc == 88
            || hit_loc == 92
            || hit_loc == 93
            || hit_loc == 97
        {
            // Arms.
            if rdot > 0.0 {
                HL_ARM_RT
            } else {
                HL_ARM_LT
            }
        } else if (107..=109).contains(&hit_loc)
            || (112..=114).contains(&hit_loc)
            || (117..=119).contains(&hit_loc)
        {
            // Head.
            HL_HEAD
        } else if (udot as f64) < 0.3 {
            HL_WAIST
        } else if fdot < 0.0 {
            if (rdot as f64) > 0.4 {
                HL_BACK_RT
            } else if (rdot as f64) < -0.4 {
                HL_BACK_LT
            } else if fdot < 0.0 {
                HL_BACK
            } else {
                HL_NONE
            }
        } else if (rdot as f64) > 0.3 {
            HL_CHEST_RT
        } else if (rdot as f64) < -0.3 {
            HL_CHEST_LT
        } else if fdot < 0.0 {
            HL_CHEST
        } else {
            HL_NONE
        }
    }
}

/// Raven `ExplodeDeath`.
///
/// Source: `oracle/codemp/game/g_combat.c:369-409`
pub fn ExplodeDeath(ctx: &mut GameContext, self_: EntityId) {
    let mut forward: vec3_t = [0.0; 3];

    let e = ctx.entity_mut(self_);
    e.takedamage = qfalse; //stop chain reaction runaway loops

    e.s.loopSound = 0;
    e.s.loopIsSoundset = qfalse;

    e.s.pos.trBase = e.r.currentOrigin;

    AngleVectors(e.s.angles, Some(&mut forward), None, None);

    if e.splashDamage > 0 && e.splashRadius > 0 {
        let attacker = match e.parent {
            Some(parent) => Some(parent),
            None => Some(self_),
        };
        let origin = e.r.currentOrigin;
        let splashDamage = e.splashDamage as f32;
        let splashRadius = e.splashRadius as f32;
        G_RadiusDamage(
            ctx,
            origin,
            attacker,
            splashDamage,
            splashRadius,
            attacker,
            None,
            meansOfDeath_t::MOD_UNKNOWN as c_int,
        );
    }

    ObjectDie(ctx, self_, Some(self_), Some(self_), 20, 0);
}

/// Raven `ScorePlum`.
///
/// Source: `oracle/codemp/game/g_combat.c:417-427`
pub fn ScorePlum(ctx: &mut GameContext, ent: EntityId, origin: vec3_t, score: c_int) {
    let plum_id = G_TempEntity(ctx, origin, entity_event_t::EV_SCOREPLUM as c_int);
    let number = ctx.entity(ent).s.number;
    let plum = ctx.entity_mut(plum_id);
    // only send this temp entity to a single client
    plum.r.svFlags |= SVF_SINGLECLIENT;
    plum.r.singleClient = number;
    //
    plum.s.otherEntityNum = number;
    plum.s.time = score;
}

/// Raven `AddScore`.
///
/// `origin` is unused by the compiled body (the `ScorePlum` call is commented out), so it stays by-value.
/// Source: `oracle/codemp/game/g_combat.c:437-460`
pub fn AddScore(ctx: &mut GameContext, ent: EntityId, origin: vec3_t, score: c_int) {
    if ctx.entity(ent).client.is_null() {
        return;
    }
    // no scoring during pre-match warmup
    if ctx.world.level.warmupTime != 0 {
        return;
    }
    // `ent` may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(ent).client;
    unsafe {
        (*client).ps.persistant[persEnum_t::PERS_SCORE as usize] += score;
        if ctx.world.cvars.g_gametype.integer == GT_TEAM
            && ctx.world.globals.g_dontPenalizeTeam == 0
        {
            let team = (*client).ps.persistant[persEnum_t::PERS_TEAM as usize] as usize;
            ctx.world.level.teamScores[team] += score;
        }
    }
    CalculateRanks(ctx);
}

/// Raven `TossClientWeapon`.
///
/// Source: `oracle/codemp/game/g_combat.c:469-559`
pub fn TossClientWeapon(ctx: &mut GameContext, self_: EntityId, direction: vec3_t, speed: f32) {
    let mut vel: vec3_t = [0.0; 3];
    // self_ may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(self_).client;
    let weapon = ctx.entity(self_).s.weapon;

    if ctx.world.cvars.g_gametype.integer == GT_SIEGE {
        //no dropping weaps
        return;
    }

    if weapon <= WP_BRYAR_PISTOL as c_int {
        //can't have this
        return;
    }

    if weapon == WP_EMPLACED_GUN as c_int || weapon == WP_TURRET as c_int {
        return;
    }

    // find the item type for this weapon
    let item = BG_FindItemForWeapon(weapon);
    // Raven re-looks the same entry up via `BG_GetItemIndexByTag(weapon, IT_WEAPON)` at three sites below.
    // `weap_quantity` holds this item's quantity.
    let weap_quantity = item.item().quantity;

    let ammoSub = unsafe { (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] }
        - weap_quantity;

    if ammoSub < 0 {
        let mut ammoQuan = weap_quantity;
        ammoQuan -= -ammoSub;

        if ammoQuan <= 0 {
            //no ammo
            return;
        }
    }

    vel[0] = direction[0] * speed;
    vel[1] = direction[1] * speed;
    vel[2] = direction[2] * speed;

    let ps_origin = unsafe { (*client).ps.origin };
    let launched = LaunchItem(ctx, item, ps_origin, vel);

    let number = ctx.entity(self_).s.number;
    let level_time = ctx.world.level.time;
    let e = ctx.entity_mut(launched);
    e.s.generic1 = number;
    e.s.powerups = level_time + 1500;

    e.count = weap_quantity;

    unsafe {
        (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] -= weap_quantity;

        if (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] < 0 {
            let ammo = (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize];
            ctx.entity_mut(launched).count -= -ammo;
            (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] = 0;
        }
    }

    let drop_weapon = unsafe {
        (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] < 1
            && weapon != WP_DET_PACK as c_int
    } || (weapon != WP_THERMAL as c_int
        && weapon != WP_DET_PACK as c_int
        && weapon != WP_TRIP_MINE as c_int);
    if drop_weapon {
        let mut i = 0;
        let mut weap = -1;

        unsafe {
            (*client).ps.stats[statIndex_t::STAT_WEAPONS as usize] &= !(1 << weapon);
        }

        while i < WP_NUM_WEAPONS as c_int {
            let has =
                unsafe { ((*client).ps.stats[statIndex_t::STAT_WEAPONS as usize] & (1 << i)) != 0 };
            if has && i != WP_NONE as c_int {
                //this one's good
                weap = i;
                break;
            }
            i += 1;
        }

        if weap != -1 {
            ctx.entity_mut(self_).s.weapon = weap;
            unsafe {
                (*client).ps.weapon = weap;
            }
        } else {
            ctx.entity_mut(self_).s.weapon = 0;
            unsafe {
                (*client).ps.weapon = 0;
            }
        }

        G_AddEvent(
            ctx.entity_mut(self_),
            entity_event_t::EV_NOAMMO as c_int,
            weapon,
        );
    }
}

/// Raven `TossClientItems`.
///
/// Source: `oracle/codemp/game/g_combat.c:568-635`
pub fn TossClientItems(ctx: &mut GameContext, self_: EntityId) {
    // self_ may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(self_).client;

    if ctx.world.cvars.g_gametype.integer == GT_SIEGE {
        // just don't drop anything then
        return;
    }

    // drop the weapon if not a gauntlet or machinegun
    let mut weapon = ctx.entity(self_).s.weapon;

    // make a special check to see if they are changing to a new
    // weapon that isn't the mg or gauntlet.  Without this, a client
    // can pick up a weapon, be killed, and not drop the weapon because
    // their weapon change hasn't completed yet and they are still holding the MG.
    if weapon == WP_BRYAR_PISTOL as c_int {
        unsafe {
            if (*client).ps.weaponstate == WEAPON_DROPPING as c_int {
                weapon = (*client).pers.cmd.weapon as c_int;
            }
            if ((*client).ps.stats[statIndex_t::STAT_WEAPONS as usize] & (1 << weapon)) == 0 {
                weapon = WP_NONE as c_int;
            }
        }
    }

    ctx.entity_mut(self_).s.bolt2 = weapon;

    if weapon > WP_BRYAR_PISTOL as c_int
        && weapon != WP_EMPLACED_GUN as c_int
        && weapon != WP_TURRET as c_int
        && unsafe { (*client).ps.ammo[weaponData[weapon as usize].ammoIndex as usize] != 0 }
    {
        // find the item type for this weapon
        let item = BG_FindItemForWeapon(weapon);

        // tell all clients to remove the weapon model on this guy until he respawns
        let te = G_TempEntity(
            ctx,
            [0.0; 3],
            entity_event_t::EV_DESTROY_WEAPON_MODEL as c_int,
        );
        let te = te;
        let number = ctx.entity(self_).s.number;
        ctx.entity_mut(te).r.svFlags |= SVF_BROADCAST;
        ctx.entity_mut(te).s.eventParm = number;

        // spawn the item
        Drop_Item(ctx, self_, item, 0.0);
    }

    // drop all the powerups if not in teamplay
    if ctx.world.cvars.g_gametype.integer != GT_TEAM
        && ctx.world.cvars.g_gametype.integer != GT_SIEGE
    {
        let mut angle: f32 = 45.0;
        for i in 1..PW_NUM_POWERUPS as c_int {
            if unsafe { (*client).ps.powerups[i as usize] } > ctx.world.level.time {
                let Some(item) = BG_FindItemForPowerup(i) else {
                    continue;
                };
                let drop = Drop_Item(ctx, self_, item, angle);
                // decide how many seconds it has left
                let count =
                    (unsafe { (*client).ps.powerups[i as usize] } - ctx.world.level.time) / 1000;
                ctx.entity_mut(drop).count = count;
                if ctx.entity(drop).count < 1 {
                    ctx.entity_mut(drop).count = 1;
                }
                angle += 45.0;
            }
        }
    }
}

/// Raven `LookAtKiller`.
///
/// Source: `oracle/codemp/game/g_combat.c:643-661`
pub fn LookAtKiller(
    self_: &mut gentity_t,
    inflictor: Option<&gentity_t>,
    attacker: Option<&gentity_t>,
) {
    let mut dir: vec3_t = [0.0; 3];
    let mut angles: vec3_t = [0.0; 3];
    // self_ may be an NPC corpse. `.client` is a pool client, so the deref stays raw.
    let client = self_.client;
    let self_ptr = self_ as *const gentity_t;

    if attacker.is_some_and(|a| !core::ptr::eq(a, self_ptr)) {
        let a = attacker.unwrap().s.pos.trBase;
        let s = self_.s.pos.trBase;
        dir = [a[0] - s[0], a[1] - s[1], a[2] - s[2]];
    } else if inflictor.is_some_and(|inf| !core::ptr::eq(inf, self_ptr)) {
        let inf = inflictor.unwrap().s.pos.trBase;
        let s = self_.s.pos.trBase;
        dir = [inf[0] - s[0], inf[1] - s[1], inf[2] - s[2]];
    } else {
        let dead_yaw = self_.s.angles[YAW] as c_int;
        unsafe {
            (*client).ps.stats[statIndex_t::STAT_DEAD_YAW as usize] = dead_yaw;
        }
        return;
    }

    unsafe {
        (*client).ps.stats[statIndex_t::STAT_DEAD_YAW as usize] = vectoyaw(dir) as c_int;
    }

    // Raven computes `angles` here but never consumes it. This dead computation stays.
    angles[YAW] = vectoyaw(dir);
    angles[PITCH] = 0.0;
    angles[ROLL] = 0.0;
}

/// Raven `GibEntity`.
///
/// Source: `oracle/codemp/game/g_combat.c:668-673`
pub fn GibEntity(ctx: &mut GameContext, self_: EntityId, killer: c_int) {
    G_AddEvent(
        ctx.entity_mut(self_),
        entity_event_t::EV_GIB_PLAYER as c_int,
        killer,
    );
    ctx.entity_mut(self_).takedamage = qfalse;
    ctx.entity_mut(self_).s.eType = entityType_t::ET_INVISIBLE as c_int;
    ctx.entity_mut(self_).r.contents = 0;
}

/// Raven `BodyRid`.
///
/// Source: `oracle/codemp/game/g_combat.c:675-679`
pub fn BodyRid(ctx: &mut GameContext, ent: EntityId) {
    // Seam: the unlink trap takes the raw entity address.
    let ent_ptr = ctx.entity_mut(ent) as *mut gentity_t;
    trap::UnlinkEntity(ctx.engine, GUnlinkentityArgs::new(ent_ptr.cast()));
    ctx.entity_mut(ent).physicsObject = qfalse;
}

/// Raven `body_die`.
///
/// Raven: `NOTENOTE No gibbing right now, this is star wars.`
/// Source: `oracle/codemp/game/g_combat.c:686-751`
pub fn body_die(
    ctx: &mut GameContext,
    self_: EntityId,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    damage: c_int,
    meansOfDeath: c_int,
) {
    let mut doDisint = false;

    if ctx.entity(self_).s.eType == entityType_t::ET_NPC as c_int {
        // well, just rem it then, so long as it's done with its death anim and it's not a standard weapon.
        // self_ is an NPC. `.client` is a pool client, so the deref stays raw.
        let client = ctx.entity(self_).client;
        if !client.is_null()
            && unsafe { (*client).ps.torsoTimer } <= 0
            && (meansOfDeath == meansOfDeath_t::MOD_UNKNOWN as c_int
                || meansOfDeath == meansOfDeath_t::MOD_WATER as c_int
                || meansOfDeath == meansOfDeath_t::MOD_SLIME as c_int
                || meansOfDeath == meansOfDeath_t::MOD_LAVA as c_int
                || meansOfDeath == meansOfDeath_t::MOD_CRUSH as c_int
                || meansOfDeath == meansOfDeath_t::MOD_TELEFRAG as c_int
                || meansOfDeath == meansOfDeath_t::MOD_FALLING as c_int
                || meansOfDeath == meansOfDeath_t::MOD_SUICIDE as c_int
                || meansOfDeath == meansOfDeath_t::MOD_TARGET_LASER as c_int
                || meansOfDeath == meansOfDeath_t::MOD_TRIGGER_HURT as c_int)
        {
            ctx.entity_mut(self_).think = Some(EntThink::G_FreeEntity).into();
            ctx.entity_mut(self_).nextthink = ctx.world.level.time;
        }
        return;
    }

    if ctx.entity(self_).health < (GIB_HEALTH + 1) {
        ctx.entity_mut(self_).health = GIB_HEALTH + 1;

        let client = ctx.entity(self_).client;
        doDisint = !(!client.is_null()
            && (ctx.world.level.time - unsafe { (*client).respawnTime }) < 2000);
    }

    let client = ctx.entity(self_).client;
    if !client.is_null() && unsafe { ((*client).ps.eFlags & EF_DISINTEGRATION) != 0 } {
        return;
    } else if (ctx.entity(self_).s.eFlags & EF_DISINTEGRATION) != 0 {
        return;
    }

    if doDisint {
        if !client.is_null() {
            unsafe {
                (*client).ps.eFlags |= EF_DISINTEGRATION;
                (*client).ps.lastHitLoc = (*client).ps.origin;
            }
        } else {
            let level_time = ctx.world.level.time;
            let e = ctx.entity_mut(self_);
            e.s.eFlags |= EF_DISINTEGRATION;
            e.s.origin2 = e.r.currentOrigin;

            // since it's the corpse entity, tell it to "remove" itself
            e.think = Some(EntThink::BodyRid).into();
            e.nextthink = level_time + 1000;
        }
        return;
    }
}

/// Raven `CheckAlmostCapture`.
///
/// Source: `oracle/codemp/game/g_combat.c:806-851`
pub fn CheckAlmostCapture(self_: &gentity_t, attacker: Option<&gentity_t>) {
    // This takes ctx-free leaf borrows: `&gentity_t` and `Option<&gentity_t>` stay unused.
    // Raven's entire body is `#if 0`'d out (disabled CTF "almost capture" event), so the compiled function is a no-op.
}

/// Raven `G_InKnockDown`.
///
/// Source: `oracle/codemp/game/g_combat.c:853-880`
pub fn G_InKnockDown(ps: *mut playerState_t) -> qboolean {
    use animNumber_t::*;
    unsafe {
        let legs = (*ps).legsAnim;
        let knockdowns = [
            BOTH_KNOCKDOWN1,
            BOTH_KNOCKDOWN2,
            BOTH_KNOCKDOWN3,
            BOTH_KNOCKDOWN4,
            BOTH_KNOCKDOWN5,
            BOTH_GETUP1,
            BOTH_GETUP2,
            BOTH_GETUP3,
            BOTH_GETUP4,
            BOTH_GETUP5,
            BOTH_FORCE_GETUP_F1,
            BOTH_FORCE_GETUP_F2,
            BOTH_FORCE_GETUP_B1,
            BOTH_FORCE_GETUP_B2,
            BOTH_FORCE_GETUP_B3,
            BOTH_FORCE_GETUP_B4,
            BOTH_FORCE_GETUP_B5,
        ];
        if knockdowns.iter().any(|&a| a as c_int == legs) {
            qtrue
        } else {
            qfalse
        }
    }
}

/// Raven `G_CheckSpecialDeathAnim`.
///
/// Source: `oracle/codemp/game/g_combat.c:882-1372`
pub fn G_CheckSpecialDeathAnim(
    ctx: &mut GameContext,
    self_: EntityId,
    point: vec3_t,
    damage: c_int,
    r#mod: c_int,
    hitLoc: c_int,
) -> c_int {
    use animNumber_t::*;
    // self_'s `.client` is a pool client for NPCs, so its derefs stay raw.
    // The anim-selection body below keeps one unsafe scope.
    // The entity-field reads (`.client`, `.localAnimIndex`) route through the accessor.
    let client = ctx.entity(self_).client;
    unsafe {
        let mut deathAnim: c_int = -1;

        // crouch-death helper: returns the thrown-back vs crouched death anim.
        // `move` captures the Copy raw pointer `client` by value, so it holds no environment borrow of `client`.
        // The later `&mut (*client).ps` reborrows do not collide with the closure's shared capture.
        let crouch_death = move |ctx: &mut GameContext| -> c_int {
            let mut fwd: vec3_t = [0.0; 3];
            AngleVectors((*client).ps.viewangles, Some(&mut fwd), None, None);
            let thrown = _DotProduct(fwd, (*client).ps.velocity);
            if thrown < -150.0 {
                BOTH_DEATHBACKWARD1 as c_int
            } else {
                BOTH_DEATH_CROUCHED as c_int
            }
        };

        let legs_anim_for_roll = (*client).ps.legsAnim;
        if mp_bg::bg_panimate::BG_InRoll(
            &mut (*client).ps as *mut playerState_t,
            legs_anim_for_roll,
        ) != qfalse
        {
            deathAnim = BOTH_DEATH_ROLL as c_int; //# Death anim from a roll
        } else if mp_bg::bg_panimate::BG_FlippingAnim((*client).ps.legsAnim) != qfalse {
            deathAnim = BOTH_DEATH_FLIP as c_int; //# Death anim from a flip
        } else if G_InKnockDown(&mut (*client).ps as *mut playerState_t) != qfalse {
            // since these happen a lot, let's handle them case by case
            let legsAnim = (*client).ps.legsAnim;
            let animLength = (*(&ctx.world.bg_state.bgAllAnims)
                [ctx.entity(self_).localAnimIndex as usize]
                .anims
                .add(legsAnim as usize))
            .numFrames as f32
                * ((&ctx.world.bg_state.bgHumanoidAnimations)[legsAnim as usize].frameLerp as f32)
                    .abs();
            let legsTimer = (*client).ps.legsTimer as f32;

            if legsAnim == BOTH_KNOCKDOWN1 as c_int {
                if animLength - legsTimer > 100.0 {
                    // on our way down
                    if (*client).ps.legsTimer > 600 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int; // still partially up
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int; // down
                    }
                }
            } else if legsAnim == BOTH_KNOCKDOWN2 as c_int {
                if animLength - legsTimer > 700.0 {
                    if (*client).ps.legsTimer > 600 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_KNOCKDOWN3 as c_int {
                if animLength - legsTimer > 100.0 {
                    if (*client).ps.legsTimer > 1300 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            } else if legsAnim == BOTH_KNOCKDOWN4 as c_int {
                if animLength - legsTimer > 300.0 {
                    if (*client).ps.legsTimer > 350 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                } else {
                    // crouch death
                    deathAnim = crouch_death(ctx);
                }
            } else if legsAnim == BOTH_KNOCKDOWN5 as c_int {
                if (*client).ps.legsTimer < 750 {
                    deathAnim = BOTH_DEATH_LYING_DN as c_int; // flat
                }
            } else if legsAnim == BOTH_GETUP1 as c_int {
                if (*client).ps.legsTimer < 350 {
                    // standing up
                } else if (*client).ps.legsTimer < 800 {
                    // crouching
                    deathAnim = crouch_death(ctx);
                } else {
                    // lying down
                    if animLength - legsTimer > 450.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP2 as c_int {
                if (*client).ps.legsTimer < 150 {
                } else if (*client).ps.legsTimer < 850 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 500.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP3 as c_int {
                if (*client).ps.legsTimer < 250 {
                } else if (*client).ps.legsTimer < 600 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 150.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP4 as c_int {
                if (*client).ps.legsTimer < 250 {
                } else if (*client).ps.legsTimer < 600 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 850.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP5 as c_int {
                if (*client).ps.legsTimer > 850 {
                    // lying down
                    if animLength - legsTimer > 1500.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP_CROUCH_B1 as c_int {
                if (*client).ps.legsTimer < 800 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 400.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_GETUP_CROUCH_F1 as c_int {
                if (*client).ps.legsTimer < 800 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 150.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B1 as c_int {
                if (*client).ps.legsTimer < 325 {
                } else if (*client).ps.legsTimer < 725 {
                    deathAnim = BOTH_DEATH_SPIN_180 as c_int; //# Death anim when facing backwards
                } else if (*client).ps.legsTimer < 900 {
                    deathAnim = crouch_death(ctx);
                } else {
                    if animLength - legsTimer > 50.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B2 as c_int {
                if (*client).ps.legsTimer < 575 {
                } else if (*client).ps.legsTimer < 875 {
                    deathAnim = BOTH_DEATH_SPIN_180 as c_int;
                } else if (*client).ps.legsTimer < 900 {
                    deathAnim = crouch_death(ctx);
                } else {
                    // partially up
                    deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B3 as c_int {
                if (*client).ps.legsTimer < 150 {
                } else if (*client).ps.legsTimer < 775 {
                    deathAnim = BOTH_DEATHBACKWARD2 as c_int; // backflip
                } else {
                    deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B4 as c_int {
                if (*client).ps.legsTimer < 325 {
                } else {
                    if animLength - legsTimer > 150.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B5 as c_int {
                if (*client).ps.legsTimer < 550 {
                } else if (*client).ps.legsTimer < 1025 {
                    deathAnim = BOTH_DEATHBACKWARD2 as c_int; // backflip
                } else {
                    if animLength - legsTimer > 50.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_B6 as c_int {
                if (*client).ps.legsTimer < 225 {
                } else if (*client).ps.legsTimer < 425 {
                    deathAnim = crouch_death(ctx);
                } else if (*client).ps.legsTimer < 825 {
                    deathAnim = BOTH_DEATHFORWARD3 as c_int; // backflip
                } else {
                    if animLength - legsTimer > 225.0 {
                        deathAnim = BOTH_DEATH_FALLING_UP as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_UP as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_F1 as c_int {
                if (*client).ps.legsTimer < 275 {
                } else if (*client).ps.legsTimer < 750 {
                    deathAnim = BOTH_DEATH14 as c_int;
                } else {
                    if animLength - legsTimer > 100.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            } else if legsAnim == BOTH_FORCE_GETUP_F2 as c_int {
                if (*client).ps.legsTimer < 1200 {
                } else {
                    if animLength - legsTimer > 225.0 {
                        deathAnim = BOTH_DEATH_FALLING_DN as c_int;
                    } else {
                        deathAnim = BOTH_DEATH_LYING_DN as c_int;
                    }
                }
            }
        }

        deathAnim
    }
}

/// Raven `G_PickDeathAnim`.
///
/// Raven: `FIXME: play dead flop anims on body if in an appropriate _DEAD anim when this func is called`.
/// `point` is read-only (forwarded to `G_GetHitLocation`), so it stays by-value.
/// Source: `oracle/codemp/game/g_combat.c:1374-1742`
pub fn G_PickDeathAnim(
    ctx: &mut GameContext,
    self_: Option<EntityId>,
    point: vec3_t,
    damage: c_int,
    r#mod: c_int,
    mut hitLoc: c_int,
) -> c_int {
    use animNumber_t::*;
    // self_ may be an NPC. `.client` is a pool client, so the deref stays raw.
    // The body keeps one unsafe scope for those derefs.
    let client = self_.map_or(core::ptr::null_mut(), |id| ctx.entity(id).client);
    unsafe {
        let mut deathAnim: c_int = -1;
        let max_health: c_int;
        let mut objVelocity: vec3_t = [0.0; 3];

        if self_.is_none() || client.is_null() {
            if self_.is_none()
                || ctx.entity(self_.unwrap()).s.eType != entityType_t::ET_NPC as c_int
            {
                // g2animent
                return 0;
            }
        }
        let self_id = self_.unwrap();

        if !client.is_null() {
            max_health = (*client).ps.stats[statIndex_t::STAT_MAX_HEALTH as usize];

            if (*client).inSpaceIndex != 0
                && (*client).inSpaceIndex != mp_qshared::shared::limits::ENTITYNUM_NONE
            {
                return BOTH_CHOKE3 as c_int;
            }
        } else {
            max_health = 60;
        }

        if !client.is_null() {
            objVelocity = (*client).ps.velocity;
        } else {
            objVelocity = ctx.entity(self_id).s.pos.trDelta;
        }

        if hitLoc == HL_NONE {
            hitLoc = G_GetHitLocation(ctx, self_id, point);
            // self->hitLoc
        }

        let legAnim = if !client.is_null() {
            (*client).ps.legsAnim
        } else {
            ctx.entity(self_id).s.legsAnim
        };

        if ctx.world.globals.gGAvoidDismember != 0 {
            return BOTH_RIGHTHANDCHOPPEDOFF as c_int;
        }

        // Dead flops. Every listed anim resolves to `deathAnim = -2` (the conditional DEADFLOP re-selects are commented out in Raven).
        let dead_flops = [
            BOTH_DEATH1,
            BOTH_DEAD1,
            BOTH_DEATH2,
            BOTH_DEAD2,
            BOTH_DEATH8,
            BOTH_DEAD8,
            BOTH_DEATH13,
            BOTH_DEAD13,
            BOTH_DEATH14,
            BOTH_DEAD14,
            BOTH_DEATH16,
            BOTH_DEAD16,
            BOTH_DEADBACKWARD1,
            BOTH_DEADBACKWARD2,
            BOTH_DEATH10,
            BOTH_DEAD10,
            BOTH_DEATH15,
            BOTH_DEAD15,
            BOTH_DEADFORWARD1,
            BOTH_DEADFORWARD2,
            BOTH_DEADFLOP1,
            BOTH_DEAD3,
            BOTH_DEAD4,
            BOTH_DEAD5,
            BOTH_DEAD6,
            BOTH_DEAD7,
            BOTH_DEAD9,
            BOTH_DEAD11,
            BOTH_DEAD12,
            BOTH_DEAD17,
            BOTH_DEAD18,
            BOTH_DEAD19,
            BOTH_LYINGDEAD1,
            BOTH_STUMBLEDEAD1,
            BOTH_FALLDEAD1LAND,
            BOTH_DEATH3,
            BOTH_DEATH4,
            BOTH_DEATH5,
            BOTH_DEATH6,
            BOTH_DEATH7,
            BOTH_DEATH9,
            BOTH_DEATH11,
            BOTH_DEATH12,
            BOTH_DEATH17,
            BOTH_DEATH18,
            BOTH_DEATH19,
            BOTH_DEATHFORWARD1,
            BOTH_DEATHFORWARD2,
            BOTH_DEATHBACKWARD1,
            BOTH_DEATHBACKWARD2,
            BOTH_DEATH1IDLE,
            BOTH_LYINGDEATH1,
            BOTH_STUMBLEDEATH1,
            BOTH_FALLDEATH1,
            BOTH_FALLDEATH1INAIR,
            BOTH_FALLDEATH1LAND,
        ];
        if dead_flops.iter().any(|&a| a as c_int == legAnim) {
            deathAnim = -2;
        }

        if deathAnim == -1 {
            if !client.is_null() {
                deathAnim = G_CheckSpecialDeathAnim(ctx, self_id, point, damage, r#mod, hitLoc);
            }

            if deathAnim == -1 {
                let dmg = damage as f32;
                let mh = max_health as f32;
                // death anims
                if hitLoc == HL_FOOT_RT || hitLoc == HL_FOOT_LT {
                    if r#mod == meansOfDeath_t::MOD_SABER as c_int
                        && ctx.world.bg_state.rng.Q_irand(0, 2) == 0
                    {
                        return BOTH_DEATH10 as c_int; // chest: back flip
                    } else if ctx.world.bg_state.rng.Q_irand(0, 2) == 0 {
                        deathAnim = BOTH_DEATH4 as c_int;
                    } else if ctx.world.bg_state.rng.Q_irand(0, 1) == 0 {
                        deathAnim = BOTH_DEATH5 as c_int;
                    } else {
                        deathAnim = BOTH_DEATH15 as c_int;
                    }
                } else if hitLoc == HL_LEG_RT || hitLoc == HL_LEG_LT {
                    if ctx.world.bg_state.rng.Q_irand(0, 2) == 0 {
                        deathAnim = BOTH_DEATH4 as c_int;
                    } else if ctx.world.bg_state.rng.Q_irand(0, 1) == 0 {
                        deathAnim = BOTH_DEATH5 as c_int;
                    } else {
                        deathAnim = BOTH_DEATH15 as c_int;
                    }
                } else if hitLoc == HL_BACK {
                    if VectorLengthSquared(objVelocity) == 0.0 {
                        deathAnim = BOTH_DEATH17 as c_int; // head/back: croak
                    } else if ctx.world.bg_state.rng.Q_irand(0, 2) == 0 {
                        deathAnim = BOTH_DEATH4 as c_int;
                    } else if ctx.world.bg_state.rng.Q_irand(0, 1) == 0 {
                        deathAnim = BOTH_DEATH5 as c_int;
                    } else {
                        deathAnim = BOTH_DEATH15 as c_int;
                    }
                } else if hitLoc == HL_CHEST_RT
                    || hitLoc == HL_ARM_RT
                    || hitLoc == HL_HAND_RT
                    || hitLoc == HL_BACK_RT
                {
                    if dmg <= mh * 0.25 {
                        deathAnim = BOTH_DEATH9 as c_int;
                    } else if dmg <= mh * 0.5 {
                        deathAnim = BOTH_DEATH3 as c_int;
                    } else if dmg <= mh * 0.75 {
                        deathAnim = BOTH_DEATH6 as c_int;
                    } else if ctx.world.bg_state.rng.Q_irand(0, 1) != 0 {
                        deathAnim = BOTH_DEATH8 as c_int; // TEMP HACK: play spinny deaths less often
                    } else {
                        deathAnim = match ctx.world.bg_state.rng.Q_irand(0, 2) {
                            1 => BOTH_DEATH3 as c_int,
                            2 => BOTH_DEATH6 as c_int,
                            _ => BOTH_DEATH9 as c_int,
                        };
                    }
                } else if hitLoc == HL_CHEST_LT
                    || hitLoc == HL_ARM_LT
                    || hitLoc == HL_HAND_LT
                    || hitLoc == HL_BACK_LT
                {
                    if dmg <= mh * 0.25 {
                        deathAnim = BOTH_DEATH11 as c_int;
                    } else if dmg <= mh * 0.5 {
                        deathAnim = BOTH_DEATH7 as c_int;
                    } else if dmg <= mh * 0.75 {
                        deathAnim = BOTH_DEATH12 as c_int;
                    } else if ctx.world.bg_state.rng.Q_irand(0, 1) != 0 {
                        deathAnim = BOTH_DEATH14 as c_int;
                    } else {
                        deathAnim = match ctx.world.bg_state.rng.Q_irand(0, 2) {
                            1 => BOTH_DEATH7 as c_int,
                            2 => BOTH_DEATH12 as c_int,
                            _ => BOTH_DEATH11 as c_int,
                        };
                    }
                } else if hitLoc == HL_CHEST || hitLoc == HL_WAIST {
                    if dmg <= mh * 0.25 || VectorLengthSquared(objVelocity) == 0.0 {
                        if ctx.world.bg_state.rng.Q_irand(0, 1) == 0 {
                            deathAnim = BOTH_DEATH18 as c_int; // gut: fall right
                        } else {
                            deathAnim = BOTH_DEATH19 as c_int; // gut: fall left
                        }
                    } else if dmg <= mh * 0.5 {
                        deathAnim = BOTH_DEATH2 as c_int;
                    } else if dmg <= mh * 0.75 {
                        if ctx.world.bg_state.rng.Q_irand(0, 1) == 0 {
                            deathAnim = BOTH_DEATH1 as c_int;
                        } else {
                            deathAnim = BOTH_DEATH16 as c_int;
                        }
                    } else {
                        deathAnim = BOTH_DEATH10 as c_int; // chest: back flip
                    }
                } else if hitLoc == HL_HEAD {
                    if dmg <= mh * 0.5 {
                        deathAnim = BOTH_DEATH17 as c_int; // head/back: croak
                    } else {
                        deathAnim = BOTH_DEATH13 as c_int; // head: stumble, fall back
                    }
                }
            }
        }

        // Validate.....
        if deathAnim == -1
            || BG_HasAnimation(
                &ctx.world.bg_state,
                ctx.entity(self_id).localAnimIndex,
                deathAnim,
            ) == qfalse
        {
            // I guess we'll take what we can get.....
            let localAnimIndex = ctx.entity(self_id).localAnimIndex;
            deathAnim = BG_PickAnim(
                &mut ctx.world.bg_state,
                localAnimIndex,
                BOTH_DEATH1 as c_int,
                BOTH_DEATH25 as c_int,
            );
        }

        deathAnim
    }
}

/// Raven `G_GetJediMaster`.
///
/// Source: `oracle/codemp/game/g_combat.c:1744-1762`
pub fn G_GetJediMaster(ctx: &mut GameContext) -> *mut gentity_t {
    for i in 0..MAX_CLIENTS as usize {
        let inuse = ctx.world.g_entities[i].inuse;
        // This is a real client slot (i < MAX_CLIENTS), so the client deref stays raw.
        let client = ctx.world.g_entities[i].client;
        if inuse != qfalse && !client.is_null() && unsafe { (*client).ps.isJediMaster } != qfalse {
            return &mut ctx.world.g_entities[i] as *mut gentity_t;
        }
    }
    core::ptr::null_mut()
}

/// Raven `G_AlertTeam`.
///
/// Source: `oracle/codemp/game/g_combat.c:1770-1858`
pub fn G_AlertTeam(
    ctx: &mut GameContext,
    victim: EntityId,
    attacker: Option<EntityId>,
    radius: f32,
    soundDist: f32,
) {
    let mut radiusEnts: [c_int; 128] = [0; 128];
    let mut mins: vec3_t = [0.0; 3];
    let mut maxs: vec3_t = [0.0; 3];
    let sndDistSq = soundDist * soundDist;

    // attacker may be an NPC. `.client` is a pool client, so the deref stays raw.
    let attacker_client = attacker.map_or(core::ptr::null_mut(), |id| ctx.entity(id).client);
    if attacker.is_none() || attacker_client.is_null() {
        return;
    }

    // Setup the bbox to search in
    for i in 0..3 {
        mins[i] = ctx.entity(victim).r.currentOrigin[i] - radius;
        maxs[i] = ctx.entity(victim).r.currentOrigin[i] + radius;
    }

    // Get the number of entities in a given space
    let numEnts = trap::EntitiesInBox(
        ctx.engine,
        mp_abi::game::syscalls::G_ENTITIES_IN_BOX::GEntitiesInBoxArgs::new(
            &mins as *const vec3_t,
            &maxs as *const vec3_t,
            radiusEnts.as_mut_ptr(),
            128,
        ),
    );

    // Cull this list
    for i in 0..numEnts {
        let check = EntityId(radiusEnts[i as usize] as u32);

        // Validate clients
        if ctx.entity(check).client.is_null() {
            continue;
        }

        // only want NPCs
        if ctx.entity(check).NPC.is_null() {
            continue;
        }

        // gNPC_t pointer. The deref stays raw.
        let npc = ctx.entity(check).NPC;

        // This NPC specifically flagged to ignore alerts
        if (unsafe { (*npc).scriptFlags } & SCF_IGNORE_ALERTS) != 0 {
            continue;
        }

        // This NPC specifically flagged to ignore alerts
        if (unsafe { (*npc).scriptFlags } & SCF_LOOK_FOR_ENEMIES) == 0 {
            continue;
        }

        // this ent does not participate in group AI
        if (unsafe { (*npc).scriptFlags } & SCF_NO_GROUPS) != 0 {
            continue;
        }

        // Skip the requested avoid check if present
        if check == victim {
            continue;
        }

        // Skip the attacker
        if Some(check) == attacker {
            continue;
        }

        // Pool clients. The deref stays raw.
        let checkClient = ctx.entity(check).client;
        let victimClient = ctx.entity(victim).client;

        // Must be on the same team
        if unsafe { (*checkClient).playerTeam } != unsafe { (*victimClient).playerTeam } {
            continue;
        }

        // Must be alive
        if ctx.entity(check).health <= 0 {
            continue;
        }

        if ctx.entity(check).enemy.is_none() {
            // only do this if they're not already mad at someone
            let distSq = DistanceSquared(
                ctx.entity(check).r.currentOrigin,
                ctx.entity(victim).r.currentOrigin,
            );
            let victim_org = ctx.entity(victim).r.currentOrigin;
            let check_org = ctx.entity(check).r.currentOrigin;
            if distSq > 16384.0
                && trap::InPVS(
                    ctx.engine,
                    mp_abi::game::syscalls::G_IN_PVS::GInPvsArgs::new(
                        &victim_org as *const vec3_t,
                        &check_org as *const vec3_t,
                    ),
                ) == qfalse
            {
                // not even potentially visible/hearable
                continue;
            }
            // NOTE: this allows sound alerts to still go through doors/PVS if the teammate is within 128 of the victim...
            if soundDist <= 0.0 || distSq > sndDistSq {
                // out of sound range
                let hfov = unsafe { (*npc).stats.hfov } as c_int;
                let vfov = unsafe { (*npc).stats.vfov } as c_int;
                if crate::NPC_senses::InFOV(ctx, Some(victim), check, hfov, vfov) == qfalse
                    || crate::NPC_utils::NPC_ClearLOS2(ctx, Some(check), victim_org) == qfalse
                {
                    // out of FOV or no LOS
                    continue;
                }
            }

            // FIXME: This can have a nasty cascading effect if setup wrong...
            crate::NPC_combat::G_SetEnemy(ctx, check, attacker);
        }
    }
}

/// Raven `G_DeathAlert`.
///
/// Raven: `FIXME: with all the other alert stuff, do we really need this?`
/// Source: `oracle/codemp/game/g_combat.c:1869-1872`
pub fn G_DeathAlert(ctx: &mut GameContext, victim: EntityId, attacker: Option<EntityId>) {
    // The EntityId/Option params forward straight through to G_AlertTeam.
    // #define DEATH_ALERT_RADIUS 512 / DEATH_ALERT_SOUND_RADIUS 512
    G_AlertTeam(ctx, victim, attacker, 512.0, 512.0);
}

/// Raven `DeathFX`.
///
/// Raven: `team no longer indicates species/race.  NPC_class should be used to identify certain npc types`.
/// Source: `oracle/codemp/game/g_combat.c:1883-1984`
pub fn DeathFX(ctx: &mut GameContext, ent: Option<EntityId>) {
    let mut effectPos: vec3_t = [0.0; 3];
    let mut right: vec3_t = [0.0; 3];

    // ent may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ent.map_or(core::ptr::null_mut(), |id| ctx.entity(id).client);
    if ent.is_none() || client.is_null() {
        return;
    }
    let ent = ent.unwrap();

    let defaultDir: vec3_t = [0.0, 0.0, 1.0];
    let npc_class = unsafe { (*client).NPC_class };

    match npc_class {
        class_t::CLASS_MOUSE => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 20.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/small_explode"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/mouse/misc/death1");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_PROBE => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] += 50.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/probeexplosion1"),
                effectPos,
                defaultDir,
            );
        }
        class_t::CLASS_ATST => {
            AngleVectors(
                ctx.entity(ent).r.currentAngles,
                None,
                Some(&mut right),
                None,
            );
            _VectorMA(ctx.entity(ent).r.currentOrigin, 20.0, right, &mut effectPos);
            effectPos[2] += 180.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            _VectorMA(effectPos, -40.0, right, &mut effectPos);
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
        }
        class_t::CLASS_SEEKER | class_t::CLASS_REMOTE => {
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/small_explode"),
                ctx.entity(ent).r.currentOrigin,
                defaultDir,
            );
        }
        class_t::CLASS_GONK => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 5.0;
            let s = format!(
                "sound/chars/gonk/misc/death{}.wav",
                ctx.world.bg_state.rng.Q_irand(1, 3)
            );
            let sound = G_SoundIndex(ctx, &s);
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/med_explode"),
                effectPos,
                defaultDir,
            );
        }
        // should list all remaining droids here, hope I didn't miss any
        class_t::CLASS_R2D2 => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 10.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/med_explode"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/mark2/misc/mark2_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_PROTOCOL | class_t::CLASS_R5D2 => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 10.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/med_explode"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/mark2/misc/mark2_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_MARK2 => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 15.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/mark2/misc/mark2_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_INTERROGATOR => {
            effectPos = ctx.entity(ent).r.currentOrigin;
            effectPos[2] -= 15.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/interrogator/misc/int_droid_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_MARK1 => {
            AngleVectors(
                ctx.entity(ent).r.currentAngles,
                None,
                Some(&mut right),
                None,
            );
            _VectorMA(ctx.entity(ent).r.currentOrigin, 10.0, right, &mut effectPos);
            effectPos[2] -= 15.0;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            _VectorMA(effectPos, -20.0, right, &mut effectPos);
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            _VectorMA(effectPos, -20.0, right, &mut effectPos);
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "explosions/droidexplosion1"),
                effectPos,
                defaultDir,
            );
            let sound = G_SoundIndex(ctx, "sound/chars/mark1/misc/mark1_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
        }
        class_t::CLASS_SENTRY => {
            let sound = G_SoundIndex(ctx, "sound/chars/sentry/misc/sentry_explo");
            crate::g_utils::G_Sound(
                ctx,
                Some(ent),
                CHAN_AUTO,
                sound,
            );
            effectPos = ctx.entity(ent).r.currentOrigin;
            crate::g_utils::G_PlayEffectID(
                G_EffectIndex(ctx, "env/med_explode"),
                effectPos,
                defaultDir,
            );
        }
        _ => {}
    }
}

/// Raven `G_CheckVictoryScript`.
///
/// Source: `oracle/codemp/game/g_combat.c:1986-2014`
pub fn G_CheckVictoryScript(ctx: &mut GameContext, self_: EntityId) {
    // self_'s `.NPC` (gNPC_t) and `.client` (pool) pointers deref raw (traps 2b/2c).
    // The body keeps one unsafe scope. Entity fields route through the accessor.
    unsafe {
        if G_ActivateBehavior(ctx, Some(self_), bSet_t::BSET_VICTORY as c_int) != qfalse {
            return;
        }

        let npc = ctx.entity(self_).NPC;
        let client = ctx.entity(self_).client;

        if !npc.is_null() && ctx.entity(self_).s.weapon == WP_SABER as c_int {
            // Jedi taunt from within their AI
            (*npc).blockedSpeechDebounceTime = 0; // get them ready to taunt
            return;
        }
        if !client.is_null() && (*client).NPC_class == class_t::CLASS_GALAKMECH {
            ctx.entity_mut(self_).wait = 1.0;
            let self_eid = Some(self_);
            let gloat_time = ctx.world.bg_state.rng.Q_irand(5000, 8000);
            TIMER_Set(ctx, self_eid, c"gloatTime".as_ptr(), gloat_time);
            (*npc).blockedSpeechDebounceTime = 0; // get him ready to taunt
            return;
        }
        // FIXME: any way to not say this *right away*?  Wait for victim's death anim/scream to finish?
        let level_time = ctx.world.level.time;
        if !npc.is_null()
            && !(*npc).group.is_null()
            && !(*(*npc).group).commander.is_null()
            && !((*(*(*npc).group).commander).NPC).is_null()
            && (*((*(*(*npc).group).commander).NPC)).rank > (*npc).rank
            && ctx.world.bg_state.rng.Q_irand(0, 2) == 0
        {
            // sometimes have the group commander speak instead
            let cmdr_npc = (*(*(*npc).group).commander).NPC;
            (*cmdr_npc).greetingDebounceTime =
                level_time + ctx.world.bg_state.rng.Q_irand(2000, 5000);
        } else if !npc.is_null() {
            (*npc).greetingDebounceTime = level_time + ctx.world.bg_state.rng.Q_irand(2000, 5000);
        }
    }
}

/// Raven `G_AddPowerDuelScore`.
///
/// Source: `oracle/codemp/game/g_combat.c:2016-2035`
pub fn G_AddPowerDuelScore(ctx: &mut GameContext, team: c_int, score: c_int) {
    let mut i = 0;
    while i < MAX_CLIENTS as c_int {
        let check = EntityId(i as u32);
        let inuse = ctx.entity(check).inuse;
        // This is a real client slot (i < MAX_CLIENTS), so the deref stays raw.
        let checkClient = ctx.entity(check).client;
        if inuse != qfalse
            && !checkClient.is_null()
            && unsafe {
                (*checkClient).pers.connected == CON_CONNECTED
                    && (*checkClient).iAmALoser == qfalse
                    && (*checkClient).ps.stats[statIndex_t::STAT_HEALTH as usize] > 0
                    && (*checkClient).sess.sessionTeam != TEAM_SPECTATOR
                    && (*checkClient).sess.duelTeam as c_int == team
            }
        {
            // found a living client on the specified team
            unsafe {
                (*checkClient).sess.wins += score;
            }
            let number = ctx.entity(check).s.number;
            crate::g_client::ClientUserinfoChanged(ctx, number);
        }
        i += 1;
    }
}

/// Raven `G_AddPowerDuelLoserScore`.
///
/// Source: `oracle/codemp/game/g_combat.c:2037-2055`
pub fn G_AddPowerDuelLoserScore(ctx: &mut GameContext, team: c_int, score: c_int) {
    let mut i = 0;
    while i < MAX_CLIENTS as c_int {
        let check = EntityId(i as u32);
        let inuse = ctx.entity(check).inuse;
        // This is a real client slot (i < MAX_CLIENTS), so the deref stays raw.
        let checkClient = ctx.entity(check).client;
        if inuse != qfalse
            && !checkClient.is_null()
            && unsafe {
                (*checkClient).pers.connected == CON_CONNECTED
                    && ((*checkClient).iAmALoser != qfalse
                        || ((*checkClient).ps.stats[statIndex_t::STAT_HEALTH as usize] <= 0
                            && (*checkClient).sess.sessionTeam != TEAM_SPECTATOR))
                    && (*checkClient).sess.duelTeam as c_int == team
            }
        {
            // found a living client on the specified team
            unsafe {
                (*checkClient).sess.losses += score;
            }
            let number = ctx.entity(check).s.number;
            crate::g_client::ClientUserinfoChanged(ctx, number);
        }
        i += 1;
    }
}

/// Raven `G_BroadcastObit`.
///
/// Source: `oracle/codemp/game/g_combat.c:2059-2105`
pub fn G_BroadcastObit(
    ctx: &mut GameContext,
    self_: EntityId,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    killer: c_int,
    meansOfDeath: c_int,
    wasInVehicle: c_int,
    wasJediMaster: qboolean,
) {
    // `SVF_BROADCAST` resolves via the crate prelude glob (`crate::g_public_consts`).
    // broadcast the death event to everyone
    if ctx.entity(self_).s.eType != entityType_t::ET_NPC as c_int
        && ctx.world.globals.g_noPDuelCheck == 0
    {
        let currentOrigin = ctx.entity(self_).r.currentOrigin;
        let ent = G_TempEntity(ctx, currentOrigin, entity_event_t::EV_OBITUARY as c_int);
        ctx.entity_mut(ent).s.eventParm = meansOfDeath;
        let self_number = ctx.entity(self_).s.number;
        ctx.entity_mut(ent).s.otherEntityNum = self_number;
        if let Some(attacker) = attacker {
            let n = ctx.entity(attacker).s.number;
            ctx.entity_mut(ent).s.otherEntityNum2 = n;
        } else {
            ctx.entity_mut(ent).s.otherEntityNum2 = killer;
        }
        if let Some(inflictor) = inflictor {
            let cn = ctx.entity(inflictor).classname_str();
            if Q_stricmp("vehicle_proj", &cn) == 0 {
                // a vehicle missile
                ctx.entity_mut(ent).s.eventParm = meansOfDeath_t::MOD_VEHICLE as c_int;
                // store index into g_vehWeaponInfo
                let oe2 = ctx.entity(inflictor).s.otherEntityNum2;
                ctx.entity_mut(ent).s.weapon = oe2 + 1;
                // store generic rocket or blaster type of missile
                let inf_weapon = ctx.entity(inflictor).s.weapon;
                ctx.entity_mut(ent).s.generic1 = inf_weapon;
            }
        }
        if wasInVehicle != 0 && ctx.entity(self_).s.number < MAX_CLIENTS as c_int {
            // target is in a vehicle, store the entnum
            ctx.entity_mut(ent).s.lookTarget = wasInVehicle;
        }
        if let Some(attacker) = attacker {
            if ctx.entity(attacker).s.m_iVehicleNum != 0
                && ctx.entity(attacker).s.number < MAX_CLIENTS as c_int
            {
                let v = ctx.entity(attacker).s.m_iVehicleNum;
                ctx.entity_mut(ent).s.brokenLimbs = v;
            } else if ctx.entity(ent).s.lookTarget != 0 && {
                let cn = ctx.entity(attacker).classname_str();
                Q_stricmp("func_rotating", &cn) == 0
            } {
                // my vehicle was killed by a func_rotating, probably an asteroid, so...
                ctx.entity_mut(ent).s.saberInFlight = qtrue;
            }
        }
        ctx.entity_mut(ent).r.svFlags = SVF_BROADCAST; // send to everyone
        ctx.entity_mut(ent).s.isJediMaster = wasJediMaster;
    }
}

/// Split from `G_Damage`.
/// This holds the full-exemption checks: friendly fire and emplaced/allied/turret-owner team rules, the Jedi Master no-bystander rule,
/// godmode, EF_INVULNERABLE (clearing it on expiry), and the siege teamnodmg object rules.
/// It returns true when the target takes no damage at all (Raven's early returns).
/// Source: `oracle/codemp/game/g_combat.c:4884-4966`
unsafe fn g_damage_check_exempt(
    ctx: &mut GameContext,
    targ: *mut gentity_t,
    attacker: *mut gentity_t,
    dflags: c_int,
) -> bool {
    // check for completely getting out of the damage
    if (dflags & DAMAGE_NO_PROTECTION) == 0 {
        if targ != attacker {
            if crate::g_team::OnSameTeam(ctx, ctx.entity_id_of(targ), ctx.entity_id_of(attacker))
                != qfalse
            {
                if ctx.world.cvars.g_friendlyFire.integer == 0 {
                    return true;
                }
            } else if !attacker.is_null()
                && (*attacker).inuse != qfalse
                && (*attacker).client.is_null()
                && (*attacker).activator.is_some()
                && {
                    let act = &mut ctx.world.g_entities[(*attacker).activator.unwrap().0 as usize]
                        as *mut gentity_t;
                    targ != act && (*act).inuse != qfalse && !(*act).client.is_null()
                }
            {
                // emplaced guns don't hurt teammates of user
                let act = &mut ctx.world.g_entities[(*attacker).activator.unwrap().0 as usize]
                    as *mut gentity_t;
                if crate::g_team::OnSameTeam(ctx, ctx.entity_id_of(targ), ctx.entity_id_of(act))
                    != qfalse
                {
                    if ctx.world.cvars.g_friendlyFire.integer == 0 {
                        return true;
                    }
                }
            } else if (*targ).inuse != qfalse
                && !(*targ).client.is_null()
                && ctx.world.cvars.g_gametype.integer >= GT_TEAM
                && (*attacker).s.number >= MAX_CLIENTS as c_int
                && (*attacker).alliedTeam != 0
                && (*((*targ).client)).sess.sessionTeam == (*attacker).alliedTeam
                && ctx.world.cvars.g_friendlyFire.integer == 0
            {
                // things allied with my team should't hurt me.. I guess
                return true;
            }
        }

        if ctx.world.cvars.g_gametype.integer == GT_JEDIMASTER
            && ctx.world.cvars.g_friendlyFire.integer == 0
            && !targ.is_null()
            && !(*targ).client.is_null()
            && !attacker.is_null()
            && !(*attacker).client.is_null()
            && targ != attacker
            && (*((*targ).client)).ps.isJediMaster == qfalse
            && (*((*attacker).client)).ps.isJediMaster == qfalse
            && G_ThereIsAMaster(ctx) != qfalse
        {
            return true;
        }

        if (*targ).s.number >= MAX_CLIENTS as c_int
            && !(*targ).client.is_null()
            && (*targ).s.shouldtarget != qfalse
            && (*targ).s.teamowner != 0
            && !attacker.is_null()
            && (*attacker).inuse != qfalse
            && !(*attacker).client.is_null()
            && (*targ).s.owner >= 0
            && (*targ).s.owner < MAX_CLIENTS as c_int
        {
            let targown = &mut ctx.world.g_entities[(*targ).s.owner as usize] as *mut gentity_t;

            if !targown.is_null()
                && (*targown).inuse != qfalse
                && !(*targown).client.is_null()
                && crate::g_team::OnSameTeam(
                    ctx,
                    ctx.entity_id_of(targown),
                    ctx.entity_id_of(attacker),
                ) != qfalse
            {
                if ctx.world.cvars.g_friendlyFire.integer == 0 {
                    return true;
                }
            }
        }

        // check for godmode
        if ((*targ).flags & FL_GODMODE) != 0 && (*targ).s.eType != entityType_t::ET_NPC as c_int {
            return true;
        }

        if !targ.is_null()
            && !(*targ).client.is_null()
            && ((*((*targ).client)).ps.eFlags & EF_INVULNERABLE) != 0
            && !attacker.is_null()
            && !(*attacker).client.is_null()
            && targ != attacker
        {
            let tc = (*targ).client;
            if (*tc).invulnerableTimer <= ctx.world.level.time {
                (*tc).ps.eFlags &= !EF_INVULNERABLE;
            } else {
                return true;
            }
        }
    }

    // check for teamnodmg
    if !attacker.is_null() && (*targ).client.is_null() {
        // attacker hit a non-client
        if ctx.world.cvars.g_gametype.integer == GT_SIEGE
            && ctx.world.cvars.g_ff_objectives.integer == 0
        {
            if (*targ).teamnodmg != 0 {
                // targ shouldn't take damage from a certain team
                if !(*attacker).client.is_null() {
                    // a client hit a non-client object
                    if (*targ).teamnodmg == (*((*attacker).client)).sess.sessionTeam {
                        return true;
                    }
                } else if (*attacker).teamnodmg != 0 {
                    // a non-client hit a non-client object
                    if (*targ).teamnodmg == (*attacker).teamnodmg {
                        if (*attacker).activator.is_some() && {
                            let a = &mut ctx.world.g_entities
                                [(*attacker).activator.unwrap().0 as usize]
                                as *mut gentity_t;
                            (*a).inuse != qfalse
                                && (*a).s.number < MAX_CLIENTS as c_int
                                && !(*a).client.is_null()
                                && (*((*a).client)).sess.sessionTeam != (*targ).teamnodmg
                        } {
                            // uh, let them damage it I guess.
                        } else {
                            return true;
                        }
                    }
                }
            }
        }
    }

    false
}

/// Split from `G_Damage`.
/// This holds the momentum add, applied even when the damage will not be taken: saber-knockback scaling (per-saber knockbackScale flags),
/// the velocity kick, and the PMF_TIME_KNOCKBACK timer with its saber-scale-zero exception.
/// Source: `oracle/codemp/game/g_combat.c:4812-4874`
unsafe fn g_damage_apply_knockback(
    ctx: &mut GameContext,
    targ: *mut gentity_t,
    attacker: *mut gentity_t,
    dir_val: vec3_t,
    knockback: c_int,
    dflags: c_int,
    r#mod: c_int,
) {
    // figure momentum add, even if the damage won't be taken
    if knockback != 0 && !(*targ).client.is_null() {
        let mut kvel: vec3_t = [0.0; 3];
        let mass: f32 = 200.0;
        let tc = (*targ).client;
        let g_knockback = ctx.world.cvars.g_knockback.value;

        if r#mod == meansOfDeath_t::MOD_SABER as c_int {
            let mut saberKnockbackScale = ctx.world.cvars.g_saberDmgVelocityScale.value;
            if (dflags & DAMAGE_SABER_KNOCKBACK1) != 0 || (dflags & DAMAGE_SABER_KNOCKBACK2) != 0 {
                if saberKnockbackScale == 0.0 {
                    saberKnockbackScale = 1.0;
                }
                if !attacker.is_null() && !(*attacker).client.is_null() {
                    let ac = (*attacker).client;
                    if (dflags & DAMAGE_SABER_KNOCKBACK1) != 0 {
                        saberKnockbackScale *= (*ac).saber[0].knockbackScale;
                    }
                    if (dflags & DAMAGE_SABER_KNOCKBACK1_B2) != 0 {
                        saberKnockbackScale *= (*ac).saber[0].knockbackScale2;
                    }
                    if (dflags & DAMAGE_SABER_KNOCKBACK2) != 0 {
                        saberKnockbackScale *= (*ac).saber[1].knockbackScale;
                    }
                    if (dflags & DAMAGE_SABER_KNOCKBACK2_B2) != 0 {
                        saberKnockbackScale *= (*ac).saber[1].knockbackScale2;
                    }
                }
            }
            _VectorScale(
                dir_val,
                (g_knockback * knockback as f32 / mass) * saberKnockbackScale,
                &mut kvel,
            );
        } else {
            _VectorScale(dir_val, g_knockback * knockback as f32 / mass, &mut kvel);
        }
        _VectorAdd((*tc).ps.velocity, kvel, &mut (*tc).ps.velocity);

        // set the timer so that the other client can't cancel
        // out the movement immediately
        if (*tc).ps.pm_time == 0
            && (ctx.world.cvars.g_saberDmgVelocityScale.integer != 0
                || r#mod != meansOfDeath_t::MOD_SABER as c_int
                || (dflags & DAMAGE_SABER_KNOCKBACK1) != 0
                || (dflags & DAMAGE_SABER_KNOCKBACK2) != 0
                || (dflags & DAMAGE_SABER_KNOCKBACK1_B2) != 0
                || (dflags & DAMAGE_SABER_KNOCKBACK2_B2) != 0)
        {
            let mut t = knockback * 2;
            if t < 50 {
                t = 50;
            }
            if t > 200 {
                t = 200;
            }
            (*tc).ps.pm_time = t;
            (*tc).ps.pm_flags |= PMF_TIME_KNOCKBACK;
        }
    }
}

/// Split from `player_die`.
/// This holds kill credit and per-gametype scoring: the PERS_KILLED bump, suicide count, victory script, the droid/collision
/// no-credit picks, self/team-kill penalties (duel opponent credit, TK autokick), Jedi Master saber handoff and scoring,
/// duel/powerduel win bookkeeping, and the world-kill penalty.
/// Source: `oracle/codemp/game/g_combat.c:2566-2743`
unsafe fn player_die_kill_credit(
    ctx: &mut GameContext,
    self_: *mut gentity_t,
    attacker: *mut gentity_t,
    meansOfDeath: c_int,
) {
    let cl = (*self_).client;
    (*cl).ps.persistant[persEnum_t::PERS_KILLED as usize] += 1;

    if self_ == attacker {
        (*cl).ps.fd.suicides += 1;
    }

    if !attacker.is_null() && !(*attacker).client.is_null() {
        // killed by a client of some kind (player, NPC or vehicle)
        let acl = (*attacker).client;
        if (*self_).s.number < MAX_CLIENTS as c_int {
            // only remember real clients
            (*acl).lastkilled_client = (*self_).s.number;
        }

        G_CheckVictoryScript(ctx, ctx.entity_id_of(attacker).unwrap());

        if (*self_).s.number >= MAX_CLIENTS as c_int
            && !(*self_).client.is_null()
            && (*cl).NPC_class != class_t::CLASS_VEHICLE
            && (*self_).s.m_iVehicleNum != 0
        {
            // no credit for droid, you do get credit for the vehicle kill and the pilot (2 points)
        } else if meansOfDeath == meansOfDeath_t::MOD_COLLISION as c_int
            || meansOfDeath == meansOfDeath_t::MOD_VEH_EXPLOSION as c_int
        {
            // no credit for veh-veh collisions?
        } else if attacker == self_
            || crate::g_team::OnSameTeam(ctx, ctx.entity_id_of(self_), ctx.entity_id_of(attacker))
                != qfalse
        {
            // killed self or teammate
            if meansOfDeath == meansOfDeath_t::MOD_FALLING as c_int
                && attacker != self_
                && (*attacker).s.number < MAX_CLIENTS as c_int
                && (*attacker).s.m_iVehicleNum != 0
            {
                // crushed by a teammate in a vehicle, no penalty
            } else if ctx.world.cvars.g_gametype.integer == GT_DUEL {
                // in duel, if you kill yourself, the person you are dueling against gets a kill for it
                let mut otherClNum: c_int = -1;
                if ctx.world.level.sortedClients[0] == (*self_).s.number {
                    otherClNum = ctx.world.level.sortedClients[1];
                } else if ctx.world.level.sortedClients[1] == (*self_).s.number {
                    otherClNum = ctx.world.level.sortedClients[0];
                }

                if otherClNum >= 0
                    && otherClNum < MAX_CLIENTS as c_int
                    && ctx.world.g_entities[otherClNum as usize].inuse != qfalse
                    && !ctx.world.g_entities[otherClNum as usize].client.is_null()
                    && otherClNum != (*attacker).s.number
                {
                    AddScore(
                        ctx,
                        EntityId::from_num(otherClNum).unwrap(),
                        (*self_).r.currentOrigin,
                        1,
                    );
                } else {
                    AddScore(
                        ctx,
                        ctx.entity_id_of(attacker).unwrap(),
                        (*self_).r.currentOrigin,
                        -1,
                    );
                }
            } else {
                AddScore(
                    ctx,
                    ctx.entity_id_of(attacker).unwrap(),
                    (*self_).r.currentOrigin,
                    -1,
                );
                if attacker != self_
                    && (*attacker).s.number < MAX_CLIENTS as c_int
                    && (*self_).s.number < MAX_CLIENTS as c_int
                {
                    crate::g_cmds::G_CheckTKAutoKickBan(ctx, ctx.entity_id_of(attacker).unwrap());
                }
            }
            if ctx.world.cvars.g_gametype.integer == GT_JEDIMASTER {
                if !(*self_).client.is_null() && (*cl).ps.isJediMaster != qfalse {
                    // killed ourself so return the saber to the original position
                    crate::g_client::ThrowSaberToAttacker(
                        ctx,
                        ctx.entity_id_of(self_).unwrap(),
                        None,
                    );
                    (*cl).ps.isJediMaster = qfalse;
                }
            }
        } else {
            if ctx.world.cvars.g_gametype.integer == GT_JEDIMASTER {
                if (!(*attacker).client.is_null() && (*acl).ps.isJediMaster != qfalse)
                    || (!(*self_).client.is_null() && (*cl).ps.isJediMaster != qfalse)
                {
                    AddScore(
                        ctx,
                        ctx.entity_id_of(attacker).unwrap(),
                        (*self_).r.currentOrigin,
                        1,
                    );

                    if !(*self_).client.is_null() && (*cl).ps.isJediMaster != qfalse {
                        crate::g_client::ThrowSaberToAttacker(
                            ctx,
                            ctx.entity_id_of(self_).unwrap(),
                            ctx.entity_id_of(attacker),
                        );
                        (*cl).ps.isJediMaster = qfalse;
                    }
                } else {
                    let jmEnt = G_GetJediMaster(ctx);

                    if !jmEnt.is_null() && !(*jmEnt).client.is_null() {
                        AddScore(
                            ctx,
                            ctx.entity_id_of(jmEnt).unwrap(),
                            (*self_).r.currentOrigin,
                            1,
                        );
                    }
                }
            } else {
                AddScore(
                    ctx,
                    ctx.entity_id_of(attacker).unwrap(),
                    (*self_).r.currentOrigin,
                    1,
                );
            }

            if meansOfDeath == meansOfDeath_t::MOD_STUN_BATON as c_int {
                // play humiliation on player
                (*acl).ps.persistant[persEnum_t::PERS_GAUNTLET_FRAG_COUNT as usize] += 1;

                (*acl).rewardTime = ctx.world.level.time + REWARD_SPRITE_TIME;

                // also play humiliation on target
                (*cl).ps.persistant[persEnum_t::PERS_PLAYEREVENTS as usize] ^=
                    PLAYEREVENT_GAUNTLETREWARD;
            }

            // check for two kills in a short amount of time
            if ctx.world.level.time - (*acl).lastKillTime < CARNAGE_REWARD_TIME {
                // play excellent on player
                (*acl).ps.persistant[persEnum_t::PERS_EXCELLENT_COUNT as usize] += 1;

                (*acl).rewardTime = ctx.world.level.time + REWARD_SPRITE_TIME;
            }
            (*acl).lastKillTime = ctx.world.level.time;
        }
    } else if meansOfDeath == meansOfDeath_t::MOD_COLLISION as c_int
        || meansOfDeath == meansOfDeath_t::MOD_VEH_EXPLOSION as c_int
    {
        // no credit for veh-veh collisions?
    } else {
        if !(*self_).client.is_null() && (*cl).ps.isJediMaster != qfalse {
            // killed ourself so return the saber to the original position
            crate::g_client::ThrowSaberToAttacker(ctx, ctx.entity_id_of(self_).unwrap(), None);
            (*cl).ps.isJediMaster = qfalse;
        }

        if ctx.world.cvars.g_gametype.integer == GT_DUEL {
            // in duel, if you kill yourself, the person you are dueling against gets a kill for it
            let mut otherClNum: c_int = -1;
            if ctx.world.level.sortedClients[0] == (*self_).s.number {
                otherClNum = ctx.world.level.sortedClients[1];
            } else if ctx.world.level.sortedClients[1] == (*self_).s.number {
                otherClNum = ctx.world.level.sortedClients[0];
            }

            if otherClNum >= 0
                && otherClNum < MAX_CLIENTS as c_int
                && ctx.world.g_entities[otherClNum as usize].inuse != qfalse
                && !ctx.world.g_entities[otherClNum as usize].client.is_null()
                && otherClNum != (*self_).s.number
            {
                AddScore(
                    ctx,
                    EntityId::from_num(otherClNum).unwrap(),
                    (*self_).r.currentOrigin,
                    1,
                );
            } else {
                AddScore(
                    ctx,
                    ctx.entity_id_of(self_).unwrap(),
                    (*self_).r.currentOrigin,
                    -1,
                );
            }
        } else {
            AddScore(
                ctx,
                ctx.entity_id_of(self_).unwrap(),
                (*self_).r.currentOrigin,
                -1,
            );
        }
    }
}

/// Split from `player_die`.
/// This holds the kill log: killer number/name and obituary resolution (with the §19 NULL-modName pick),
/// the "Kill:" log line, the g_austrian duel-details block, and the weapon kill/death/frag logs.
/// It returns the resolved killer entity number for the obit broadcast.
/// Source: `oracle/codemp/game/g_combat.c:2510-2562`
unsafe fn player_die_log_kill(
    ctx: &mut GameContext,
    self_: *mut gentity_t,
    attacker: *mut gentity_t,
    meansOfDeath: c_int,
) -> c_int {
    let cl = (*self_).client;
    let mut killer: c_int;
    let mut killer_name: String;
    if !attacker.is_null() {
        killer = (*attacker).s.number;
        if !(*attacker).client.is_null() {
            killer_name = (*((*attacker).client)).pers.netname.clone();
        } else {
            killer_name = "<non-client>".to_string();
        }
    } else {
        killer = ENTITYNUM_WORLD;
        killer_name = "<world>".to_string();
    }

    if killer < 0 || killer >= MAX_CLIENTS as c_int {
        killer = ENTITYNUM_WORLD;
        killer_name = "<world>".to_string();
    }

    let obit: String;
    if meansOfDeath < 0 || meansOfDeath as usize >= modNames.len() {
        obit = "<bad obituary>".to_string();
    } else if modNames[meansOfDeath as usize].is_null() {
        // C's `G_LogPrintf("%s", NULL)` for the three trailing NULL modNames slots (42-44) is undefined behavior (§19).
        // glibc renders NULL as "(null)".
        obit = "(null)".to_string();
    } else {
        obit = cstr_to_str(modNames[meansOfDeath as usize]);
    }

    let s = format!(
        "Kill: {} {} {}: {} killed {} by {}\n",
        killer,
        (*self_).s.number,
        meansOfDeath,
        killer_name,
        (*cl).pers.netname.clone(),
        obit
    );
    crate::g_main::G_LogPrintf(ctx, &s);

    if ctx.world.cvars.g_austrian.integer != 0
        && ctx.world.cvars.g_gametype.integer == GT_DUEL
        && ctx.world.level.numPlayingClients >= 2
    {
        let spawnTime = if (*ctx
            .world
            .level
            .clients
            .add(ctx.world.level.sortedClients[0] as usize))
        .respawnTime
            > (*ctx
                .world
                .level
                .clients
                .add(ctx.world.level.sortedClients[1] as usize))
            .respawnTime
        {
            (*ctx
                .world
                .level
                .clients
                .add(ctx.world.level.sortedClients[0] as usize))
            .respawnTime
        } else {
            (*ctx
                .world
                .level
                .clients
                .add(ctx.world.level.sortedClients[1] as usize))
            .respawnTime
        };
        crate::g_main::G_LogPrintf(ctx, "Duel Kill Details:\n");
        let s = format!("Kill Time: {}\n", ctx.world.level.time - spawnTime);
        crate::g_main::G_LogPrintf(ctx, &s);
        let s = format!(
            "victim: {}, hits on enemy {}\n",
            (*cl).pers.netname.clone(),
            (*cl).ps.persistant[persEnum_t::PERS_HITS as usize]
        );
        crate::g_main::G_LogPrintf(ctx, &s);
        if !attacker.is_null() && !(*attacker).client.is_null() {
            let acl = (*attacker).client;
            let s = format!(
                "killer: {}, hits on enemy {}, health: {}\n",
                (*acl).pers.netname.clone(),
                (*acl).ps.persistant[persEnum_t::PERS_HITS as usize],
                (*attacker).health
            );
            crate::g_main::G_LogPrintf(ctx, &s);
            // also - if MOD_SABER, list the animation and saber style
            if meansOfDeath == meansOfDeath_t::MOD_SABER as c_int {
                let s = format!(
                    "killer saber style: {}, killer saber anim {}\n",
                    (*acl).ps.fd.saberAnimLevel,
                    cstr_to_str(animTable[(*acl).ps.torsoAnim as usize].name)
                );
                crate::g_main::G_LogPrintf(ctx, &s);
            }
        }
    }

    crate::g_log::G_LogWeaponKill(ctx, killer, meansOfDeath);
    crate::g_log::G_LogWeaponDeath(ctx, (*self_).s.number, (*self_).s.weapon);
    if !attacker.is_null() && !(*attacker).client.is_null() && (*attacker).inuse != qfalse {
        crate::g_log::G_LogWeaponFrag(ctx, killer, (*self_).s.number);
    }

    killer
}

/// Split from `player_die`.
/// This holds the normal-death animation phase: draw the death anim (world-owned `death_anim_i` rotation plus `G_PickDeathAnim`),
/// play it with pm_type shielded, saber/heavy-melee dismemberment check, the EV_DEATH1+i event (Jedi Master flagged),
/// the death alert, and the corpse's body_die/takedamage arming.
/// Source: `oracle/codemp/game/g_combat.c:2845-2928`
unsafe fn player_die_death_anim(
    ctx: &mut GameContext,
    self_: *mut gentity_t,
    attacker: *mut gentity_t,
    damage: c_int,
    meansOfDeath: c_int,
    wasJediMaster: qboolean,
) {
    let cl = (*self_).client;
    let mut sPMType: c_int;
    // NOTENOTE No gib deaths right now, this is star wars.
    {
        // normal death
        // Raven's function-`static int i` rotates EV_DEATH1 0..2 across deaths.
        // This value is world-owned per §B3 (`game_globals.rs`).
        let mut i_val = ctx.world.globals.death_anim_i;

        // Referee probe capture: holdrand before the death-anim draw.
        let probe_rin = ctx.world.bg_state.rng.dbg_holdrand();
        let anim = G_PickDeathAnim(
            ctx,
            ctx.entity_id_of(self_),
            (*self_).pos1,
            damage,
            meansOfDeath,
            HL_NONE,
        );
        // Referee probe: G_PickDeathAnim inputs (pos, dmg, mod) + RNG in/out.
        probe!(
            "DEATH_ANIM",
            "t={} en={} dmg={} mod={} p={:08x},{:08x},{:08x} rin={:08x} rout={:08x} anim={}",
            ctx.world.level.time,
            (*self_).s.number,
            damage,
            meansOfDeath,
            (*self_).pos1[0].to_bits(),
            (*self_).pos1[1].to_bits(),
            (*self_).pos1[2].to_bits(),
            probe_rin,
            ctx.world.bg_state.rng.dbg_holdrand(),
            anim,
        );

        if anim >= 1 {
            // Some droids don't have death anims
            if (*self_).health <= GIB_HEALTH {
                (*self_).health = GIB_HEALTH + 1;
            }

            (*cl).respawnTime = ctx.world.level.time + 1000;

            sPMType = (*cl).ps.pm_type;
            (*cl).ps.pm_type = pmtype_t::PM_NORMAL as c_int; // don't want pm type interfering with our setanim calls.

            if (*self_).inuse != qfalse {
                // not disconnecting
                crate::g_utils::G_SetAnim(
                    ctx,
                    ctx.entity_id_of(self_).unwrap(),
                    core::ptr::null_mut(),
                    SETANIM_BOTH,
                    anim,
                    SETANIM_FLAG_OVERRIDE | SETANIM_FLAG_HOLD | SETANIM_FLAG_RESTART,
                    0,
                );
            }

            (*cl).ps.pm_type = sPMType;

            if meansOfDeath == meansOfDeath_t::MOD_SABER as c_int
                || (meansOfDeath == meansOfDeath_t::MOD_MELEE as c_int
                    && G_HeavyMelee(ctx, ctx.entity_id_of(attacker)) != qfalse)
            {
                // saber or heavy melee (claws)
                crate::g_client::G_UpdateClientAnims(ctx, ctx.entity_id_of(self_).unwrap(), 1.0);
                G_CheckForDismemberment(
                    ctx,
                    ctx.entity_id_of(self_).unwrap(),
                    ctx.entity_id_of(attacker),
                    (*self_).pos1,
                    damage,
                    anim,
                    qfalse,
                );
            }
        } else if !(*self_).NPC.is_null()
            && !(*self_).client.is_null()
            && (*cl).NPC_class != class_t::CLASS_MARK1
            && (*cl).NPC_class != class_t::CLASS_VEHICLE
        {
            // in this case if we're an NPC it's my guess that we want to get removed straight away.
            (*self_).think = Some(EntThink::G_FreeEntity).into();
            (*self_).nextthink = ctx.world.level.time;
        }

        if wasJediMaster != qfalse {
            G_AddEvent(&mut *(self_), entity_event_t::EV_DEATH1 as c_int + i_val, 1);
        } else {
            G_AddEvent(&mut *(self_), entity_event_t::EV_DEATH1 as c_int + i_val, 0);
        }

        if self_ != attacker {
            // don't make NPCs want to murder you on respawn for killing yourself!
            G_DeathAlert(
                ctx,
                ctx.entity_id_of(self_).unwrap(),
                ctx.entity_id_of(attacker),
            );
        }

        // the body can still be gibbed
        if (*self_).NPC.is_null() {
            // don't remove NPCs like this!
            (*self_).die = Some(EntDie::body_die).into();
        }

        // It won't gib, it will disintegrate (because this is Star Wars).
        (*self_).takedamage = qtrue;

        // globally cycle through the different death animations
        i_val = (i_val + 1) % 3;
        ctx.world.globals.death_anim_i = i_val;
    }
}

/// Split from `player_die`.
/// This holds post-obit scoring: team frag bonuses, the suicide/nodrop CTF flag returns, item drops (non-NPC, outside nodrop),
/// the MOD_TEAM_CHANGE score refund versus score display, and the following-spectator score refresh.
/// Source: `oracle/codemp/game/g_combat.c:2745-2810`
unsafe fn player_die_scoring_and_drops(
    ctx: &mut GameContext,
    self_: *mut gentity_t,
    inflictor: *mut gentity_t,
    attacker: *mut gentity_t,
    meansOfDeath: c_int,
) {
    let cl = (*self_).client;
    // Add team bonuses
    crate::g_team::Team_FragBonuses(
        ctx,
        ctx.entity_id_of(self_).unwrap(),
        ctx.entity_id_of(inflictor),
        ctx.entity_id_of(attacker),
    );

    // if I committed suicide, the flag does not fall, it returns.
    if meansOfDeath == meansOfDeath_t::MOD_SUICIDE as c_int {
        if (*cl).ps.powerups[PW_NEUTRALFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_FREE as c_int);
            (*cl).ps.powerups[PW_NEUTRALFLAG as usize] = 0;
        } else if (*cl).ps.powerups[PW_REDFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_RED as c_int);
            (*cl).ps.powerups[PW_REDFLAG as usize] = 0;
        } else if (*cl).ps.powerups[PW_BLUEFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_BLUE as c_int);
            (*cl).ps.powerups[PW_BLUEFLAG as usize] = 0;
        }
    }

    // if client is in a nodrop area, don't drop anything (but return CTF flags!)
    let contents = trap::PointContents(
        ctx.engine,
        mp_abi::game::syscalls::G_POINTCONTENTS::GPointcontentsArgs::new(
            &(*self_).r.currentOrigin as *const vec3_t,
            -1,
        ),
    );
    if (contents & CONTENTS_NODROP) == 0 && (*cl).ps.fallingToDeath == qfalse {
        if (*self_).s.eType != entityType_t::ET_NPC as c_int {
            TossClientItems(ctx, ctx.entity_id_of(self_).unwrap());
        }
    } else {
        if (*cl).ps.powerups[PW_NEUTRALFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_FREE as c_int);
        } else if (*cl).ps.powerups[PW_REDFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_RED as c_int);
        } else if (*cl).ps.powerups[PW_BLUEFLAG as usize] != 0 {
            crate::g_team::Team_ReturnFlag(ctx, TEAM_BLUE as c_int);
        }
    }

    if meansOfDeath_t::MOD_TEAM_CHANGE as c_int == meansOfDeath {
        // Give them back a point since they didn't really die.
        AddScore(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            (*self_).r.currentOrigin,
            1,
        );
    } else {
        crate::g_cmds::Cmd_Score_f(ctx, ctx.entity_id_of(self_).unwrap()); // show scores
    }

    // send updated scores to any clients that are following this one,
    // or they would get stale scoreboards
    for i in 0..ctx.world.level.maxclients {
        let client = ctx.world.level.clients.add(i as usize);
        if (*client).pers.connected != CON_CONNECTED {
            continue;
        }
        if (*client).sess.sessionTeam != TEAM_SPECTATOR {
            continue;
        }
        if (*client).sess.spectatorClient == (*self_).s.number {
            crate::g_cmds::Cmd_Score_f(ctx, EntityId(i as u32));
        }
    }
}

/// Split from `player_die`.
/// This holds the vehicle-explosion cascade: when a no-delay vehicle dies with riders, credit a murderer (last attacker, attacking
/// vehicle's pilot, or own pilot), kill the hidden pilot, eject and kill the passengers, then kill the droid unit.
/// It is self-contained. The temp-inflictor fake-up is freed and every local resets before returning.
/// Source: `oracle/codemp/game/g_combat.c:2155-2289`
unsafe fn player_die_vehicle_cascade(
    ctx: &mut GameContext,
    self_: *mut gentity_t,
    inflictor: *mut gentity_t,
    attacker: *mut gentity_t,
    meansOfDeath: c_int,
) {
    let cl = (*self_).client;
    let mut tempInflictorEnt: *mut gentity_t = inflictor;
    let mut tempInflictor: qboolean = qfalse;
    let mut actualMOD = meansOfDeath;
    let selfVeh = (*self_).m_pVehicle;
    if (*self_).s.eType == entityType_t::ET_NPC as c_int
        && (*self_).s.NPC_class == class_t::CLASS_VEHICLE as c_int
        && !(*self_).m_pVehicle.is_null()
        && (*(*selfVeh).m_pVehicleInfo).explosionDelay == 0
        && (!(*selfVeh).m_pPilot.is_null()
            || (*selfVeh).m_iNumPassengers > 0
            || !(*selfVeh).m_pDroidUnit.is_null())
    {
        // kill everyone on board in the name of the attacker... if the vehicle has no death delay
        let mut murderer: *mut gentity_t = core::ptr::null_mut();
        let mut killEnt: *mut gentity_t;
        let mut i = 0;

        if (*cl).ps.otherKillerTime >= ctx.world.level.time {
            // use the last attacker
            murderer = &mut ctx.world.g_entities[(*cl).ps.otherKiller as usize] as *mut gentity_t;
            if (*murderer).inuse == qfalse || (*murderer).client.is_null() {
                murderer = core::ptr::null_mut();
            } else {
                let murdererVeh = (*murderer).m_pVehicle;
                if (*murderer).s.number >= MAX_CLIENTS as c_int
                    && (*murderer).s.eType == entityType_t::ET_NPC as c_int
                    && (*murderer).s.NPC_class == class_t::CLASS_VEHICLE as c_int
                    && !(*murderer).m_pVehicle.is_null()
                    && !(*murdererVeh).m_pPilot.is_null()
                {
                    let murderPilot = &mut ctx.world.g_entities
                        [(*((*murdererVeh).m_pPilot as *mut gentity_t)).s.number as usize]
                        as *mut gentity_t;
                    if (*murderPilot).inuse != qfalse && !(*murderPilot).client.is_null() {
                        // give the pilot of the offending vehicle credit for the kill
                        murderer = murderPilot;
                        actualMOD = (*cl).otherKillerMOD;
                        if (*cl).otherKillerVehWeapon > 0 {
                            let __teid14 = crate::g_utils::G_Spawn(ctx);
                            tempInflictorEnt = ctx.entity_mut(__teid14);
                            if !tempInflictorEnt.is_null() {
                                // fake up the inflictor
                                tempInflictor = qtrue;
                                ctx.ent_set(__teid14, PrefixSet::ClassnameStatic(c"vehicle_proj"));
                                (*tempInflictorEnt).s.otherEntityNum2 =
                                    (*cl).otherKillerVehWeapon - 1;
                                (*tempInflictorEnt).s.weapon = (*cl).otherKillerWeaponType;
                            }
                        }
                    }
                }
            }
        } else if !attacker.is_null()
            && (*attacker).inuse != qfalse
            && !(*attacker).client.is_null()
        {
            let attackerVeh = (*attacker).m_pVehicle;
            if (*attacker).s.number >= MAX_CLIENTS as c_int
                && (*attacker).s.eType == entityType_t::ET_NPC as c_int
                && (*attacker).s.NPC_class == class_t::CLASS_VEHICLE as c_int
                && !(*attacker).m_pVehicle.is_null()
                && !(*attackerVeh).m_pPilot.is_null()
            {
                // set vehicles pilot's killer as murderer
                murderer = &mut ctx.world.g_entities
                    [(*((*attackerVeh).m_pPilot as *mut gentity_t)).s.number as usize]
                    as *mut gentity_t;
                if (*murderer).inuse != qfalse
                    && !(*murderer).client.is_null()
                    && (*((*murderer).client)).ps.otherKillerTime >= ctx.world.level.time
                {
                    murderer = &mut ctx.world.g_entities
                        [(*((*murderer).client)).ps.otherKiller as usize]
                        as *mut gentity_t;
                    if (*murderer).inuse == qfalse || (*murderer).client.is_null() {
                        murderer = core::ptr::null_mut();
                    }
                } else {
                    murderer = core::ptr::null_mut();
                }
            } else {
                murderer =
                    &mut ctx.world.g_entities[(*attacker).s.number as usize] as *mut gentity_t;
            }
        } else if !(*selfVeh).m_pPilot.is_null() {
            murderer = (*selfVeh).m_pPilot as *mut gentity_t;
            if (*murderer).inuse == qfalse || (*murderer).client.is_null() {
                murderer = core::ptr::null_mut();
            }
        }

        // no valid murderer.. just use self I guess
        if murderer.is_null() {
            if attacker.is_null() {
                murderer = self_;
            } else {
                murderer = attacker;
            }
        }

        if (*(*selfVeh).m_pVehicleInfo).hideRider != qfalse {
            // pilot is *inside* me, so kill him, too
            killEnt = (*selfVeh).m_pPilot as *mut gentity_t;
            if !killEnt.is_null() && (*killEnt).inuse != qfalse && !(*killEnt).client.is_null() {
                G_Damage(
                    ctx,
                    ctx.entity_id_of(killEnt),
                    ctx.entity_id_of(tempInflictorEnt),
                    ctx.entity_id_of(murderer),
                    None,
                    (*((*killEnt).client)).ps.origin,
                    99999,
                    DAMAGE_NO_PROTECTION,
                    actualMOD,
                );
            }
            if !(*selfVeh).m_pVehicleInfo.is_null() {
                let numPass = (*selfVeh).m_iNumPassengers;
                i = 0;
                while i < numPass && (*selfVeh).m_iNumPassengers != 0 {
                    // go through and eject the last passenger
                    killEnt = (*selfVeh).m_ppPassengers[((*selfVeh).m_iNumPassengers - 1) as usize]
                        as *mut gentity_t;
                    if !killEnt.is_null() {
                        crate::veh_dispatch::eject(ctx, selfVeh, killEnt as *mut bgEntity_t, qtrue);
                        if (*killEnt).inuse != qfalse && !(*killEnt).client.is_null() {
                            G_Damage(
                                ctx,
                                ctx.entity_id_of(killEnt),
                                ctx.entity_id_of(tempInflictorEnt),
                                ctx.entity_id_of(murderer),
                                None,
                                (*((*killEnt).client)).ps.origin,
                                99999,
                                DAMAGE_NO_PROTECTION,
                                actualMOD,
                            );
                        }
                    }
                    i += 1;
                }
            }
        }
        killEnt = (*selfVeh).m_pDroidUnit as *mut gentity_t;
        if !killEnt.is_null() && (*killEnt).inuse != qfalse && !(*killEnt).client.is_null() {
            (*killEnt).flags &= !FL_UNDYING;
            G_Damage(
                ctx,
                ctx.entity_id_of(killEnt),
                ctx.entity_id_of(tempInflictorEnt),
                ctx.entity_id_of(murderer),
                None,
                (*((*killEnt).client)).ps.origin,
                99999,
                DAMAGE_NO_PROTECTION,
                actualMOD,
            );
        }
        if tempInflictor != qfalse {
            crate::g_utils::G_FreeEntity(ctx, ctx.entity_id_of(tempInflictorEnt));
        }
        tempInflictorEnt = inflictor;
        tempInflictor = qfalse;
        actualMOD = meansOfDeath;
    }
}

/// Raven `player_die`.
///
/// Source: `oracle/codemp/game/g_combat.c:2123-3011`
pub fn player_die(
    ctx: &mut GameContext,
    self_: EntityId,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    damage: c_int,
    meansOfDeath: c_int,
) {
    // The EntityId/Option params carry body null-checks for inflictor/attacker.
    // The raw pointers re-derive verbatim, matching Raven's large mega-function shape.
    let self_: *mut gentity_t = ctx.entity_mut(self_);
    let inflictor: *mut gentity_t =
        unsafe { crate::ent_id::resolve(ctx.world.g_entities.as_mut_ptr(), inflictor) };
    let mut attacker: *mut gentity_t =
        unsafe { crate::ent_id::resolve(ctx.world.g_entities.as_mut_ptr(), attacker) };
    unsafe {
        let base = ctx.world.g_entities.as_mut_ptr();
        let cl = (*self_).client;
        let mut wasJediMaster: qboolean = qfalse;
        let mut wasInVehicle: c_int = 0;
        let mut tempInflictorEnt: *mut gentity_t = inflictor;
        let mut tempInflictor: qboolean = qfalse;
        let mut actualMOD = meansOfDeath;

        if (*cl).ps.pm_type == pmtype_t::PM_DEAD as c_int {
            return;
        }

        if ctx.world.level.intermissiontime != 0 {
            return;
        }

        // check player stuff
        ctx.world.globals.g_dontFrickinCheck = qfalse;

        if ctx.world.cvars.g_gametype.integer == GT_POWERDUEL {
            // don't want to wait til later in the frame if this is the case
            crate::g_main::CheckExitRules(ctx);

            if ctx.world.level.intermissiontime != 0 {
                return;
            }
        }

        player_die_vehicle_cascade(ctx, self_, inflictor, attacker, meansOfDeath);

        (*cl).ps.emplacedIndex = 0;

        crate::g_client::G_BreakArm(ctx, ctx.entity_id_of(self_).unwrap(), 0); // unbreak anything we have broken
        (*cl).ps.saberEntityNum = (*cl).saberStoredIndex; // in case we died while our saber was knocked away.

        (*cl).bodyGrabIndex = ENTITYNUM_NONE;
        (*cl).bodyGrabTime = 0;

        if (*cl).holdingObjectiveItem > 0 {
            // carrying a siege objective item - make sure it updates and removes itself from us now
            // in case this is an instant death-respawn situation
            let objectiveItem =
                &mut ctx.world.g_entities[(*cl).holdingObjectiveItem as usize] as *mut gentity_t;

            if (*objectiveItem).inuse != qfalse {
                if let Some(think_fn) = (*objectiveItem).think.get() {
                    crate::ent_fn_enums::dispatch_think(ctx, think_fn, objectiveItem);
                }
            }
        }

        if ((*cl).inSpaceIndex != 0
            && (*cl).inSpaceIndex != mp_qshared::shared::limits::ENTITYNUM_NONE)
            || ((*cl).ps.eFlags2 & EF2_SHIP_DEATH) != 0
        {
            (*cl).noCorpse = qtrue;
        }

        if (*cl).NPC_class != class_t::CLASS_VEHICLE && (*cl).ps.m_iVehicleNum != 0 {
            // I'm riding a vehicle
            // tell it I'm getting off
            let veh = &mut ctx.world.g_entities[(*cl).ps.m_iVehicleNum as usize] as *mut gentity_t;
            let vehVeh = (*veh).m_pVehicle;

            if (*veh).inuse != qfalse && !(*veh).client.is_null() && !(*veh).m_pVehicle.is_null() {
                // remember it for obit
                wasInVehicle = (*vehVeh)
                    .m_pVehicleInfo
                    .offset_from(ctx.world.bg_state.g_vehicleInfo.as_ptr())
                    as c_int;
                crate::veh_dispatch::eject(ctx, vehVeh, self_ as *mut bgEntity_t, qtrue);

                if (*(*vehVeh).m_pVehicleInfo).r#type == VH_FIGHTER {
                    // go into "die in ship" mode with flag
                    (*cl).ps.eFlags2 |= EF2_SHIP_DEATH;

                    // put me over where my vehicle exploded
                    crate::g_utils::G_SetOrigin(&mut *(self_), (*((*veh).client)).ps.origin);
                    _VectorCopy((*((*veh).client)).ps.origin, &mut (*cl).ps.origin);
                }
            }
            // droids throw heads if they haven't yet
            if (*cl).NPC_class == class_t::CLASS_R2D2 {
                if trap::G2API_GetSurfaceRenderStatus(ctx.engine, (*self_).ghoul2, 0, "head") == 0
                {
                    let mut up: vec3_t = [0.0; 3];
                    AngleVectors((*self_).r.currentAngles, None, None, Some(&mut up));
                    crate::g_utils::G_PlayEffectID(
                        G_EffectIndex(ctx, "chunks/r2d2head_veh"),
                        (*self_).r.currentOrigin,
                        up,
                    );
                }
            } else if (*cl).NPC_class == class_t::CLASS_R5D2 {
                if trap::G2API_GetSurfaceRenderStatus(ctx.engine, (*self_).ghoul2, 0, "head") == 0
                {
                    let mut up: vec3_t = [0.0; 3];
                    AngleVectors((*self_).r.currentAngles, None, None, Some(&mut up));
                    crate::g_utils::G_PlayEffectID(
                        G_EffectIndex(ctx, "chunks/r5d2head_veh"),
                        (*self_).r.currentOrigin,
                        up,
                    );
                }
            }
        }

        if !(*self_).NPC.is_null() {
            let npc = (*self_).NPC;
            if !(*self_).client.is_null()
                && crate::NPC_AI_Jedi::Jedi_WaitingAmbush(&*self_) != qfalse
            {
                // ambushing trooper
                (*cl).noclip = qfalse;
            }
            crate::NPC_combat::NPC_FreeCombatPoint(ctx, (*npc).combatPoint, qfalse);
            if !(*npc).group.is_null() {
                crate::NPC_AI_Utils::AI_GroupMemberKilled(ctx, ctx.entity_id_of(self_));
                crate::NPC_AI_Utils::AI_DeleteSelfFromGroup(ctx, ctx.entity_id_of(self_).unwrap());
            }

            if (*npc).tempGoal.is_some() {
                let tg = (*npc).tempGoal.unwrap();
                crate::g_utils::G_FreeEntity(ctx, Some(tg));
                (*npc).tempGoal = None;
            }
            if false {
                crate::NPC_AI_Jedi::Boba_FlyStop(ctx, ctx.entity_id_of(self_).unwrap());
            }
            if (*self_).s.NPC_class == class_t::CLASS_RANCOR as c_int {
                crate::NPC_AI_Rancor::Rancor_DropVictim(ctx, ctx.entity_id_of(self_).unwrap());
            }
        }
        if !attacker.is_null() && !(*attacker).NPC.is_null() {
            let anpc = (*attacker).NPC;
            if !(*anpc).group.is_null() && (*(*anpc).group).enemy == self_ {
                (*(*anpc).group).enemy = core::ptr::null_mut();
            }
        }

        // Cheap method until/if I decide to put fancier stuff in (e.g. sabers falling out of hand and slowly
        // holstering on death like sp)
        if (*cl).ps.weapon == WP_SABER as c_int
            && (*cl).ps.saberHolstered == 0
            && (*cl).ps.saberEntityNum != 0
        {
            if (*cl).ps.saberInFlight == qfalse && (*cl).saber[0].soundOff != 0 {
                crate::g_utils::G_Sound(
                    ctx,
                    ctx.entity_id_of(self_),
                    CHAN_AUTO,
                    (*cl).saber[0].soundOff,
                );
            }
            if (*cl).saber[1].soundOff != 0 && (*cl).saber[1].model[0] != 0 {
                crate::g_utils::G_Sound(
                    ctx,
                    ctx.entity_id_of(self_),
                    CHAN_AUTO,
                    (*cl).saber[1].soundOff,
                );
            }
        }

        // Use any target we had
        G_UseTargets(ctx, ctx.entity_id_of(self_), ctx.entity_id_of(self_));

        if ctx.world.cvars.g_slowmoDuelEnd.integer != 0
            && (ctx.world.cvars.g_gametype.integer == GT_DUEL
                || ctx.world.cvars.g_gametype.integer == GT_POWERDUEL)
            && !attacker.is_null()
            && (*attacker).inuse != qfalse
            && !(*attacker).client.is_null()
        {
            if ctx.world.globals.gDoSlowMoDuel == qfalse {
                ctx.world.globals.gDoSlowMoDuel = qtrue;
                ctx.world.globals.gSlowMoDuelTime = ctx.world.level.time;
            }
        }

        // Make sure the jetpack is turned off.
        crate::g_items::Jetpack_Off(&mut *self_);

        (*cl).ps.heldByClient = 0;
        (*cl).beingThrown = 0;
        (*cl).doingThrow = 0;
        mp_bg::bg_pmove::BG_ClearRocketLock(&mut (*cl).ps as *mut playerState_t);
        (*cl).isHacking = 0;
        (*cl).ps.hackingTime = 0;

        if !inflictor.is_null()
            && (*inflictor).activator.is_some()
            && (*inflictor).client.is_null()
            && (*attacker).client.is_null()
            && (*inflictor).s.weapon == WP_TURRET as c_int
        {
            let act = &mut ctx.world.g_entities[(*inflictor).activator.unwrap().0 as usize]
                as *mut gentity_t;
            if !(*act).client.is_null() && (*act).inuse != qfalse {
                attacker = act;
            }
        }

        if !(*self_).client.is_null() && (*cl).ps.isJediMaster != qfalse {
            wasJediMaster = qtrue;
        }

        // if he was charging or anything else, kill the sound
        crate::g_utils::G_MuteSound(ctx, (*self_).s.number, CHAN_WEAPON);

        crate::g_weapon::BlowDetpacks(ctx, ctx.entity_id_of(self_).unwrap()); // blow detpacks if they're planted

        (*cl).ps.fd.forceDeactivateAll = 1;

        if (self_ == attacker || (*attacker).client.is_null())
            && (meansOfDeath == meansOfDeath_t::MOD_CRUSH as c_int
                || meansOfDeath == meansOfDeath_t::MOD_FALLING as c_int
                || meansOfDeath == meansOfDeath_t::MOD_TRIGGER_HURT as c_int
                || meansOfDeath == meansOfDeath_t::MOD_UNKNOWN as c_int
                || meansOfDeath == meansOfDeath_t::MOD_SUICIDE as c_int)
            && (*cl).ps.otherKillerTime > ctx.world.level.time
        {
            // remember who last attacked us
            attacker = &mut ctx.world.g_entities[(*cl).ps.otherKiller as usize] as *mut gentity_t;
            if (*cl).otherKillerMOD != meansOfDeath_t::MOD_UNKNOWN as c_int {
                actualMOD = (*cl).otherKillerMOD;
            }
            if (*cl).otherKillerVehWeapon > 0 {
                let __teid15 = crate::g_utils::G_Spawn(ctx);
                tempInflictorEnt = ctx.entity_mut(__teid15);
                if !tempInflictorEnt.is_null() {
                    tempInflictor = qtrue;
                    ctx.ent_set(__teid15, PrefixSet::ClassnameStatic(c"vehicle_proj"));
                    (*tempInflictorEnt).s.otherEntityNum2 = (*cl).otherKillerVehWeapon - 1;
                    (*tempInflictorEnt).s.weapon = (*cl).otherKillerWeaponType;
                }
            }
        }

        // check for an almost capture
        CheckAlmostCapture(&*self_, attacker.as_ref());

        (*cl).ps.pm_type = pmtype_t::PM_DEAD as c_int;
        (*cl).ps.pm_flags &= !PMF_STUCK_TO_WALL;

        let killer = player_die_log_kill(ctx, self_, attacker, meansOfDeath);

        G_BroadcastObit(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(inflictor),
            ctx.entity_id_of(attacker),
            killer,
            actualMOD,
            wasInVehicle,
            wasJediMaster,
        );
        if tempInflictor != qfalse {
            crate::g_utils::G_FreeEntity(ctx, ctx.entity_id_of(tempInflictorEnt));
        }

        (*self_).enemy = Some(ent_id(base, attacker));

        player_die_kill_credit(ctx, self_, attacker, meansOfDeath);

        player_die_scoring_and_drops(ctx, self_, inflictor, attacker, meansOfDeath);

        (*self_).takedamage = qtrue; // can still be gibbed

        (*self_).s.weapon = WP_NONE as c_int;
        (*self_).s.powerups = 0;
        if (*self_).s.eType != entityType_t::ET_NPC as c_int {
            // handled differently for NPCs
            (*self_).r.contents = CONTENTS_CORPSE;
        }
        (*cl).ps.zoomMode = 0; // Turn off zooming when we die

        (*self_).s.loopSound = 0;
        (*self_).s.loopIsSoundset = qfalse;

        if (*self_).s.eType != entityType_t::ET_NPC as c_int {
            // handled differently for NPCs
            (*self_).r.maxs[2] = -8.0;
        }

        // don't allow respawn until the death anim is done
        (*cl).respawnTime = ctx.world.level.time + 1700;

        // remove powerups
        (*cl).ps.powerups.fill(0);

        player_die_death_anim(ctx, self_, attacker, damage, meansOfDeath, wasJediMaster);

        if !(*self_).NPC.is_null() {
            // If an NPC, make sure we start running our scripts again- this gets set to infinite while we fall to our deaths
            (*((*self_).NPC)).nextBStateThink = ctx.world.level.time;
        }

        if G_ActivateBehavior(ctx, ctx.entity_id_of(self_), bSet_t::BSET_DEATH as c_int) != qfalse {
            //deathScript = qtrue;
        }

        if !(*self_).NPC.is_null() && ((*((*self_).NPC)).scriptFlags & SCF_FFDEATH) != 0 {
            if G_ActivateBehavior(ctx, ctx.entity_id_of(self_), bSet_t::BSET_FFDEATH as c_int)
                != qfalse
            {
                //deathScript = qtrue;
            }
            let target4 = (*self_).target4.clone();
            let target4 = if target4.is_empty() { None } else { Some(target4) };
            crate::g_utils::G_UseTargets2(
                ctx,
                ctx.entity_id_of(self_),
                ctx.entity_id_of(self_),
                target4.as_deref(),
            );
        }

        // Free up any timers we may have on us.
        crate::g_timer::TIMER_Clear2(ctx, ctx.entity_id_of(self_));

        trap::LinkEntity(
            ctx.engine,
            mp_abi::game::syscalls::G_LINKENTITY::GLinkentityArgs::new(self_.cast()),
        );

        if !(*self_).NPC.is_null() {
            (*((*self_).NPC)).timeOfDeath = ctx.world.level.time;
        }

        // Start any necessary death fx for this entity
        DeathFX(ctx, ctx.entity_id_of(self_));

        if ctx.world.cvars.g_gametype.integer == GT_POWERDUEL
            && ctx.world.globals.g_noPDuelCheck == 0
        {
            // powerduel checks
            if (*cl).sess.duelTeam == DUELTEAM_LONE as c_int {
                // automatically means a win as there is only one
                G_AddPowerDuelScore(ctx, DUELTEAM_DOUBLE as c_int, 1);
                G_AddPowerDuelLoserScore(ctx, DUELTEAM_LONE as c_int, 1);
                ctx.world.globals.g_endPDuel = qtrue;
            } else if (*cl).sess.duelTeam == DUELTEAM_DOUBLE as c_int {
                let mut i = 0;
                let mut heLives: qboolean = qfalse;

                while i < MAX_CLIENTS as c_int {
                    let check = &mut ctx.world.g_entities[i as usize] as *mut gentity_t;
                    let checkClient = (*check).client;
                    if (*check).inuse != qfalse
                        && !checkClient.is_null()
                        && (*check).s.number != (*self_).s.number
                        && (*checkClient).pers.connected == CON_CONNECTED
                        && (*checkClient).iAmALoser == qfalse
                        && (*checkClient).ps.stats[statIndex_t::STAT_HEALTH as usize] > 0
                        && (*checkClient).sess.sessionTeam != TEAM_SPECTATOR
                        && (*checkClient).sess.duelTeam == DUELTEAM_DOUBLE as c_int
                    {
                        // still an active living paired duelist so it's not over yet.
                        heLives = qtrue;
                        break;
                    }
                    i += 1;
                }

                if heLives == qfalse {
                    // they're all dead, give the lone duelist the win.
                    G_AddPowerDuelScore(ctx, DUELTEAM_LONE as c_int, 1);
                    G_AddPowerDuelLoserScore(ctx, DUELTEAM_DOUBLE as c_int, 1);
                    ctx.world.globals.g_endPDuel = qtrue;
                }
            }
        }
    }
}

/// Raven `CheckArmor`.
///
/// Source: `oracle/codemp/game/g_combat.c:3019-3071`
pub fn CheckArmor(ctx: &mut GameContext, ent: EntityId, damage: c_int, dflags: c_int) -> c_int {
    if damage == 0 {
        return 0;
    }

    // ent may be an NPC or a vehicle. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(ent).client;

    if client.is_null() {
        return 0;
    }

    if (dflags & DAMAGE_NO_ARMOR) != 0 {
        return 0;
    }

    if unsafe { (*client).NPC_class } == class_t::CLASS_VEHICLE
        && !ctx.entity(ent).m_pVehicle.is_null()
        && unsafe { (*client).ps.electrifyTime } > ctx.world.level.time
    {
        // ion-cannon has disabled this ship's shields, take damage on hull!
        return 0;
    }
    // armor
    let count = unsafe { (*client).ps.stats[statIndex_t::STAT_ARMOR as usize] };

    let mut save;
    if (dflags & DAMAGE_HALF_ABSORB) != 0 {
        // Half the damage gets absorbed by the shields, rather than 100%
        save = (damage as f32 * ARMOR_PROTECTION).ceil() as c_int;
    } else {
        // All the damage gets absorbed by the shields.
        save = damage;
    }

    // save is the most damage that the armor is elibigle to protect, of course, but it's limited by the total armor.
    if save >= count {
        save = count;
    }

    if save == 0 {
        return 0;
    }

    unsafe {
        if (dflags & DAMAGE_HALF_ARMOR_REDUCTION) != 0 {
            // Armor isn't whittled so easily by sniper shots.
            (*client).ps.stats[statIndex_t::STAT_ARMOR as usize] -=
                (save as f32 * ARMOR_REDUCTION_FACTOR) as c_int;
        } else {
            (*client).ps.stats[statIndex_t::STAT_ARMOR as usize] -= save;
        }
    }

    save
}

/// Raven `G_ApplyKnockback`.
///
/// `newDir` is read-only (`VectorScale` reads it into `kvel`), so it stays by-value.
/// Source: `oracle/codemp/game/g_combat.c:3074-3121`
pub fn G_ApplyKnockback(ctx: &mut GameContext, targ: EntityId, newDir: vec3_t, knockback: f32) {
    let mut kvel: vec3_t = [0.0; 3];

    let mass: f32 = if ctx.entity(targ).physicsBounce > 0.0 {
        ctx.entity(targ).physicsBounce // override the mass
    } else {
        200.0
    };

    let g_knockback = ctx.world.cvars.g_knockback.value;
    if ctx.world.cvars.g_gravity.value > 0.0 {
        _VectorScale(
            newDir,
            ((g_knockback * knockback / mass) as f64 * 0.8) as f32,
            &mut kvel,
        );
        kvel[2] = ((newDir[2] * g_knockback * knockback / mass) as f64 * 1.5) as f32;
    } else {
        _VectorScale(newDir, g_knockback * knockback / mass, &mut kvel);
    }

    // targ may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(targ).client;
    if !client.is_null() {
        unsafe {
            _VectorAdd((*client).ps.velocity, kvel, &mut (*client).ps.velocity);
        }
    } else if ctx.entity(targ).s.pos.trType != trType_t::TR_STATIONARY
        && ctx.entity(targ).s.pos.trType != trType_t::TR_LINEAR_STOP
        && ctx.entity(targ).s.pos.trType != trType_t::TR_NONLINEAR_STOP
    {
        let level_time = ctx.world.level.time;
        let e = ctx.entity_mut(targ);
        let trDelta = e.s.pos.trDelta;
        _VectorAdd(trDelta, kvel, &mut e.s.pos.trDelta);
        e.s.pos.trBase = e.r.currentOrigin;
        e.s.pos.trTime = level_time;
    }

    // set the timer so that the other client can't cancel
    // out the movement immediately
    if !client.is_null() && unsafe { (*client).ps.pm_time } == 0 {
        let mut t = (knockback * 2.0) as c_int;
        if t < 50 {
            t = 50;
        }
        if t > 200 {
            t = 200;
        }
        unsafe {
            (*client).ps.pm_time = t;
            (*client).ps.pm_flags |= PMF_TIME_KNOCKBACK;
        }
    }
}

/// Raven `RaySphereIntersections`.
///
/// `dir` is normalized in place (mutate-through becomes `&mut`).
/// `origin` and `point` are read-only.
/// C's `vec3_t intersections[2]` becomes `*mut vec3_t` to the first of the two slots.
/// Source: `oracle/codemp/game/g_combat.c:3128-3158`
pub fn RaySphereIntersections(
    origin: vec3_t,
    radius: f32,
    point: vec3_t,
    dir: &mut vec3_t,
    intersections: *mut vec3_t,
) -> c_int {
    unsafe {
        // normalize dir so a = 1
        VectorNormalize(dir);
        let b = 2.0
            * (dir[0] * (point[0] - origin[0])
                + dir[1] * (point[1] - origin[1])
                + dir[2] * (point[2] - origin[2]));
        let c = (point[0] - origin[0]) * (point[0] - origin[0])
            + (point[1] - origin[1]) * (point[1] - origin[1])
            + (point[2] - origin[2]) * (point[2] - origin[2])
            - radius * radius;

        let d = b * b - 4.0 * c;
        // VectorMA(point, t, dir, out): out[i] = point[i] + t * dir[i]
        let ma = |t: f32| {
            [
                point[0] + t * dir[0],
                point[1] + t * dir[1],
                point[2] + t * dir[2],
            ]
        };
        if d > 0.0 {
            // Raven computes `t = (- b + sqrt(d)) / 2`.
            // `sqrt` is libm's `double sqrt`, so `d` promotes to double and the whole root evaluates in f64 before narrowing to the f32 `t`.
            // Evaluating in f32 here would diverge by a last bit on irrational `sqrt(d)`.
            let t = ((-(b as f64) + (d as f64).sqrt()) / 2.0) as f32;
            *intersections.add(0) = ma(t);
            let t = ((-(b as f64) - (d as f64).sqrt()) / 2.0) as f32;
            *intersections.add(1) = ma(t);
            2
        } else if d == 0.0 {
            // Raven computes `t = (- b) / 2`. There is no `sqrt`, so this stays f32 (`2` is an int divisor, promoting only to float).
            let t = -b / 2.0;
            *intersections.add(0) = ma(t);
            1
        } else {
            0
        }
    }
}

/// Raven `G_GetDismemberLoc`. Gets the general area without server-side ghoul2.
///
/// `boltPoint` is written (mutate-through becomes `&mut`).
/// Source: `oracle/codemp/game/g_combat.c:3192-3262`
pub fn G_GetDismemberLoc(self_: &gentity_t, boltPoint: &mut vec3_t, limbType: c_int) {
    use g2ModelParts_t::*;
    {
        let mut fwd: vec3_t = [0.0; 3];
        let mut right: vec3_t = [0.0; 3];
        let mut up: vec3_t = [0.0; 3];

        AngleVectors(
            self_.r.currentAngles,
            Some(&mut fwd),
            Some(&mut right),
            Some(&mut up),
        );

        *boltPoint = self_.r.currentOrigin;

        let m = limbType;
        if m == G2_MODELPART_HEAD as c_int {
            for i in 0..3 {
                boltPoint[i] += up[i] * 24.0;
            }
        } else if m == G2_MODELPART_WAIST as c_int {
            for i in 0..3 {
                boltPoint[i] += up[i] * 4.0;
            }
        } else if m == G2_MODELPART_LARM as c_int {
            for i in 0..3 {
                boltPoint[i] += up[i] * 18.0;
                boltPoint[i] -= right[i] * 10.0;
            }
        } else if m == G2_MODELPART_RARM as c_int {
            for i in 0..3 {
                boltPoint[i] += up[i] * 18.0;
                boltPoint[i] += right[i] * 10.0;
            }
        } else if m == G2_MODELPART_RHAND as c_int {
            for i in 0..3 {
                boltPoint[i] += up[i] * 8.0;
                boltPoint[i] += right[i] * 10.0;
            }
        } else if m == G2_MODELPART_LLEG as c_int {
            for i in 0..3 {
                boltPoint[i] -= up[i] * 4.0;
                boltPoint[i] -= right[i] * 10.0;
            }
        } else if m == G2_MODELPART_RLEG as c_int {
            for i in 0..3 {
                boltPoint[i] -= up[i] * 4.0;
                boltPoint[i] += right[i] * 10.0;
            }
        }
    }
}

/// Raven `G_GetDismemberBolt`.
///
/// Source: `oracle/codemp/game/g_combat.c:3264-3385`
pub fn G_GetDismemberBolt(
    ctx: &mut GameContext,
    self_: EntityId,
    boltPoint: &mut vec3_t,
    limbType: c_int,
) {
    use g2ModelParts_t::*;
    let mut useBolt = ctx.entity(self_).genericValue5;
    let mut properOrigin: vec3_t = [0.0; 3];
    let mut properAngles: vec3_t = [0.0; 3];
    let mut addVel: vec3_t = [0.0; 3];
    let mut boltMatrix: mdxaBone_t = unsafe { core::mem::zeroed() };
    let mut fVSpeed: f32 = 0.0;
    // self_ may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(self_).client;

    let rotateBone: *const c_char = if limbType == G2_MODELPART_HEAD as c_int {
        c"cranium".as_ptr()
    } else if limbType == G2_MODELPART_WAIST as c_int {
        if ctx.entity(self_).localAnimIndex <= 1 {
            c"thoracic".as_ptr() // humanoid
        } else {
            c"pelvis".as_ptr()
        }
    } else if limbType == G2_MODELPART_LARM as c_int {
        c"lradius".as_ptr()
    } else if limbType == G2_MODELPART_RARM as c_int {
        c"rradius".as_ptr()
    } else if limbType == G2_MODELPART_RHAND as c_int {
        c"rhand".as_ptr()
    } else if limbType == G2_MODELPART_LLEG as c_int {
        c"ltibia".as_ptr()
    } else if limbType == G2_MODELPART_RLEG as c_int {
        c"rtibia".as_ptr()
    } else {
        c"rtibia".as_ptr()
    };

    let ghoul2 = ctx.entity(self_).ghoul2;
    useBolt = trap::G2API_AddBolt(ctx.engine, ghoul2, 0, &unsafe { cstr_to_str(rotateBone) });

    unsafe {
        _VectorCopy((*client).ps.origin, &mut properOrigin);
        _VectorCopy((*client).ps.viewangles, &mut properAngles);

        // try to predict the origin based on velocity so it's more like what the client is seeing
        _VectorCopy((*client).ps.velocity, &mut addVel);
        VectorNormalize(&mut addVel);

        if (*client).ps.velocity[0] < 0.0 {
            fVSpeed += -(*client).ps.velocity[0];
        } else {
            fVSpeed += (*client).ps.velocity[0];
        }
        if (*client).ps.velocity[1] < 0.0 {
            fVSpeed += -(*client).ps.velocity[1];
        } else {
            fVSpeed += (*client).ps.velocity[1];
        }
        if (*client).ps.velocity[2] < 0.0 {
            fVSpeed += -(*client).ps.velocity[2];
        } else {
            fVSpeed += (*client).ps.velocity[2];
        }
    }

    fVSpeed = (fVSpeed as f64 * 0.08) as f32;

    properOrigin[0] += addVel[0] * fVSpeed;
    properOrigin[1] += addVel[1] * fVSpeed;
    properOrigin[2] += addVel[2] * fVSpeed;

    properAngles[0] = 0.0;
    properAngles[1] = unsafe { (*client).ps.viewangles[YAW] };
    properAngles[2] = 0.0;

    let level_time = ctx.world.level.time;
    let modelScale = ctx.entity(self_).modelScale;

    trap::G2API_GetBoltMatrix(
        ctx.engine,
        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
            ghoul2,
            0,
            useBolt,
            &mut boltMatrix as *mut mdxaBone_t,
            &properAngles as *const vec3_t,
            &properOrigin as *const vec3_t,
            level_time,
            core::ptr::null_mut(),
            &modelScale as *const vec3_t,
        ),
    );

    boltPoint[0] = boltMatrix.matrix[0][3];
    boltPoint[1] = boltMatrix.matrix[1][3];
    boltPoint[2] = boltMatrix.matrix[2][3];

    trap::G2API_GetBoltMatrix(
        ctx.engine,
        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
            ghoul2,
            1,
            0,
            &mut boltMatrix as *mut mdxaBone_t,
            &properAngles as *const vec3_t,
            &properOrigin as *const vec3_t,
            level_time,
            core::ptr::null_mut(),
            &modelScale as *const vec3_t,
        ),
    );

    if !client.is_null() && limbType == G2_MODELPART_RHAND as c_int {
        // Make some saber hit sparks over the severed wrist area
        let mut boltAngles: vec3_t = [0.0; 3];

        boltAngles[0] = -boltMatrix.matrix[0][1];
        boltAngles[1] = -boltMatrix.matrix[1][1];
        boltAngles[2] = -boltMatrix.matrix[2][1];

        let te = G_TempEntity(ctx, *boltPoint, entity_event_t::EV_SABER_HIT as c_int);
        let self_number = ctx.entity(self_).s.number;
        ctx.entity_mut(te).s.otherEntityNum = self_number;
        ctx.entity_mut(te).s.otherEntityNum2 = ENTITYNUM_NONE;
        ctx.entity_mut(te).s.weapon = 0; // saberNum
        ctx.entity_mut(te).s.legsAnim = 0; // bladeNum

        let bp = *boltPoint;
        ctx.entity_mut(te).s.origin = bp;
        ctx.entity_mut(te).s.angles = boltAngles;

        if ctx.entity(te).s.angles[0] == 0.0
            && ctx.entity(te).s.angles[1] == 0.0
            && ctx.entity(te).s.angles[2] == 0.0
        {
            // don't let it play with no direction
            ctx.entity_mut(te).s.angles[1] = 1.0;
        }

        ctx.entity_mut(te).s.eventParm = 16; // lots of sparks
    }
}

/// Raven `LimbTouch`.
///
/// Source: `oracle/codemp/game/g_combat.c:3387-3389`
pub fn LimbTouch(self_: EntityId, other: Option<EntityId>, trace: *mut trace_t) {
    // The touch-dispatch EntityId/Option params stay unused.
    // Raven's body is empty. The touch handler is a deliberate no-op.
}

/// Raven `LimbThink`.
///
/// Source: `oracle/codemp/game/g_combat.c:3391-3430`
pub fn LimbThink(ctx: &mut GameContext, ent: EntityId) {
    use g2ModelParts_t::*;
    let gravity: f32 = 3.0;
    let mut mass: f32 = 0.09;
    let mut bounce: f32 = 1.3;

    if ctx.entity(ent).s.modelGhoul2 == G2_MODELPART_HEAD as c_int {
        mass = 0.08;
        bounce = 1.4;
    } else if ctx.entity(ent).s.modelGhoul2 == G2_MODELPART_WAIST as c_int {
        mass = 0.1;
        bounce = 1.2;
    }
    // G2_MODELPART_LARM/RARM/RHAND/LLEG/RLEG/default: leave defaults

    let level_time = ctx.world.level.time;
    if ctx.entity(ent).speed < level_time as f32 {
        let e = ctx.entity_mut(ent);
        e.think = Some(EntThink::G_FreeEntity).into();
        e.nextthink = level_time;
        return;
    }

    if ctx.entity(ent).genericValue5 <= level_time {
        // this will be every frame by standard, but we want to compensate in case sv_fps is not 20.
        crate::g_exphysics::G_RunExPhys(
            ctx,
            ent,
            gravity,
            mass,
            bounce,
            true,
            core::ptr::null_mut(),
            0,
        );
        ctx.entity_mut(ent).genericValue5 = level_time + 50;
    }

    ctx.entity_mut(ent).nextthink = level_time;
}

/// Raven `G_Dismember`.
///
/// Source: `oracle/codemp/game/g_combat.c:3436-3620`
pub fn G_Dismember(
    ctx: &mut GameContext,
    ent: EntityId,
    enemy: Option<EntityId>,
    point: vec3_t,
    limbType: c_int,
    limbRollBase: f32,
    limbPitchBase: f32,
    deathAnim: c_int,
    postDeath: qboolean,
) {
    use animNumber_t::*;
    use g2ModelParts_t::*;
    // ent/enemy `.client` are pool clients (deref raw).
    // The body keeps one unsafe scope for those. Entity/limb fields route through the accessor.
    let ghoul2 = ctx.entity(ent).ghoul2;
    unsafe {
        let mut newPoint: vec3_t = [0.0; 3];
        let mut dir: vec3_t = [0.0; 3];
        let mut vel: vec3_t = [0.0; 3];
        let mut limbName: String = String::new();
        let mut stubName: String = String::new();
        let mut stubCapName: String = String::new();

        if limbType == G2_MODELPART_HEAD as c_int {
            limbName = "head".to_string();
            stubCapName = "torso_cap_head".to_string();
        } else if limbType == G2_MODELPART_WAIST as c_int {
            limbName = "torso".to_string();
            stubCapName = "hips_cap_torso".to_string();
        } else {
            // (limb root surf, stub root surf, cap suffix) per part.
            let (limb_root, stub_root, cap_suffix) = if limbType == G2_MODELPART_LARM as c_int {
                ("l_arm", "torso", "l_arm")
            } else if limbType == G2_MODELPART_RARM as c_int {
                ("r_arm", "torso", "r_arm")
            } else if limbType == G2_MODELPART_RHAND as c_int {
                ("r_hand", "r_arm", "r_hand")
            } else if limbType == G2_MODELPART_LLEG as c_int {
                ("l_leg", "hips", "l_leg")
            } else {
                // umm... just default to the right leg, I guess (same as on client)
                ("r_leg", "hips", "r_leg")
            };
            limbName = BG_GetRootSurfNameWithVariant(
                ghoul2,
                limb_root,
                &ctx.world.bg_state,
                &GameBgTraps::new(ctx.engine),
            );
            stubName = BG_GetRootSurfNameWithVariant(
                ghoul2,
                stub_root,
                &ctx.world.bg_state,
                &GameBgTraps::new(ctx.engine),
            );
            stubCapName = format!("{}_cap_{}", stubName, cap_suffix);
        }

        if !ghoul2.is_null()
            && trap::G2API_GetSurfaceRenderStatus(ctx.engine, ghoul2, 0, &limbName) != 0
        {
            // is it already off? If so there's no reason to be doing it again, so get out of here.
            return;
        }

        newPoint = point;
        let limb = crate::g_utils::G_Spawn(ctx);

        let speed = (ctx.world.level.time + ctx.world.bg_state.rng.Q_irand(8000, 16000)) as f32;
        let nextthink = ctx.world.level.time + FRAMETIME;
        let ent_number = ctx.entity(ent).s.number;
        // ent may be an NPC. `.client` is a pool client, so the deref stays raw.
        let ent_client = ctx.entity(ent).client;
        let ent_angles = if !ent_client.is_null() {
            unsafe { (*ent_client).ps.viewangles }
        } else {
            ctx.entity(ent).r.currentAngles
        };
        let currentOrigin = ctx.entity(ent).r.currentOrigin;
        vel = if !ent_client.is_null() {
            unsafe { (*ent_client).ps.velocity }
        } else {
            ctx.entity(ent).s.pos.trDelta
        };

        ctx.ent_set(limb, PrefixSet::ClassnameStatic(c"playerlimb"));
        let e = ctx.entity_mut(limb);

        crate::g_utils::G_SetOrigin(e, newPoint);
        e.s.pos.trBase = newPoint;
        e.think = Some(EntThink::LimbThink).into();
        e.touch = Some(EntTouch::LimbTouch).into();
        e.speed = speed;
        e.nextthink = nextthink;

        e.r.svFlags = SVF_USE_CURRENT_ORIGIN;
        e.clipmask = MASK_SOLID;
        e.r.contents = CONTENTS_TRIGGER;
        e.physicsObject = qtrue;
        e.r.mins = [-6.0, -6.0, -3.0];
        e.r.maxs = [6.0, 6.0, 6.0];

        e.s.g2radius = 200;

        e.s.eType = entityType_t::ET_GENERAL as c_int;
        e.s.weapon = G2_MODEL_PART;
        e.s.modelGhoul2 = limbType;
        e.s.modelindex = ent_number;
        if ent_client.is_null() {
            e.s.modelindex = -1;
            e.s.otherEntityNum2 = ent_number;
        }

        e.s.apos.trDelta = [0.0; 3];

        e.r.currentAngles = ent_angles;
        e.s.apos.trBase = ent_angles;

        // Set up the ExPhys values for the entity.
        e.epGravFactor = 0.0;
        e.epVelocity = [0.0; 3];
        _VectorSubtract(point, currentOrigin, &mut dir);
        VectorNormalize(&mut dir);
        _VectorMA(vel, 80.0, dir, &mut e.epVelocity);

        // add some vertical velocity
        if limbType == G2_MODELPART_HEAD as c_int || limbType == G2_MODELPART_WAIST as c_int {
            e.epVelocity[2] += 10.0;
        }

        // enemy may be an NPC. `.client` is a pool client, so the deref stays raw.
        let saber_toss = match enemy {
            Some(enemy_id) => {
                let ecl = ctx.entity(enemy_id).client;
                !ecl.is_null()
                    && ent != enemy_id
                    && ctx.entity(ent).s.number != ctx.entity(enemy_id).s.number
                    && unsafe { (*ecl).ps.weapon } == WP_SABER as c_int
                    && unsafe { (*ecl).olderIsValid } != qfalse
                    && (ctx.world.level.time - unsafe { (*ecl).lastSaberStorageTime }) < 200
            }
            None => false,
        };
        if saber_toss {
            let enemy_id = enemy.unwrap();
            let ecl = ctx.entity(enemy_id).client;
            let mut dif: vec3_t = [0.0; 3];
            let mut totalDistance;
            let distScale: f32 = 1.2;

            // scale down the initial velocity first, which is based on the speed of the limb owner.
            // ExPhys object velocity operates on a slightly different scale than Q3-based physics velocity.
            let ev = ctx.entity(limb).epVelocity;
            _VectorScale(ev, 0.4f32, &mut ctx.entity_mut(limb).epVelocity);

            _VectorSubtract(
                unsafe { (*ecl).lastSaberBase_Always },
                unsafe { (*ecl).olderSaberBase },
                &mut dif,
            );
            totalDistance = VectorNormalize(&mut dif);

            _VectorScale(dif, totalDistance * distScale, &mut dif);
            let ev = ctx.entity(limb).epVelocity;
            _VectorAdd(ev, dif, &mut ctx.entity_mut(limb).epVelocity);

            if !ent_client.is_null()
                && (unsafe { (*ent_client).ps.torsoTimer } > 0
                    || mp_bg::bg_panimate::BG_InDeathAnim(unsafe { (*ent_client).ps.torsoAnim })
                        == qfalse)
            {
                // if he's done with his death anim we don't actually want the limbs going far
                let mut preVel: vec3_t = [0.0; 3];

                let ev = ctx.entity(limb).epVelocity;
                preVel = ev;
                preVel[2] = 0.0;
                totalDistance = VectorNormalize(&mut preVel);

                if totalDistance < 40.0 {
                    let mAmt: f32 = 40.0;

                    ctx.entity_mut(limb).epVelocity[0] = preVel[0] * mAmt;
                    ctx.entity_mut(limb).epVelocity[1] = preVel[1] * mAmt;
                }
            } else if !ent_client.is_null() {
                let ev = ctx.entity(limb).epVelocity;
                _VectorScale(ev, 0.3f32, &mut ctx.entity_mut(limb).epVelocity);
            }
        }

        if ctx.entity(ent).s.eType == entityType_t::ET_NPC as c_int && !ghoul2.is_null() {
            // if it's an npc remove these surfs on the server too. For players we don't even care cause there's no further dismemberment after death.
            trap::G2API_SetSurfaceOnOff(ctx.engine, ghoul2, &limbName, 0x00000100);
            trap::G2API_SetSurfaceOnOff(ctx.engine, ghoul2, &stubCapName, 0);
        }

        let customRGBA = ctx.entity(ent).s.customRGBA;
        ctx.entity_mut(limb).s.customRGBA = customRGBA;

        let limb_ptr = ctx.entity_mut(limb) as *mut gentity_t;
        trap::LinkEntity(
            ctx.engine,
            mp_abi::game::syscalls::G_LINKENTITY::GLinkentityArgs::new(limb_ptr.cast()),
        );
    }
}

/// Raven `DismembermentTest`.
///
/// Source: `oracle/codemp/game/g_combat.c:3622-3633`
pub fn DismembermentTest(ctx: &mut GameContext, self_: EntityId) {
    use animNumber_t::*;
    use g2ModelParts_t::*;
    let mut sect = G2_MODELPART_HEAD as c_int;
    let mut boltPoint: vec3_t = [0.0; 3];

    while sect <= G2_MODELPART_RLEG as c_int {
        G_GetDismemberBolt(ctx, self_, &mut boltPoint, sect);
        G_Dismember(
            ctx,
            self_,
            Some(self_),
            boltPoint,
            sect,
            90.0,
            0.0,
            BOTH_DEATH1 as c_int,
            qfalse,
        );
        sect += 1;
    }
}

/// Raven `DismembermentByNum`.
///
/// Source: `oracle/codemp/game/g_combat.c:3635-3669`
pub fn DismembermentByNum(ctx: &mut GameContext, self_: EntityId, num: c_int) {
    use animNumber_t::*;
    use g2ModelParts_t::*;
    let mut boltPoint: vec3_t = [0.0; 3];

    let sect = match num {
        0 => G2_MODELPART_HEAD as c_int,
        1 => G2_MODELPART_WAIST as c_int,
        2 => G2_MODELPART_LARM as c_int,
        3 => G2_MODELPART_RARM as c_int,
        4 => G2_MODELPART_RHAND as c_int,
        5 => G2_MODELPART_LLEG as c_int,
        6 => G2_MODELPART_RLEG as c_int,
        _ => G2_MODELPART_HEAD as c_int,
    };

    G_GetDismemberBolt(ctx, self_, &mut boltPoint, sect);
    G_Dismember(
        ctx,
        self_,
        Some(self_),
        boltPoint,
        sect,
        90.0,
        0.0,
        BOTH_DEATH1 as c_int,
        qfalse,
    );
}

/// Raven `G_GetHitQuad`.
///
/// `hitloc` is read-only (subtracted into a local `diff`), so it stays by-value.
/// Source: `oracle/codemp/game/g_combat.c:3671-3751`
pub fn G_GetHitQuad(ctx: &mut GameContext, self_: EntityId, hitloc: vec3_t) -> c_int {
    use g2ModelParts_t::*;
    let mut diff: vec3_t = [0.0; 3];
    let mut fwdangles: vec3_t = [0.0, 0.0, 0.0];
    let mut right: vec3_t = [0.0; 3];
    let mut clEye: vec3_t;
    let mut hitLoc = ctx.world.globals.gPainHitLoc;

    // self_ may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(self_).client;
    if !client.is_null() {
        clEye = unsafe { (*client).ps.origin };
        clEye[2] += unsafe { (*client).ps.viewheight } as f32;
    } else {
        clEye = ctx.entity(self_).s.pos.trBase;
        clEye[2] += 16.0;
    }

    for i in 0..3 {
        diff[i] = hitloc[i] - clEye[i];
    }
    diff[2] = 0.0;
    VectorNormalize(&mut diff);

    if !client.is_null() {
        fwdangles[1] = unsafe { (*client).ps.viewangles[1] };
    } else {
        fwdangles[1] = ctx.entity(self_).s.apos.trBase[1];
    }
    // Ultimately we might care if the shot was ahead or behind, but for now, just quadrant is fine.
    AngleVectors(fwdangles, None, Some(&mut right), None);

    let rightdot = right[0] * diff[0] + right[1] * diff[1] + right[2] * diff[2];
    let zdiff = hitloc[2] - clEye[2];

    if zdiff > 0.0 {
        if (rightdot as f64) > 0.3 {
            hitLoc = G2_MODELPART_RARM as c_int;
        } else if (rightdot as f64) < -0.3 {
            hitLoc = G2_MODELPART_LARM as c_int;
        } else {
            hitLoc = G2_MODELPART_HEAD as c_int;
        }
    } else if zdiff > -20.0 {
        if (rightdot as f64) > 0.1 {
            hitLoc = G2_MODELPART_RARM as c_int;
        } else if (rightdot as f64) < -0.1 {
            hitLoc = G2_MODELPART_LARM as c_int;
        } else {
            hitLoc = G2_MODELPART_HEAD as c_int;
        }
    } else if rightdot >= 0.0 {
        hitLoc = G2_MODELPART_RLEG as c_int;
    } else {
        hitLoc = G2_MODELPART_LLEG as c_int;
    }

    hitLoc
}

/// Raven `G_GetHitLocFromSurfName`.
///
/// Callers encode Raven's NULL `vec3_t point` as the zero vector, so the `if (!point)` guard becomes a zero test.
/// Source: `oracle/codemp/game/g_combat.c:3757-4216`
pub fn G_GetHitLocFromSurfName(
    ctx: &mut GameContext,
    ent: EntityId,
    surfName: &str,
    hitLoc: *mut c_int,
    point: vec3_t,
    dir: vec3_t,
    bladeDir: vec3_t,
    r#mod: c_int,
) -> qboolean {
    // ent's `.client` is a pool client for NPCs, so the deref stays raw.
    // `*hitLoc` is a raw out-param, and the ghoul2 bolt-matrix calls take raw field pointers.
    // The body keeps one unsafe scope. The ent field reads route through the accessor.
    unsafe {
        let mut dismember: qboolean = qfalse;
        let mut kneeLBolt: c_int = -1;
        let mut kneeRBolt: c_int = -1;
        let mut handRBolt: c_int = -1;
        let mut handLBolt: c_int = -1;
        let mut footRBolt: c_int = -1;
        let mut footLBolt: c_int = -1;

        *hitLoc = HL_NONE;

        if surfName.is_empty() {
            return qfalse;
        }

        let client = ctx.entity(ent).client;
        if client.is_null() {
            return qfalse;
        }
        // Raven `if (!point) return qfalse;`. Zero vector is the NULL sentinel.
        if point == [0.0, 0.0, 0.0] {
            return qfalse;
        }

        if !ctx.entity(ent).client.is_null()
            && ((*client).NPC_class == class_t::CLASS_R2D2
                || (*client).NPC_class == class_t::CLASS_R2D2
                || (*client).NPC_class == class_t::CLASS_GONK
                || (*client).NPC_class == class_t::CLASS_MOUSE
                || (*client).NPC_class == class_t::CLASS_SENTRY
                || (*client).NPC_class == class_t::CLASS_INTERROGATOR
                || (*client).NPC_class == class_t::CLASS_SENTRY
                || (*client).NPC_class == class_t::CLASS_PROBE)
        {
            // we don't care about per-surface hit-locations or dismemberment for these guys
            return qfalse;
        }

        if ctx.entity(ent).localAnimIndex <= 1 {
            // humanoid
            handLBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*l_hand");
            handRBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*r_hand");
            kneeLBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*hips_l_knee");
            kneeRBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*hips_r_knee");
            footLBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*l_leg_foot");
            footRBolt = trap::G2API_AddBolt(ctx.engine, ctx.entity(ent).ghoul2, 0, "*r_leg_foot");
        }

        // surfName is proven non-empty by the early guard above.
        let surf_name_s = surfName;
        let stricmp = |lit: &str| Q_stricmp(lit, surf_name_s) == 0;

        if !ctx.entity(ent).client.is_null() && (*client).NPC_class == class_t::CLASS_ATST {
            if stricmp("head_light_blaster_cann") {
                *hitLoc = HL_ARM_LT;
            } else if stricmp("head_concussion_charger") {
                *hitLoc = HL_ARM_RT;
            }
            return qfalse;
        } else if !ctx.entity(ent).client.is_null() && (*client).NPC_class == class_t::CLASS_MARK1 {
            if stricmp("l_arm") {
                *hitLoc = HL_ARM_LT;
            } else if stricmp("r_arm") {
                *hitLoc = HL_ARM_RT;
            } else if stricmp("torso_front") {
                *hitLoc = HL_CHEST;
            } else if stricmp("torso_tube1") {
                *hitLoc = HL_GENERIC1;
            } else if stricmp("torso_tube2") {
                *hitLoc = HL_GENERIC2;
            } else if stricmp("torso_tube3") {
                *hitLoc = HL_GENERIC3;
            } else if stricmp("torso_tube4") {
                *hitLoc = HL_GENERIC4;
            } else if stricmp("torso_tube5") {
                *hitLoc = HL_GENERIC5;
            } else if stricmp("torso_tube6") {
                *hitLoc = HL_GENERIC6;
            }
            return qfalse;
        } else if !ctx.entity(ent).client.is_null() && (*client).NPC_class == class_t::CLASS_MARK2 {
            if stricmp("torso_canister1") {
                *hitLoc = HL_GENERIC1;
            } else if stricmp("torso_canister2") {
                *hitLoc = HL_GENERIC2;
            } else if stricmp("torso_canister3") {
                *hitLoc = HL_GENERIC3;
            }
            return qfalse;
        } else if !ctx.entity(ent).client.is_null()
            && (*client).NPC_class == class_t::CLASS_GALAKMECH
        {
            if stricmp("torso_antenna") || stricmp("torso_antenna_base") {
                *hitLoc = HL_GENERIC1;
            } else if stricmp("torso_shield") {
                *hitLoc = HL_GENERIC2;
            } else {
                *hitLoc = HL_CHEST;
            }
            return qfalse;
        }

        let strncmp = |lit: &str, n: usize| Q_strncmp(lit, surf_name_s, n) == 0;

        let actualTime = ctx.world.level.time;
        if strncmp("hips", 4) {
            // FIXME: test properly for legs
            *hitLoc = HL_WAIST;
            if !ctx.entity(ent).client.is_null() && !ctx.entity(ent).ghoul2.is_null() {
                let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                let mut tagOrg: vec3_t = [0.0; 3];

                let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];
                if kneeLBolt >= 0 {
                    trap::G2API_GetBoltMatrix(
                        ctx.engine,
                        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                            ctx.entity(ent).ghoul2,
                            0,
                            kneeLBolt,
                            &mut boltMatrix as *mut mdxaBone_t,
                            &angles as *const vec3_t,
                            &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                            actualTime,
                            core::ptr::null_mut(),
                            &ctx.entity(ent).modelScale as *const vec3_t,
                        ),
                    );
                    BG_GiveMeVectorFromMatrix(
                        &boltMatrix as *const mdxaBone_t,
                        ORIGIN as c_int,
                        &mut tagOrg,
                    );
                    if DistanceSquared(point, tagOrg) < 100.0 {
                        *hitLoc = HL_LEG_LT; // actually hit the knee
                    }
                }
                if *hitLoc == HL_WAIST {
                    if kneeRBolt >= 0 {
                        trap::G2API_GetBoltMatrix(
                            ctx.engine,
                            mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                                ctx.entity(ent).ghoul2,
                                0,
                                kneeRBolt,
                                &mut boltMatrix as *mut mdxaBone_t,
                                &angles as *const vec3_t,
                                &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                                actualTime,
                                core::ptr::null_mut(),
                                &ctx.entity(ent).modelScale as *const vec3_t,
                            ),
                        );
                        BG_GiveMeVectorFromMatrix(
                            &boltMatrix as *const mdxaBone_t,
                            ORIGIN as c_int,
                            &mut tagOrg,
                        );
                        if DistanceSquared(point, tagOrg) < 100.0 {
                            *hitLoc = HL_LEG_RT; // actually hit the knee
                        }
                    }
                }
            }
        } else if strncmp("torso", 5) {
            if ctx.entity(ent).client.is_null() {
                *hitLoc = HL_CHEST;
            } else {
                let mut t_fwd: vec3_t = [0.0; 3];
                let mut t_rt: vec3_t = [0.0; 3];
                let mut t_up: vec3_t = [0.0; 3];
                let mut dirToImpact: vec3_t = [0.0; 3];
                AngleVectors(
                    (*client).renderInfo.torsoAngles,
                    Some(&mut t_fwd),
                    Some(&mut t_rt),
                    Some(&mut t_up),
                );

                if (*client).renderInfo.boltValidityTime != ctx.world.level.time {
                    let renderAng: vec3_t = [0.0, (*client).ps.viewangles[YAW], 0.0];
                    let origin = (*client).ps.origin;
                    UpdateClientRenderBolts(ctx, ent, origin, renderAng);
                }

                _VectorSubtract(point, (*client).renderInfo.torsoPoint, &mut dirToImpact);
                let frontSide = _DotProduct(t_fwd, dirToImpact);
                let rightSide = _DotProduct(t_rt, dirToImpact);
                let upSide = _DotProduct(t_up, dirToImpact);
                if upSide < -10.0 {
                    *hitLoc = HL_WAIST; // hit at waist
                } else {
                    // hit on upper torso
                    if rightSide > 4.0 {
                        *hitLoc = HL_ARM_RT;
                    } else if rightSide < -4.0 {
                        *hitLoc = HL_ARM_LT;
                    } else if rightSide > 2.0 {
                        if frontSide > 0.0 {
                            *hitLoc = HL_CHEST_RT;
                        } else {
                            *hitLoc = HL_BACK_RT;
                        }
                    } else if rightSide < -2.0 {
                        if frontSide > 0.0 {
                            *hitLoc = HL_CHEST_LT;
                        } else {
                            *hitLoc = HL_BACK_LT;
                        }
                    } else if upSide > -3.0 && r#mod == meansOfDeath_t::MOD_SABER as c_int {
                        *hitLoc = HL_HEAD;
                    } else if frontSide > 0.0 {
                        *hitLoc = HL_CHEST;
                    } else {
                        *hitLoc = HL_BACK;
                    }
                }
            }
        } else if strncmp("head", 4) {
            *hitLoc = HL_HEAD;
        } else if strncmp("r_arm", 5) {
            *hitLoc = HL_ARM_RT;
            if !ctx.entity(ent).client.is_null() && !ctx.entity(ent).ghoul2.is_null() {
                let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                let mut tagOrg: vec3_t = [0.0; 3];
                let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];
                if handRBolt >= 0 {
                    trap::G2API_GetBoltMatrix(
                        ctx.engine,
                        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                            ctx.entity(ent).ghoul2,
                            0,
                            handRBolt,
                            &mut boltMatrix as *mut mdxaBone_t,
                            &angles as *const vec3_t,
                            &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                            actualTime,
                            core::ptr::null_mut(),
                            &ctx.entity(ent).modelScale as *const vec3_t,
                        ),
                    );
                    BG_GiveMeVectorFromMatrix(
                        &boltMatrix as *const mdxaBone_t,
                        ORIGIN as c_int,
                        &mut tagOrg,
                    );
                    if DistanceSquared(point, tagOrg) < 256.0 {
                        *hitLoc = HL_HAND_RT; // actually hit the hand
                    }
                }
            }
        } else if strncmp("l_arm", 5) {
            *hitLoc = HL_ARM_LT;
            if !ctx.entity(ent).client.is_null() && !ctx.entity(ent).ghoul2.is_null() {
                let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                let mut tagOrg: vec3_t = [0.0; 3];
                let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];
                if handLBolt >= 0 {
                    trap::G2API_GetBoltMatrix(
                        ctx.engine,
                        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                            ctx.entity(ent).ghoul2,
                            0,
                            handLBolt,
                            &mut boltMatrix as *mut mdxaBone_t,
                            &angles as *const vec3_t,
                            &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                            actualTime,
                            core::ptr::null_mut(),
                            &ctx.entity(ent).modelScale as *const vec3_t,
                        ),
                    );
                    BG_GiveMeVectorFromMatrix(
                        &boltMatrix as *const mdxaBone_t,
                        ORIGIN as c_int,
                        &mut tagOrg,
                    );
                    if DistanceSquared(point, tagOrg) < 256.0 {
                        *hitLoc = HL_HAND_LT; // actually hit the hand
                    }
                }
            }
        } else if strncmp("r_leg", 5) {
            *hitLoc = HL_LEG_RT;
            if !ctx.entity(ent).client.is_null() && !ctx.entity(ent).ghoul2.is_null() {
                let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                let mut tagOrg: vec3_t = [0.0; 3];
                let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];
                if footRBolt >= 0 {
                    trap::G2API_GetBoltMatrix(
                        ctx.engine,
                        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                            ctx.entity(ent).ghoul2,
                            0,
                            footRBolt,
                            &mut boltMatrix as *mut mdxaBone_t,
                            &angles as *const vec3_t,
                            &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                            actualTime,
                            core::ptr::null_mut(),
                            &ctx.entity(ent).modelScale as *const vec3_t,
                        ),
                    );
                    BG_GiveMeVectorFromMatrix(
                        &boltMatrix as *const mdxaBone_t,
                        ORIGIN as c_int,
                        &mut tagOrg,
                    );
                    if DistanceSquared(point, tagOrg) < 100.0 {
                        *hitLoc = HL_FOOT_RT; // actually hit the foot
                    }
                }
            }
        } else if strncmp("l_leg", 5) {
            *hitLoc = HL_LEG_LT;
            if !ctx.entity(ent).client.is_null() && !ctx.entity(ent).ghoul2.is_null() {
                let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                let mut tagOrg: vec3_t = [0.0; 3];
                let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];
                if footLBolt >= 0 {
                    trap::G2API_GetBoltMatrix(
                        ctx.engine,
                        mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                            ctx.entity(ent).ghoul2,
                            0,
                            footLBolt,
                            &mut boltMatrix as *mut mdxaBone_t,
                            &angles as *const vec3_t,
                            &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                            actualTime,
                            core::ptr::null_mut(),
                            &ctx.entity(ent).modelScale as *const vec3_t,
                        ),
                    );
                    BG_GiveMeVectorFromMatrix(
                        &boltMatrix as *const mdxaBone_t,
                        ORIGIN as c_int,
                        &mut tagOrg,
                    );
                    if DistanceSquared(point, tagOrg) < 100.0 {
                        *hitLoc = HL_FOOT_LT; // actually hit the foot
                    }
                }
            }
        } else if strncmp("r_hand", 6) || strncmp("w_", 2) {
            // right hand or weapon
            *hitLoc = HL_HAND_RT;
        } else if strncmp("l_hand", 6) {
            *hitLoc = HL_HAND_LT;
        }

        if ctx.world.cvars.g_dismember.integer == 100 {
            // full probability...
            if !ctx.entity(ent).client.is_null() && (*client).NPC_class == class_t::CLASS_PROTOCOL {
                dismember = qtrue;
            } else if (dir[0] != 0.0 || dir[1] != 0.0 || dir[2] != 0.0)
                && (bladeDir[0] != 0.0 || bladeDir[1] != 0.0 || bladeDir[2] != 0.0)
            {
                // we care about direction (presumably for dismemberment)
                if true {
                    // Fix me?
                    let mut tagName: *const c_char = core::ptr::null();
                    let mut aoa: f32 = 0.5;
                    // dir must be roughly perpendicular to the hitLoc's cap bolt
                    let hl = *hitLoc;
                    if hl == HL_LEG_RT {
                        tagName = c"*hips_cap_r_leg".as_ptr();
                    } else if hl == HL_LEG_LT {
                        tagName = c"*hips_cap_l_leg".as_ptr();
                    } else if hl == HL_WAIST {
                        tagName = c"*hips_cap_torso".as_ptr();
                        aoa = 0.25;
                    } else if hl == HL_CHEST_RT || hl == HL_ARM_RT || hl == HL_BACK_LT {
                        tagName = c"*torso_cap_r_arm".as_ptr();
                    } else if hl == HL_CHEST_LT || hl == HL_ARM_LT || hl == HL_BACK_RT {
                        tagName = c"*torso_cap_l_arm".as_ptr();
                    } else if hl == HL_HAND_RT {
                        tagName = c"*r_arm_cap_r_hand".as_ptr();
                    } else if hl == HL_HAND_LT {
                        tagName = c"*l_arm_cap_l_hand".as_ptr();
                    } else if hl == HL_HEAD {
                        tagName = c"*torso_cap_head".as_ptr();
                        aoa = 0.25;
                    }
                    // HL_CHEST/HL_BACK/HL_FOOT_RT/HL_FOOT_LT/default: no dismemberment possible

                    if !tagName.is_null() {
                        let tagBolt = trap::G2API_AddBolt(
                            ctx.engine,
                            ctx.entity(ent).ghoul2,
                            0,
                            &cstr_to_str(tagName),
                        );
                        if tagBolt != -1 {
                            let mut boltMatrix: mdxaBone_t = core::mem::zeroed();
                            let mut tagOrg: vec3_t = [0.0; 3];
                            let mut tagDir: vec3_t = [0.0; 3];
                            let angles: vec3_t = [0.0, ctx.entity(ent).r.currentAngles[YAW], 0.0];

                            trap::G2API_GetBoltMatrix(
                                ctx.engine,
                                mp_abi::game::syscalls::G_G2_GETBOLT::GG2GetboltArgs::new(
                                    ctx.entity(ent).ghoul2,
                                    0,
                                    tagBolt,
                                    &mut boltMatrix as *mut mdxaBone_t,
                                    &angles as *const vec3_t,
                                    &ctx.entity(ent).r.currentOrigin as *const vec3_t,
                                    actualTime,
                                    core::ptr::null_mut(),
                                    &ctx.entity(ent).modelScale as *const vec3_t,
                                ),
                            );
                            BG_GiveMeVectorFromMatrix(
                                &boltMatrix as *const mdxaBone_t,
                                ORIGIN as c_int,
                                &mut tagOrg,
                            );
                            BG_GiveMeVectorFromMatrix(
                                &boltMatrix as *const mdxaBone_t,
                                NEGATIVE_Y as c_int,
                                &mut tagDir,
                            );
                            if DistanceSquared(point, tagOrg) < 256.0 {
                                // hit close
                                let mut dot = _DotProduct(dir, tagDir);
                                if dot < aoa && dot > -aoa {
                                    // hit roughly perpendicular
                                    dot = _DotProduct(bladeDir, tagDir);
                                    if dot < aoa && dot > -aoa {
                                        // blade was roughly perpendicular
                                        dismember = qtrue;
                                    }
                                }
                            }
                        }
                    }
                }
            } else {
                // hmm, no direction supplied.
                dismember = qtrue;
            }
        }
        dismember
    }
}

/// Raven `G_CheckForDismemberment`.
///
/// Source: `oracle/codemp/game/g_combat.c:4218-4344`
pub fn G_CheckForDismemberment(
    ctx: &mut GameContext,
    ent: EntityId,
    enemy: Option<EntityId>,
    point: vec3_t,
    damage: c_int,
    deathAnim: c_int,
    postDeath: qboolean,
) {
    use g2ModelParts_t::*;
    // ent's `.client` is a pool client for NPCs (deref raw).
    // The body keeps one unsafe scope. Entity fields (all reads) route through the accessor.
    unsafe {
        let mut hitLoc: c_int = -1;
        let mut hitLocUse: c_int;
        let mut boltPoint: vec3_t = [0.0; 3];
        let dismember = ctx.world.cvars.g_dismember.integer;
        let client = ctx.entity(ent).client;

        if ctx.entity(ent).localAnimIndex > 1 {
            if ctx.entity(ent).NPC.is_null() {
                return;
            }

            if (*client).NPC_class != class_t::CLASS_PROTOCOL {
                // this is the only non-humanoid allowed to do dismemberment.
                return;
            }
        }

        if dismember == 0 {
            return;
        }

        if ctx.world.globals.gGAvoidDismember == 1 {
            return;
        }

        if ctx.world.globals.gGAvoidDismember != 2 {
            // this means do the dismemberment regardless of randomness and damage
            if ctx.world.bg_state.rng.Q_irand(0, 100) > dismember {
                return;
            }

            if damage < 5 {
                return;
            }
        }

        if ctx.world.globals.gGAvoidDismember == 2 {
            hitLoc = HL_HAND_RT;
        } else {
            if ctx.world.cvars.d_saberGhoul2Collision.integer != 0
                && !ctx.entity(ent).client.is_null()
                && (*client).g2LastSurfaceTime == ctx.world.level.time
            {
                let hitSurfaceName = trap::G2API_GetSurfaceName(
                    ctx.engine,
                    ctx.entity(ent).ghoul2,
                    (*client).g2LastSurfaceHit,
                    0,
                    MAX_QPATH as usize,
                );
                let hitSurface = strncpyz_string(hitSurfaceName.as_bytes(), MAX_QPATH as usize);

                if !hitSurface.is_empty() {
                    G_GetHitLocFromSurfName(
                        ctx,
                        ent,
                        &hitSurface,
                        &mut hitLoc,
                        point,
                        [0.0; 3],
                        [0.0; 3],
                        meansOfDeath_t::MOD_UNKNOWN as c_int,
                    );
                }
            }

            if hitLoc == -1 {
                hitLoc = G_GetHitLocation(ctx, ent, point);
            }
        }

        hitLocUse = match hitLoc {
            HL_FOOT_RT | HL_LEG_RT => G2_MODELPART_RLEG as c_int,
            HL_FOOT_LT | HL_LEG_LT => G2_MODELPART_LLEG as c_int,
            HL_WAIST => G2_MODELPART_WAIST as c_int,
            HL_ARM_RT => G2_MODELPART_RARM as c_int,
            HL_HAND_RT => G2_MODELPART_RHAND as c_int,
            HL_ARM_LT | HL_HAND_LT => G2_MODELPART_LARM as c_int,
            HL_HEAD => G2_MODELPART_HEAD as c_int,
            _ => G_GetHitQuad(ctx, ent, point),
        };

        if hitLocUse == -1 {
            return;
        }

        if !ctx.entity(ent).client.is_null() {
            G_GetDismemberBolt(ctx, ent, &mut boltPoint, hitLocUse);
            if ctx.world.cvars.g_austrian.integer != 0
                && (ctx.world.cvars.g_gametype.integer == GT_DUEL
                    || ctx.world.cvars.g_gametype.integer == GT_POWERDUEL)
            {
                let s = format!(
                    "Duel Dismemberment: {} dismembered at {}\n",
                    (*client).pers.netname.clone(),
                    cstr_to_str(hitLocName[hitLoc as usize].as_ptr())
                );
                crate::g_main::G_LogPrintf(ctx, &s);
            }
        } else {
            G_GetDismemberLoc(ctx.entity(ent), &mut boltPoint, hitLocUse);
        }
        G_Dismember(
            ctx, ent, enemy, boltPoint, hitLocUse, 90.0, 0.0, deathAnim, postDeath,
        );
    }
}

/// Raven `G_LocationBasedDamageModifier`.
///
/// Source: `oracle/codemp/game/g_combat.c:4346-4425`
pub fn G_LocationBasedDamageModifier(
    ctx: &mut GameContext,
    ent: EntityId,
    point: vec3_t,
    r#mod: c_int,
    dflags: c_int,
    damage: *mut c_int,
) {
    // ent's `.client` is a pool client for NPCs (deref raw), and `*damage` is a raw out-param.
    // The body keeps one unsafe scope. Entity fields (all reads) route through the accessor.
    unsafe {
        let mut hitLoc: c_int = -1;
        let client = ctx.entity(ent).client;

        if ctx.world.cvars.g_locationBasedDamage.integer == 0 {
            // then leave it alone
            return;
        }

        if (dflags & DAMAGE_NO_HIT_LOC) != 0 {
            // then leave it alone
            return;
        }

        if r#mod == meansOfDeath_t::MOD_SABER as c_int && *damage <= 1 {
            // don't bother for idle damage
            return;
        }
        // Raven `if (!point) return;`. Zero vector is the NULL sentinel.
        if point == [0.0, 0.0, 0.0] {
            return;
        }

        if !ctx.entity(ent).client.is_null() && (*client).NPC_class == class_t::CLASS_VEHICLE {
            // no location-based damage on vehicles
            return;
        }

        if (ctx.world.cvars.d_saberGhoul2Collision.integer != 0
            && !ctx.entity(ent).client.is_null()
            && (*client).g2LastSurfaceTime == ctx.world.level.time
            && r#mod == meansOfDeath_t::MOD_SABER as c_int)
            || (ctx.world.cvars.d_projectileGhoul2Collision.integer != 0
                && !ctx.entity(ent).client.is_null()
                && (*client).g2LastSurfaceTime == ctx.world.level.time)
        {
            let hitSurfaceName = trap::G2API_GetSurfaceName(
                ctx.engine,
                ctx.entity(ent).ghoul2,
                (*client).g2LastSurfaceHit,
                0,
                MAX_QPATH as usize,
            );
            let hitSurface = strncpyz_string(hitSurfaceName.as_bytes(), MAX_QPATH as usize);

            if !hitSurface.is_empty() {
                G_GetHitLocFromSurfName(
                    ctx,
                    ent,
                    &hitSurface,
                    &mut hitLoc,
                    point,
                    [0.0; 3],
                    [0.0; 3],
                    meansOfDeath_t::MOD_UNKNOWN as c_int,
                );
            }
        }

        if hitLoc == -1 {
            hitLoc = G_GetHitLocation(ctx, ent, point);
        }

        match hitLoc {
            HL_FOOT_RT | HL_FOOT_LT => {
                *damage = (*damage as f64 * 0.5) as c_int;
            }
            HL_LEG_RT | HL_LEG_LT => {
                *damage = (*damage as f64 * 0.7) as c_int;
            }
            HL_WAIST | HL_BACK_RT | HL_BACK_LT | HL_BACK | HL_CHEST_RT | HL_CHEST_LT | HL_CHEST => {
                // normal damage
            }
            HL_ARM_RT | HL_ARM_LT => {
                *damage = (*damage as f64 * 0.85) as c_int;
            }
            HL_HAND_RT | HL_HAND_LT => {
                *damage = (*damage as f64 * 0.6) as c_int;
            }
            HL_HEAD => {
                *damage = (*damage as f64 * 1.3) as c_int;
            }
            _ => {
                // do nothing then
            }
        }
    }
}

/// Raven `G_ThereIsAMaster`.
///
/// Source: `oracle/codemp/game/g_combat.c:4432-4450`
pub fn G_ThereIsAMaster(ctx: &mut GameContext) -> qboolean {
    for i in 0..MAX_CLIENTS as usize {
        // This is a real client slot (i < MAX_CLIENTS), so the deref stays raw.
        let client = ctx.world.g_entities[i].client;
        if !client.is_null() && unsafe { (*client).ps.isJediMaster } != qfalse {
            return qtrue;
        }
    }
    qfalse
}

/// Raven `G_Knockdown`.
///
/// Source: `oracle/codemp/game/g_combat.c:4452-4461`
pub fn G_Knockdown(ctx: &mut GameContext, victim: Option<EntityId>) {
    let Some(victim) = victim else {
        return;
    };
    // victim may be an NPC. `.client` is a pool client, so the deref stays raw.
    let client = ctx.entity(victim).client;
    if !client.is_null() {
        unsafe {
            if BG_KnockDownable(&mut (*client).ps as *mut playerState_t) != qfalse {
                (*client).ps.forceHandExtend = HANDEXTEND_KNOCKDOWN as c_int;
                (*client).ps.forceDodgeAnim = 0;
                (*client).ps.forceHandExtendTime = ctx.world.level.time + 1100;
                (*client).ps.quickerGetup = qfalse;
            }
        }
    }
}

/// Raven `G_ApplyVehicleOtherKiller`.
///
/// Source: `oracle/codemp/game/g_combat.c:4463-4526`
pub fn G_ApplyVehicleOtherKiller(
    ctx: &mut GameContext,
    targ: Option<EntityId>,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    r#mod: c_int,
    vehicleDying: qboolean,
) {
    // targ/pilot/passenger `.client` are pool clients, and the vehicle-struct derefs (m_pVehicle, m_pPilot, m_ppPassengers) stay raw (traps 2b/2c).
    // The body keeps one unsafe scope. Entity fields route through the accessor.
    let (Some(targ), Some(attacker)) = (targ, attacker) else {
        return;
    };
    let targClient = ctx.entity(targ).client;
    if targClient.is_null() {
        return;
    }
    let targVeh = ctx.entity(targ).m_pVehicle;
    unsafe {
        if (*targClient).ps.otherKillerDebounceTime > ctx.world.level.time {
            // wait a minute, I already have a last damager
            if ctx.entity(targ).health < 0
                || (!ctx.entity(targ).m_pVehicle.is_null() && (*targVeh).m_iRemovedSurfaces != 0)
            {
                // already dying?  don't let credit transfer to anyone else
                return;
            }
            // otherwise, still alive, so, fine, use this damager...
        }

        let attacker_number = ctx.entity(attacker).s.number;
        (*targClient).ps.otherKiller = attacker_number;
        (*targClient).ps.otherKillerTime = ctx.world.level.time + 25000;
        (*targClient).ps.otherKillerDebounceTime = ctx.world.level.time + 25000;
        (*targClient).otherKillerMOD = r#mod;
        if inflictor.is_some_and(|inf| {
            let cn = ctx.entity(inf).classname_str();
            Q_stricmp("vehicle_proj", &cn) == 0
        }) {
            let inf = inflictor.unwrap();
            (*targClient).otherKillerVehWeapon = ctx.entity(inf).s.otherEntityNum2 + 1;
            (*targClient).otherKillerWeaponType = ctx.entity(inf).s.weapon;
        } else {
            (*targClient).otherKillerVehWeapon = 0;
            (*targClient).otherKillerWeaponType = WP_NONE as c_int;
        }
        if vehicleDying != qfalse {
            // propogate otherkiller down to pilot and passengers so that proper credit is given if they suicide or eject...
            if !ctx.entity(targ).m_pVehicle.is_null() {
                if !(*targVeh).m_pPilot.is_null() {
                    let pilot_num = (*((*targVeh).m_pPilot as *mut gentity_t)).s.number;
                    let pilot = EntityId(pilot_num as u32);
                    let pc = ctx.entity(pilot).client;
                    if !pc.is_null() {
                        (*pc).ps.otherKiller = (*targClient).ps.otherKiller;
                        (*pc).ps.otherKillerTime = (*targClient).ps.otherKillerTime;
                        (*pc).ps.otherKillerDebounceTime = (*targClient).ps.otherKillerDebounceTime;
                        (*pc).otherKillerMOD = (*targClient).otherKillerMOD;
                        (*pc).otherKillerVehWeapon = (*targClient).otherKillerVehWeapon;
                        (*pc).otherKillerWeaponType = (*targClient).otherKillerWeaponType;
                    }
                }
                let mut passNum = 0;
                while passNum < (*targVeh).m_iNumPassengers {
                    let pass_num = (*((*targVeh).m_ppPassengers[passNum as usize]
                        as *mut gentity_t))
                        .s
                        .number;
                    let pass = EntityId(pass_num as u32);
                    let pc = ctx.entity(pass).client;
                    if !pc.is_null() {
                        (*pc).ps.otherKiller = (*targClient).ps.otherKiller;
                        (*pc).ps.otherKillerTime = (*targClient).ps.otherKillerTime;
                        (*pc).ps.otherKillerDebounceTime = (*targClient).ps.otherKillerDebounceTime;
                        (*pc).otherKillerMOD = (*targClient).otherKillerMOD;
                        (*pc).otherKillerVehWeapon = (*targClient).otherKillerVehWeapon;
                        (*pc).otherKillerWeaponType = (*targClient).otherKillerWeaponType;
                    }
                    passNum += 1;
                }
            }
        }
    }
}

/// Raven `G_CheckVehicleNPCTeamDamage`.
///
/// Raven: `NOTE: this covers both the vehicle and NPCs riding vehicles (droids)`.
/// Source: `oracle/codemp/game/g_combat.c:4528-4544`
pub fn G_CheckVehicleNPCTeamDamage(ent: Option<&gentity_t>) -> qboolean {
    // not valid or a real client or not an NPC
    let Some(ent) = ent else {
        return qfalse;
    };
    if ent.s.number < MAX_CLIENTS as c_int || ent.s.eType != entityType_t::ET_NPC as c_int {
        return qfalse;
    }

    if ent.s.NPC_class != class_t::CLASS_VEHICLE as c_int {
        // regular NPC
        if ent.s.m_iVehicleNum != 0 {
            // an NPC in a vehicle, check for team damage
            return qtrue;
        }
    }
    qfalse
}

/// Raven `G_Damage`.
///
/// Source: `oracle/codemp/game/g_combat.c:4577-5715`
pub fn G_Damage(
    ctx: &mut GameContext,
    targ: Option<EntityId>,
    inflictor: Option<EntityId>,
    attacker: Option<EntityId>,
    mut dir: Option<&mut vec3_t>,
    point: vec3_t,
    mut damage: c_int,
    mut dflags: c_int,
    r#mod: c_int,
) {
    // The Option params carry body null-checks for all three.
    // The raw pointers re-derive verbatim, matching Raven's large mega-function shape. This is the G_Damage hub.
    unsafe {
        let base = ctx.world.g_entities.as_mut_ptr();
        let targ: *mut gentity_t = crate::ent_id::resolve(base, targ);
        let mut inflictor: *mut gentity_t = crate::ent_id::resolve(base, inflictor);
        let mut attacker: *mut gentity_t = crate::ent_id::resolve(base, attacker);
        let mut take: c_int;
        let mut save: c_int;
        let asave: c_int;
        let mut knockback: c_int;
        let max: c_int;
        let mut subamt: c_int = 0;
        let mut famt: f32 = 0.0;
        let mut hamt: f32 = 0.0;
        let mut shieldAbsorbed: f32 = 0.0;

        if !targ.is_null() && (*targ).damageRedirect != qfalse {
            G_Damage(
                ctx,
                EntityId::from_num((*targ).damageRedirectTo),
                ctx.entity_id_of(inflictor),
                ctx.entity_id_of(attacker),
                dir.as_deref_mut(),
                point,
                damage,
                dflags,
                r#mod,
            );
            return;
        }

        if r#mod == meansOfDeath_t::MOD_DEMP2 as c_int
            && !targ.is_null()
            && (*targ).inuse != qfalse
            && !(*targ).client.is_null()
        {
            let tc = (*targ).client;
            let tv = (*targ).m_pVehicle;
            if (*tc).ps.electrifyTime < ctx.world.level.time {
                // electrocution effect
                if (*targ).s.eType == entityType_t::ET_NPC as c_int
                    && (*targ).s.NPC_class == class_t::CLASS_VEHICLE as c_int
                    && !(*targ).m_pVehicle.is_null()
                    && ((*(*tv).m_pVehicleInfo).r#type == VH_SPEEDER
                        || (*(*tv).m_pVehicleInfo).r#type == VH_WALKER)
                {
                    // do some extra stuff to speeders/walkers
                    (*tc).ps.electrifyTime =
                        ctx.world.level.time + ctx.world.bg_state.rng.Q_irand(3000, 4000);
                } else if (*targ).s.NPC_class != class_t::CLASS_VEHICLE as c_int
                    || (!(*targ).m_pVehicle.is_null()
                        && (*(*tv).m_pVehicleInfo).r#type != VH_FIGHTER)
                {
                    // don't do this to fighters
                    (*tc).ps.electrifyTime =
                        ctx.world.level.time + ctx.world.bg_state.rng.Q_irand(300, 800);
                }
            }
        }

        if ctx.world.cvars.g_gametype.integer == GT_SIEGE && ctx.world.globals.gSiegeRoundBegun == 0
        {
            // nothing can be damaged til the round starts.
            return;
        }

        if (*targ).takedamage == qfalse {
            return;
        }

        if ((*targ).flags & FL_SHIELDED) != 0
            && r#mod != meansOfDeath_t::MOD_SABER as c_int
            && (*targ).client.is_null()
        {
            // magnetically protected, this thing can only be damaged by lightsabers
            return;
        }

        if ((*targ).flags & FL_DMG_BY_SABER_ONLY) != 0
            && r#mod != meansOfDeath_t::MOD_SABER as c_int
        {
            // saber-only damage
            return;
        }

        if !(*targ).client.is_null() {
            // don't take damage when in a walker, or fighter
            // unless the walker/fighter is dead!!! -rww
            let tc = (*targ).client;
            if (*tc).ps.clientNum < MAX_CLIENTS as c_int && (*tc).ps.m_iVehicleNum != 0 {
                let veh =
                    &mut ctx.world.g_entities[(*tc).ps.m_iVehicleNum as usize] as *mut gentity_t;
                let vv = (*veh).m_pVehicle;
                if !(*veh).m_pVehicle.is_null() && (*veh).health > 0 {
                    if (*(*vv).m_pVehicleInfo).r#type == VH_WALKER
                        || (*(*vv).m_pVehicleInfo).r#type == VH_FIGHTER
                    {
                        if (dflags & DAMAGE_NO_PROTECTION) == 0 {
                            return;
                        }
                    }
                }
            }
        }

        if ((*targ).flags & FL_DMG_BY_HEAVY_WEAP_ONLY) != 0 {
            // only take damage from explosives and such
            let m = r#mod;
            use meansOfDeath_t::*;
            if m != MOD_REPEATER_ALT as c_int
                && m != MOD_ROCKET as c_int
                && m != MOD_FLECHETTE_ALT_SPLASH as c_int
                && m != MOD_ROCKET_HOMING as c_int
                && m != MOD_THERMAL as c_int
                && m != MOD_THERMAL_SPLASH as c_int
                && m != MOD_TRIP_MINE_SPLASH as c_int
                && m != MOD_TIMED_MINE_SPLASH as c_int
                && m != MOD_DET_PACK_SPLASH as c_int
                && m != MOD_VEHICLE as c_int
                && m != MOD_CONC as c_int
                && m != MOD_CONC_ALT as c_int
                && m != MOD_SABER as c_int
                && m != MOD_TURBLAST as c_int
                && m != MOD_TARGET_LASER as c_int
                && m != MOD_SUICIDE as c_int
                && m != MOD_FALLING as c_int
                && m != MOD_CRUSH as c_int
                && m != MOD_TELEFRAG as c_int
                && m != MOD_COLLISION as c_int
                && m != MOD_VEH_EXPLOSION as c_int
                && m != MOD_TRIGGER_HURT as c_int
            {
                if r#mod != MOD_MELEE as c_int
                    || G_HeavyMelee(ctx, ctx.entity_id_of(attacker)) == qfalse
                {
                    // let classes with heavy melee ability damage heavy wpn dmg doors with fists
                    return;
                }
            }
        }

        if ((*targ).flags & FL_BBRUSH) != 0 {
            use meansOfDeath_t::*;
            if r#mod == MOD_DEMP2 as c_int
                || r#mod == MOD_DEMP2_ALT as c_int
                || r#mod == MOD_BRYAR_PISTOL as c_int
                || r#mod == MOD_BRYAR_PISTOL_ALT as c_int
                || r#mod == MOD_MELEE as c_int
            {
                // these don't damage bbrushes.. ever
                if r#mod != MOD_MELEE as c_int
                    || G_HeavyMelee(ctx, ctx.entity_id_of(attacker)) == qfalse
                {
                    // let classes with heavy melee ability damage breakable brushes with fists
                    return;
                }
            }
        }

        if !targ.is_null()
            && !(*targ).client.is_null()
            && (*((*targ).client)).ps.duelInProgress != 0
        {
            let tc = (*targ).client;
            if !attacker.is_null()
                && !(*attacker).client.is_null()
                && (*attacker).s.number != (*tc).ps.duelIndex
            {
                return;
            } else if !attacker.is_null()
                && !(*attacker).client.is_null()
                && r#mod != meansOfDeath_t::MOD_SABER as c_int
            {
                return;
            }
        }
        if !attacker.is_null()
            && !(*attacker).client.is_null()
            && (*((*attacker).client)).ps.duelInProgress != 0
        {
            let ac = (*attacker).client;
            if !targ.is_null()
                && !(*targ).client.is_null()
                && (*targ).s.number != (*ac).ps.duelIndex
            {
                return;
            } else if !targ.is_null()
                && !(*targ).client.is_null()
                && r#mod != meansOfDeath_t::MOD_SABER as c_int
            {
                return;
            }
        }

        if (dflags & DAMAGE_NO_PROTECTION) == 0 {
            // rage overridden by no_protection
            if !targ.is_null()
                && !(*targ).client.is_null()
                && ((*((*targ).client)).ps.fd.forcePowersActive & (1 << FP_RAGE as c_int)) != 0
            {
                damage = (damage as f64 * 0.5) as c_int;
            }
        }

        // the intermission has allready been qualified for, so don't
        // allow any extra scoring
        if ctx.world.level.intermissionQueued != 0 {
            return;
        }
        if inflictor.is_null() {
            inflictor = &mut ctx.world.g_entities[ENTITYNUM_WORLD as usize] as *mut gentity_t;
        }
        if attacker.is_null() {
            attacker = &mut ctx.world.g_entities[ENTITYNUM_WORLD as usize] as *mut gentity_t;
        }

        // shootable doors / buttons don't actually have any health
        if (*targ).s.eType == entityType_t::ET_MOVER as c_int && (*targ).genericValue4 != 1 {
            if (*targ).use_.is_some() && (*targ).moverState == MOVER_POS1 {
                crate::g_utils::GlobalUse(
                    ctx,
                    ctx.entity_id_of(targ),
                    ctx.entity_id_of(inflictor),
                    ctx.entity_id_of(attacker),
                );
            }
            return;
        }
        // reduce damage by the attacker's handicap value
        // unless they are rocket jumping
        if !(*attacker).client.is_null()
            && attacker != targ
            && (*attacker).s.eType == entityType_t::ET_PLAYER as c_int
            && ctx.world.cvars.g_gametype.integer != GT_SIEGE
        {
            max = (*((*attacker).client)).ps.stats[statIndex_t::STAT_MAX_HEALTH as usize];
            damage = damage * max / 100;
        }

        if (dflags & DAMAGE_NO_HIT_LOC) == 0 {
            // see if we should modify it by damage location
            if (*targ).inuse != qfalse
                && (!(*targ).client.is_null() || (*targ).s.eType == entityType_t::ET_NPC as c_int)
                && (*attacker).inuse != qfalse
                && (!(*attacker).client.is_null()
                    || (*attacker).s.eType == entityType_t::ET_NPC as c_int)
            {
                G_LocationBasedDamageModifier(
                    ctx,
                    ctx.entity_id_of(targ).unwrap(),
                    point,
                    r#mod,
                    dflags,
                    &mut damage,
                );
            }
        }

        if !(*targ).client.is_null()
            && (*((*targ).client)).NPC_class == class_t::CLASS_RANCOR
            && (attacker.is_null()
                || (*attacker).client.is_null()
                || (*((*attacker).client)).NPC_class != class_t::CLASS_RANCOR)
        {
            // I guess always do 10 points of damage...feel free to tweak as needed
            if damage < 10 {
                damage = 0;
            } else if damage >= 10 {
                damage = 10;
            }
        }

        let client = (*targ).client;

        if !client.is_null() {
            if (*client).noclip != qfalse {
                return;
            }
        }

        // Raven `if (!dir) dflags|=DAMAGE_NO_KNOCKBACK; else VectorNormalize(dir);`
        if let Some(d) = dir.as_deref_mut() {
            VectorNormalize(d);
        } else {
            dflags |= DAMAGE_NO_KNOCKBACK;
        }
        // This snapshots the (now-normalized) direction and its presence for the read sites below.
        // `dir_val` is zero when NULL. Those reads are gated by DAMAGE_NO_KNOCKBACK or an explicit `dir_present` branch.
        // This follows §19: the one NULL-deref path, `DirToByte`, becomes `DirToByte(vec3_origin)`.
        let dir_present = dir.is_some();
        let dir_val: vec3_t = dir.as_deref().copied().unwrap_or([0.0, 0.0, 0.0]);

        knockback = damage;
        if knockback > 200 {
            knockback = 200;
        }
        if ((*targ).flags & FL_NO_KNOCKBACK) != 0 {
            knockback = 0;
        }
        if (dflags & DAMAGE_NO_KNOCKBACK) != 0 {
            knockback = 0;
        }

        g_damage_apply_knockback(ctx, targ, attacker, dir_val, knockback, dflags, r#mod);

        if (ctx.world.cvars.g_trueJedi.integer != 0
            || ctx.world.cvars.g_gametype.integer == GT_SIEGE)
            && !client.is_null()
        {
            // less explosive damage for jedi, more saber damage for non-jedi
            use meansOfDeath_t::*;
            if (*client).ps.trueJedi != 0
                || (ctx.world.cvars.g_gametype.integer == GT_SIEGE
                    && (*client).ps.weapon == WP_SABER as c_int)
            {
                // if the target is a trueJedi, reduce splash and explosive damage to 1/2
                let m = r#mod;
                if m == MOD_REPEATER_ALT as c_int
                    || m == MOD_REPEATER_ALT_SPLASH as c_int
                    || m == MOD_DEMP2_ALT as c_int
                    || m == MOD_FLECHETTE_ALT_SPLASH as c_int
                    || m == MOD_ROCKET as c_int
                    || m == MOD_ROCKET_SPLASH as c_int
                    || m == MOD_ROCKET_HOMING as c_int
                    || m == MOD_ROCKET_HOMING_SPLASH as c_int
                    || m == MOD_THERMAL as c_int
                    || m == MOD_THERMAL_SPLASH as c_int
                    || m == MOD_TRIP_MINE_SPLASH as c_int
                    || m == MOD_TIMED_MINE_SPLASH as c_int
                    || m == MOD_DET_PACK_SPLASH as c_int
                {
                    damage = (damage as f64 * 0.75) as c_int;
                }
            } else if ((*client).ps.trueNonJedi != 0
                || (ctx.world.cvars.g_gametype.integer == GT_SIEGE
                    && (*client).ps.weapon != WP_SABER as c_int))
                && r#mod == MOD_SABER as c_int
            {
                // trueNonJedi takes more saber damage
                if damage < 100 {
                    damage *= 4;
                    if damage > 100 {
                        damage = 100;
                    }
                }
            }
        }

        if !(*attacker).client.is_null()
            && !(*targ).client.is_null()
            && ctx.world.cvars.g_gametype.integer == GT_SIEGE
            && (*((*targ).client)).siegeClass != -1
            && ((&ctx.world.bg_state.bgSiegeClasses)[(*((*targ).client)).siegeClass as usize]
                .classflags
                & (1 << CFL_STRONGAGAINSTPHYSICAL as c_int))
                != 0
        {
            // this class is flagged to take less damage from physical attacks.
            damage = (damage as f64 * 0.5) as c_int;
        }

        if g_damage_check_exempt(ctx, targ, attacker, dflags) {
            return;
        }

        // battlesuit protects from all radius damage (but takes knockback)
        // and protects 50% against all damage
        if !client.is_null() && (*client).ps.powerups[PW_BATTLESUIT as usize] != 0 {
            G_AddEvent(
                &mut *(targ),
                entity_event_t::EV_POWERUP_BATTLESUIT as c_int,
                0,
            );
            if (dflags & DAMAGE_RADIUS) != 0 || r#mod == meansOfDeath_t::MOD_FALLING as c_int {
                return;
            }
            damage = (damage as f64 * 0.5) as c_int;
        }

        // add to the attacker's hit counter (if the target isn't a general entity like a prox mine)
        if !(*attacker).client.is_null()
            && targ != attacker
            && (*targ).health > 0
            && (*targ).s.eType != entityType_t::ET_MISSILE as c_int
            && (*targ).s.eType != entityType_t::ET_GENERAL as c_int
            && !client.is_null()
        {
            let ac = (*attacker).client;
            if crate::g_team::OnSameTeam(ctx, ctx.entity_id_of(targ), ctx.entity_id_of(attacker))
                != qfalse
            {
                (*ac).ps.persistant[persEnum_t::PERS_HITS as usize] -= 1;
            } else {
                (*ac).ps.persistant[persEnum_t::PERS_HITS as usize] += 1;
            }
            (*ac).ps.persistant[persEnum_t::PERS_ATTACKEE_ARMOR as usize] =
                ((*targ).health << 8) | ((*client).ps.stats[statIndex_t::STAT_ARMOR as usize]);
        }

        // always give half damage if hurting self... but not in siege.  Heavy weapons need a counter.
        // calculated after knockback, so rocket jumping works
        if targ == attacker && (dflags & DAMAGE_NO_SELF_PROTECTION) == 0 {
            if ctx.world.cvars.g_gametype.integer == GT_SIEGE {
                damage = (damage as f64 * 1.5) as c_int;
            } else {
                damage = (damage as f64 * 0.5) as c_int;
            }
        }

        if damage < 1 {
            damage = 1;
        }

        // store the attacker as the potential killer in case we die by other means
        if !(*targ).client.is_null() && !attacker.is_null() && attacker != targ {
            let tc = (*targ).client;
            if (*targ).s.eType == entityType_t::ET_NPC as c_int
                && (*targ).s.NPC_class == class_t::CLASS_VEHICLE as c_int
            {
                // vehicle
                G_ApplyVehicleOtherKiller(
                    ctx,
                    ctx.entity_id_of(targ),
                    ctx.entity_id_of(inflictor),
                    ctx.entity_id_of(attacker),
                    r#mod,
                    qfalse,
                );
            } else {
                // other client
                (*tc).ps.otherKiller = (*attacker).s.number;
                (*tc).ps.otherKillerTime = ctx.world.level.time + 5000;
                (*tc).ps.otherKillerDebounceTime = ctx.world.level.time + 100;
                (*tc).otherKillerMOD = r#mod;
                (*tc).otherKillerVehWeapon = 0;
                (*tc).otherKillerWeaponType = WP_NONE as c_int;
            }
        }

        take = damage;
        save = 0;

        // save some from armor
        asave = CheckArmor(ctx, ctx.entity_id_of(targ).unwrap(), take, dflags);

        if asave != 0 {
            shieldAbsorbed = asave as f32;
        }

        take -= asave;
        if !(*targ).client.is_null() {
            // update vehicle shields and armor, check for explode
            let tc = (*targ).client;
            let tv = (*targ).m_pVehicle;
            if (*tc).NPC_class == class_t::CLASS_VEHICLE && !(*targ).m_pVehicle.is_null() {
                let mut surface: c_int = -1;
                if !attacker.is_null() {
                    // so we know the last guy who shot at us
                    (*targ).enemy = Some(ent_id(base, attacker));
                }

                (*tv).m_iShields = (*tc).ps.stats[statIndex_t::STAT_ARMOR as usize];
                crate::g_vehicles::G_VehUpdateShields(&mut *targ);
                (*tv).m_iArmor -= take;
                if (*tv).m_iArmor <= 0 {
                    (*targ).s.eFlags |= EF_DEAD;
                    (*tc).ps.eFlags |= EF_DEAD;
                    (*tv).m_iArmor = 0;
                }
                if (*(*tv).m_pVehicleInfo).r#type == VH_FIGHTER {
                    // get the last surf that was hit
                    if !(*targ).client.is_null() && (*tc).g2LastSurfaceTime == ctx.world.level.time
                    {
                        let hitSurfaceName = trap::G2API_GetSurfaceName(
                            ctx.engine,
                            (*targ).ghoul2,
                            (*tc).g2LastSurfaceHit,
                            0,
                            MAX_QPATH as usize,
                        );
                        let hitSurface =
                            strncpyz_string(hitSurfaceName.as_bytes(), MAX_QPATH as usize);

                        if !hitSurface.is_empty() {
                            surface = G_ShipSurfaceForSurfName(&hitSurface);

                            if take != 0 && surface > 0 {
                                // hit a certain part of the ship
                                let mut deathPoint: c_int = 0;

                                (*targ).locationDamage[surface as usize] += take;

                                match surface {
                                    SHIPSURF_FRONT => {
                                        deathPoint = (*(*tv).m_pVehicleInfo).health_front;
                                    }
                                    SHIPSURF_BACK => {
                                        deathPoint = (*(*tv).m_pVehicleInfo).health_back;
                                    }
                                    SHIPSURF_RIGHT => {
                                        deathPoint = (*(*tv).m_pVehicleInfo).health_right;
                                    }
                                    SHIPSURF_LEFT => {
                                        deathPoint = (*(*tv).m_pVehicleInfo).health_left;
                                    }
                                    _ => {}
                                }

                                if deathPoint != 0 {
                                    if (*targ).locationDamage[surface as usize] >= deathPoint {
                                        // this area of the ship is now dead
                                        let wasDying = (*tv).m_iRemovedSurfaces != 0;
                                        if crate::g_vehicles::G_FlyVehicleDestroySurface(
                                            ctx,
                                            ctx.entity_id_of(targ).unwrap(),
                                            surface,
                                        ) != qfalse
                                        {
                                            crate::g_vehicles::G_VehicleSetDamageLocFlags(
                                                ctx,
                                                ctx.entity_id_of(targ).unwrap(),
                                                surface,
                                                deathPoint,
                                            );
                                            if !wasDying {
                                                (*tc).ps.otherKillerDebounceTime = 0;
                                            }
                                            G_ApplyVehicleOtherKiller(
                                                ctx,
                                                ctx.entity_id_of(targ),
                                                ctx.entity_id_of(inflictor),
                                                ctx.entity_id_of(attacker),
                                                r#mod,
                                                qtrue,
                                            );
                                        }
                                    } else {
                                        crate::g_vehicles::G_VehicleSetDamageLocFlags(
                                            ctx,
                                            ctx.entity_id_of(targ).unwrap(),
                                            surface,
                                            deathPoint,
                                        );
                                    }
                                }
                            }
                        }
                    }
                }
                if (*(*tv).m_pVehicleInfo).r#type != VH_ANIMAL {
                    if !attacker.is_null()
                        && targ != attacker
                        && !VectorCompare((*tc).ps.origin, point)
                        && (*tv).m_LandTrace.fraction >= 1.0
                    {
                        // just took a hit, knock us around
                        let mut vUp: vec3_t = [0.0; 3];
                        let mut impactDir: vec3_t = [0.0; 3];
                        let mut impactStrength = (damage as f32 / 200.0) * 10.0;
                        let mut dot: f32;
                        if impactStrength > 10.0 {
                            impactStrength = 10.0;
                        }
                        // pitch or roll us based on where we were hit
                        AngleVectors(
                            *((*tv).m_vOrientation as *const vec3_t),
                            None,
                            None,
                            Some(&mut vUp),
                        );
                        _VectorSubtract(point, (*targ).r.currentOrigin, &mut impactDir);
                        VectorNormalize(&mut impactDir);
                        if surface <= 0 {
                            // no surf guess where we were hit, then
                            let mut vFwd: vec3_t = [0.0; 3];
                            let mut vRight: vec3_t = [0.0; 3];
                            AngleVectors(
                                *((*tv).m_vOrientation as *const vec3_t),
                                Some(&mut vFwd),
                                Some(&mut vRight),
                                Some(&mut vUp),
                            );
                            dot = _DotProduct(vRight, impactDir);
                            if dot > 0.4 {
                                surface = SHIPSURF_RIGHT;
                            } else if dot < -0.4 {
                                surface = SHIPSURF_LEFT;
                            } else {
                                dot = _DotProduct(vFwd, impactDir);
                                if dot > 0.0 {
                                    surface = SHIPSURF_FRONT;
                                } else {
                                    surface = SHIPSURF_BACK;
                                }
                            }
                        }
                        match surface {
                            SHIPSURF_FRONT => {
                                dot = _DotProduct(vUp, impactDir);
                                if dot > 0.0 {
                                    *(*tv).m_vOrientation.add(PITCH) += impactStrength;
                                } else {
                                    *(*tv).m_vOrientation.add(PITCH) -= impactStrength;
                                }
                            }
                            SHIPSURF_BACK => {
                                dot = _DotProduct(vUp, impactDir);
                                if dot > 0.0 {
                                    *(*tv).m_vOrientation.add(PITCH) -= impactStrength;
                                } else {
                                    *(*tv).m_vOrientation.add(PITCH) += impactStrength;
                                }
                            }
                            SHIPSURF_RIGHT => {
                                dot = _DotProduct(vUp, impactDir);
                                if dot > 0.0 {
                                    *(*tv).m_vOrientation.add(ROLL) -= impactStrength;
                                } else {
                                    *(*tv).m_vOrientation.add(ROLL) += impactStrength;
                                }
                            }
                            SHIPSURF_LEFT => {
                                dot = _DotProduct(vUp, impactDir);
                                if dot > 0.0 {
                                    *(*tv).m_vOrientation.add(ROLL) += impactStrength;
                                } else {
                                    *(*tv).m_vOrientation.add(ROLL) -= impactStrength;
                                }
                            }
                            _ => {}
                        }
                    }
                }
            }
        }

        if r#mod == meansOfDeath_t::MOD_DEMP2 as c_int
            || r#mod == meansOfDeath_t::MOD_DEMP2_ALT as c_int
        {
            if !client.is_null() {
                let tv = (*targ).m_pVehicle;
                if (*client).NPC_class == class_t::CLASS_VEHICLE
                    && !(*targ).m_pVehicle.is_null()
                    && !(*tv).m_pVehicleInfo.is_null()
                    && (*(*tv).m_pVehicleInfo).r#type == VH_FIGHTER
                {
                    // all damage goes into the disruption of shields and systems
                    take = 0;
                } else {
                    if (*client).jetPackOn != qfalse {
                        // disable jetpack temporarily
                        crate::g_items::Jetpack_Off(&mut *targ);
                        (*client).jetPackToggleTime =
                            ctx.world.level.time + ctx.world.bg_state.rng.Q_irand(3000, 10000);
                    }

                    if (*client).NPC_class == class_t::CLASS_PROTOCOL
                        || (*client).NPC_class == class_t::CLASS_SEEKER
                        || (*client).NPC_class == class_t::CLASS_R2D2
                        || (*client).NPC_class == class_t::CLASS_R5D2
                        || (*client).NPC_class == class_t::CLASS_MOUSE
                        || (*client).NPC_class == class_t::CLASS_GONK
                    {
                        take *= 2;
                    } else if (*client).NPC_class == class_t::CLASS_PROBE
                        || (*client).NPC_class == class_t::CLASS_INTERROGATOR
                        || (*client).NPC_class == class_t::CLASS_MARK1
                        || (*client).NPC_class == class_t::CLASS_MARK2
                        || (*client).NPC_class == class_t::CLASS_SENTRY
                        || (*client).NPC_class == class_t::CLASS_ATST
                    {
                        take *= 5;
                    } else {
                        if take > 0 {
                            take /= 3;
                            if take < 1 {
                                take = 1;
                            }
                        }
                    }
                }
            }
        }

        if ctx.world.cvars.g_debugDamage.integer != 0 {
            let s = format!(
                "{}: client:{} health:{} damage:{} armor:{}\n",
                ctx.world.level.time,
                (*targ).s.number,
                (*targ).health,
                take,
                asave
            );
            crate::g_main::G_Printf(ctx, &s);
        }

        // add to the damage inflicted on a player this frame
        if !client.is_null() {
            if !attacker.is_null() {
                (*client).ps.persistant[persEnum_t::PERS_ATTACKER as usize] = (*attacker).s.number;
            } else {
                (*client).ps.persistant[persEnum_t::PERS_ATTACKER as usize] = ENTITYNUM_WORLD;
            }
            (*client).damage_armor += asave;
            (*client).damage_blood += take;
            (*client).damage_knockback += knockback;
            // Raven `if (dir) { VectorCopy(dir, damage_from); fromWorld=qfalse }
            // else { VectorCopy(targ->r.currentOrigin, damage_from); fromWorld=qtrue }`.
            if dir_present {
                _VectorCopy(dir_val, &mut (*client).damage_from);
                (*client).damage_fromWorld = qfalse;
            } else {
                _VectorCopy((*targ).r.currentOrigin, &mut (*client).damage_from);
                (*client).damage_fromWorld = qtrue;
            }

            if !attacker.is_null() && !(*attacker).client.is_null() {
                crate::ai_main::BotDamageNotification(
                    ctx,
                    ctx.entity_id_of(targ).unwrap(),
                    ctx.entity_id_of(attacker),
                );
            } else if !inflictor.is_null() && !(*inflictor).client.is_null() {
                crate::ai_main::BotDamageNotification(
                    ctx,
                    ctx.entity_id_of(targ).unwrap(),
                    ctx.entity_id_of(inflictor),
                );
            }
        }

        // See if it's the player hurting the emeny flag carrier
        if ctx.world.cvars.g_gametype.integer == GT_CTF
            || ctx.world.cvars.g_gametype.integer == GT_CTY
        {
            crate::g_team::Team_CheckHurtCarrier(
                ctx,
                ctx.entity_id_of(targ),
                ctx.entity_id_of(attacker),
            );
        }

        if !(*targ).client.is_null() {
            // set the last client who damaged the target
            let tc = (*targ).client;
            (*tc).lasthurt_client = (*attacker).s.number;
            (*tc).lasthurt_mod = r#mod;
        }

        if (dflags & DAMAGE_NO_PROTECTION) == 0 {
            if take != 0
                && !(*targ).client.is_null()
                && ((*((*targ).client)).ps.fd.forcePowersActive & (1 << FP_PROTECT as c_int)) != 0
            {
                let tc = (*targ).client;
                if (*tc).ps.fd.forcePower != 0 {
                    let mut maxtake = take;

                    if (*tc).forcePowerSoundDebounce < ctx.world.level.time {
                        crate::w_force::G_PreDefSound(
                            ctx,
                            (*tc).ps.origin,
                            (PDSOUND_PROTECTHIT) as i32,
                        );
                        (*tc).forcePowerSoundDebounce = ctx.world.level.time + 400;
                    }

                    if (*tc).ps.fd.forcePowerLevel[FP_PROTECT as usize] == FORCE_LEVEL_1 {
                        famt = 1.0;
                        hamt = 0.40;
                        if maxtake > 100 {
                            maxtake = 100;
                        }
                    } else if (*tc).ps.fd.forcePowerLevel[FP_PROTECT as usize] == FORCE_LEVEL_2 {
                        famt = 0.5;
                        hamt = 0.60;
                        if maxtake > 200 {
                            maxtake = 200;
                        }
                    } else if (*tc).ps.fd.forcePowerLevel[FP_PROTECT as usize] == FORCE_LEVEL_3 {
                        famt = 0.25;
                        hamt = 0.80;
                        if maxtake > 400 {
                            maxtake = 400;
                        }
                    }

                    if (*tc).ps.powerups[PW_FORCE_BOON as usize] == 0 {
                        (*tc).ps.fd.forcePower =
                            ((*tc).ps.fd.forcePower as f32 - (maxtake as f32 * famt)) as c_int;
                    } else {
                        (*tc).ps.fd.forcePower = ((*tc).ps.fd.forcePower as f32
                            - (maxtake as f32 * famt) / 2.0)
                            as c_int;
                    }
                    subamt = ((maxtake as f32 * hamt) + (take - maxtake) as f32) as c_int;
                    if (*tc).ps.fd.forcePower < 0 {
                        subamt += (*tc).ps.fd.forcePower;
                        (*tc).ps.fd.forcePower = 0;
                    }
                    if subamt != 0 {
                        take -= subamt;

                        if take < 0 {
                            take = 0;
                        }
                    }
                }
            }
        }

        if shieldAbsorbed != 0.0 {
            let evEnt_eid = G_TempEntity(
                ctx,
                (*targ).r.currentOrigin,
                entity_event_t::EV_SHIELD_HIT as c_int,
            );
            let evEnt = ctx.entity_mut(evEnt_eid) as *mut gentity_t;
            (*evEnt).s.otherEntityNum = (*targ).s.number;
            // Raven passes `dir` here even when it may be NULL (which would UB-deref).
            // `dir_val` is the zero vector when absent.
            (*evEnt).s.eventParm = DirToByte(dir_val);
            (*evEnt).s.time2 = shieldAbsorbed as c_int;
        }

        // do the damage
        if take != 0 {
            if !(*targ).client.is_null()
                && (*targ).s.number < MAX_CLIENTS as c_int
                && (r#mod == meansOfDeath_t::MOD_DEMP2 as c_int
                    || r#mod == meansOfDeath_t::MOD_DEMP2_ALT as c_int)
            {
                let tc = (*targ).client;
                if (*tc).ps.weaponTime <= 0 {
                    (*tc).ps.weaponTime = 2000;
                    (*tc).ps.electrifyTime = ctx.world.level.time + 2000;
                    if (*tc).ps.weaponstate == WEAPON_CHARGING as c_int
                        || (*tc).ps.weaponstate == WEAPON_CHARGING_ALT as c_int
                    {
                        (*tc).ps.weaponstate = WEAPON_READY as c_int;
                    }
                }
            }

            if (dflags & DAMAGE_NO_PROTECTION) == 0 {
                if !(*targ).client.is_null()
                    && ((*((*targ).client)).ps.fd.forcePowersActive & (1 << FP_RAGE as c_int)) != 0
                    && (!(*inflictor).client.is_null() || !(*attacker).client.is_null())
                {
                    let tc = (*targ).client;
                    take /= (*tc).ps.fd.forcePowerLevel[FP_RAGE as usize] + 1;
                }
            }
            (*targ).health = (*targ).health - take;

            if ((*targ).flags & FL_UNDYING) != 0 {
                if (*targ).health < 1 {
                    (*targ).health = 1;
                }
            }

            if !(*targ).client.is_null() {
                (*((*targ).client)).ps.stats[statIndex_t::STAT_HEALTH as usize] = (*targ).health;
            }

            if (dflags & DAMAGE_NO_PROTECTION) == 0 {
                if !(*targ).client.is_null()
                    && ((*((*targ).client)).ps.fd.forcePowersActive & (1 << FP_RAGE as c_int)) != 0
                    && (!(*inflictor).client.is_null() || !(*attacker).client.is_null())
                {
                    let tc = (*targ).client;
                    if (*targ).health <= 0 {
                        (*targ).health = 1;
                    }
                    if (*tc).ps.stats[statIndex_t::STAT_HEALTH as usize] <= 0 {
                        (*tc).ps.stats[statIndex_t::STAT_HEALTH as usize] = 1;
                    }
                }
            }

            // We want to go ahead and set gPainHitLoc regardless of if we have a pain func,
            if !(*targ).client.is_null()
                && !(*targ).ghoul2.is_null()
                && (*((*targ).client)).g2LastSurfaceTime == ctx.world.level.time
            {
                let tc = (*targ).client;
                let hitSurfaceName = trap::G2API_GetSurfaceName(
                    ctx.engine,
                    (*targ).ghoul2,
                    (*tc).g2LastSurfaceHit,
                    0,
                    MAX_QPATH as usize,
                );
                let hitSurface = strncpyz_string(hitSurfaceName.as_bytes(), MAX_QPATH as usize);

                if !hitSurface.is_empty() {
                    let targ_eid = ctx.entity_id_of(targ).unwrap();
                    // This is irreducible.
                    // The `&mut world.globals.gPainHitLoc` out-param aliases the ctx passed to the same `G_GetHitLocFromSurfName` call.
                    let hit_loc_out = &mut (*ctx.world_raw()).globals.gPainHitLoc;
                    G_GetHitLocFromSurfName(
                        ctx,
                        targ_eid,
                        &hitSurface,
                        hit_loc_out,
                        point,
                        dir_val,
                        [0.0; 3],
                        r#mod,
                    );
                } else {
                    ctx.world.globals.gPainHitLoc = -1;
                }

                if ctx.world.globals.gPainHitLoc < (crate::entity::hit_location::HL_MAX) as i32
                    && ctx.world.globals.gPainHitLoc >= 0
                    && (*targ).locationDamage[ctx.world.globals.gPainHitLoc as usize] < Q3_INFINITE
                    && ((*targ).s.eType == entityType_t::ET_PLAYER as c_int
                        || (*targ).s.NPC_class != class_t::CLASS_VEHICLE as c_int)
                {
                    (*targ).locationDamage[ctx.world.globals.gPainHitLoc as usize] += take;

                    if ctx.world.cvars.g_armBreakage.integer != 0
                        && (*tc).ps.brokenLimbs == 0
                        && (*tc).ps.stats[statIndex_t::STAT_HEALTH as usize] > 0
                        && (*targ).health > 0
                        && ((*targ).s.eFlags & EF_DEAD) == 0
                    {
                        if (*targ).locationDamage[HL_ARM_RT as usize]
                            + (*targ).locationDamage[HL_HAND_RT as usize]
                            >= 80
                        {
                            crate::g_client::G_BreakArm(
                                ctx,
                                ctx.entity_id_of(targ).unwrap(),
                                (BROKENLIMB_RARM) as i32,
                            );
                        } else if (*targ).locationDamage[HL_ARM_LT as usize]
                            + (*targ).locationDamage[HL_HAND_LT as usize]
                            >= 80
                        {
                            crate::g_client::G_BreakArm(
                                ctx,
                                ctx.entity_id_of(targ).unwrap(),
                                (BROKENLIMB_LARM) as i32,
                            );
                        }
                    }
                }
            } else {
                ctx.world.globals.gPainHitLoc = -1;
            }

            if (*targ).maxHealth != 0 {
                crate::g_utils::G_ScaleNetHealth(&mut *(targ));
            }

            if (*targ).health <= 0 {
                if !client.is_null() {
                    (*targ).flags |= FL_NO_KNOCKBACK;

                    // Raven's `if (point) ... else VectorCopy(ps.origin, pos1)`.
                    // Zero vector is the NULL sentinel.
                    // For no-point deaths, for example falling, this records the victim's origin for the death anim.
                    if point != [0.0, 0.0, 0.0] {
                        _VectorCopy(point, &mut (*targ).pos1);
                    } else {
                        _VectorCopy((*((*targ).client)).ps.origin, &mut (*targ).pos1);
                    }
                } else if (*targ).s.eType == entityType_t::ET_NPC as c_int {
                    // g2animent
                    _VectorCopy(point, &mut (*targ).pos1);
                }

                if (*targ).health < -999 {
                    (*targ).health = -999;
                }

                // If we are a breaking glass brush, store the damage point so we can do cool things with it.
                if ((*targ).r.svFlags & SVF_GLASS_BRUSH) != 0 {
                    _VectorCopy(point, &mut (*targ).pos1);
                    // Raven `if (dir) VectorCopy(dir,pos2); else VectorClear(pos2);`
                    if dir_present {
                        _VectorCopy(dir_val, &mut (*targ).pos2);
                    } else {
                        (*targ).pos2 = [0.0, 0.0, 0.0];
                    }
                }

                if (*targ).s.eType == entityType_t::ET_NPC as c_int
                    && !(*targ).client.is_null()
                    && ((*targ).s.eFlags & EF_DEAD) != 0
                {
                    // an NPC that's already dead. Maybe we can cut some more limbs off!
                    if (r#mod == meansOfDeath_t::MOD_SABER as c_int
                        || (r#mod == meansOfDeath_t::MOD_MELEE as c_int
                            && G_HeavyMelee(ctx, ctx.entity_id_of(attacker)) != qfalse))
                        && take > 2
                        && (dflags & DAMAGE_NO_DISMEMBER) == 0
                    {
                        G_CheckForDismemberment(
                            ctx,
                            ctx.entity_id_of(targ).unwrap(),
                            ctx.entity_id_of(attacker),
                            (*targ).pos1,
                            take,
                            (*((*targ).client)).ps.torsoAnim,
                            qtrue,
                        );
                    }
                }

                (*targ).enemy = Some(ent_id(base, attacker));
                if let Some(die_fn) = (*targ).die.get() {
                    crate::ent_fn_enums::dispatch_die(
                        ctx, die_fn, targ, inflictor, attacker, take, r#mod,
                    );
                }
                G_ActivateBehavior(ctx, ctx.entity_id_of(targ), bSet_t::BSET_DEATH as c_int);
                return;
            } else {
                if ctx.world.cvars.g_debugMelee.integer != 0 {
                    // getting hurt makes you let go of the wall
                    if !(*targ).client.is_null()
                        && ((*((*targ).client)).ps.pm_flags & PMF_STUCK_TO_WALL) != 0
                    {
                        crate::w_force::G_LetGoOfWall(ctx, ctx.entity_id_of(targ));
                    }
                }
                if (*targ).pain.is_some() {
                    if (*targ).s.eType != entityType_t::ET_NPC as c_int
                        || r#mod != meansOfDeath_t::MOD_SABER as c_int
                        || take > 1
                    {
                        // don't even notify NPCs of pain if it's just idle saber damage
                        ctx.world.globals.gPainMOD = r#mod;
                        // Raven's `if (point) ... else VectorCopy(r.currentOrigin, gPainPoint)`.
                        // Zero vector is the NULL sentinel.
                        if point != [0.0, 0.0, 0.0] {
                            ctx.world.globals.gPainPoint = point;
                        } else {
                            _VectorCopy((*targ).r.currentOrigin, &mut ctx.world.globals.gPainPoint);
                        }
                        if let Some(pain_fn) = (*targ).pain.get() {
                            crate::ent_fn_enums::dispatch_pain(ctx, pain_fn, targ, attacker, take);
                        }
                    }
                }
            }

            crate::g_log::G_LogWeaponDamage(ctx, (*attacker).s.number, r#mod, take);
        }
    }
}

/// Raven `G_DamageFromKiller`.
///
/// Source: `oracle/codemp/game/g_combat.c:5717-5758`
pub fn G_DamageFromKiller(
    ctx: &mut GameContext,
    pEnt: Option<EntityId>,
    pVehEnt: Option<EntityId>,
    attacker: Option<EntityId>,
    org: vec3_t,
    damage: c_int,
    dflags: c_int,
    mut r#mod: c_int,
) {
    let mut killer = attacker;
    let mut inflictor = attacker;
    let mut tempInflictor: qboolean = qfalse;
    let (Some(pEnt), Some(pVehEnt)) = (pEnt, pVehEnt) else {
        return;
    };
    // pVehEnt's `.client` is a pool client, so the deref stays raw.
    let vc = ctx.entity(pVehEnt).client;
    if vc.is_null() {
        return;
    }
    if unsafe { (*vc).ps.otherKiller } < ENTITYNUM_WORLD
        && unsafe { (*vc).ps.otherKillerTime } > ctx.world.level.time
    {
        let potentialKiller = EntityId(unsafe { (*vc).ps.otherKiller } as u32);

        if ctx.entity(potentialKiller).inuse != qfalse {
            // he's valid I guess
            killer = Some(potentialKiller);
            r#mod = unsafe { (*vc).otherKillerMOD };
            inflictor = killer;
            if unsafe { (*vc).otherKillerVehWeapon } > 0 {
                let inflictor_ptr = crate::g_utils::G_Spawn(ctx);
                inflictor = Some(inflictor_ptr);
                if let Some(inflictor_id) = inflictor {
                    // fake up the inflictor
                    tempInflictor = qtrue;
                    ctx.ent_set(inflictor_id, PrefixSet::ClassnameStatic(c"vehicle_proj"));
                    let vw = unsafe { (*vc).otherKillerVehWeapon };
                    ctx.entity_mut(inflictor_id).s.otherEntityNum2 = vw - 1;
                    let wt = unsafe { (*vc).otherKillerWeaponType };
                    ctx.entity_mut(inflictor_id).s.weapon = wt;
                }
            }
        }
    }
    // FIXME: damage hitEnt, some, too?  Our explosion should hurt them some, but...
    if let Some(killer_id) = killer {
        let veh = ctx.entity(killer_id).m_pVehicle;
        if ctx.entity(killer_id).s.eType == entityType_t::ET_NPC as c_int
            && ctx.entity(killer_id).s.NPC_class == class_t::CLASS_VEHICLE as c_int
            && !veh.is_null()
            && !unsafe { (*veh).m_pPilot }.is_null()
        {
            // vehicle-struct/pilot pointers deref raw.
            let pilot_ptr = unsafe { (*veh).m_pPilot } as *mut gentity_t;
            killer = ctx.entity_id_of(pilot_ptr);
        }
    }
    G_Damage(
        ctx,
        Some(pEnt),
        inflictor,
        killer,
        None,
        org,
        damage,
        dflags,
        r#mod,
    );
    if tempInflictor != qfalse {
        crate::g_utils::G_FreeEntity(ctx, inflictor);
    }
}

/// Raven `CanDamage`.
///
/// `origin` is read-only (a trace start point), so it stays by-value.
/// Source: `oracle/codemp/game/g_combat.c:5768-5815`
pub fn CanDamage(ctx: &mut GameContext, targ: EntityId, origin: vec3_t) -> bool {
    // `core::mem::zeroed` keeps a tight unsafe scope.
    // The trap Trace macro and entity reads route through the accessor.
    unsafe {
        let mut tr: trace_t = core::mem::zeroed();
        let mut midpoint: vec3_t = [0.0; 3];
        // Raven passes `vec3_origin` (zero vector) as the trace bbox.
        let zero: vec3_t = [0.0; 3];

        // use the midpoint of the bounds instead of the origin, because
        // bmodels may have their origin is 0,0,0
        for i in 0..3 {
            midpoint[i] = (ctx.entity(targ).r.absmin[i] + ctx.entity(targ).r.absmax[i]) * 0.5;
        }

        // This inlines `trap_Trace(&tr, origin, vec3_origin, vec3_origin, dest, ENTITYNUM_NONE, MASK_SOLID)`, one per candidate destination.
        macro_rules! trace_to {
            ($dest:expr) => {
                trap::Trace(
                    ctx.engine,
                    GTraceArgs::new(
                        &mut tr as *mut trace_t,
                        &origin as *const vec3_t,
                        &zero as *const vec3_t,
                        &zero as *const vec3_t,
                        &$dest as *const vec3_t,
                        ENTITYNUM_NONE,
                        MASK_SOLID,
                    ),
                )
            };
        }

        let mut dest = midpoint;
        trace_to!(dest);
        if tr.fraction == 1.0 || tr.entityNum as c_int == ctx.entity(targ).s.number {
            return true;
        }

        // this should probably check in the plane of projection,
        // rather than in world coordinate, and also include Z
        dest = midpoint;
        dest[0] += 15.0;
        dest[1] += 15.0;
        trace_to!(dest);
        if tr.fraction == 1.0 {
            return true;
        }

        dest = midpoint;
        dest[0] += 15.0;
        dest[1] -= 15.0;
        trace_to!(dest);
        if tr.fraction == 1.0 {
            return true;
        }

        dest = midpoint;
        dest[0] -= 15.0;
        dest[1] += 15.0;
        trace_to!(dest);
        if tr.fraction == 1.0 {
            return true;
        }

        dest = midpoint;
        dest[0] -= 15.0;
        dest[1] -= 15.0;
        trace_to!(dest);
        if tr.fraction == 1.0 {
            return true;
        }

        false
    }
}

/// Raven `G_RadiusDamage`.
///
/// Source: `oracle/codemp/game/g_combat.c:5823-5943`
pub fn G_RadiusDamage(
    ctx: &mut GameContext,
    origin: vec3_t,
    attacker: Option<EntityId>,
    damage: f32,
    mut radius: f32,
    ignore: Option<EntityId>,
    missile: Option<EntityId>,
    r#mod: c_int,
) -> bool {
    let mut entityList: [c_int; MAX_GENTITIES as usize] = [0; MAX_GENTITIES as usize];
    let mut mins: vec3_t = [0.0; 3];
    let mut maxs: vec3_t = [0.0; 3];
    let mut v: vec3_t = [0.0; 3];
    let mut dir: vec3_t = [0.0; 3];
    let mut hitClient = false;
    let roastPeople = false;

    // (missile burn-mark selection block is #if 0'd out in Raven)

    if radius < 1.0 {
        radius = 1.0;
    }

    for i in 0..3 {
        mins[i] = origin[i] - radius;
        maxs[i] = origin[i] + radius;
    }

    let numListedEntities = trap::EntitiesInBox(
        ctx.engine,
        mp_abi::game::syscalls::G_ENTITIES_IN_BOX::GEntitiesInBoxArgs::new(
            &mins as *const vec3_t,
            &maxs as *const vec3_t,
            entityList.as_mut_ptr(),
            (MAX_GENTITIES) as i32,
        ),
    );

    for e in 0..numListedEntities {
        let ent = EntityId(entityList[e as usize] as u32);

        if Some(ent) == ignore {
            continue;
        }
        if ctx.entity(ent).takedamage == qfalse {
            continue;
        }

        // find the distance from the edge of the bounding box
        for i in 0..3 {
            if origin[i] < ctx.entity(ent).r.absmin[i] {
                v[i] = ctx.entity(ent).r.absmin[i] - origin[i];
            } else if origin[i] > ctx.entity(ent).r.absmax[i] {
                v[i] = origin[i] - ctx.entity(ent).r.absmax[i];
            } else {
                v[i] = 0.0;
            }
        }

        let dist = VectorLength(v);
        if dist >= radius {
            continue;
        }

        // C's `1.0` is a double literal, so the subtract and multiply run in f64 (narrowed at the store).
        // `dist / radius` is still an f32 divide.
        let points = (damage as f64 * (1.0 - (dist / radius) as f64)) as f32;

        if CanDamage(ctx, ent, origin) {
            if LogAccuracyHit(ctx, ent, attacker) {
                hitClient = true;
            }
            let currentOrigin = ctx.entity(ent).r.currentOrigin;
            _VectorSubtract(currentOrigin, origin, &mut dir);
            // push the center of mass higher than the origin so players
            // get knocked into the air more
            dir[2] += 24.0;
            // Match the oracle's short-circuit ordering: `attacker->m_pVehicle`
            // must only be read after the `attacker` / `m_pVehicle` null guards.
            // The vehicle-struct/pilot pointers deref raw.
            let use_pilot = match attacker {
                Some(attacker_id) => {
                    let veh = ctx.entity(attacker_id).m_pVehicle;
                    ctx.entity(attacker_id).inuse != qfalse
                        && !ctx.entity(attacker_id).client.is_null()
                        && ctx.entity(attacker_id).s.eType == entityType_t::ET_NPC as c_int
                        && ctx.entity(attacker_id).s.NPC_class == class_t::CLASS_VEHICLE as c_int
                        && !veh.is_null()
                        && !unsafe { (*veh).m_pPilot }.is_null()
                }
                None => false,
            };
            if use_pilot {
                let attacker_id = attacker.unwrap();
                let veh = ctx.entity(attacker_id).m_pVehicle;
                let pilot_ptr = unsafe { (*veh).m_pPilot } as *mut gentity_t;
                let pilot = ctx.entity_id_of(pilot_ptr);
                // say my pilot did it.
                G_Damage(
                    ctx,
                    Some(ent),
                    missile,
                    pilot,
                    Some(&mut dir),
                    origin,
                    points as c_int,
                    DAMAGE_RADIUS,
                    r#mod,
                );
            } else {
                G_Damage(
                    ctx,
                    Some(ent),
                    missile,
                    attacker,
                    Some(&mut dir),
                    origin,
                    points as c_int,
                    DAMAGE_RADIUS,
                    r#mod,
                );
            }

            let roast = !ctx.entity(ent).client.is_null()
                && roastPeople
                && missile.is_some_and(|missile_id| {
                    !VectorCompare(
                        ctx.entity(ent).r.currentOrigin,
                        ctx.entity(missile_id).r.currentOrigin,
                    )
                });
            if roast {
                let missile_id = missile.unwrap();
                // the thing calling this function can create burn marks on people, so create an event to do so
                let ent_origin = ctx.entity(ent).r.currentOrigin;
                let ev_id = G_TempEntity(ctx, ent_origin, entity_event_t::EV_GHOUL2_MARK as c_int);

                let ent_number = ctx.entity(ent).s.number;
                let missile_origin = ctx.entity(missile_id).r.currentOrigin;
                let evEnt = ctx.entity_mut(ev_id);
                evEnt.s.otherEntityNum = ent_number;
                evEnt.s.weapon = WP_ROCKET_LAUNCHER as c_int;

                evEnt.s.origin = missile_origin;
                evEnt.s.origin2 = ent_origin;

                if missile_origin[2] < ent_origin[2] {
                    evEnt.s.origin2[2] += 8.0;
                } else {
                    evEnt.s.origin2[2] += 24.0;
                }

                evEnt.s.eventParm = 1;
            }
        }
    }

    hitClient
}
