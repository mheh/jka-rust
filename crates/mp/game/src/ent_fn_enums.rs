// GENERATED by tools/closure-prototype/fngen.py (out/gen/).
// A later port pass consumes this file.
// Regenerate it rather than hand-edit it.

//! Entity fn-pointer dispatch as fn-ID enums, with a central match dispatch.
//!
//! Entity fn-pointer fields are ported as per-field fn-ID enums.
//! `PartialEq` replaces Raven's fn-address compares.
//! Rust stores `Option<EntXxx>` where Raven stored a nullable fn pointer.
//! Field signatures source: `oracle/codemp/game/g_local.h:285-291`
//! Targets: the fnsweep manifest fn-pointer write census (assignments in `game/*.c`).
//! Vehicle-vtable fields are excluded because `mp_bg` dispatches them through its own enum.
#![allow(non_snake_case, non_camel_case_types, unused)]

// Dispatch targets are the entity fn-pointer handlers, each defined in one landed skeleton module.
// This file glob-imports every module so the central match arms can name handlers unqualified.
// A Raven fn name is globally unique, so the glob imports never collide.
use crate::ai_main::*;
use crate::ai_util::*;
use crate::ai_wpnav::*;
...
// fn-ID enums hoisted to `mp_qshared` (below `mp_game`) so `gentity_t`'s dispatch fields can name them.
// The central match dispatch stays here because it names the handler fns.
// Re-exported so existing `crate::ent_fn_enums::EntXxx` imports keep resolving.
...
/// The raw-pointer params match Raven's signature for staging.
/// A later port pass re-shapes them to (world, EntityId, …).
...
pub fn dispatch_pain(
    ...
    match id {
        ...
        EntPain::NPC_ATST_Pain => NPC_ATST_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Droid_Pain => NPC_Droid_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_GM_Pain => NPC_GM_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Howler_Pain => NPC_Howler_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ...
        EntPain::NPC_Mark1_Pain => NPC_Mark1_Pain(
            ctx,
            ctx.entity_id_of(self_),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Mark2_Pain => NPC_Mark2_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_MineMonster_Pain => NPC_MineMonster_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ...
        EntPain::NPC_Probe_Pain => NPC_Probe_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Rancor_Pain => NPC_Rancor_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Remote_Pain => NPC_Remote_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Seeker_Pain => NPC_Seeker_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ),
        EntPain::NPC_Sentry_Pain => NPC_Sentry_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
        ...
        EntPain::NPC_Wampa_Pain => NPC_Wampa_Pain(
            ctx,
            ctx.entity_id_of(self_).unwrap(),
            ctx.entity_id_of(attacker),
            damage,
...
// ==== EntSpawn: classname->SP_ dispatch ====
// GENERATED addendum. The spawns[] table is a dual of oracle g_spawn.c:435-673 (185 rows).
// The classname strcmp becomes an EntSpawn lookup plus a central match dispatch, the EntThink pattern.
...
/// The `spawns[]` classname->`EntSpawn` table (185 rows, oracle order).
/// Raven `G_CallSpawn` walks this with a case-sensitive strcmp on `ent->classname`.
/// `spawn_for_classname` is that lookup.
...
/// Central spawn dispatch (replaces the `spawns[].spawn(ent)` call in `G_CallSpawn`).
/// The raw-pointer `ent` param matches Raven's signature for staging.
/// A later port pass re-shapes it to `(world, EntityId)`.
/// Source: `oracle/codemp/game/g_spawn.c:435-673`
