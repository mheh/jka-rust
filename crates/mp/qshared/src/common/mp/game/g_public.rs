...
//!
//! Following the `Q3_INFINITE` migration (NAV-D3 / RULING 39d, sibling
//! `q3_infinite.rs`): these `#define` bit flags are `g_public.h`'s shared
//! server<->game contract, so they live at a tier both the game module
//! (`mp_game`) and the engine server (`mp_engine_server`, which cannot reach
//! `mp_game`) can see. `mp_game`'s `g_public_consts` re-exports them so its own
//! call sites keep resolving.
//!
//! Plain `#define` bit flags (not an enum), so §C8 makes them `const`s
//! directly.
//!
//! Source: `oracle/codemp/game/g_public.h:11-47`
