...
pub fn run(events: PlatformEventSource, command_line: String) -> ! {
    let outcome = catch_unwind(AssertUnwindSafe(move || boot_and_run(events, command_line)));
    if outcome.is_err() {
...
fn boot_and_run(events: PlatformEventSource, command_line: String) -> ! {
    ...
    engine.re = Some(RendererFrontend::new());
