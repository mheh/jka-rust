...
use crate::client_host::{g2_from_view, Client};
use crate::fx::fx_harness::{fx_f32, fx_v3, fx_zero_trace, FxHarness};
use crate::snd_stubs::{S_RegisterSound, S_StartLocalSound, S_StartSound};
...
impl FxHost<'_, '_> {
    ...
    pub fn RegisterSound(&mut self, name: &str) -> c_int {
        match self {
            FxHost::Engine { cl, .. } => S_RegisterSound(cl, name),
            FxHost::Harness(h) => {
    ...
    pub fn PlayLocalSound(&mut self, sfx_handle: c_int, entchannel: c_int) {
        match self {
            FxHost::Engine { cl, .. } => S_StartLocalSound(cl, sfx_handle, entchannel),
            FxHost::Harness(h) => h.emit(format!(
