...
use mp_bg::public::configstring::CS_SYSTEMINFO;
...
use mp_engine_qcommon::files_common::{FS_FCloseFile, FS_PureServerSetLoadedPaks, FS_Write};
...
use mp_engine_qcommon::qcommon::svc_ops_e::svc_ops_e::{
    svc_EOF, svc_baseline, svc_configstring, svc_download, svc_gamestate, svc_mapchange, svc_nop,
    svc_serverCommand, svc_setgame, svc_snapshot,
};
...
use native_string::info::{Info_NextPair, Info_ValueForKey};
...
use crate::svc_strings::svc_strings;

/// Raven `SHOWNET` - the debug trace line for one read op at `cl_shownet >= 2`.
///
/// Source: `oracle/codemp/client/cl_parse.cpp:41-45`
pub fn SHOWNET(common: &mut Common, cl: &mut Client, msg: *mut msg_t, s: *mut c_char) {
    if common.cvar(cl.cl_shownet).integer >= 2 {
...
pub fn CL_DeltaEntity(
    ...
    if unchanged != qfalse {
        ...
        MSG_ReadDeltaEntity(view, msg, old, state, newnum);
    }
...
pub fn CL_SystemInfoChanged(view: &mut EngineHostView, cl: &mut Client) {
    ...
    loop {
        ...
        Cvar_Set(view, &key, &value);
    }
    // if game folder should not be set and it is set at the client side
    if game_set == qfalse && !Cvar_VariableString(view.common, "fs_game").is_empty() {
        Cvar_Set(view, "fs_game", "");
    }
    cl.cl_connectedToPureServer = Cvar_VariableValue(view.common, "sv_pure") as c_int;
...
pub fn CL_ParseAutomapSymbols(common: &mut Common, cl: &mut Client, msg: *mut msg_t) {
    let count = MSG_ReadShort(common, msg) as u16 as c_int;
    cl.clc.rmgAutomapSymbolCount = count;

    for i in 0..count {
        let m_type = MSG_ReadByte(common, msg);
        let m_side = MSG_ReadByte(common, msg);
        let origin_x = MSG_ReadLong(common, msg) as f32;
        let origin_y = MSG_ReadLong(common, msg) as f32;
...
pub fn CL_ParseCommandString(common: &mut Common, cl: &mut Client, msg: *mut msg_t) {
    let seq = MSG_ReadLong(common, msg);
    let s = MSG_ReadString(common, msg);
...
pub fn CL_ParsePacketEntities(
    ...
    loop {
        ...
        while oldnum < newnum {
            ...
            CL_DeltaEntity(view, cl, msg, newframe, oldnum, oldstate, qtrue);
        ...
        if oldnum == newnum {
            ...
            CL_DeltaEntity(view, cl, msg, newframe, newnum, oldstate, qfalse);
        ...
        if oldnum > newnum {
            ...
            CL_DeltaEntity(view, cl, msg, newframe, newnum, baseline, qfalse);
            continue;
        }
    }
...
pub fn CL_ParseRMG(common: &mut Common, cl: &mut Client, msg: *mut msg_t) {
    cl.clc.rmgHeightMapSize = MSG_ReadShort(common, msg) as u16 as c_int;
    ...
    size = MSG_ReadShort(common, msg) as u16 as c_int;

    if MSG_ReadBits(common, msg, 1) != 0 {
        // Read the flatten map
        MSG_ReadData(common, msg, heightmap1.as_mut_ptr() as *mut (), size);
        ...
        MSG_ReadData(
            common,
            msg,
            cl.clc.rmgFlattenMap.as_mut_ptr() as *mut (),
            size,
        );
    }

    // Read the seed
    cl.clc.rmgSeed = MSG_ReadLong(common, msg);

    CL_ParseAutomapSymbols(common, cl, msg);
}

/// Raven `CL_ParseSnapshot` - decompresses one server snapshot into `cl.snap`.
///
/// Source: `oracle/codemp/client/cl_parse.cpp:207-328`
pub fn CL_ParseSnapshot(view: &mut EngineHostView, cl: &mut Client, msg: *mut msg_t) {
    ...
    new_snap.serverTime = MSG_ReadLong(view.common, msg);

    new_snap.messageNum = cl.clc.serverMessageSequence;

    let delta_num = MSG_ReadByte(view.common, msg);
    if delta_num == 0 {
        new_snap.deltaNum = -1;
    } else {
        new_snap.deltaNum = new_snap.messageNum - delta_num;
    }
    new_snap.snapFlags = MSG_ReadByte(view.common, msg);
    ...
    // read areamask
    let len = MSG_ReadByte(view.common, msg);
    MSG_ReadData(
        view.common,
        msg,
        new_snap.areamask.as_mut_ptr() as *mut (),
        len,
    );

    // read playerinfo
    SHOWNET(view.common, cl, msg, c"playerstate".as_ptr() as *mut c_char);
    if !old.is_null() {
    ...
    SHOWNET(
        ...
        cl,
        msg,
        c"packet entities".as_ptr() as *mut c_char,
    );
    CL_ParsePacketEntities(view, cl, msg, old, &mut new_snap);
...
pub fn CL_ParseDownload(view: &mut EngineHostView, cl: &mut Client, msg: *mut msg_t) {
    let mut data = [0u8; MAX_MSGLEN];

    // read the data
    let block = MSG_ReadShort(view.common, msg) as u16 as c_int;

    if block == 0 {
        // block zero is special, contains file size
        cl.clc.downloadSize = MSG_ReadLong(view.common, msg);

        Cvar_SetValue(view, "cl_downloadSize", cl.clc.downloadSize as f32);
    ...
    let size = MSG_ReadShort(view.common, msg) as u16 as c_int;
    if size > 0 {
        MSG_ReadData(view.common, msg, data.as_mut_ptr() as *mut (), size);
    }
    ...
    if cl.clc.download == 0 {
        ...
        cl.clc.download = FS_SV_FOpenFileWrite(view.common, &temp_name);
    ...
    Cvar_SetValue(view, "cl_downloadCount", cl.clc.downloadCount as f32);
    ...
    if size == 0 {
        ...
        if cl.clc.download != 0 {
            ...
            let download_name = unsafe { CStr::from_ptr(cl.clc.downloadName.as_ptr()) }
                .to_string_lossy()
                .into_owned();
            FS_SV_Rename(view.common, &temp_name, &download_name);
        }
...
pub fn CL_ParseGamestate(view: &mut EngineHostView, cl: &mut Client, msg: *mut msg_t) {
    ...
    cl.clc.serverCommandSequence = MSG_ReadLong(view.common, msg);
    ...
    loop {
        ...
        if cmd == svc_EOF as c_int {
            break;
        }

        if cmd == svc_configstring as c_int {
            let start = unsafe { (*msg).readcount };

            let i = MSG_ReadShort(view.common, msg);
            ...
            let s = MSG_ReadBigString(view.common, msg);

            if view.common.cvar(cl.cl_shownet).integer >= 2 {
                com_printf(view.common, &format!("{:3}: {}: {}\n", start, i, s));
            }
            ...
            MSG_ReadDeltaEntity(view, msg, &mut nullstate, es, newnum);
        } else {
            com_error(
                errorParm_t::ERR_DROP,
    ...
    cl.clc.clientNum = MSG_ReadLong(view.common, msg);
    // read the checksum feed
    cl.clc.checksumFeed = MSG_ReadLong(view.common, msg);

    CL_ParseRMG(view.common, cl, msg); // rwwRMG - get info for it from the server

    // parse serverId and other cvars
    CL_SystemInfoChanged(view, cl);

    // reinitialize the filesystem if the game directory has changed
    if FS_ConditionalRestart(view, cl.clc.checksumFeed) != qfalse {
...
pub fn CL_ParseServerMessage(view: &mut EngineHostView, cl: &mut Client, msg: *mut msg_t) {
    if view.common.cvar(cl.cl_shownet).integer == 1 {
        com_printf(view.common, &format!("{} ", unsafe { (*msg).cursize }));
    } else if view.common.cvar(cl.cl_shownet).integer >= 2 {
    ...
    loop {
        ...
        if cmd == svc_EOF as c_int {
            SHOWNET(
                ...
                cl,
                msg,
                c"END OF MESSAGE".as_ptr() as *mut c_char,
            );
            break;
        }
        ...
        if view.common.cvar(cl.cl_shownet).integer >= 2 {
            if svc_strings[cmd as usize].is_empty() {
                ...
                SHOWNET(
                    ...
                    cl,
                    msg,
                    svc_strings[cmd as usize].as_ptr() as *mut c_char,
                );
            }
        }

        // other commands
        if cmd == svc_nop as c_int {
            // no-op
        } else if cmd == svc_serverCommand as c_int {
            CL_ParseCommandString(view.common, cl, msg);
        } else if cmd == svc_gamestate as c_int {
            CL_ParseGamestate(view, cl, msg);
        } else if cmd == svc_snapshot as c_int {
            CL_ParseSnapshot(view, cl, msg);
        } else if cmd == svc_setgame as c_int {
            CL_ParseSetGame(view, msg);
        } else if cmd == svc_download as c_int {
            CL_ParseDownload(view, cl, msg);
        } else if cmd == svc_mapchange as c_int {
            if !cl.cgvm.is_null() {
                VM_Call(
                    view.common,
                    cl.cgvm,
                    MpCgameExport::CG_MAP_CHANGE as c_int,
                    &[],
                );
            }
        } else {
