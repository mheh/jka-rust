...
pub struct CollisionWorld {
    ...
    // Raven's `cm_draw.cpp` scan-converter statics (`n`, `pt`, `nact`,
    // `active`) live for one `CDraw32::DrawPolygon` call, so they are a local
    // `PolyScan` (`cm/poly_scan.rs`) and not fields here.
    // Source: `oracle/codemp/qcommon/cm_draw.cpp:1080-1085`
