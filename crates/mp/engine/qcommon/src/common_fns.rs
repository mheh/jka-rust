...
pub fn Com_SafeMode(common: &mut Common) -> qboolean {
    for i in 0..common.com_numConsoleLines as usize {
        crate::cmd::Cmd_TokenizeString(common, common.com_consoleLines[i]);
        let argv0 = crate::cmd::Cmd_Argv(common, 0);
...
pub fn Com_StartupVariable(
    ...
    for i in 0..common.com_numConsoleLines as usize {
        ...
        let s = crate::cmd::Cmd_Argv(common, 1);
        if r#match.is_null() || q_strcmp(s, r#match as *mut c_char) == 0 {
            crate::cvar::Cvar_Set(common, cm, rm, host, s, crate::cmd::Cmd_Argv(common, 2));
            let cv: *mut cvar_t =
                crate::cvar::Cvar_Get(common, cm, rm, host, s, c"".as_ptr() as *mut c_char, 0);
            unsafe {
                (*cv).flags |= mp_game::q_shared_cvar_flags::CVAR_USER_CREATED;
            }
...
pub fn Com_Error_f(common: &mut Common) {
    if crate::cmd::Cmd_Argc(common) > 1 {
        crate::common::com_error::Com_Error(common, errorParm_t::ERR_DROP, "Testing drop error");
    } else {
        crate::common::com_error::Com_Error(common, errorParm_t::ERR_FATAL, "Testing fatal error");
    }
}
...
pub fn Com_Freeze_f(
    ...
    if crate::cmd::Cmd_Argc(common) != 2 {
        crate::common::com_printf(common, "freeze <seconds>\n");
        return;
    }
    let s: f32 = c_str_to_string(crate::cmd::Cmd_Argv(common, 1))
        .trim()
        .parse()
        .unwrap_or(0.0);
...
pub fn Com_InitJournaling(
    ...
    common.com_journal = crate::cvar::Cvar_Get(
        ...
        mp_game::q_shared_cvar_flags::CVAR_INIT,
    );
    unsafe {
...
pub fn Com_WriteConfigToFile(common: &mut Common, filename: *const c_char) {
    ...
    crate::files::fs_printf::FS_Printf(
        common,
        f,
...
pub fn Com_WriteConfiguration(common: &mut Common) {
    ...
    if common.cvar_modifiedFlags & mp_game::q_shared_cvar_flags::CVAR_ARCHIVE == 0 {
        return;
    }
    common.cvar_modifiedFlags &= !mp_game::q_shared_cvar_flags::CVAR_ARCHIVE;
...
pub fn Com_EventLoop(
    ...
    loop {
        ...
        if matches!(ev.evType, sysEventType_t::SE_NONE) {
            ...
            while crate::qcommon::net_loopback::NET_GetLoopPacket(
                common,
...
pub fn Com_Frame(
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        crate::cmd::Cbuf_Execute(common, cm, sv, rm, host);
...
pub fn Com_ParseTextFile(
    ...
    let mut f: mp_qshared_filehandle::fileHandle_t = 0;
    let length = crate::files::fs_fopen_file_by_mode::FS_FOpenFileByMode(
        common,
        cm,
        rm,
        host,
...
pub fn Com_Init(
    ...
    let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        ...
        crate::cmd::Cbuf_Init(common);

        crate::z_memman::com_init_zone_memory::Com_InitZoneMemory(common, cm, rm, host);

        crate::cmd::Cmd_Init(common, cm, rm, host);
        ...
        crate::files::fs_init_filesystem::FS_InitFilesystem(common, cm, rm, host);
        ...
        crate::z_memman::com_init_hunk_memory::Com_InitHunkMemory(common, sv, rm, g2, host);
        ...
        common.com_maxfps = crate::cvar::Cvar_Get(
            ...
            mp_game::q_shared_cvar_flags::CVAR_ARCHIVE,
        );
        ...
        common.com_developer = crate::cvar::Cvar_Get(
            ...
            c"0".as_ptr() as *mut c_char,
            mp_game::q_shared_cvar_flags::CVAR_TEMP,
        );
        ...
        common.com_timescale = crate::cvar::Cvar_Get(
            ...
            c"1".as_ptr() as *mut c_char,
            mp_game::q_shared_cvar_flags::CVAR_CHEAT | mp_game::q_shared_cvar_flags::CVAR_SYSTEMINFO,
        );
        common.com_fixedtime = crate::cvar::Cvar_Get(
            ...
            c"0".as_ptr() as *mut c_char,
            mp_game::q_shared_cvar_flags::CVAR_CHEAT,
        );
        ...
        common.cl_paused = crate::cvar::Cvar_Get(
            ...
            c"0".as_ptr() as *mut c_char,
            mp_game::q_shared_cvar_flags::CVAR_ROM,
        );
        ...
        crate::cvar::Cvar_Get(
            ...
            mp_game::q_shared_cvar_flags::CVAR_ARCHIVE | mp_game::q_shared_cvar_flags::CVAR_SYSTEMINFO,
        );
        crate::cvar::Cvar_Get(
            common, cm, rm, host,
        ...
        crate::cmd::Cmd_AddCommand(
            ...
            host,
            "changeVectors",
            crate::qcommon::msg::MSG_ReportChangeVectors_f,
        );
        ...
        common.com_version = crate::cvar::Cvar_Get(
            ...
            mp_game::q_shared_cvar_flags::CVAR_ROM | mp_game::q_shared_cvar_flags::CVAR_SERVERINFO,
        );

        crate::common::stringed::SE_Init(common, host);

        host.sys_init();
        crate::qcommon::net_chan::Netchan_Init(
            common,
            (crate::common::com_milliseconds::Com_Milliseconds(common, cm, rm, host) & 0xffff) as c_int,
        );
        crate::vm::vm_init::VM_Init(common, cm, rm, host);
