...
## Amendment 2026-08-05 - DEC-65 ruling 1: the model blocks go behind `Arc` and publish to the render thread (gh#31 steps 002-004)

The FROZEN content above is unchanged. This records the DEC-65 landing (`docs/audits/2026-08-04-model-block-publication-landing.md`, `docs/audits/2026-08-05-draw-arm-migration-landing.md`) that reshaped the cache entry and added a publication half.

- **Block ownership.** `CachedEndianedModelBinary.disk_image` is now `Option<Arc<ModelBlock>>` (`crates/mp/renderer/src/tr_model/cached_model_binary.rs`), not the `Option<AlignedBytes>` the `## State ownership` table, `## Seam definition`, and divergences entries spell. `ModelBlock` (`crates/mp/renderer/src/render_state/model_block.rs`) owns the `AlignedBytes` allocation, a copy-on-write `generation`, and the two DEC-35 sidecars as `Option<Arc<MdxmParsed>>`/`Option<Arc<MdxaParsed>>`. This supersedes the DEC-35 blockquote's "stored beside `disk_image`" placement: the sidecars moved inside the block. The `model_mdxm`/`model_mdxa` pointers now address into the block the `Arc` shares. The `TRM-D4` 16-byte alignment and cast discipline are untouched.
- **Deallocation.** "Drop deallocs the same Layout" now happens at the last `Arc` drop, not at cache-entry drop. Eviction (`models_level_load_end`, `re_register_models_dump_non_pure`, `re_register_models_delete_all`) removes the cache entry and purges every published slot that names the block (`ModelBlocks::remove_block`), so `r_modelpoolmegs` reclamation still frees memory. A frame in flight keeps an evicted block alive until the frame drops.
- **Publication.** `RenderModels` gains `blocks: ModelBlocks` and `blocks_dirty`, with `mark_block` recording each finished slot as a `PublishedModel` (block `Arc` plus byte offset, never a pointer) and `publish_blocks` handing a clone over when dirty. `RE_EndFrame` (`crates/mp/renderer/src/tr_cmds.rs`) drains the flag once per frame into `RenderAssets.models: Arc<ModelBlocks>`. This corrects the 2026-07-27 amendment's retirement note: `RenderAssets` carries a `models` field again, at the published-registry type, while the registry itself stays here.
- **Read path.** The draw arms resolve model bytes through `PublishedModel::md3_ptr(lod)` and `mdxm_view()` (`crates/mp/renderer/src/render_state/model_blocks.rs`), the one SAFETY home for the base-plus-offset casts. DEC-65 ruling 4 splits the entity dispatch: the brush test reads `BModelTable`, and `model_type` resolves from the published entry.
- **The shader poke is copy-on-write.** `poke_shader_index` writes through `Arc::make_mut`: a block a published frame still holds is cloned, the write lands in the unique clone, and the generation bumps. The caller re-reads the entry's base pointer afterwards, because the entry can now name a different allocation.
