...
Batch-4 addendum: the rule covers every header pointing at confirmed-retired generation tooling, not just `PORT-COMPLETE` (the `game_cvars.rs` `GENERATED by fngen.py` case). The confirmation bar stays high: the tooling's own source or a `docs/` record must show it retired. A worker who cannot confirm keeps the header and flags it.
...
Batch-4 addendum: inter-token whitespace inside restored C call text is part of "the text" and restores exactly (the `g_target.rs` `gi.Printf (...)` paren-spacing case).
...
Batch-4 addendum: fabricated and confirmed citations are always removed, never repaired into a valid-looking form (the `ai_util.rs` `§F19` case). A frozen campaign's completion-assurance prose deletes with its label, since both narrate finished process rather than a live constraint.
...
Batch-4 addendum: the protection covers the named tag and its anchor, not the surrounding prose. A DEC-anchored banner's prose restyles like any port prose, em dashes stripped (the `game_impl.rs` `DEC-36 D5` banner case). Only Raven's own text gets verbatim protection.
...
## CB-14 - port-status notes state the constraint, not the progress (batch-4 walk, 2026-08-18)
