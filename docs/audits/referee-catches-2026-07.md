...
| Commit | Finding |
|---|---|
| `2a9b171e` | **Preprocessor-polarity: `#ifndef METROID_JUMP` arms ported as live code** (`w_force.c:5444-5454`; METROID_JUMP is `#define`d unconditionally at `:7`). Re-enabled the cut charge-jump mechanic for every grounded force-capable player holding jump — live-combat behavior retail never has, referee-blind because bots never produce the input pattern. New audit lens: check `#ifdef` polarity around every transcribed block; excluded-in-retail code is as dangerous as a dropped guard. Same batch: PM_StepSlideMove `< 0.7` double compare (movement spine), BotUtilizePersonality temp-alloc imbalance (2 failed personality loads exhausted the 3-slot BG_TempAlloc table). |
