...
## DEC-63 — three-builder landing batch: decoder, FX host seam, FX retail leg (ruled 2026-08-02)

Four rulings from the gh#22/gh#25/gh#27 landing sit-down.

1. **The MP3 decoder is symphonia-bundle-mp3.** Amends the minimp3 naming in DEC-03 and DEC-57.3: the replaced decoder dependency is `symphonia-bundle-mp3` (pure Rust, MPL-2.0, one call site at `mp3::mp3_decoder::Mp3Decoder`). Every Rust minimp3 binding is a cc build of the unmaintained C decoder with an open RUSTSEC advisory, so the zune-jpeg posture (DEC-49) applies. Frame framing stays ours (`Mp3FrameHeader`) because Raven's wrap counts whole-file source bytes. Decoder correctness pins on committed synthetic decode fixtures.
2. **The `FxHost` Harness arm stands.** Amends DEC-61.3: a dissolved wrapper's receiver may be a closed two-arm enum (`Engine` with the direct calls, `Harness` feeding the §18 rig), because a call-shaped output surface has no ring to dump and the capture seam must sit at the call boundary. A service trait or function table stays forbidden. This mirrors DEC-61.1's closed-enum treatment of the primitive hierarchy.
3. **FX takes the retail `FINAL_BUILD` leg, and a subsystem follows its rig's macro set.** Retail and OpenJK release both define `FINAL_BUILD` (OpenJK `CMakeLists.txt:324`), so the `#ifndef FINAL_BUILD` guards in `StopEffect` and the axis `PlayEffect` (`FxScheduler.cpp:96-102,760-766`) compile out: no print, no early return, and the `mEffectIDs[sfile]` lookup inserts the name with id 0. The port flips to that leg behind new fx-oracle scenarios that pin the retail behavior first. The repo-wide non-final convention (`perf_flags.rs`) stays for the server tree because the lockstep referee-oracle builds without `FINAL_BUILD` — each subsystem matches the macro set of the oracle rig that gates it.
4. **DEC-60.1 discharged.** The gh#22 thread split ran the `gpu_res` re-audit and found nothing to re-home: `GpuResources` holds one empty placeholder and every live GPU object is `mp_renderer_gpu`'s, owned by the render thread. The obligation transfers to the R4 wave that gives `GpuResources` real wgpu fields — that wave moves it to the render thread rather than keep it in `RendererFrontend` (recorded at `hook_install.rs`, `renderer_frontend.rs`, `gpu_resources.rs`).

## DEC-64 — the `image` crate is the workspace codec umbrella (ruled 2026-08-03)

`mp_renderer` takes `image` as a plain dependency, `default-features = false`, features `png` and `jpeg`. It covers two legs. PNG decode is the first. Retail pk3s ship PNG-only textures, and `mp/ffa2` alone draws six of them, so the deferred `LoadPNG32` and `LoadPNG8` arms dropped those stages and left fallback squares in the live client. JPEG encode is the second, for the `screenshot` command that `SaveJPG` writes at Raven's quality 95. zune-jpeg stays the live decode path for `LoadJPG` under DEC-49, and `image` bundles that same decoder, so the family stays coherent. One exception sits beside the umbrella. `LoadPNG8` returns Raven's unexpanded 8-bit samples, and `image` expands a palette to RGB before a caller sees it, so `png` joins as a direct dependency for that one load. It adds no crate to the tree, because `image` already pulls it.

## DEC-65 — model blocks to the render thread: read-only publication, sim-side skeletons, tokens everywhere (ruled 2026-08-03)

Three rulings open wave 3 (gh#31), where MD3 and Ghoul2 entities start to draw render-side.

First, block ownership. The parsed md3/mdxm/mdxa blocks move into Arc-published immutable storage with generation discipline in the Handle style, and the render thread reads them directly. This requires the ModelPool ownership restructure (the pool today is Box-pinned, not Clone, and never Arc-published per its own doc), and we accept that cost. The Send justification is real, not hopeful: the blocks are immutable after load. The recorded evolution path: this shape later becomes GPU-resident static geometry plus per-frame bone matrices (GPU skinning) without rework. Frame-level upscaling and post stay orthogonal in the pass graph, per the DEC-53 R4 backend entry. Sim-side vertex baking was rejected because it walks back DEC-50 render-side traversal, ships every animated vertex every frame, and forecloses motion vectors, GPU skinning, and a decoupled render rate.

Second, the Ghoul2 skeleton. The transform runs sim-side at scene-add time, and plain per-entity bone matrices cross in the frame package. Ghoul2System stays thread-confined, and the incremental CBoneCache stays sim-side where it is already warm. The one deviation from Raven is timing: Raven transforms at backend draw time, we transform at scene-add, but both read the same refdef time within one frame, and the gh#31 image-golden gate verifies the parity. MD3 needs no crossing state, because frame interpolation is stateless from the published blocks plus the frame and oldframe fields already on RefEntity. The matrices-per-frame payload is also exactly what GPU skinning consumes, so this ruling is the second half of the evolution path above. Ruling 2 executed 2026-08-05 in gh#31 step-005 (commits `c34ec3fc` through `a3501348`, merge `ec7c934c`). Two step-005 survey refinements: the crossing carries the per-model instance snapshot (handle, skins, LOD bias, overrides, bolts) beside the matrices, and the timing-parity sentence above holds trivially, because `G2API_GetTime` ignores its time argument, so the builder needs no refdef. The transform-before-cull deviation is accepted in the step packet (its ruling A), and the record is `docs/audits/2026-08-05-bone-matrix-crossing-landing.md`.

Third, the ghoul2 reference crossing. The ghoul2 reference crosses every module boundary as the Ghoul2Handle token (handle plus one, encoded pointer-width, null round-trips to None), and raw pointers never leave the engine. This closes the unreconciled split flagged at tr_scene.rs: the render trap decodes tokens while the server boundary (sv_game.rs:3116-3125) hands out real Box pointers in the same void* slot. The server hand-out converts to the token. Guards: a scout confirms no module dereferences the ghoul2 void* before the change lands, and the lockstep referee suite plus the image goldens gate it. If a module does read through the pointer, the recorded fallback is translation at entry (a pointer-to-handle map at the render trap), but that debt is not the ruling. Ruling 3 executed 2026-08-03 in commits `7833dc68` through `62b2e7af`, which converted all three dispatch files plus `sv_world.rs`, so the present-tense description above and the `sv_game.rs:3116-3125` cite read as history.

## DEC-66 — the render-side RNG owner (ruled 2026-08-07)

Four rulings from the gh#31 step-009 ratification walk. They unblock the electricity chain (`CreateShape`, `DoBoltSeg`, `RB_SurfaceSaberGlow`) — the oriented-quad and cylinder arms draw no random numbers.

1. **The backend owns the CRT stream.** The render-side backend owns one persistent `Rng` (`native_math::rng`), seeded `Rng::new()` at boot (the MSVC compile-time seed, holdrand = 1), beside the backend's frame-to-frame state and threaded into the arms. The `sh1`/`sh2`/`f_count` file statics land in `TrSurfaceShapeState` under the same owner, with `LIGHTNING_RECURSION_LEVEL` as a const beside them. The stream persists across frames — a per-frame reset would replay the same jitter every frame and freeze the bolt shimmer. Owner-embedded instances are the `rng.rs` doctrine, and world-effects already runs the pattern renderer-side.

2. **The `DoBoltSeg` seed is a threaded local.** The dispatch shape stays `Option<&RefEntity>`. `RB_SurfaceElectricity` hoists `e.frame` into a local `mut seed: c_int` and threads `&mut seed` through the `DoBoltSeg`/`ApplyShape` recursion. Faithful because `backEnd.currentEntity` is a per-frame copy, so the oracle's seed write-back never outlives one draw chain. This overrules the `Option<&mut RefEntity>` rewire named in the deferred comment at `tr_surface.rs:1083`, and the lane deletes that note.

3. **The stream split is an accepted divergence.** Retail links one process-wide CRT `rand()` state, so a renderer draw advances the same stream the client, cgame, and FX code read next. The render-side instance is independent in both directions; the observable effect is a different jitter phase on cosmetic geometry. No gate observes cross-stream interleaving, so no gate changes.

4. **Fixed-dt carries the goldens.** The boot seed is fixed and the `--fixed-dt` frame sequence is deterministic, so electricity and saber-glow captures are byte-stable with no new machinery. No reseed hook now. If a golden proves fragile against scene-order changes, a test-only `srand` reseed before capture graduates then, as its own ruling.

Amendment (ruled 2026-08-31, the gh#54 weather golden): the ruling-4 clause graduates. The weather path carries Raven's own wall-clock reseed, `srand(Com_Milliseconds())` (`oracle/codemp/renderer/tr_WorldEffects.cpp:1491`), so a faithful fixture cannot be byte-stable without pinning. A golden fixture may reseed both `Rng` streams to fixed constants, twice: once before the weather command triple, which pins the draws inside `Initialize`, and once after the commands and before stepping, which keeps the stepped stream independent of the command path's draw count. The live path keeps the faithful wall-clock reseed untouched. First exercised by `golden_world_weather_ctf2`, proved by three isolated byte-identical runs before the bless.

## DEC-67 — every change reaches master through a branch and a pull request (ruled 2026-08-16)

Every commit goes to master through a work branch and a GitHub pull request. The rule covers everything: code lanes, docs, DEC entries, packet folders, and one-line touch-ups. The PR merges on GitHub with a merge commit, never a squash, so the lane's per-commit gate history stays visible in master. Each merge fires the module-matrix build and rolls the `latest` release, and that cost is accepted: the batch-push HOLD rule (user 2026-07-12) retires. The user maintains the master branch rulesets on GitHub. Branch names keep the 2026-08-01 pattern (`wf/<ticket>-<slug>`, `issue/<n>-<slug>`, `research/<name>`), and work that no issue drives uses `chore/<slug>`. Amends the disposition rule in `.claude/skills/packet/SKILL.md` and the branch-pattern section of `docs/agents/issue-tracker.md`.

Amendment (executed 2026-08-16): the gate mechanics. The workflow's `approve` job binds to the `ci-approval` environment, whose required reviewer is the repo owner with self-review allowed, because GitHub forbids a PR-review self-approval but allows a deployment self-approval. A pull request waits at that gate until the owner approves the deployment, then the compile gate, the module matrix, and the server lanes run, and the `ci-green` summary job is the one status check the active master ruleset requires. The ruleset also requires a pull request, allows only merge commits, blocks force pushes and branch deletion, and grants the repository admin an always-bypass as the owner's override. The repo merge settings allow merge commits only. A PR run uploads the module and server zips as run artifacts and never touches the rolling release, and the post-merge master push publishes the release. First exercised on PR #45.

## DEC-68 — the jampgame comment pass: one combined triage-and-restyle sweep (ruled 2026-08-16)

One pass over every comment in the `mp_game` crate replaces DEC-39's two-pass shape for jampgame: each comment is judged by its information, residue deletes, load-bearing port notes and behavioral or divergence documentation restyle into STE house style with every fact kept, and Raven-documented text (the QUAKED/spawnflag blocks, Raven's own comments, `Source:` cites, layout asserts) stays verbatim, character for character. DEC-39 stands unchanged for the other modules. The behavior case law lives in `docs/comment-behavior.md`, this ruling's executable appendix - review walks append rules there, and wave briefs carry it verbatim. The mechanical gate is DEC-39 rule 3's comments-only token check, built as `tools/comment-gate` (proc-macro2 token-stream identity per changed file), with `cargo build --workspace` per lane and one lockstep-referee plus full-test run at campaign end. Lanes run on sonnet medium over waves balanced by comment-line count, the campaign anchors on a plain GitHub issue, and everything lands on the DEC-67 docs-pass PR branch. The plan is `docs/plans/2026-08-16-jampgame-comment-pass.md`, and the ten-file pilot (reviewed 2026-08-16, net -292 lines, token-clean) seeded CB-1 through CB-5.

## DEC-69 — the live community master server replaces the dead retail default (ruled 2026-08-30)

Retail's default master, `masterjk3.ravensoft.com` in `sv_master1`, is dead, so a faithful client finds zero servers. The port keeps the retail `sv_master1` default and puts the live community master `master.jkhub.org` in `sv_master2`, a slot Raven ships empty. `CL_GlobalServers_f` takes the cvar-indexed shape: a `globalservers 0 <protocol>` request fans out and re-issues `globalservers <i> <protocol>` for each non-empty `sv_masterN`, and an address with no port gets `PORT_MASTER` (29060) explicitly, because `NET_StringToAdr` fills a missing port with `PORT_SERVER` (29070) and a master never listens there. `CL_ServersResponsePacket` routes every master response into the global list - the mplayer branch is dead surface, because `AS_MPLAYER` is obsolete (`oracle/codemp/game/q_shared.h:3025-3029`). The divergence is deliberate and user-ruled (live-play session 2026-08-30): the wire protocol to a master is unchanged, and only the default address set and the fan-out differ from retail. Landed in commit 29c7ed37 (`crates/mp/engine/client/src/cl_main.rs`, `crates/mp/engine/server/src/sv_init.rs`).
